...
import { r as stripAnsi, t as sanitizeForLog } from "./ansi-haTfqSzd.js";
...
import { i as updateSkillsFromClawHub, n as readTrackedClawHubSkillSlugs, r as searchSkillsFromClawHub, t as installSkillFromClawHub } from "./skills-clawhub-PcPgjhgL.js";
...
function formatSkillsList(report, opts) {
	...
	if (skills.length === 0) return appendClawHubHint(opts.eligible ? `No eligible skills found. Run \`${formatCliCommand("enclawed skills list")}\` to see all skills.` : "No skills found.", opts.json);
...
function formatSkillInfo(report, skillName, opts) {
	...
	if (!skill) {
		...
		return appendClawHubHint(`Skill "${skillName}" not found. Run \`${formatCliCommand("enclawed skills list")}\` to see available skills.`, opts.json);
	}
	...
	if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
		...
		lines.push(`  Save via CLI: ${formatCliCommand(`enclawed config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`);
		lines.push(`  Stored in: ${theme.muted("$ENCLAWED_CONFIG_PATH")} ${theme.muted("(default: ~/.enclawed/enclawed.json)")}`);
	}
	return appendClawHubHint(lines.join("\n"), opts.json);
}
...
async function loadSkillsStatusReport() {
	...
	const { buildWorkspaceSkillStatus } = await import("./skills-status-BrAFRhGT.js");
