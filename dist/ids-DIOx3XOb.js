...
function listPackageRoots() {
	return [resolveEnclawedPackageRootSync({ cwd: process.cwd() }), resolveEnclawedPackageRootSync({ moduleUrl: import.meta.url })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
}
