...
import { r as parseOptionalDelimitedEntries } from "./helpers-icsK1N4v.js";
...
function resolveIrcAccount(params) {
	...
	const resolve = (accountId) => {
		...
		const username = (merged.username?.trim() || (accountId === "default" ? process.env.IRC_USERNAME?.trim() : "") || nick || "enclawed").trim();
		const realname = (merged.realname?.trim() || (accountId === "default" ? process.env.IRC_REALNAME?.trim() : "") || "Enclawed").trim();
...
function buildFallbackNick(nick) {
	const base = nick.replace(/\s+/g, "").replace(/[^A-Za-z0-9_\-[\]\\`^{}|]/g, "") || "enclawed";
	const suffix = "_";
	const maxNickLen = 30;
