...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-BPigC1a6.js";
import "./config-BDd-c7HD.js";
import { t as WizardCancelledError } from "./prompts-DZ3FVXpm.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-DcaEkiOH.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BHyRThzF.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-b9TTkz0v.js");
	...
	if (snapshot.exists && !snapshot.valid) {
		...
		await prompter.outro(`Config invalid. Run \`${formatCliCommand("enclawed doctor")}\` to repair it, then re-run setup.`);
		runtime.exit(1);
		return;
	}
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CQlPMz3l.js");
		const { logConfigUpdated } = await import("./logging-Bujaid2T.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-9JhxDBwn.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-DhiPGEav.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-bDl6iLUV.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Df3FH48p.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-gydfqrN0.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-gydfqrN0.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Df3FH48p.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CzUERh3b.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-B-qX5vSJ.js");
		const { setupChannels } = await import("./onboard-channels-u5unK06-.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BIspmwlu.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-C3muk8EK.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-DhovrAbM.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-YhSRc0vt.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-B_rlB1lB.js");
