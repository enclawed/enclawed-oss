...
import { n as resolveBoundaryPath, s as isPathInside } from "./boundary-path-CUdfnLBJ.js";
import { r as openBoundaryFile } from "./boundary-file-read-CQh3KACR.js";
import { a as loadConfig } from "./io-b4s6ivfp.js";
import { a as DEFAULT_IDENTITY_FILENAME, c as DEFAULT_SOUL_FILENAME, d as ensureAgentWorkspace, i as DEFAULT_HEARTBEAT_FILENAME, l as DEFAULT_TOOLS_FILENAME, n as DEFAULT_AGENT_WORKSPACE_DIR, r as DEFAULT_BOOTSTRAP_FILENAME, t as DEFAULT_AGENTS_FILENAME, u as DEFAULT_USER_FILENAME } from "./workspace-Dg5fnKWT.js";
import "./config-DDWYoiuw.js";
import { t as loadActivatedBundledPluginPublicSurfaceModuleSync } from "./facade-runtime-Oph1J7lc.js";
import { a as isToolAllowed, f as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, n as resolveSandboxConfigForAgent, t as resolveSandboxBrowserDockerCreateConfig, u as SANDBOX_AGENT_WORKSPACE_MOUNT } from "./config-DW2vjdts.js";
...
async function createSshSandboxSessionFromConfigText(params) {
	...
	const configDir = await fs$1.mkdtemp(path.join(resolveSshTmpRoot(), "enclawed-sandbox-ssh-"));
...
const NOVNC_PASSWORD_ENV_KEY = "ENCLAWED_BROWSER_NOVNC_PASSWORD";
...
async function ensureSandboxBrowser(params) {
	...
	const browserImage = params.cfg.browser.image ?? "enclawed-sandbox-browser:bookworm-slim";
	...
	let noVncPassword;
	if (hasContainer) {
		if (noVncEnabled) noVncPassword = await readDockerContainerEnvVar(containerName, "ENCLAWED_BROWSER_NOVNC_PASSWORD") ?? void 0;
		...
		if (hashMismatch) {
			...
			if (running && (typeof lastUsedAtMs !== "number" || now - lastUsedAtMs < HOT_BROWSER_WINDOW_MS)) {
				const hint = (() => {
					if (params.cfg.scope === "session") return `enclawed sandbox recreate --browser --session ${params.scopeKey}`;
					if (params.cfg.scope === "agent") return `enclawed sandbox recreate --browser --agent ${resolveSandboxAgentId(params.scopeKey) ?? "main"}`;
	...
	if (!hasContainer) {
		...
		args.push("-e", `ENCLAWED_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`);
	...
	if (noVncEnabled && !noVncPassword) noVncPassword = await readDockerContainerEnvVar(containerName, "ENCLAWED_BROWSER_NOVNC_PASSWORD") ?? void 0;
	const existing = BROWSER_BRIDGES.get(params.scopeKey);
	const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME) : null;
