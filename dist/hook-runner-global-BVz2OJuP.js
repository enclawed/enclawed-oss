...
function createHookRunner(registry, options = {}) {
	...
	*/
	async function runBeforeAgentFinalize(event, ctx) {
		...
		if (hooks.length === 0) return;
		logger?.debug?.(`[hooks] running before_agent_finalize (${hooks.length} handlers, first-decision wins)`);
		for (const hook of hooks) try {
			const handlerResult = await hook.handler(event, ctx);
			if (handlerResult && handlerResult.action && handlerResult.action !== "continue") return handlerResult;
		} catch (err) {
			handleHookError({
