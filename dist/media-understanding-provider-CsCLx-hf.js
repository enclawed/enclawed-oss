import { i as FALLBACK_CODEX_MODELS, r as CODEX_PROVIDER_ID } from "./provider-catalog-k9U3olG6.js";
import { i as resolveCodexAppServerRuntimeOptions } from "./config-COjeGYaA.js";
import { a as readCodexErrorNotification, c as readCodexTurnCompletedNotification, n as assertCodexThreadStartResponse, r as assertCodexTurnStartResponse } from "./protocol-validators-wvIjUPTe.js";
import { r as readModelListResult } from "./models-C6hpvDWO.js";
...
async function describeCodexImages(req, options) {
	...
	const client = options.clientFactory ? await options.clientFactory(appServer.start, req.profile) : await import("./shared-client-BeN9uz6A.js").then(({ createIsolatedCodexAppServerClient }) => createIsolatedCodexAppServerClient({
	...
	try {
		...
		const thread = assertCodexThreadStartResponse(await client.request("thread/start", {
			...
			developerInstructions: "You are Enclawed's bounded image-understanding worker. Describe only the provided image content. Do not call tools, edit files, or ask follow-up questions.",
			dynamicTools: [],
