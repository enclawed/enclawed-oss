...
import { B as Union, E as Integer, H as _Object_, I as Unsafe, T as Literal, X as Optional, g as String$1, v as Number$1 } from "./build-BqpZINkL.js";
import { d as extractReplyText, f as formatFeishuApiError, g as readString, i as listFeishuAccountIds, l as resolveFeishuRuntimeAccount, p as isRecord, r as listEnabledFeishuAccounts, s as resolveFeishuAccount, u as encodeQuery, y as parseFeishuCommentTarget } from "./accounts-Bop3Pflv.js";
import { o as createFeishuClient } from "./client-B55PKUcX.js";
...
const FeishuChatSchema = _Object_({
	action: Unsafe({
		type: "string",
		enum: [...CHAT_ACTION_VALUES],
		description: "Action to run: members | info | member_info"
	}),
	chat_id: Optional(String$1({ description: "Chat ID (from URL or event payload)" })),
	member_id: Optional(String$1({ description: "Member ID for member_info lookups" })),
	page_size: Optional(Number$1({ description: "Page size (1-100, default 50)" })),
	page_token: Optional(String$1({ description: "Pagination token" })),
	member_id_type: Optional(Unsafe({
		type: "string",
...
const FeishuDriveSchema = Union([
	_Object_({
		action: Literal("list"),
		folder_token: Optional(String$1({ description: "Folder token (optional, omit for root directory)" }))
	}),
	_Object_({
		action: Literal("info"),
		file_token: String$1({ description: "File or folder token" }),
		type: FileType
	}),
	_Object_({
		action: Literal("create_folder"),
		name: String$1({ description: "Folder name" }),
		folder_token: Optional(String$1({ description: "Parent folder token (optional, omit for root)" }))
	}),
	_Object_({
		action: Literal("move"),
		file_token: String$1({ description: "File token to move" }),
		type: FileType,
		folder_token: String$1({ description: "Target folder token" })
	}),
	_Object_({
		action: Literal("delete"),
		file_token: String$1({ description: "File token to delete" }),
		type: FileType
	}),
	_Object_({
		action: Literal("list_comments"),
		file_token: String$1({ description: "Document token" }),
		file_type: Optional(CommentFileType),
		page_size: Optional(Integer({
			minimum: 1,
			maximum: 100,
			description: "Page size"
		})),
		page_token: Optional(String$1({ description: "Comment page token" }))
	}),
	_Object_({
		action: Literal("list_comment_replies"),
		file_token: String$1({ description: "Document token" }),
		file_type: Optional(CommentFileType),
		comment_id: String$1({ description: "Comment id" }),
		page_size: Optional(Integer({
			minimum: 1,
			maximum: 100,
			description: "Page size"
		})),
		page_token: Optional(String$1({ description: "Reply page token" }))
	}),
	_Object_({
		action: Literal("add_comment"),
		file_token: String$1({ description: "Document token" }),
		file_type: Optional(Union([Literal("doc"), Literal("docx")], { description: "Document type. Defaults to docx when omitted." })),
		content: String$1({ description: "Comment text content" }),
		block_id: Optional(String$1({ description: "Optional docx block id for a local comment. Omit to create a full-document comment." }))
	}),
	_Object_({
		action: Literal("reply_comment"),
		file_token: String$1({ description: "Document token" }),
		file_type: Optional(CommentFileType),
		comment_id: String$1({ description: "Comment id" }),
		content: String$1({ description: "Reply text content" })
	})
]);
//#endregion
