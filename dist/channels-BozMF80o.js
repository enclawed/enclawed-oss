...
import { t as isChannelVisibleInConfiguredLists } from "./exposure-CJLYbfjs.js";
...
import { i as loadAuthProfileStore } from "./store-DMcipGx3.js";
import "./plugins-iuTgQA7g.js";
import "./logging-BIHkOBbx.js";
import { c as resolveMessageActionDiscoveryForPlugin, r as createMessageActionDiscoveryContext } from "./message-action-discovery-CRt1rORA.js";
import "./auth-profiles-A-k-drnz.js";
import { r as callGateway } from "./call-CmVOwPMM.js";
import { n as getChannelsCommandSecretTargetIds } from "./command-secret-targets-AiCLh2A8.js";
...
import { i as resolveChannelDefaultAccountId, r as parseOptionalDelimitedEntries } from "./helpers-Db-VEnkz.js";
import { c as moveSingleAccountChannelSectionToDefaultAccount } from "./setup-helpers-BXCkKBqj.js";
import { n as withProgress } from "./progress-Bnz7HAqX.js";
import { t as createClackPrompter } from "./clack-prompter-CEq8K4xW.js";
import { n as hasResolvedCredentialValue, t as hasConfiguredUnavailableCredentialStatus } from "./account-snapshot-fields-BUrPgrA9.js";
import { n as formatUsageReportLines, t as loadProviderUsageSummary } from "./provider-usage-D0RWqPjJ.js";
import { t as resolveCommandConfigWithSecrets } from "./command-config-resolution-DETi_iss.js";
import { n as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-DHvxXX5_.js";
import { t as parseLogLine } from "./parse-log-line-BYNsUi0H.js";
import { r as describeBinding, t as applyAgentBindings } from "./agents.bindings-CVSiqyY3.js";
import { n as requireValidConfigSnapshot, t as requireValidConfigFileSnapshot } from "./config-validation-Dy7dJ56q.js";
import { t as collectChannelStatusIssues } from "./channels-status-issues-B3thq04P.js";
import { n as buildReadOnlySourceChannelAccountSnapshot, t as buildChannelAccountSnapshot } from "./status-BPy_gPC_.js";
...
function formatAccountLabel$1(params) {
...
function formatChannelAccountLabel$1(params) {
	const channelText = channelLabel(params.channel);
	const accountText = formatAccountLabel$1({
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	const { listChannelPluginCatalogEntries } = await import("./catalog-B8l3_G5J.js");
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-HVPvYNbw.js"), import("./onboard-channels-D7JGW_VJ.js")]);
	...
	const loadScopedPlugin = async (channelId, pluginId) => {
		...
		const { loadChannelSetupPluginRegistrySnapshotForChannel } = await import("./plugin-install-C69EzfGy.js");
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-aeN3_o9c.js");
		if (!isCatalogChannelInstalled({
			cfg: nextConfig,
			entry: catalogEntry,
			workspaceDir
		})) {
			const { ensureChannelSetupPluginInstalled } = await import("./plugin-install-C69EzfGy.js");
			const prompter = createClackPrompter();
	...
	if (afterAccountConfigWritten) {
		const { runCollectedChannelOnboardingPostWriteHooks } = await import("./onboard-channels-D7JGW_VJ.js");
...
async function channelsCapabilitiesCommand(opts, runtime = defaultRuntime) {
	...
	for (const report of reports) {
		const label = formatChannelAccountLabel$1({
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = !useWizard && rawChannel && (!channel || !getChannelPlugin(channel)) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-BX2peom2.js");
...
function formatChannelAccountLabel(params) {
	return `${listChannelPlugins().find((plugin) => plugin.id === params.channel)?.meta.label ?? params.channel} ${formatAccountLabel({
		accountId: params.accountId,
		name: params.name
	})}`;
}
function appendEnabledConfiguredLinkedBits$1(bits, account) {
...
function appendModeBit$1(bits, account) {
	if (typeof account.mode === "string" && account.mode.length > 0) bits.push(`mode:${account.mode}`);
}
function appendTokenSourceBits$1(bits, account) {
...
function appendBaseUrlBit$1(bits, account) {
	if (typeof account.baseUrl === "string" && account.baseUrl) bits.push(`url:${account.baseUrl}`);
}
function buildChannelAccountLine$1(provider, account, bits) {
...
async function formatConfigChannelsStatusLines(cfg, meta, opts) {
	...
	const accountLines = (provider, accounts) => accounts.map((account) => {
		const bits = [];
		appendEnabledConfiguredLinkedBits$1(bits, account);
		appendModeBit$1(bits, account);
		appendTokenSourceBits$1(bits, account);
		appendBaseUrlBit$1(bits, account);
		return buildChannelAccountLine$1(provider, account, bits);
	});
	const plugins = listChannelPlugins();
...
function buildChannelAccountLine(provider, account, bits) {
	return `- ${formatChannelAccountLabel$1({
		channel: provider,
		accountId: typeof account.accountId === "string" ? account.accountId : "default",
		name: (normalizeOptionalString(account.name) ?? "") || void 0
	})}: ${bits.join(", ")}`;
