...
import { n as writeJsonFileAtomically } from "./json-store-omP8GdLt.js";
import "./string-coerce-runtime-C86BPY1x.js";
import "./state-paths-DdDaGCxI.js";
import { a as resolveMatrixDefaultOrOnlyAccountId, i as resolveMatrixChannelConfig, n as requiresExplicitMatrixDefaultAccount, o as resolveMatrixAccountStringValues, r as resolveConfiguredMatrixAccountIds, t as findMatrixAccountEntry } from "./account-selection-ENajVOSq.js";
import { i as resolveScopedMatrixEnvConfig, n as resolveGlobalMatrixEnvConfig } from "./env-auth-DDCFP_Xx.js";
import { a as resolveMatrixCredentialsPath, n as resolveMatrixAccountStorageRoot, s as resolveMatrixLegacyFlatStoragePaths } from "./storage-paths-C8UhrqzP.js";
import { t as formatMatrixErrorMessage } from "./errors-DFW-R2p9.js";
import "./migration-snapshot-backup-B5v3eT80.js";
...
function resolveLegacyMatrixFlatStoreTarget(params) {
	if (!resolveMatrixChannelConfig(params.cfg)) return { warning: `Legacy Matrix ${params.detectedKind} detected at ${params.detectedPath}, but channels.matrix is not configured yet. Configure Matrix, then rerun "enclawed doctor --fix" or restart the gateway.` };
	if (requiresExplicitMatrixDefaultAccount(params.cfg)) return { warning: `Legacy Matrix ${params.detectedKind} detected at ${params.detectedPath}, but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set. Set "channels.matrix.defaultAccount" to the intended target account before rerunning "enclawed doctor --fix" or restarting the gateway.` };
	...
	if (!target) {
		...
		return { warning: `Legacy Matrix ${params.detectedKind} detected at ${params.detectedPath}, but ${targetDescription} could not be resolved yet (need homeserver, userId, and access token for channels.matrix${accountId === "default" ? "" : `.accounts.${accountId}`}). Start the gateway once with a working Matrix login, or rerun "enclawed doctor --fix" after cached credentials are available.` };
	}
	return {
		...target,
...
async function loadMatrixLegacyCryptoInspector() {
	return (await import("./legacy-crypto-inspector-CE3KHqvy.js")).inspectLegacyMatrixCryptoStore;
}
function detectLegacyBotSdkCryptoStore(cryptoRootDir) {
	try {
		if (!fs.statSync(cryptoRootDir).isDirectory()) return {
			detected: false,
			warning: `Legacy Matrix encrypted state path exists but is not a directory: ${cryptoRootDir}. Enclawed skipped automatic crypto migration for that path.`
		};
	} catch (err) {
		return {
			detected: false,
			warning: `Failed reading legacy Matrix encrypted state path (${cryptoRootDir}): ${String(err)}. Enclawed skipped automatic crypto migration for that path.`
		};
	}
	try {
		...
		return {
			detected: false,
			warning: `Failed scanning legacy Matrix encrypted state path (${cryptoRootDir}): ${String(err)}. Enclawed skipped automatic crypto migration for that path.`
		};
	}
}
...
async function autoPrepareLegacyMatrixCrypto(params) {
	...
	for (const plan of detection.plans) {
		...
		if (!plan.deviceId) {
			warnings.push(`Legacy Matrix encrypted state detected at ${plan.legacyCryptoPath}, but no device ID was found for account "${plan.accountId}". Enclawed will continue, but old encrypted history cannot be recovered automatically.`);
			continue;
		}
		let summary;
		try {
		...
		if (!summary.decryptionKeyBase64 && (summary.roomKeyCounts?.backedUp ?? 0) > 0) warnings.push(`Legacy Matrix encrypted state for account "${plan.accountId}" has backed-up room keys, but no local backup decryption key was found. Ask the operator to run "enclawed matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`);
