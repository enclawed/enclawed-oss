...
import { n as getBridgeAuthForPort } from "./bridge-auth-registry-CGe6hegm.js";
import { h as resolveBrowserRateLimitMessage } from "./cdp.helpers-Dodvjzj3.js";
import { n as resolveBrowserControlAuth } from "./control-auth-C-gwKOQn.js";
...
function withLoopbackBrowserAuthImpl(url, init, deps) {
	...
	if (headers.has("authorization") || headers.has("x-enclawed-password")) return {
		...init,
		headers
	};
	...
	try {
		...
		else if (bridgeAuth?.password) headers.set("x-enclawed-password", bridgeAuth.password);
	} catch {}
	return {
		...init,
		headers
...
function resolveDispatcherBrowserControlOwnership(url) {
	...
	try {
		...
		return profile.driver === "enclawed" && profile.cdpIsLoopback && !profile.attachOnly ? "local-managed" : "external-browser";
	} catch {
		return "unknown";
	}
}
function resolveBrowserFetchOperatorHint(url, opts) {
	if (opts?.ownership === "external-browser") return "The browser profile is external to Enclawed; make sure its browser/CDP endpoint is running and reachable. Restarting the Enclawed gateway will not launch it.";
	return !isAbsoluteHttp(url) ? `Restart the Enclawed gateway (Enclawed.app menubar, or \`${formatCliCommand("enclawed gateway")}\`).` : "If this is a sandboxed session, ensure the sandbox browser is running.";
}
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
	...
	const msg = normalizeErrorMessage(err);
	const kind = classifyBrowserFetchFailure(err);
	if (kind === "timeout") return new Error(`Can't reach the Enclawed browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`, err instanceof Error ? { cause: err } : void 0);
	...
	return new Error(appendBrowserToolModelHint(`Can't reach the Enclawed browser control service. ${operatorHint} (${msg})`), err instanceof Error ? { cause: err } : void 0);
}
...
async function fetchBrowserJson(url, init) {
	...
	try {
		...
		const { dispatchBrowserControlRequest } = await import("./local-dispatch.runtime-TIwoCNeS.js");
