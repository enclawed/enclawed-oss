...
import { i as normalizeEnvVarKey, n as isDangerousHostEnvOverrideVarName, r as isDangerousHostEnvVarName } from "./host-env-security-CSPdLugP.js";
import { t as collectDurableServiceEnvVars } from "./state-dir-dotenv-DXchXP6U.js";
import { d as resolveGatewayLaunchAgentLabel } from "./paths-HxbQFXGH.js";
import { i as resolveGatewayProgramArguments, n as resolveDaemonInstallRuntimeInputs, r as resolveDaemonNodeBinDir, t as emitDaemonInstallRuntimeWarning } from "./daemon-install-plan.shared-D7-fTNl2.js";
import { c as buildServiceEnvironment } from "./runtime-paths-C0Np3ULF.js";
...
import { g as writeConfigFile, u as readConfigFileSnapshotForWrite } from "./io-b4s6ivfp.js";
...
function loadDaemonInstallAuthProfileSourceRuntime() {
	daemonInstallAuthProfileSourceRuntimePromise ??= import("./daemon-install-auth-profiles-source.runtime-CUNyNwKl.js");
	return daemonInstallAuthProfileSourceRuntimePromise;
}
function loadDaemonInstallAuthProfileStoreRuntime() {
	daemonInstallAuthProfileStoreRuntimePromise ??= import("./daemon-install-auth-profiles-store.runtime-7RtiFUPi.js");
...
function collectPreservedExistingServiceEnvVars(existingEnvironment, managedServiceEnvKeys) {
	...
	for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
		...
		if (upper === "HOME" || upper === "PATH" || upper === "TMPDIR" || upper.startsWith("ENCLAWED_")) continue;
...
async function buildGatewayInstallPlan(params) {
	...
	const serviceEnvironment = buildServiceEnvironment({
		env: params.env,
		port: params.port,
		launchdLabel: process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(params.env.ENCLAWED_PROFILE) : void 0,
...
function gatewayInstallErrorHint(platform = process.platform) {
	return platform === "win32" ? "Tip: native Windows now falls back to a per-user Startup-folder login item when Scheduled Task creation is denied; if install still fails, rerun from an elevated PowerShell or skip service install." : `Tip: rerun \`${formatCliCommand("enclawed gateway install")}\` after fixing the error.`;
}
//#endregion
...
function hasDurableGatewayPasswordEnvForInstall(cfg, env) {
	...
	return Boolean(normalizeOptionalString(durableServiceEnv.ENCLAWED_GATEWAY_PASSWORD) || normalizeOptionalString(durableServiceEnv.CLAWDBOT_GATEWAY_PASSWORD));
}
...
function formatAmbiguousGatewayAuthModeReason() {
	return ["gateway.auth.token and gateway.auth.password are both configured while gateway.auth.mode is unset.", `Set ${formatCliCommand("enclawed config set gateway.auth.mode token")} or ${formatCliCommand("enclawed config set gateway.auth.mode password")}.`].join(" ");
}
