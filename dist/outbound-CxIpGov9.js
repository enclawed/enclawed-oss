import { i as normalizeOptionalString, n as normalizeLowercaseStringOrEmpty, s as sanitizeFileName } from "./string-normalize-Blt_o5di.js";
import { A as fileExistsAsync, C as debugLog, D as UPLOAD_PREPARE_FALLBACK_CODE, F as readFileAsync, L as formatErrorMessage, M as getFileTypeName, N as getImageMimeType, O as checkFileSize, P as getMaxUploadSize, R as MediaFileType, S as debugError, T as UploadDailyLimitExceededError, g as sendText$1, h as sendMedia$1, j as formatFileSize, k as downloadFile, t as accountToCreds, u as initApiConfig, w as debugWarn } from "./sender-VizoFyMG.js";
import { c as getQQBotDataDir, g as resolveQQBotPayloadLocalFilePath, h as normalizePath, p as isLocalPath, r as parseTarget$1, u as getQQBotMediaDir } from "./target-parser-DLnL6VYB.js";
...
async function sendPhotoFromLocal(ctx, mediaPath) {
	...
	if (!sizeCheck.ok) return buildFileTooLargeResult(MediaFileType.IMAGE, sizeCheck.size);
...
async function sendVoiceFromLocal(ctx, mediaPath, directUploadFormats, transcodeEnabled) {
	...
	if (fileSize > getMaxUploadSize(MediaFileType.VOICE)) return buildFileTooLargeResult(MediaFileType.VOICE, fileSize);
...
async function sendVideoFromLocal(ctx, mediaPath) {
	...
	if (!sizeCheck.ok) return buildFileTooLargeResult(MediaFileType.VIDEO, sizeCheck.size);
...
async function sendDocumentFromLocal(ctx, mediaPath) {
	...
	if (!sizeCheck.ok) return buildFileTooLargeResult(MediaFileType.FILE, sizeCheck.size);
...
//#endregion
export { DEFAULT_MEDIA_SEND_ERROR as C, recordMessageReply as S, setOutboundAudioPort as T, resolveUserFacingMediaError as _, encodePayloadForCron as a, getMessageReplyConfig as b, parseQQBotPayload as c, parseTarget as d, resolveOutboundMediaPath as f, sendVoice as g, sendVideoMsg as h, sendText as i, normalizeMediaTags as l, sendPhoto as m, sendMedia as n, isCronReminderPayload as o, sendDocument as p, sendProactiveMessage as r, isMediaPayload as s, sendCronMessage as t, buildMediaTarget as u, MESSAGE_REPLY_LIMIT as v, OUTBOUND_ERROR_CODES as w, getMessageReplyStats as x, checkMessageReplyLimit as y };
