...
import { i as resolveProviderRequestCapabilities } from "./provider-attribution-BhAkATy2.js";
import { L as resolveProviderTransportTurnStateWithPlugin } from "./provider-runtime-DB4wLsLH.js";
import { s as detectOpenAICompletionsCompat } from "./provider-model-compat-BTKr4bsR.js";
import { t as normalizeToolParameterSchema } from "./pi-tools.schema-DERW_M3Q.js";
import { d as resolveDebugProxySettings } from "./runtime-B0vRcVVh.js";
import { y as stripNullHeaderValues } from "./provider-stream-shared-CP0ypOLf.js";
import { i as stripSystemPromptCacheBoundary } from "./system-prompt-cache-boundary-fpsgXwVt.js";
import { r as hasCopilotVisionInput, t as buildCopilotDynamicHeaders } from "./copilot-dynamic-headers-BjkJWSEq.js";
import { n as resolveOpenAIResponsesPayloadPolicy, r as flattenCompletionMessagesToStringContent, t as applyOpenAIResponsesPayloadPolicy } from "./openai-responses-payload-policy-whVi9F9B.js";
import { a as mergeModelProviderRequestOverrides, i as getModelProviderRequestTransport, l as resolveProviderRequestPolicyConfig, n as buildProviderRequestDispatcherPolicy } from "./provider-request-config-DpuK1s8u.js";
...
import { createAssistantMessageEventStream as createAssistantMessageEventStream$1 } from "@mariozechner/pi-ai";
...
function createWritableTransportEventStream() {
	const eventStream = createAssistantMessageEventStream$1();
	return {
		eventStream,
		stream: eventStream
	};
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				const client = createOpenAIResponsesClient(model, context, apiKey, stripNullHeaderValues(options?.headers), turnState?.headers);
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				const client = createAzureOpenAIClient(model, context, apiKey, stripNullHeaderValues(options?.headers), turnState?.headers);
...
function createOpenAICompletionsTransportStreamFn() {
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				const client = createOpenAICompletionsClient(model, context, options?.apiKey || getEnvApiKey(model.provider) || "", stripNullHeaderValues(options?.headers));
