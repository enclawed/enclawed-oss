...
import { i as resolveCodexAppServerRuntimeOptions, n as codexSandboxPolicyForTurn } from "./config-COjeGYaA.js";
import { t as isJsonObject } from "./protocol-B5MYjtGn.js";
import { i as getSharedCodexAppServerClient } from "./shared-client-ZMYHf916.js";
import { t as CODEX_CONTROL_METHODS } from "./capabilities-ACWsG7b8.js";
...
async function requireThreadBinding(sessionFile) {
	...
	if (!binding?.threadId) throw new Error("No Codex thread is attached to this Enclawed session yet.");
	return binding;
}
...
async function createThread(params) {
	...
	const response = await (await getSharedCodexAppServerClient({
		...
		sandbox: runtime.sandbox,
		...runtime.serviceTier ? { serviceTier: runtime.serviceTier } : {},
		developerInstructions: "This Codex thread is bound to an Enclawed conversation. Answer normally; Enclawed will deliver your final response back to the conversation.",
...
async function runBoundTurn(params) {
	...
	const requestCleanup = client.addRequestHandler(async (request) => {
		...
		if (request.method === "item/commandExecution/requestApproval" || request.method === "item/fileChange/requestApproval") return {
			decision: "decline",
			reason: "Enclawed native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow."
		};
