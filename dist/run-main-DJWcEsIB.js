...
import { a as enableConsoleCapture } from "./subsystem-DTyALtnK.js";
...
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-CmlPFX-5.js";
import { t as ensureEnclawedCliOnPath } from "./path-env-BACXGVB9.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-CNB0eIy8.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-C407i02B.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-DpOk0yvN.js");
			...
			const { statusCommand } = await import("./status-Cy27LYeg.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-BoMJP5Re.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-BGg_g2xW.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-TUn-pIoC.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-CNNOubcj.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-CNNOubcj.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-CIzHid5R.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-CIzHid5R.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-BjP38QjN.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-C36AEf2j.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-CgPI_zkk.js");
