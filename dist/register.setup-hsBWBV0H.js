...
async function createDefaultConfigIO() {
	const { createConfigIO } = await import("./io-DeP5arBW.js");
...
async function resolveDefaultAgentWorkspaceDir(deps) {
	...
	const { DEFAULT_AGENT_WORKSPACE_DIR } = await import("./workspace-BYkxUEM0.js");
	return DEFAULT_AGENT_WORKSPACE_DIR;
}
async function ensureDefaultAgentWorkspace(params) {
	const { ensureAgentWorkspace } = await import("./workspace-BYkxUEM0.js");
	return ensureAgentWorkspace(params);
}
async function writeDefaultConfigFile(config) {
	const { writeConfigFile } = await import("./io-DeP5arBW.js");
	await writeConfigFile(config);
}
async function formatDefaultConfigPath(configPath) {
	const { formatConfigPath } = await import("./logging-BCYZcbDn.js");
...
async function resolveDefaultSessionTranscriptsDir() {
	const { resolveSessionTranscriptsDir } = await import("./sessions-DQ-R94qJ.js");
