...
import { g as resolveUserPath, t as CONFIG_DIR } from "./utils-CrVQlOZJ.js";
import { t as formatCliCommand } from "./command-format-CAEA84sd.js";
import { a as resolvePackageExtensionEntries } from "./manifest-CWMW2Is2.js";
import { a as safePathSegmentHashed, i as safeDirName, n as packageNameMatchesId, o as unscopedPackageName, r as resolveSafeInstallDir } from "./install-safe-path-DHbDtG8y.js";
import path from "node:path";
import fs from "node:fs/promises";
//#region src/plugins/install.ts
let pluginInstallRuntimePromise;
async function loadPluginInstallRuntime() {
	pluginInstallRuntimePromise ??= import("./install.runtime-Dy31hbil.js");
	return pluginInstallRuntimePromise;
}
const MISSING_EXTENSIONS_ERROR = "package.json missing enclawed.extensions; update the plugin package to include enclawed.extensions (for example [\"./dist/index.js\"]). See https://docs.enclawed.ai/help/troubleshooting#plugin-install-fails-with-missing-enclawed-extensions";
...
function ensureEnclawedExtensions(params) {
...
async function detectNativePackageInstallSource(packageDir) {
	...
	try {
		return ensureEnclawedExtensions({ manifest: await runtime.readJsonFile(manifestPath) }).ok;
	} catch {
		return false;
	}
}
async function installPluginFromPackageDir(params) {
	...
	const extensionsResult = ensureEnclawedExtensions({ manifest });
