...
import { S as truncateUtf16Safe, g as resolveUserPath } from "./utils-BcQ7uNrT.js";
...
import { t as extractKeywords } from "./query-expansion-CM44xUgG.js";
import { i as getMemoryMultimodalExtensions, r as classifyMemoryMultimodalPath, t as buildCaseInsensitiveExtensionGlob } from "./multimodal-CntkuRhS.js";
...
import { n as listMemoryEmbeddingProviders, r as listRegisteredMemoryEmbeddingProviderAdapters, t as getMemoryEmbeddingProvider } from "./memory-embedding-provider-runtime-B-QrJN7t.js";
import { D as createLocalEmbeddingProvider, E as DEFAULT_LOCAL_MODEL, u as enforceEmbeddingMaxInputTokens } from "./engine-embeddings-_CpcCUSS.js";
import { a as ensureDir, c as listMemoryFiles, d as remapChunkLines, f as runWithConcurrency, h as hasNonTextEmbeddingParts, i as cosineSimilarity, l as normalizeExtraMemoryPaths, n as buildMultimodalChunkForIndexing, o as hashText, p as isFileMissingError, r as chunkMarkdown, t as buildFileEntry, u as parseEmbedding } from "./internal-eXimu566.js";
import "./memory-core-host-engine-embeddings-DLsydSMj.js";
import "./memory-core-host-engine-foundation-B7wTRjlU.js";
import { c as buildSessionEntry, f as sessionPathForFile, l as listSessionFilesForAgent } from "./engine-qmd-PE3eK5Dl.js";
import "./memory-core-host-engine-qmd-COEwxeIF.js";
import { r as readMemoryFile } from "./backend-config-DUgC4uH2.js";
import { n as loadSqliteVecExtension, r as ensureMemoryIndexSchema, t as requireNodeSqlite } from "./engine-storage-B_mB3Pwz.js";
import "./memory-core-host-engine-storage-BJiANKgt.js";
import "./provider-env-vars-D7GzYfVv.js";
import "./dreaming-shared-ClG8Ai8P.js";
...
function formatLocalSetupError(err) {
	...
	return [
		...
		missing ? "2) Reinstall Enclawed (this should install node-llama-cpp): npm i -g enclawed@latest" : null,
...
function resolveConfiguredSourcesForMeta(sources) {
	const normalized = Array.from(sources).filter((source) => source === "memory" || source === "sessions").toSorted((a, b) => a < b ? -1 : a > b ? 1 : 0);
	return normalized.length > 0 ? normalized : ["memory"];
}
function normalizeMetaSources(meta) {
	if (!Array.isArray(meta.sources)) return ["memory"];
	const normalized = Array.from(new Set(meta.sources.filter((source) => source === "memory" || source === "sessions"))).toSorted((a, b) => a < b ? -1 : a > b ? 1 : 0);
	return normalized.length > 0 ? normalized : ["memory"];
}
...
function resolveConfiguredScopeHash(params) {
	const extraPaths = normalizeExtraMemoryPaths(params.workspaceDir, params.extraPaths).map((value) => value.replace(/\\/g, "/")).toSorted((a, b) => a < b ? -1 : a > b ? 1 : 0);
...
var MemoryManagerSyncOps = class {
	...
	async runSync(params) {
		...
		const targetedSessionSync = await runMemoryTargetedSessionSync({
			...
			useUnsafeReindex: process.env.ENCLAWED_TEST_FAST === "1" && process.env.ENCLAWED_TEST_MEMORY_UNSAFE_REINDEX === "1",
			sessionsDirtyFiles: this.sessionsDirtyFiles,
		...
		try {
			if (needsFullReindex) {
				if (process.env.ENCLAWED_TEST_FAST === "1" && process.env.ENCLAWED_TEST_MEMORY_UNSAFE_REINDEX === "1") await this.runUnsafeReindex({
					reason: params?.reason,
					force: params?.force,
					progress: progress ?? void 0
				});
