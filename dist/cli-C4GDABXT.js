...
import { n as matrixReplaceConfigFile, r as readMatrixRuntimeConfig, t as getMatrixRuntime } from "./runtime-oC-Hfm9f.js";
import { i as resolveMatrixAccount } from "./accounts-SYpoCi5T.js";
import { t as formatMatrixErrorMessage } from "./errors-D19xOr-n.js";
import { n as resolveMatrixConfigPath } from "./config-paths-Hom4Nv4q.js";
import { t as updateMatrixAccountConfig } from "./config-update-BMbN2Cae.js";
import { n as matrixSetupAdapter } from "./setup-core-CZQBQqgs.js";
import { t as resolveMatrixRoomKeyBackupIssue } from "./backup-health-DPO5XU9A.js";
import { i as setMatrixSdkLogMode, r as setMatrixSdkConsoleLogging } from "./logging-7_zyXjgr.js";
import { t as withResolvedActionClient } from "./client-C3IgeTrM.js";
import { r as resolveMatrixAuthContext } from "./config-BZ019aml.js";
import "./client-QjRLHs6i.js";
import "./runtime-api-CVf04Q_1.js";
import { n as summarizeMatrixDeviceHealth, t as isEnclawedManagedMatrixDevice } from "./device-health-D3UYl6Lm.js";
import { n as updateMatrixOwnProfile, t as applyMatrixProfileUpdate } from "./profile-update-DoaLmnL2.js";
import { a as confirmMatrixVerificationSas, c as getMatrixRoomKeyBackupStatus, d as listMatrixVerifications, f as mismatchMatrixVerificationSas, g as runMatrixSelfVerification, h as restoreMatrixRoomKeyBackup, l as getMatrixVerificationSas, m as resetMatrixRoomKeyBackup, n as bootstrapMatrixVerification, p as requestMatrixVerification, r as cancelMatrixVerification, t as acceptMatrixVerification, u as getMatrixVerificationStatus, v as startMatrixVerification, y as verifyMatrixRecoveryKey } from "./verification-BAGGT-y3.js";
...
async function pruneMatrixStaleGatewayDevices(opts = {}) {
	return await withResolvedActionClient(opts, async (client) => {
		...
		const staleGatewayDeviceIds = summarizeMatrixDeviceHealth(devices).staleEnclawedDevices.map((device) => device.deviceId);
		return {
			before: devices,
...
function loadMatrixActionClientModule() {
	matrixActionClientModulePromise ??= import("./client-Czb3Ca0Q.js");
	return matrixActionClientModulePromise;
}
function loadMatrixDirectManagementModule() {
	matrixDirectManagementModulePromise ??= import("./direct-management-DhW2CQvl.js");
...
function resolveMatrixCliAccountContext(accountId) {
	const cfg = readMatrixRuntimeConfig();
	return {
...
async function addMatrixAccount(params) {
	...
	const cfg = readMatrixRuntimeConfig(runtime);
	...
	if (accountConfig.encryption === true) {
		const { maybeBootstrapNewEncryptedMatrixAccount } = await import("./setup-bootstrap-CdsdsLCe.js");
	...
	if (desiredDisplayName || desiredAvatarUrl) try {
		...
		if (synced.convertedAvatarFromHttp && synced.resolvedAvatarUrl) {
			await matrixReplaceConfigFile({
				nextConfig: updateMatrixAccountConfig(readMatrixRuntimeConfig(runtime), accountId, { avatarUrl: synced.resolvedAvatarUrl }),
				afterWrite: { mode: "auto" }
			});
	...
	try {
		...
		deviceHealth = {
			...
			staleEnclawedDeviceIds: addedDevices.filter((device) => !device.current && isEnclawedManagedMatrixDevice(device.displayName)).map((device) => device.deviceId)
		};
	} catch (err) {
		deviceHealth = {
			currentDeviceId: null,
...
async function setupMatrixEncryption(params) {
	...
	if (encryptionChanged) await matrixReplaceConfigFile({
		nextConfig: updated,
		afterWrite: { mode: "auto" }
	});
...
function registerMatrixCli(params) {
		...
		await runMatrixCliCommand({
			...
			onText: (result) => {
				...
				if (result.deviceHealth.error) console.error(`Matrix device health warning: ${formatMatrixCliText(result.deviceHealth.error)}`);
					...
					const staleDeviceIds = result.deviceHealth.staleEnclawedDeviceIds.map((deviceId) => formatMatrixCliText(deviceId)).join(", ");
					console.log(`Matrix device hygiene warning: stale Enclawed devices detected (${staleDeviceIds}). Run ${formatMatrixCliCommand("devices prune-stale", result.accountId)}.`);
				}
	...
	devices.command("prune-stale").description("Delete stale Enclawed-managed devices for this account").option("--account <id>", "Account ID (for multi-account setups)").option("--verbose", "Show detailed diagnostics").option("--json", "Output as JSON").action(async (options) => {
		const { accountId, cfg } = resolveMatrixCliAccountContext(options.account);
		await runMatrixCliCommand({
			verbose: options.verbose === true,
			json: options.json === true,
			...
			onText: (result, verbose) => {
				printAccountLabel(accountId);
				console.log(`Deleted stale Enclawed devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.map((deviceId) => formatMatrixCliText(deviceId)).join(", ") : "none"}`);
