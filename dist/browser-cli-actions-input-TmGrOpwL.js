...
import { c as resolveExistingPathsWithinRoot, s as DEFAULT_UPLOAD_DIR } from "./config-DdC3GGxL.js";
import { o as normalizeBrowserFormField, s as normalizeBrowserFormFieldValue } from "./pw-role-snapshot-BzSS6-KL.js";
import "./core-api-bMMphq2o.js";
import { t as callBrowserRequest } from "./browser-cli-shared-DuNOlHHF.js";
import { t as runBrowserResizeWithOutput } from "./browser-cli-resize-B8lV6I-k.js";
...
function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
	...
	browser.command("upload").description("Arm file upload for the next file chooser").argument("<paths...>", "File paths to upload (must be within Enclawed temp uploads dir, e.g. /tmp/enclawed/uploads/file.pdf)").option("--ref <ref>", "Ref id from snapshot to click after arming").option("--input-ref <ref>", "Ref id for <input type=file> to set directly").option("--element <selector>", "CSS selector for <input type=file>").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next file chooser (default: 120000)", (v) => Number(v)).action(async (paths, opts, cmd) => {
		const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
	...
	browser.command("waitfordownload").description("Wait for the next download (and save it)").argument("[path]", "Save path within enclawed temp downloads dir (default: /tmp/enclawed/downloads/...; fallback: os.tmpdir()/enclawed/downloads/...)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next download (default: 120000)", (v) => Number(v)).action(async (outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/wait/download",
			body: { path: normalizeOptionalString(outPath) }
		});
	});
	browser.command("download").description("Click a ref and save the resulting download").argument("<ref>", "Ref id from snapshot to click").argument("<path>", "Save path within enclawed temp downloads dir (e.g. report.pdf or /tmp/enclawed/downloads/report.pdf)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the download to start (default: 120000)", (v) => Number(v)).action(async (ref, outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/download",
			body: {
				ref,
