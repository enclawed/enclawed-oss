...
import { a as GATEWAY_SERVICE_MARKER, d as resolveGatewayLaunchAgentLabel, m as resolveGatewayWindowsTaskName, n as resolveHomeDir, p as resolveGatewaySystemdServiceName } from "./paths-HxbQFXGH.js";
import { t as execSchtasks } from "./schtasks-exec-Prgnbixh.js";
import path from "node:path";
import fs from "node:fs/promises";
//#region src/daemon/inspect.ts
const EXTRA_MARKERS = ["enclawed", "clawdbot"];
...
function isEnclawedGatewayLaunchdService(label, contents) {
...
function isEnclawedGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("enclawed-gateway")) return false;
	return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}
function isEnclawedGatewayTaskName(name) {
	const normalized = normalizeLowercaseStringOrEmpty(name);
	if (!normalized) return false;
	return normalized === normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName()) || normalized.startsWith("enclawed gateway");
}
...
async function scanLaunchdDir(params) {
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		...
		if (marker === "enclawed" && isEnclawedGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
			detail: `plist: ${fullPath}`,
...
async function scanSystemdDir(params) {
	...
	for (const { entry, name, fullPath, contents } of candidates) {
		...
		if (!marker) continue;
		if (marker === "enclawed" && isEnclawedGatewaySystemdService(name, contents)) continue;
		results.push({
			platform: "linux",
			label: entry,
...
async function findExtraGatewayServices(env, opts = {}) {
	...
	if (process.platform === "win32") {
		...
		for (const task of tasks) {
			...
			if (!name) continue;
			if (isEnclawedGatewayTaskName(name)) continue;
