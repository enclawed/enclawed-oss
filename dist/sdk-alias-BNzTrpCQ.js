...
function hasTrustedEnclawedRootIndicator(params) {
	...
	const hasEnclawedBin = typeof params.packageJson.bin === "string" && normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("enclawed") || typeof params.packageJson.bin === "object" && params.packageJson.bin !== null && typeof params.packageJson.bin.enclawed === "string";
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot) {
	...
	if (!pkg) return null;
	if (!hasTrustedEnclawedRootIndicator({
		packageRoot,
		packageJson: pkg
	})) return null;
...
function resolveTrustedEnclawedRootFromArgvHint(params) {
	...
	return hasTrustedEnclawedRootIndicator({
		packageRoot,
		packageJson
	}) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(params) {
	...
	const fromExplicitHints = resolveTrustedEnclawedRootFromArgvHint({
		cwd,
		argv1: params.argv1
	}) ?? (params.moduleUrl ? resolveEnclawedPackageRootSync({
		cwd,
...
const PLUGIN_SDK_PACKAGE_NAMES = ["@enclawed/plugin-sdk", "@enclawed/plugin-sdk"];
...
function resolvePluginSdkScopedAliasMap(params = {}) {
	...
	for (const subpath of listPluginSdkExportedSubpaths({
		...
		if (Object.prototype.hasOwnProperty.call(aliasMap, `@enclawed/plugin-sdk/${subpath}`)) break;
	}
...
function buildPluginLoaderAliasMap(modulePath, argv1 = STARTUP_ARGV1, moduleUrl, pluginSdkResolution = "auto") {
	...
	return {
		...extensionApiAlias ? { "@enclawed/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) } : {},
