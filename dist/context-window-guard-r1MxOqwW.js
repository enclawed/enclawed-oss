...
function formatContextWindowWarningMessage(params) {
	...
	if (params.guard.source === "agentContextTokens") return `${base}; Enclawed is capped by agents.defaults.contextTokens, so raise that cap if you want to use more of the model context window`;
	if (params.guard.source === "modelsConfig") return `${base}; Enclawed is using the configured model context limit for this model, so raise contextWindow/contextTokens if it is set too low`;
...
function formatContextWindowBlockMessage(params) {
	...
	if (!resolveContextWindowGuardHint({ runtimeBaseUrl: params.runtimeBaseUrl }).likelySelfHosted) return base;
	if (params.guard.source === "agentContextTokens") return `${base} Enclawed is capped by agents.defaults.contextTokens. Raise that cap.`;
	if (params.guard.source === "modelsConfig") return `${base} Enclawed is using the configured model context limit for this model. Raise contextWindow/contextTokens or choose a larger model.`;
	return `${base} This looks like a local model endpoint. Raise the server/model context limit or choose a larger model. Enclawed local/self-hosted runs work best at ${CONTEXT_WINDOW_WARN_BELOW_TOKENS}+ tokens.`;
}
