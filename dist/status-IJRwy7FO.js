...
import { n as resolvePreferredEnclawedTmpDir, t as POSIX_ENCLAWED_TMP_DIR } from "./tmp-enclawed-dir-BTrLrKyp.js";
...
import { _ as resolveStateDir, o as resolveConfigPath, u as resolveGatewayPort } from "./paths-CDjhyzOH.js";
import { d as resolveGatewayLaunchAgentLabel, p as resolveGatewaySystemdServiceName } from "./paths-HxbQFXGH.js";
import { a as trimToUndefined } from "./credential-planner-7-s3g1Jp.js";
import "./credentials-C-3FfE6S.js";
import { r as createConfigIO } from "./io-BPigC1a6.js";
import { i as parseStrictPositiveInteger } from "./parse-finite-number-HbZldjE_.js";
import { c as resolveGatewayLogPaths } from "./launchd-DLvjqB_1.js";
import { a as inspectPortUsage, s as formatPortDiagnostics } from "./ports-CNWE5-2x.js";
import { p as classifySystemdUnavailableDetail } from "./systemd-B-zqDlea.js";
import { r as resolveGatewayService } from "./service-DafdpbVq.js";
import { a as filterDaemonEnv, c as pickProbeHostForBind, d as resolveRuntimeStatusColor, f as safeDaemonEnv, o as normalizeListenerAddress, s as parsePortFromArgs, t as createCliStatusTextStyles, u as renderRuntimeHints } from "./shared-B1Rkh6AD.js";
import { i as resolveDaemonContainerContext, r as formatRuntimeStatus } from "./runtime-hints-JaQ37b3Y.js";
import "./config-BDd-c7HD.js";
import { t as formatConfigIssueLine } from "./issue-format-B5juDRgp.js";
import { n as renderSystemdUnavailableHints, t as isSystemdUnavailableDetail } from "./systemd-hints-CCexgvGB.js";
import { r as isWSLEnv } from "./wsl-B2FYKJDu.js";
import { t as readLastGatewayErrorLine } from "./diagnostics-21WZiFgV.js";
import { r as resolveBestEffortGatewayBindHostForDisplay, t as inspectBestEffortPrimaryTailnetIPv4 } from "./network-discovery-display-ITOLCHF6.js";
import { r as renderGatewayServiceCleanupHints } from "./inspect-CU9-zFpD.js";
...
function loadGatewayProbeAuthModule() {
	gatewayProbeAuthModulePromise ??= import("./probe-auth-DAr-Sefc.js");
	return gatewayProbeAuthModulePromise;
}
function loadDaemonInspectModule() {
	daemonInspectModulePromise ??= import("./inspect-BsvS_t1Q.js");
	return daemonInspectModulePromise;
}
function loadServiceAuditModule() {
	serviceAuditModulePromise ??= import("./service-audit-BRKI0yUp.js");
	return serviceAuditModulePromise;
}
function loadGatewayTlsModule() {
	gatewayTlsModulePromise ??= import("./gateway-Bh2B7VEK.js");
	return gatewayTlsModulePromise;
}
function loadDaemonProbeModule() {
	daemonProbeModulePromise ??= import("./probe-MOlxMgmb.js");
	return daemonProbeModulePromise;
}
function loadRestartHealthModule() {
	restartHealthModulePromise ??= import("./restart-health-BqzPDN_X.js");
...
function printDaemonStatus(status, opts) {
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("enclawed doctor")}" (or "${formatCliCommand("enclawed doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("enclawed gateway install --force")}\` from the same --profile / ENCLAWED_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (status.health && status.health.staleGatewayPids.length > 0 && service.runtime?.status === "running" && typeof service.runtime.pid === "number") {
		...
		defaultRuntime.error(errorText(`Fix: run ${formatCliCommand("enclawed gateway restart")} and re-check with ${formatCliCommand("enclawed gateway status --deep")}.`));
		spacer();
	}
	...
	if (service.runtime?.cachedLabel) {
		const labelValue = resolveGatewayLaunchAgentLabel((service.command?.environment ?? process.env).ENCLAWED_PROFILE);
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("enclawed gateway install")}`));
		spacer();
	}
	...
	if (service.loaded && service.runtime?.status === "running" && status.port && status.port.status !== "busy") {
		...
		if (process.platform === "linux") {
			const unit = resolveGatewaySystemdServiceName((service.command?.environment ?? process.env).ENCLAWED_PROFILE);
