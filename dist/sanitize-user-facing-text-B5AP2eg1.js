...
import { a as parseApiErrorPayload, i as parseApiErrorInfo, n as formatRawAssistantErrorForUi, r as isCloudflareOrHtmlErrorPage, t as extractLeadingHttpStatus } from "./assistant-error-format-BGQs2PwF.js";
import { c as isTimeoutErrorMessage, i as isOverloadedErrorMessage, o as isRateLimitErrorMessage, r as isBillingErrorMessage } from "./failover-matches-GPB2UP4c.js";
...
const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_ENCLAWED_INTERNAL_CONTEXT>>>";
const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_ENCLAWED_INTERNAL_CONTEXT>>>";
...
function hasInternalRuntimeContext(text) {
	...
	return findDelimitedTokenIndex(text, "<<<BEGIN_ENCLAWED_INTERNAL_CONTEXT>>>", 0) !== -1 || text.includes(LEGACY_INTERNAL_CONTEXT_HEADER);
}
//#endregion
...
function formatDiskSpaceErrorCopy(raw) {
	if (!raw) return;
	const lower = normalizeLowercaseStringOrEmpty(raw);
	if (/\benospc\b/i.test(raw) || lower.includes("no space left on device") || lower.includes("disk full")) return "Enclawed could not write local session data because the disk is full. Free some disk space and try again.";
}
...
//#endregion
export { isExecDeniedResultText as _, formatTransportErrorCopy as a, isLikelyHttpErrorText as c, stableStringify as d, INTERNAL_RUNTIME_CONTEXT_BEGIN as f, formatExecDeniedUserMessage as g, hasInternalRuntimeContext as h, formatRateLimitOrOverloadedErrorCopy as i, isRawApiErrorPayload as l, escapeInternalRuntimeContextDelimiters as m, formatBillingErrorMessage as n, getApiErrorPayloadFingerprint as o, INTERNAL_RUNTIME_CONTEXT_END as p, formatDiskSpaceErrorCopy as r, isInvalidStreamingEventOrderError as s, BILLING_ERROR_USER_MESSAGE as t, sanitizeUserFacingText as u, parseExecApprovalResultText as v };
