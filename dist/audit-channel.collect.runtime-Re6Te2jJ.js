...
async function collectChannelSecurityFindings$1(params) {
	...
	const warnDmPolicy = async (input) => {
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + formatCliCommand("enclawed config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
