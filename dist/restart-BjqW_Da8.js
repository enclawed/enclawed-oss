...
import { o as quoteCmdScriptArg, t as cleanStaleGatewayProcessesSync } from "./restart-stale-pids-Ci1fldoQ.js";
import { a as resolveTaskScriptPath } from "./schtasks-U2b4RK50.js";
...
function relaunchGatewayScheduledTask(env = process.env) {
	...
	const scriptPath = path.join(resolvePreferredEnclawedTmpDir(), `enclawed-schtasks-restart-${randomUUID()}.cmd`);
...
function triggerEnclawedRestart() {
	...
	if (process.platform === "linux") {
		const unit = normalizeSystemdUnit(process.env.ENCLAWED_SYSTEMD_UNIT, process.env.ENCLAWED_PROFILE);
		const userArgs = [
			"--user",
			"restart",
			unit
	...
	const label = process.env.ENCLAWED_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.ENCLAWED_PROFILE);
...
//#endregion
export { markGatewaySigusr1RestartHandled as a, setPreRestartDeferralCheck as c, isGatewaySigusr1RestartExternallyAllowed as i, triggerEnclawedRestart as l, deferGatewayRestartUntilIdle as n, scheduleGatewaySigusr1Restart as o, emitGatewayRestart as r, setGatewaySigusr1RestartPolicy as s, consumeGatewaySigusr1RestartAuthorization as t };
