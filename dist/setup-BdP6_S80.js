...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-gK4mTn0s.js";
import "./config-zbBVmEjy.js";
import { t as WizardCancelledError } from "./prompts-DZ3FVXpm.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-CXEfICzu.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-CzX-Ic2c.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CJgPGZaR.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-D2lzp6O2.js");
		const { logConfigUpdated } = await import("./logging-DOiDKHwy.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-DD9-09_e.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-DjuE1Ibj.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CubP0aGH.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-DkizYGN7.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DkizYGN7.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CubP0aGH.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-BAxy1fFb.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-COfEf5bT.js");
		const { setupChannels } = await import("./onboard-channels-BIq6KAj7.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CujqEriv.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-ZBWBnLcl.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-Bdd9x8sD.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CEdd7S_R.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-Butit0sf.js");
