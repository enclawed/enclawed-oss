...
import { a as signDevicePayload, n as loadOrCreateDeviceIdentity } from "./device-identity-CEc1YLN9.js";
...
function resolveApnsRelayConfigFromEnv(env = process.env, gatewayConfig) {
	...
	const envBaseUrl = normalizeNonEmptyString$1(env.ENCLAWED_APNS_RELAY_BASE_URL);
	const configBaseUrl = normalizeNonEmptyString$1(configuredRelay?.baseUrl);
	const baseUrl = envBaseUrl ?? configBaseUrl;
	const baseUrlSource = envBaseUrl ? "ENCLAWED_APNS_RELAY_BASE_URL" : "gateway.push.apns.relay.baseUrl";
	if (!baseUrl) return {
		ok: false,
		error: "APNs relay config missing: set gateway.push.apns.relay.baseUrl or ENCLAWED_APNS_RELAY_BASE_URL"
	};
	try {
		const parsed = new URL(baseUrl);
		if (parsed.protocol !== "https:" && parsed.protocol !== "http:") throw new Error("unsupported protocol");
		if (!parsed.hostname) throw new Error("host required");
		if (parsed.protocol === "http:" && !readAllowHttp(env.ENCLAWED_APNS_RELAY_ALLOW_HTTP)) throw new Error("http relay URLs require ENCLAWED_APNS_RELAY_ALLOW_HTTP=true (development only)");
		...
		return {
			...
			value: {
				...
				timeoutMs: normalizeTimeoutMs(env.ENCLAWED_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs)
			}
		};
	} catch (err) {
		return {
...
async function resolveApnsAuthConfigFromEnv(env = process.env) {
	...
	const inlineKeyRaw = normalizeNonEmptyString(env.ENCLAWED_APNS_PRIVATE_KEY_P8) ?? normalizeNonEmptyString(env.ENCLAWED_APNS_PRIVATE_KEY);
	...
	if (!keyPath) return {
		ok: false,
		error: "APNs private key missing: set ENCLAWED_APNS_PRIVATE_KEY_P8 or ENCLAWED_APNS_PRIVATE_KEY_PATH"
	};
	try {
		return {
			ok: true,
		...
		return {
			ok: false,
			error: `failed reading ENCLAWED_APNS_PRIVATE_KEY_PATH (${keyPath}): ${formatErrorMessage(err)}`
		};
	}
}
