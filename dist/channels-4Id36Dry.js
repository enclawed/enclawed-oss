...
import { i as loadAuthProfileStore } from "./store-CFRep4YQ.js";
import "./plugins-lmHd1cqM.js";
import "./logging-1_Acw1mv.js";
import { c as resolveMessageActionDiscoveryForPlugin, r as createMessageActionDiscoveryContext } from "./message-action-discovery-xACgYzpm.js";
...
import { c as moveSingleAccountChannelSectionToDefaultAccount } from "./setup-helpers-DOTBbn4V.js";
...
import { n as formatUsageReportLines, t as loadProviderUsageSummary } from "./provider-usage-B6CI1qvc.js";
import { t as resolveCommandConfigWithSecrets } from "./command-config-resolution-Bixq89P5.js";
import { n as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-BwmdTeqk.js";
import { t as parseLogLine } from "./parse-log-line-BYNsUi0H.js";
import { r as describeBinding, t as applyAgentBindings } from "./agents.bindings-Dbc2zEgz.js";
import { n as requireValidConfigSnapshot, t as requireValidConfigFileSnapshot } from "./config-validation-M8txYvao.js";
import { t as collectChannelStatusIssues } from "./channels-status-issues-BII4w3Af.js";
import { n as buildReadOnlySourceChannelAccountSnapshot, t as buildChannelAccountSnapshot } from "./status-DEdQPQDv.js";
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-DAM3Sviw.js"), import("./onboard-channels-CdG8TgJL.js")]);
	...
	const loadScopedPlugin = async (channelId, pluginId) => {
		...
		const { loadChannelSetupPluginRegistrySnapshotForChannel } = await import("./plugin-install-DrBJ_dOI.js");
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-B-yfPcyA.js");
		if (!isCatalogChannelInstalled({
			cfg: nextConfig,
			entry: catalogEntry,
			workspaceDir
		})) {
			const { ensureChannelSetupPluginInstalled } = await import("./plugin-install-DrBJ_dOI.js");
			const prompter = createClackPrompter();
	...
	if (afterAccountConfigWritten) {
		const { runCollectedChannelOnboardingPostWriteHooks } = await import("./onboard-channels-CdG8TgJL.js");
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = !useWizard && rawChannel && (!channel || !getChannelPlugin(channel)) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-CIfxjGrp.js");
