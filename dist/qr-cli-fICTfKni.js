...
import { a as hasConfiguredSecretInput, d as resolveSecretInputRef, l as normalizeSecretInputString } from "./types.secrets-B-g1z55T.js";
...
import { r as resolveRequiredConfiguredSecretRefInputString } from "./resolve-configured-secret-input-string-DJikbSTU.js";
import { i as pickMatchingExternalInterfaceAddress, o as safeNetworkInterfaces } from "./tailnet-_A2c3oXt.js";
import { a as isCarrierGradeNatIpv4Address, c as isIpv6Address, f as isRfc1918Ipv4Address, m as parseCanonicalIpAddress, s as isIpv4Address } from "./ip-BWffDmx-.js";
import { a as isLoopbackHost, c as isSecureWebSocketUrl } from "./net-SK24IRt9.js";
...
import { i as getQrRemoteCommandSecretTargetIds } from "./command-secret-targets-BhK2K9pt.js";
import { n as resolveGatewayBindUrl, t as resolveTailnetHostWithRunner } from "./tailscale-status-C1YaIByP.js";
import { n as materializeGatewayAuthSecretRefs } from "./auth-config-utils-DNF-RREh.js";
import { t as PAIRING_SETUP_BOOTSTRAP_PROFILE } from "./device-bootstrap-profile-z_9L1_Lj.js";
import { i as issueDeviceBootstrapToken } from "./device-bootstrap-BhmzzHF2.js";
...
function shouldResolveLocalGatewayPasswordSecret(cfg, env) {
	if (trimToUndefined(env.ENCLAWED_GATEWAY_PASSWORD)) return false;
...
function registerQrCli(program) {
	program.command("qr").description("Generate a mobile pairing QR code and setup code").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.enclawed.ai/cli/qr")}\n`).option("--remote", "Use gateway.remote.url and gateway.remote token/password (ignores device-pair publicUrl)", false).option("--url <url>", "Override gateway URL used in the setup payload").option("--public-url <url>", "Override gateway public URL used in the setup payload").option("--token <token>", "Override gateway token for setup payload").option("--password <password>", "Override gateway password for setup payload").option("--setup-code-only", "Print only the setup code", false).option("--no-ascii", "Skip ASCII QR rendering").option("--json", "Output JSON", false).action(async (opts) => {
		try {
			...
			lines.push(`${theme.muted("Setup code:")} ${setupCode}`, `${theme.muted("Gateway:")} ${resolved.payload.url}`, `${theme.muted("Auth:")} ${resolved.authLabel}`, `${theme.muted("Source:")} ${resolved.urlSource}`, "", "Approve after scan with:", `  ${theme.command("enclawed devices list")}`, `  ${theme.command("enclawed devices approve <requestId>")}`);
