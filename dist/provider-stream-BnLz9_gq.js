...
import { N as resolveProviderStreamFn } from "./provider-runtime-DB4wLsLH.js";
import { a as coerceTransportToolCallArguments, c as failTransportStream, f as sanitizeTransportPayloadText, i as createOpenAIResponsesTransportStreamFn, l as finalizeTransportStream, m as buildGuardedModelFetch, n as createAzureOpenAIResponsesTransportStreamFn, o as createEmptyTransportUsage, p as transformTransportMessages, r as createOpenAICompletionsTransportStreamFn, s as createWritableTransportEventStream, u as mergeTransportHeaders } from "./openai-transport-stream-BAYkDZaP.js";
import { A as applyAnthropicPayloadPolicyToParams, j as resolveAnthropicPayloadPolicy, y as stripNullHeaderValues } from "./provider-stream-shared-CP0ypOLf.js";
...
import { i as getModelProviderRequestTransport } from "./provider-request-config-DpuK1s8u.js";
import { t as Anthropic } from "./sdk-Bfw2wS5n.js";
import { t as parseGeminiAuth } from "./gemini-auth-BbYajsCG.js";
import { calculateCost, getApiProvider, getEnvApiKey, parseStreamingJson, registerApiProvider } from "@mariozechner/pi-ai/compat";
	...
	if (model.provider === "github-copilot") {
		...
		return {
			client: new Anthropic({
				...
				}), stripNullHeaderValues(options?.headers)),
				fetch
			}),
			isOAuthToken: false
		};
	...
	if (isAnthropicOAuthToken(apiKey)) return {
		client: new Anthropic({
			...
			defaultHeaders: mergeTransportHeaders({
				...
				"x-app": "cli"
			}, model.headers, stripNullHeaderValues(options?.headers)),
			fetch
		}),
		isOAuthToken: true
	};
		...
		(async () => {
			...
			try {
				...
				const response = await fetch(buildGoogleRequestUrl(model), {
					method: "POST",
					headers: buildGoogleHeaders(model, apiKey, stripNullHeaderValues(options?.headers)),
