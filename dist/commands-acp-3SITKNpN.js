...
import { a as requireGatewayClientScopeForInternalChannel } from "./command-gates-BCr5R7qD.js";
import { C as stopWithText, b as resolveAcpAction, d as COMMAND, x as resolveAcpHelpText } from "./shared-BXAOv6HB.js";
...
async function loadAcpActionHandler(action) {
	if (action === "spawn" || action === "cancel" || action === "steer" || action === "close") {
		lifecycleHandlersPromise ??= import("./lifecycle-DJ8Ntk8r.js");
	...
	if (action === "status" || action === "set-mode" || action === "set" || action === "cwd" || action === "permissions" || action === "timeout" || action === "model" || action === "reset-options") {
		runtimeOptionHandlersPromise ??= import("./runtime-options-i3Sh8m_N.js");
	...
	diagnosticHandlersPromise ??= import("./diagnostics-DK-QTywl.js");
