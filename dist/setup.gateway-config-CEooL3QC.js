import { d as resolveSecretInputRef, l as normalizeSecretInputString } from "./types.secrets-BpVPfGSB.js";
...
import { t as resolveSecretInputModeForEnvSelection } from "./provider-auth-mode-BUXYUkdf.js";
import { n as promptSecretRefForSetup } from "./provider-auth-ref-BNW4UpzB.js";
import { t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-CZnYCqwa.js";
import { c as normalizeGatewayTokenInput, p as validateGatewayPasswordInput } from "./onboard-helpers-DL_wxoBY.js";
...
async function configureGatewayForSetup(opts) {
	...
	if (authMode === "token") {
		...
		if ((flow === "quickstart" && opts.secretInputMode !== "ref" ? quickstartTokenRef ? "ref" : "plaintext" : await resolveSecretInputModeForEnvSelection({
			...
			gatewayToken = (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.ENCLAWED_GATEWAY_TOKEN)) || randomToken();
			gatewayTokenInput = gatewayToken;
		} else {
			gatewayToken = normalizeGatewayTokenInput(await prompter.text({
				message: "Gateway token (blank to generate)",
				placeholder: "Needed for multi-machine or non-loopback access",
				initialValue: quickstartTokenString ?? normalizeGatewayTokenInput(process.env.ENCLAWED_GATEWAY_TOKEN) ?? ""
			})) || randomToken();
			gatewayTokenInput = gatewayToken;
		}
	}
