...
import { r as STATE_DIR } from "./paths-CDjhyzOH.js";
...
import { c as expandToolGroups, l as normalizeToolName } from "./tool-policy-Bkov0fJO.js";
...
const DEFAULT_SANDBOX_IMAGE = "enclawed-sandbox:bookworm-slim";
...
const DEFAULT_SANDBOX_BROWSER_IMAGE = "enclawed-sandbox-browser:bookworm-slim";
const DEFAULT_SANDBOX_COMMON_IMAGE = "enclawed-sandbox-common:bookworm-slim";
...
function resolveSandboxBrowserDockerCreateConfig(params) {
	...
	const base = {
		...params.docker,
		network: browserNetwork || "enclawed-sandbox-browser",
...
function resolveSandboxDockerConfig(params) {
	...
	return {
		image: agentDocker?.image ?? globalDocker?.image ?? "enclawed-sandbox:bookworm-slim",
		containerPrefix: agentDocker?.containerPrefix ?? globalDocker?.containerPrefix ?? "enclawed-sbx-",
...
function resolveSandboxBrowserConfig(params) {
	...
	return {
		...
		image: agentBrowser?.image ?? globalBrowser?.image ?? "enclawed-sandbox-browser:bookworm-slim",
		containerPrefix: agentBrowser?.containerPrefix ?? globalBrowser?.containerPrefix ?? "enclawed-sbx-browser-",
		network: agentBrowser?.network ?? globalBrowser?.network ?? "enclawed-sandbox-browser",
