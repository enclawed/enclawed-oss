...
var CardDavTransport = class {
		...
		try {
			switch (name) {
				...
				case "add_contact": return {
					ok: true,
					result: await this.addContact(args)
				};
				default: return {
					ok: false,
	...
	*/
	async addContact(args) {
		...
		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error(`add_contact: "${email}" is not a valid email address`);
		const client = await this.getClient();
		const book = await this.resolvePrimaryBook(client);
		const uid = vcardUid();
		const filename = `${uid}.vcf`;
		const vCardString = renderVCard({
			uid,
			name,
			email,
			relationship
		...
		if (!resp.ok) throw new Error(`createVCard ${resp.status} ${resp.statusText}`);
		return {
			uid,
			href: resp.headers.get("location") ?? `${book.url}${filename}`
		};
	}
};
function vcardUid() {
	return Array.from(globalThis.crypto.getRandomValues(new Uint8Array(8))).map((b) => b.toString(16).padStart(2, "0")).join("");
}
function renderVCard(input) {
	...
	if (input.relationship) lines.push(`NOTE:${esc(input.relationship)}`);
	lines.push("END:VCARD");
	return lines.join("\r\n") + "\r\n";
}
//#endregion
//#region extensions/mcp-carddav/src/index.ts
const CARDDAV_TOOLS = Object.freeze([
	"search_contacts",
	"list_contacts",
	"add_contact"
]);
