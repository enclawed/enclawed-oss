...
import { i as resolveWideAreaDiscoveryDomain, t as getWideAreaZonePath } from "./widearea-dns-CgOhJJq2.js";
...
function registerDnsCli(program) {
	program.command("dns").description("DNS helpers for wide-area discovery (Tailscale + CoreDNS)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.enclawed.ai/cli/dns")}\n`).command("setup").description("Set up CoreDNS to serve your discovery domain for unicast DNS-SD (Wide-Area Bonjour)").option("--domain <domain>", "Wide-area discovery domain (e.g. enclawed.internal)").option("--apply", "Install/update CoreDNS config and (re)start the service (requires sudo)", false).action(async (opts) => {
		...
		defaultRuntime.log("");
		defaultRuntime.log(theme.heading("Recommended config ($ENCLAWED_CONFIG_PATH, default ~/.enclawed/enclawed.json):"));
		...
		if (cfg.discovery?.wideArea?.enabled !== true) {
			defaultRuntime.log("");
			defaultRuntime.log(theme.muted("Note: enable discovery.wideArea.enabled in the active Enclawed config ($ENCLAWED_CONFIG_PATH, default ~/.enclawed/enclawed.json) on the gateway and restart the gateway so it writes the DNS-SD zone."));
		}
	});
}
//#endregion
