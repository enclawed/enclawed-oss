...
import { d as pathExists, g as resolveUserPath } from "./utils-BcQ7uNrT.js";
import { n as resolveCliName } from "./cli-name-B67WgIx7.js";
...
import { r as DEFAULT_BOOTSTRAP_FILENAME } from "./workspace-dcdTJd2I.js";
import { i as isSystemdUserServiceAvailable } from "./systemd-C_DkmzYn.js";
...
import { n as listConfiguredWebSearchProviders } from "./runtime-DwQynVDO.js";
...
import { n as openUrl, t as detectBrowserOpenSupport } from "./browser-open-D2OecYAS.js";
import { i as formatControlUiSshHint, m as waitForGatewayReachable, u as probeGatewayReachable } from "./onboard-helpers-CHJZcheF.js";
...
import { t as runTui } from "./tui-dbd37fM6.js";
...
async function finalizeSetupWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger--Y758IoI.js");
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control Enclawed." : "Copy/paste this URL in a browser on this machine to control Enclawed.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("enclawed dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.enclawed.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.enclawed.ai/security", "Security");
	...
	if (webSearchProvider) {
		const { resolveExistingKey, hasExistingKey, hasKeyInEnv } = await import("./onboard-search-7moanDAk.js");
		...
		const { hasExistingKey, hasKeyInEnv } = await import("./onboard-search-7moanDAk.js");
		const legacyDetected = configuredSearchProviders.find((e) => hasExistingKey(nextConfig, e.id) || hasKeyInEnv(e));
		if (legacyDetected) await prompter.note([`Web search is available via ${legacyDetected.label} (auto-detected).`, "Docs: https://docs.enclawed.ai/tools/web"].join("\n"), "Web search");
	...
	await prompter.note("What now: https://enclawed.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "Onboarding complete. Dashboard opened; keep that tab to control Enclawed." : "Onboarding complete. Use the dashboard link above to control Enclawed.");
	return { launchedTui };
}
//#endregion
