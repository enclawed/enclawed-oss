...
import { s as ensureDir, t as CONFIG_DIR } from "./utils-CrVQlOZJ.js";
...
function resolveWideAreaDiscoveryDomain(params) {
	...
	return normalizeWideAreaDomain(params?.configDomain ?? env.ENCLAWED_WIDE_AREA_DOMAIN ?? null);
}
...
function extractContentHash(zoneText) {
	return zoneText.match(/^\s*;\s*enclawed-content-hash:\s*(\S+)\s*$/m)?.[1] ?? null;
}
...
function renderZone(opts) {
	...
	return `; enclawed-content-hash: ${computeContentHash(`${records.map((line) => line === soaLine ? `@ IN SOA ns1 hostmaster SERIAL 7200 3600 1209600 60` : line).join("\n")}\n`)}\n${contentBody}`;
}
