...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-Dzvn-wpi.js";
import "./config-DOKACpaG.js";
import { t as WizardCancelledError } from "./prompts-CfYXJ8rO.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-CcPrdmqQ.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-B0BvHBw5.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-C6uRY72v.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BPK02J-j.js");
		const { logConfigUpdated } = await import("./logging-BCYZcbDn.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-TR7XlIXd.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CZS7S4DH.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-AsCHu1q1.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DAjNgh6P.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-BHtnJSJE.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-BHtnJSJE.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DAjNgh6P.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CIhLKbbN.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-D0v238xT.js");
		const { setupChannels } = await import("./onboard-channels-BVqCpHME.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-7moanDAk.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BgesUZAR.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-DFIkYLA-.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CjdA9hOG.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-C1YsKxhV.js");
