import { d as pathExists, u as normalizeE164 } from "./utils-BcQ7uNrT.js";
...
import { J as setSetupChannelEnabled, Q as splitSetupEntries, y as normalizeAllowFromEntries } from "./setup-wizard-helpers-A7VVNxLn.js";
...
import { a as resolveWhatsAppAccount, o as resolveWhatsAppAuthDir } from "./accounts-GA33UQfL.js";
import { i as formatWhatsAppWebAuthStatusState, m as readWebAuthState } from "./auth-store-kaif7PfY.js";
...
async function promptWhatsAppOwnerAllowFrom(params) {
	...
	await prompter.note("We need the sender/owner number so Enclawed can allowlist you.", "WhatsApp number");
...
async function promptWhatsAppDmAccess(params) {
	...
	if (await params.prompter.select({
		...
		options: [{
			...
			value: "separate",
			label: "Separate phone just for Enclawed"
		}]
...
async function finalizeWhatsAppSetup(params) {
	...
	if (await params.prompter.confirm({
		...
		const { loginWeb } = await import("./login-BjLfxDff.js");
	...
	else if (!linked) await params.prompter.note(`Run \`${formatCliCommand("enclawed channels login")}\` later to link WhatsApp.`, "WhatsApp");
