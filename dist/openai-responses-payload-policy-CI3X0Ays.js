...
import { n as isOpenAIResponsesApi } from "./provider-attribution-CSlLJnzJ.js";
...
function readSupportsStoreFlag(compat) {
	if (compat && typeof compat === "object" && "supportsStore" in compat) {
		const value = compat.supportsStore;
		if (typeof value === "boolean") return value;
	}
}
function narrowSupportsStoreCompat(compat) {
	if (!compat || typeof compat !== "object") return;
	const record = compat;
	const out = {};
	if (typeof record.supportsStore === "boolean") out.supportsStore = record.supportsStore;
	if (typeof record.supportsPromptCacheKey === "boolean") out.supportsPromptCacheKey = record.supportsPromptCacheKey;
	return out;
}
...
function resolveOpenAIResponsesPayloadPolicy(model, options = {}) {
	...
	return {
		...
		shouldStripStore: explicitStore !== true && readSupportsStoreFlag(model.compat) === false && isResponsesApi,
