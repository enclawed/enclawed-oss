...
import { n as info, t as danger } from "./globals-CYDryU7g.js";
import { n as runCommandWithRuntime } from "./cli-utils-BR0c6z6r.js";
import { f as redactCdpUrl } from "./cdp.helpers-Dodvjzj3.js";
...
async function runBrowserDoctor(parent, profile, deep) {
	...
	checks.push({
		name: "browser",
		ok: status.running,
		detail: status.running ? `running${status.cdpReady === false ? ", CDP not ready" : ""}` : "not running; run `enclawed browser start`"
	});
	try {
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("create-profile").description("Create a new browser profile").requiredOption("--name <name>", "Profile name (lowercase, numbers, hyphens)").option("--color <hex>", "Profile color (hex format, e.g. #0066CC)").option("--cdp-url <url>", "CDP URL for remote Chrome (http/https)").option("--user-data-dir <path>", "User data dir for existing-session Chromium attach").option("--driver <driver>", "Profile driver (enclawed|existing-session). Default: enclawed").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		await runBrowserCommand(async () => {
			const result = await callBrowserRequest(parent, {
				method: "POST",
