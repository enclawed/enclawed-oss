...
import { t as CONFIG_DIR } from "./utils-BcQ7uNrT.js";
import { t as createSubsystemLogger } from "./subsystem-WGqvYWrS.js";
import { a as isLoopbackHost } from "./net-SK24IRt9.js";
import { r as ensurePortAvailable } from "./ports-CNWE5-2x.js";
import { t as normalizeHostname } from "./hostname-B-ACgZ7V.js";
import { d as isPrivateNetworkAllowedByPolicy, f as matchesHostnameAllowlist, h as resolvePinnedHostnameWithPolicy } from "./ssrf-DQDx1s1G.js";
import "./text-runtime-CAI4yT_W.js";
import { t as prepareOomScoreAdjustedSpawn } from "./process-runtime-C6o-Q4Vf.js";
import "./sdk-security-runtime-DQ2o4Mo0.js";
import { I as DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME, S as CHROME_STOP_TIMEOUT_MS, _ as CHROME_BOOTSTRAP_EXIT_TIMEOUT_MS, c as isWebSocketUrl, f as redactCdpUrl, i as fetchJson, l as normalizeCdpHttpBaseForJsonEndpoints, n as assertCdpEndpointAllowed, p as withCdpSocket, r as fetchCdpChecked, s as isDirectCdpWebSocketEndpoint, t as appendCdpPath, u as openCdpWebSocket, v as CHROME_BOOTSTRAP_PREFS_TIMEOUT_MS, x as CHROME_STDERR_HINT_MAX_CHARS, y as CHROME_LAUNCH_READY_WINDOW_MS } from "./cdp.helpers-Dodvjzj3.js";
import { a as BrowserProfileUnavailableError, t as BrowserCdpEndpointBlockedError } from "./errors-CIFawFUF.js";
import { a as DEFAULT_DOWNLOAD_DIR, r as resolveManagedBrowserHeadlessMode, t as getManagedBrowserMissingDisplayError } from "./config-D7LuOO2G.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-B5bjM8O0.js";
import "./subsystem-CoKrvbmn.js";
...
function resolveBrowserNavigationProxyMode(params) {
	if (params.profile.driver === "enclawed" && params.profile.cdpIsLoopback && hasExplicitChromeProxyRoutingArg(params.resolved.extraArgs)) return "explicit-browser-proxy";
	return "direct";
}
//#endregion
...
*/
function decorateEnclawedProfile(userDataDir, opts) {
	const desiredName = opts?.name ?? "enclawed";
...
function chromeLaunchHints(params) {
	...
	if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) hints.push("No DISPLAY/X server was detected. Set ENCLAWED_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.");
	if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) hints.push(`The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.enclawed/browser/${params.profile.name}/user-data.`);
...
function resolveEnclawedUserDataDir(profileName = DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME) {
	return path.join(CONFIG_DIR, "browser", profileName, "user-data");
}
function cdpUrlForPort(cdpPort) {
	return `http://127.0.0.1:${cdpPort}`;
}
function buildEnclawedChromeLaunchArgs(params) {
...
async function launchEnclawedChrome(resolved, profile, launchOptions = {}) {
	...
	const launchOnceAndWait = async (allowSingletonRecovery) => {
		...
		try {
			...
			log.info(`🦞 enclawed browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`);
			return {
				pid,
				exe,
				userDataDir,
...
async function stopEnclawedChrome(running, timeoutMs = CHROME_STOP_TIMEOUT_MS) {
...
//#endregion
export { assertBrowserNavigationResultAllowed as C, assertBrowserNavigationRedirectChainAllowed as S, withBrowserNavigationPolicy as T, INTERACTIVE_ROLES as _, resolveEnclawedUserDataDir as a, InvalidBrowserNavigationUrlError as b, formatChromeCdpDiagnostic as c, createTargetViaCdp as d, formatAriaSnapshot as f, CONTENT_ROLES as g, snapshotRoleViaCdp as h, launchEnclawedChrome as i, AX_REF_PATTERN as l, snapshotAria as m, isChromeCdpReady as n, stopEnclawedChrome as o, normalizeCdpWsUrl as p, isChromeReachable as r, diagnoseChromeCdp as s, getChromeWebSocketUrl as t, captureScreenshot as u, STRUCTURAL_ROLES as v, requiresInspectableBrowserNavigationRedirectsForUrl as w, assertBrowserNavigationAllowed as x, resolveBrowserNavigationProxyMode as y };
