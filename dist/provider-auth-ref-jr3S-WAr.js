...
import { s as isValidEnvSecretRefId } from "./types.secrets-B-g1z55T.js";
import { a as formatExecSecretRefIdValidationMessage, l as resolveDefaultSecretProviderAlias, o as isValidExecSecretRefId, s as isValidFileSecretRefId } from "./ref-contract-B_vjonjo.js";
import { t as encodeJsonPointerToken } from "./json-pointer-kmcD6c-k.js";
...
function loadSecretResolve() {
	secretResolvePromise ??= import("./resolve-DHRabqBN.js");
...
async function promptEnvSecretRefForSetup(params) {
	...
	await params.prompter.note(params.copy?.envValidatedMessage?.(envVar) ?? `Validated environment variable ${envVar}. Enclawed will store a reference, not the key value.`, "Reference validated");
	return {
		ref,
		resolvedValue
	};
...
async function promptProviderSecretRefForSetup(params) {
	...
	try {
		...
		await params.prompter.note(params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ?? `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Enclawed will store a reference, not the key value.`, "Reference validated");
		return {
			ref,
			resolvedValue
		};
