...
import { r as resolveWorkspaceDirInfo, t as logWorkspaceDirResolution } from "./workspace-dir-CwLpjIOc.js";
...
if (!isMainModule({
	...
	try {
		...
		const { logInfo } = await import("./logger-L99QmbH9.js");
		logWorkspaceDirResolution(resolution, (msg) => logInfo(msg));
	} catch {}
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-CrbpQrwo.js");
	await installGaxiosFetchCompat();
	const { bootstrapEnclawed } = await import("./bootstrap-Bvbl876_.js");
	const { resolveBundledPluginsDir } = await import("./bundled-dir-Dt7lI-xU.js");
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-DvE3wY8s.js"), import("./git-commit-f5kKkX3Q.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-metadata-Cbvq5DrT.js").then(async ({ outputPrecomputedRootHelpText }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-By3hbIRl.js").then(({ runCli }) => runCli(argv)).catch((error) => {
