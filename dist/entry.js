...
if (!isMainModule({
	...
	try {
		...
		const { logInfo } = await import("./logger-Cw0feCig.js");
		logWorkspaceDirResolution(resolution, (msg) => logInfo(msg));
	} catch {}
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-Jx4z2hIT.js");
	await installGaxiosFetchCompat();
	const { bootstrapEnclawed } = await import("./bootstrap-Bz6g0E-N.js");
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-vVis1eEz.js"), import("./git-commit-DxWQ58k6.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-metadata-BS1GeSoo.js").then(async ({ outputPrecomputedRootHelpText }) => {
...
function runMainOrRootHelp(argv) {
	...
	import("./run-main-9LhJbQAD.js").then(({ runCli }) => runCli(argv)).catch((error) => {
