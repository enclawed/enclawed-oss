...
import { o as resolveConfigPath, u as resolveGatewayPort } from "./paths-CDjhyzOH.js";
...
function buildGatewayConnectionDetailsWithResolvers(options = {}, resolvers = {}) {
	...
	const envUrlOverride = cliUrlOverride ? void 0 : normalizeOptionalString(process.env.ENCLAWED_GATEWAY_URL);
	...
	const urlSource = urlOverride ? urlSourceHint === "env" ? "env ENCLAWED_GATEWAY_URL" : "cli --url" : remoteUrl ? "config gateway.remote.url" : remoteMisconfigured ? "missing gateway.remote.url (fallback local)" : "local loopback";
	...
	if (!isSecureWebSocketUrl(url, { allowPrivateWs })) throw new Error([
		...
		allowPrivateWs ? void 0 : "Break-glass (trusted private networks only): set ENCLAWED_ALLOW_INSECURE_PRIVATE_WS=1",
