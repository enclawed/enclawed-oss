...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-CXz21W7w.js";
import "./config-D3uukyZR.js";
import { t as WizardCancelledError } from "./prompts-DC9uBmdr.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-BSwGNVSk.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-CQQ25dA-.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-0oJKb3qA.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-ZzoTRAK8.js");
		const { logConfigUpdated } = await import("./logging-CMS8hSaG.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-DnwagJAQ.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-DUIvhQI5.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-CVgApMQM.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DeNte2hF.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-CgMpO1_Y.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-CgMpO1_Y.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DeNte2hF.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-DqBIUvOZ.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-BFSDmYX4.js");
		const { setupChannels } = await import("./onboard-channels-D7JGW_VJ.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-Br-qQNfq.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-D6z2RtUf.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-DKgCwRK8.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-BjCN5oFu.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-ByqXRA4D.js");
