...
import { a as parseSemver } from "./runtime-guard-DVKiBAxK.js";
import { t as resolveStableNodePath } from "./stable-node-path-DoLMatqs.js";
...
import { I as applyPathPrepend } from "./bash-tools.exec-runtime-Cbax6svY.js";
import { t as BUNDLED_RUNTIME_SIDECAR_PATHS } from "./runtime-sidecar-paths-mbgNwm7V.js";
import { i as resolveControlUiDistIndexPathForRoot, r as resolveControlUiDistIndexHealth } from "./control-ui-assets-DQSwUaZR.js";
import { f as isBetaTag, l as DEV_BRANCH, n as compareSemverStrings, p as isStableTag, s as detectPackageManager, u as channelToNpmTag } from "./update-check-CBD-B9DA.js";
...
const NPM_UPDATE_COMPAT_SIDECAR_PATHS = new Set([{
	...
	content: "// Compatibility stub for older Enclawed updaters. The QA channel implementation is not packaged.\nexport {};\n"
}, {
	path: "dist/extensions/qa-lab/runtime-api.js",
	content: "// Compatibility stub for older Enclawed updaters. The QA lab implementation is not packaged.\nexport {};\n"
...
const ENCLAWED_MAIN_PACKAGE_SPEC = "github:enclawed/enclawed#main";
...
async function collectCriticalInstalledPackageDistPaths(packageRoot) {
	...
	await Promise.all(BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
		...
		if (await pathExists(path.join(packageRoot, pluginRoot, "package.json")) || await pathExists(path.join(packageRoot, pluginRoot, "enclawed.plugin.json"))) expectedFiles.add(relativePath);
	}));
	return [...expectedFiles].toSorted((left, right) => left.localeCompare(right));
}
...
function resolveGlobalInstallSpec(params) {
	const override = params.env?.ENCLAWED_UPDATE_PACKAGE_SPEC?.trim() || process.env.ENCLAWED_UPDATE_PACKAGE_SPEC?.trim();
	if (override) return override;
	const target = normalizePackageTarget(params.tag);
	if (isMainPackageTarget(target)) return ENCLAWED_MAIN_PACKAGE_SPEC;
...
async function bootstrapPnpmViaNpm(params) {
	const tempRoot = await fs$1.mkdtemp(path.join(os.tmpdir(), "enclawed-update-pnpm-"));
	const cleanup = async () => {
...
const PREFLIGHT_TEMP_PREFIX = process.platform === "win32" ? "ocu-pf-" : "enclawed-update-preflight-";
...
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && await pathsReferToSameLocation(gitRoot, pkgRoot)) {
		...
		try {
			...
			const doctorStep = await runStep(step("enclawed doctor", [
				await resolveStableNodePath(process.execPath),
				doctorEntry,
				"doctor",
				"--non-interactive",
				"--fix"
			], gitRoot, { ENCLAWED_UPDATE_IN_PROGRESS: "1" }));
