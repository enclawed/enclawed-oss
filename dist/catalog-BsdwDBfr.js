...
import { t as listChannelCatalogEntries } from "./channel-catalog-registry-CW2s1IvG.js";
import { r as resolveChannelExposure } from "./exposure-CJLYbfjs.js";
...
function resolveOfficialCatalogPaths(options) {
	...
	const candidates = [resolveEnclawedPackageRootSync({ cwd: process.cwd() }), resolveEnclawedPackageRootSync({ moduleUrl: import.meta.url })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index).map((packageRoot) => path.join(packageRoot, OFFICIAL_CHANNEL_CATALOG_RELATIVE_PATH));
	if (process.execPath) {
