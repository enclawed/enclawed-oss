...
import { d as setConsoleTimestampPrefix, t as createSubsystemLogger, u as setConsoleSubsystemFilter } from "./subsystem-WGqvYWrS.js";
import { t as formatDocsLink } from "./links-CjFnnUDy.js";
import { n as isRich, r as theme, t as colorize } from "./theme-hNdBadll.js";
...
import { _ as resolveStateDir, t as CONFIG_PATH, u as resolveGatewayPort } from "./paths-CDjhyzOH.js";
...
import { a as hasConfiguredSecretInput } from "./types.secrets-B-g1z55T.js";
import "./globals-CYDryU7g.js";
import { n as isContainerEnvironment, t as defaultGatewayBindMode } from "./net-SK24IRt9.js";
import { o as resolveGatewayAuth } from "./auth-B8XaTaMr.js";
import { a as loadConfig, g as writeConfigFile, l as readConfigFileSnapshot, r as createConfigIO } from "./io-BPigC1a6.js";
import { _ as resolveDefaultAgentWorkspaceDir, y as resolveWorkspaceTemplateDir } from "./workspace-BOVAZCiV.js";
import { t as cleanStaleGatewayProcessesSync } from "./restart-stale-pids-Ci1fldoQ.js";
...
import { t as parsePort } from "./parse-port-CnZutYBl.js";
import "./config-BDd-c7HD.js";
import { g as GATEWAY_CLIENT_NAMES, h as GATEWAY_CLIENT_MODES } from "./message-channel-CP7lSRvA.js";
import { a as getActiveTaskCount, c as markGatewayDraining, d as waitForActiveTasks, l as resetAllLanes } from "./command-queue-DMUvsGAm.js";
import { r as callGateway } from "./call-DtxbZweG.js";
import { a as markGatewaySigusr1RestartHandled, i as isGatewaySigusr1RestartExternallyAllowed, l as triggerEnclawedRestart, o as scheduleGatewaySigusr1Restart, t as consumeGatewaySigusr1RestartAuthorization } from "./restart-BjqW_Da8.js";
import { d as waitForActiveEmbeddedRuns, r as getActiveEmbeddedRunCount, t as abortEmbeddedPiRun } from "./runs-cE9sGCqK.js";
import { n as parseTimeoutMsWithFallback } from "./parse-timeout-rS8x7MdD.js";
import { n as withProgress } from "./progress-CDY9v3hd.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-3MDFJqY_.js";
import { t as formatHelpExamples } from "./help-format-02fn-Ugc.js";
import { n as runCommandWithRuntime } from "./cli-utils-BR0c6z6r.js";
import { t as readSecretFromFile } from "./secret-file-BydFFI6c.js";
import { n as buildGatewayDiscoveryTarget } from "./gateway-discovery-targets-DOENGoLp.js";
import { o as resolveControlUiRootSync } from "./control-ui-assets-DBgVwg6a.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-isNKDKw7.js";
import { n as forceFreePortAndWait, r as waitForPortBindable } from "./ports-BbdPu8a4.js";
import { o as handleReset } from "./onboard-helpers-DnqWC8bO.js";
...
const resolveDevWorkspaceDir = (env = process.env) => {
	...
	if (normalizeOptionalLowercaseString(env.ENCLAWED_PROFILE) === "dev") return baseDir;
...
async function ensureDevWorkspace(dir) {
	...
	const [agents, soul, tools, identity, user] = await Promise.all([
		loadDevTemplate("AGENTS.dev.md", `# AGENTS.md - Enclawed Dev Workspace\n\nDefault dev workspace for enclawed gateway --dev.\n`),
...
function restartGatewayProcessWithFreshPid() {
	if (isTruthy(process.env.ENCLAWED_NO_RESPAWN)) return { mode: "disabled" };
...
async function runGatewayLoop(params) {
	...
	const handleRestartAfterServerClose = async () => {
		...
		else gatewayLog$1.info(`restart mode: in-process restart (${respawn.detail ?? "ENCLAWED_NO_RESPAWN"})`);
...
function renderGatewayServiceStopHints(env = process.env) {
	...
	switch (process.platform) {
		case "darwin": return [`Tip: ${formatCliCommand("enclawed gateway stop")}`, `Or: launchctl bootout gui/$UID/${resolveGatewayLaunchAgentLabel(profile)}`];
		case "linux": return [`Tip: ${formatCliCommand("enclawed gateway stop")}`, `Or: systemctl --user stop ${resolveGatewaySystemdServiceName(profile)}.service`];
		case "win32": return [`Tip: ${formatCliCommand("enclawed gateway stop")}`, `Or: schtasks /End /TN "${resolveGatewayWindowsTaskName(profile)}"`];
		default: return [`Tip: ${formatCliCommand("enclawed gateway stop")}`];
	}
}
...
function warnInlinePasswordFlag() {
	defaultRuntime.error("Warning: --password can be exposed via process listings. Prefer --password-file or ENCLAWED_GATEWAY_PASSWORD.");
}
...
function getGatewayStartGuardErrors(params) {
	...
	if (!params.configExists) return [`Missing config. Run \`${formatCliCommand("enclawed setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`];
...
async function runGatewayCommand$1(opts) {
	...
	if (opts.rawStream) process.env.ENCLAWED_RAW_STREAM = "1";
	const rawStreamPath = toOptionString(opts.rawStreamPath);
	if (rawStreamPath) process.env.ENCLAWED_RAW_STREAM_PATH = rawStreamPath;
	const { startGatewayServer } = await withProgress({
		label: "Loading gateway modules…",
		indeterminate: true
	}, async () => import("./server-BPfDHDx2.js"));
	...
	if (opts.token) {
		...
		if (token) process.env.ENCLAWED_GATEWAY_TOKEN = token;
	}
	...
	if (bind !== "loopback" && !hasSharedSecret && !canBootstrapToken && resolvedAuthMode !== "trusted-proxy") {
		defaultRuntime.error([
			...
			...isContainerEnvironment() ? ["Container environment detected — the gateway defaults to bind=auto (0.0.0.0) for port-forwarding compatibility.", "Set ENCLAWED_GATEWAY_TOKEN or ENCLAWED_GATEWAY_PASSWORD, or pass --token/--password to start with auth."] : ["Set gateway.auth.token/password (or ENCLAWED_GATEWAY_TOKEN/ENCLAWED_GATEWAY_PASSWORD) or pass --token/--password."],
			...authHints
		].filter(Boolean).join("\n"));
		defaultRuntime.exit(EXIT_CONFIG_ERROR);
		return;
...
function loadConfigModule() {
	configModulePromise ??= import("./read-best-effort-config.runtime-DL6hDGCf.js");
	return configModulePromise;
}
function loadGatewayStatusModule() {
	gatewayStatusModulePromise ??= import("./gateway-status-L5s8LkhU.js");
	return gatewayStatusModulePromise;
}
function loadGatewayHealthModule() {
	gatewayHealthModulePromise ??= import("./health-u3BCGQMq.js");
	return gatewayHealthModulePromise;
}
function loadBonjourDiscoveryModule() {
	bonjourDiscoveryModulePromise ??= import("./bonjour-discovery-D-Mizz5H.js");
	return bonjourDiscoveryModulePromise;
}
function loadWideAreaDnsModule() {
	wideAreaDnsModulePromise ??= import("./widearea-dns-u5PsCh_9.js");
	return wideAreaDnsModulePromise;
}
function loadHealthStyleModule() {
	healthStyleModulePromise ??= import("./health-style-05ITTkkL.js");
	return healthStyleModulePromise;
}
function loadUsageFormatModule() {
	usageFormatModulePromise ??= import("./usage-format-1DKdGuVC.js");
