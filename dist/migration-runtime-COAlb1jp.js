import { g as redactMigrationPlan, l as markMigrationItemConflict, n as MIGRATION_REASON_TARGET_EXISTS, t as MIGRATION_REASON_MISSING_SOURCE_OR_TARGET, u as markMigrationItemError } from "./migration-DOSnICX3.js";
...
function withCachedMigrationConfigRuntime(runtime, fallbackConfig) {
	...
	if (!currentFn || !mutateFn) return runtime;
	const replaceFn = configApi?.replaceConfigFile;
	let cachedConfig;
	const current = () => {
		cachedConfig ??= structuredClone(currentFn() ?? fallbackConfig);
		return cachedConfig;
	};
	return {
		...runtime,
		config: {
			...
			...replaceFn ? { replaceConfigFile: async (params) => {
