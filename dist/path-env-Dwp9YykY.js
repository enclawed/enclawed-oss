...
import { n as resolveBrewPathDirs } from "./brew-CPUDSDGs.js";
...
function candidateBinDirs(opts) {
	...
	try {
		const execDir = path.dirname(execPath);
		if (isExecutable(path.join(execDir, "enclawed"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.ENCLAWED_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "enclawed"))) append.push(localBinDir);
	}
...
*/
function ensureEnclawedCliOnPath(opts = {}) {
	if (isTruthyEnvValue(process.env.ENCLAWED_PATH_BOOTSTRAPPED)) return;
...
//#endregion
export { ensureEnclawedCliOnPath as t };
