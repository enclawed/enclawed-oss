import { i as registerServer } from "./src-BiTUS33n.js";
import { t as index } from "./tsdav.esm-s7QXhpRT.js";
...
//#endregion
//#region extensions/mcp-caldav/src/caldav-transport.ts
const { createDAVClient } = index;
function isGoogleCalDavUrl(url) {
...
function formatLocalIcalTime(d, tz) {
...
function buildVTimezone(tz) {
	...
	const summer = new Date(Date.UTC(year, 6, 1));
...
function tzOffsetMinutes(d, tz) {
	const nameStr = new Intl.DateTimeFormat("en-CA", {
		timeZone: tz,
		timeZoneName: "longOffset"
	}).formatToParts(d).find((p) => p.type === "timeZoneName")?.value ?? "GMT+0:00";
...
var CalDavTransport = class {
	constructor(opts) {
		this.clientPromise = null;
		if (!opts.serverUrl) throw new TypeError("CalDavTransport: serverUrl required");
		if (!opts.username) throw new TypeError("CalDavTransport: username required");
		if (!opts.password) throw new TypeError("CalDavTransport: password required");
	...
	async call(method, params) {
		if (method !== "tools/call") return {
			ok: false,
			reason: `CalDAV bridge: unsupported MCP method ${method}`
		};
		...
		try {
			switch (name) {
				...
				case "get_event": return {
					ok: true,
					result: await this.getEvent(args)
				};
				...
				case "update_event": return {
					ok: true,
					result: await this.updateEvent(args)
				};
				case "delete_event": return {
					ok: true,
					result: await this.deleteEvent(args)
				};
				default: return {
					ok: false,
					reason: `CalDAV bridge: unknown tool: ${name}`
				};
			}
		} catch (err) {
			this.clientPromise = null;
			return {
				ok: false,
				reason: `caldav ${name}: ${err.message}`
			};
		}
	}
	getClient() {
	...
	async resolvePrimaryCalendar(client) {
		if (this.defaultCalendarId?.startsWith("http")) return { url: this.defaultCalendarId };
		if (/googleusercontent\.com|google\.com\/calendar/.test(this.serverUrl)) return { url: `https://apidata.googleusercontent.com/caldav/v2/${encodeURIComponent(this.username)}/events/` };
		const calendars = await client.fetchCalendars();
		if (!calendars || calendars.length === 0) throw new Error("no calendars available on this account");
		if (this.defaultCalendarId) {
			const match = calendars.find((c) => c.url === this.defaultCalendarId || c.displayName === this.defaultCalendarId);
			if (match) return {
				url: match.url,
				displayName: stringOrUndef(match.displayName)
			};
		}
		const lowerUser = this.username.toLowerCase();
		const chosen = calendars.find((c) => typeof c.url === "string" ? c.url.toLowerCase().includes(lowerUser) : false) ?? calendars[0];
		return {
			url: chosen.url,
			displayName: stringOrUndef(chosen.displayName)
		};
	}
	async fetchObjectsForPrimary(timeRange) {
		if (isGoogleCalDavUrl(this.serverUrl)) return this.googleCalendarReport(timeRange);
	...
	async googleCalendarReport(timeRange) {
		...
		const reportBody = `<?xml version="1.0" encoding="utf-8"?><C:calendar-query xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav"><D:prop><D:getetag/><C:calendar-data/></D:prop><C:filter><C:comp-filter name="VCALENDAR"><C:comp-filter name="VEVENT">${timeRange ? `<C:time-range start="${icalTime(timeRange.start)}" end="${icalTime(timeRange.end)}"/>` : ""}</C:comp-filter></C:comp-filter></C:filter></C:calendar-query>`;
		...
		while ((m = re.exec(text)) !== null) {
			...
			if (data.includes("BEGIN:VCALENDAR")) objects.push({
				url: "",
				etag: "",
				data
			});
	...
	async getEvent(args) {
		...
		for (const obj of objects) {
			const parsed = safeParse(obj);
			for (const vevent of parsed) if (new ICALmodule.Event(vevent).uid === eventId) return toDetail(vevent);
		}
	...
	async createEvent(args) {
		...
		if (isGoogleCalDavUrl(this.serverUrl)) {
			...
			process.stderr.write(`[caldav info] create_event PUT ${url}\n[caldav info] body=${ics.replace(/\r?\n/g, " | ").slice(0, 600)}\n`);
	...
	async updateEvent(args) {
		...
		const client = await this.getClient();
		const url = `${(await this.resolvePrimaryCalendar(client)).url.replace(/\/$/, "")}/${eventId}.ics`;
	...
	async googleEventPut(url, ics, opts) {
		const headers = {
	...
	async googleEventVerify(url, uid) {
		const auth = Buffer.from(`${this.username}:${this.password}`, "utf8").toString("base64");
		let res;
		try {
			res = await fetch(url, {
			...
			throw new Error(`Google CalDAV verify GET ${url} threw: ${err.message}`, { cause: err });
		}
		if (!res.ok) {
			const snippet = (await res.text()).slice(0, 200);
			throw new Error(`Google CalDAV verify GET ${url}: HTTP ${res.status} — the PUT was accepted but the event did not persist server-side. Response: ${snippet}`);
		}
		const body = await res.text();
		if (!body.includes(`UID:${uid}`)) throw new Error(`Google CalDAV verify GET ${url}: 200 but body has no UID:${uid} (${body.length} bytes). Google accepted the PUT but did not persist the event in a readable form. Body preview: ${body.slice(0, 200)}`);
...
function parseCreateEventInput(args) {
	const summary = typeof args.summary === "string" ? args.summary.trim() : "";
	const start = typeof args.start === "string" ? args.start.trim() : "";
	const end = typeof args.end === "string" ? args.end.trim() : "";
	if (!summary) throw new Error("summary is required");
	if (!start || Number.isNaN(Date.parse(start))) throw new Error("start must be an ISO 8601 datetime");
	if (!end || Number.isNaN(Date.parse(end))) throw new Error("end must be an ISO 8601 datetime");
	...
	const attendees = Array.isArray(args.attendees) ? args.attendees.filter((x) => typeof x === "string" && x.includes("@")).map((x) => x.trim()) : void 0;
	...
	return Object.freeze({
		...
		...attendees !== void 0 ? { attendees: Object.freeze(attendees) } : {},
...
function normalizeTimezone(raw) {
	...
	const tz = raw.trim();
	if (!tz) return;
	if (!/^[A-Za-z]+(?:\/[A-Za-z_-]+)+$/.test(tz) && tz !== "UTC") return;
	try {
		new Intl.DateTimeFormat("en-CA", { timeZone: tz }).format(/* @__PURE__ */ new Date());
		return tz;
	} catch {
		return;
	}
...
function normalizeRrule(raw) {
	if (typeof raw !== "string") return;
	const trimmed = raw.trim().replace(/^RRULE:/i, "").trim();
	if (!trimmed) return;
	...
	for (const p of parts) {
		...
		if (key === "FREQ") {
			hasFreq = true;
			if (!FREQ_VOCAB.has(val.toUpperCase())) return;
		}
	}
	if (!hasFreq) return;
	return parts.map((p) => {
...
function buildVCalendar(input) {
	const now = icalTime(/* @__PURE__ */ new Date());
	const lines = [
		"BEGIN:VCALENDAR",
		"VERSION:2.0",
		"PRODID:-//enclawed//executive assistant//EN",
		"CALSCALE:GREGORIAN",
		"METHOD:PUBLISH"
	];
	if (input.timezone) lines.push(...buildVTimezone(input.timezone));
	lines.push("BEGIN:VEVENT", `UID:${input.uid}`, `DTSTAMP:${now}`, `CREATED:${now}`, `LAST-MODIFIED:${now}`, `SEQUENCE:0`, `STATUS:CONFIRMED`, `TRANSP:OPAQUE`);
	if (input.timezone) {
	...
	if (input.description) lines.push(`DESCRIPTION:${icalEscape(input.description)}`);
	if (input.location) lines.push(`LOCATION:${icalEscape(input.location)}`);
	if (input.recurrence) lines.push(`RRULE:${input.recurrence}`);
	lines.push(`ORGANIZER;CN=${input.organizer}:mailto:${input.organizer}`);
	for (const email of input.attendees ?? []) lines.push(`ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=NEEDS-ACTION;RSVP=TRUE;CN=${email}:mailto:${email}`);
...
function parseIso(value, field, fallback) {
	if (value === void 0 || value === null) return fallback;
	if (value instanceof Date) return value;
	if (typeof value !== "string") throw new Error(`${field} must be an ISO-8601 string`);
	const t = Date.parse(value);
	if (Number.isNaN(t)) throw new Error(`${field} is not a valid ISO-8601 timestamp: ${value}`);
	return new Date(t);
}
function safeParse(obj) {
	if (!obj || typeof obj.data !== "string" || obj.data.length === 0) return [];
	try {
		const jcal = ICALmodule.parse(obj.data);
		return new ICALmodule.Component(jcal).getAllSubcomponents("vevent");
	} catch {
		return [];
	}
}
function attendeesFrom(vevent) {
	const result = {
		organizer: null,
		attendees: []
	};
	const organizerProp = vevent.getFirstProperty("organizer");
	if (organizerProp) result.organizer = propToAttendee(organizerProp);
	for (const att of vevent.getAllProperties("attendee")) {
		const a = propToAttendee(att);
		if (a) result.attendees.push(a);
	}
	return result;
}
function propToAttendee(prop) {
	const raw = prop.getFirstValue();
	if (typeof raw !== "string" || !raw.startsWith("mailto:")) return null;
...
function toSummary(vevent) {
	...
	return Object.freeze({
		...
		location: typeof event.location === "string" && event.location.length > 0 ? event.location : null,
		organizer,
		attendees: Object.freeze(attendees)
	});
}
function toDetail(vevent) {
	const event = new ICALmodule.Event(vevent);
	const { organizer, attendees } = attendeesFrom(vevent);
...
//#endregion
//#region extensions/mcp-caldav/src/index.ts
const CALDAV_TOOLS = Object.freeze([
	"list_events",
	"get_event",
	"create_event",
	"update_event",
...
function loadCalDavBridge(opts) {
...
//#endregion
export { loadCalDavBridge as n, CalDavTransport as r, CALDAV_TOOLS as t };
