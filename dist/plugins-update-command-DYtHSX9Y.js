...
import { g as resolveUserPath, t as CONFIG_DIR, y as shortenHomePath } from "./utils-CrVQlOZJ.js";
import { n as defaultRuntime } from "./runtime-DVd7lkz0.js";
import { r as theme } from "./theme-hNdBadll.js";
import { t as promptYesNo } from "./prompt-BsEkqrC0.js";
import { n as MANIFEST_KEY } from "./legacy-names-CmFGZWLL.js";
...
import { r as parseRegistryNpmSpec } from "./npm-registry-spec-Cvql3WDg.js";
import { o as unscopedPackageName, r as resolveSafeInstallDir } from "./install-safe-path-DHbDtG8y.js";
import { i as installPluginFromPath, r as installPluginFromNpmSpec, t as PLUGIN_INSTALL_ERROR_CODE } from "./install-CmSaci9p.js";
import { l as resolveArchiveKind } from "./archive-Bx4mRIBv.js";
import { t as parseFrontmatter } from "./frontmatter-BnFskQX7.js";
import { t as findBundledPluginSource } from "./bundled-sources-DtjAR7c6.js";
import { a as resolvePinnedNpmInstallRecordForCli, n as persistPluginInstall, r as recordHookInstall, t as persistHookPackInstall } from "./plugins-install-persist-BGKyNjpS.js";
import { m as parseClawHubPluginSpec } from "./clawhub-CHEjoiE_.js";
import { n as formatClawHubSpecifier, r as installPluginFromClawHub } from "./clawhub-BLcBOLYD.js";
import { a as decidePreferredClawHubFallback, d as extractInstalledNpmHookPackageName, f as extractInstalledNpmPackageName, i as createPluginInstallLogger, n as buildPreferredClawHubSpec, r as createHookPackInstallLogger, s as formatPluginInstallWithHookFallbackError } from "./plugins-command-helpers-wQjP9rFc.js";
import { r as resolveBundledInstallPlanForNpmFailure, t as resolveBundledInstallPlanBeforeNpm } from "./plugin-install-plan-IdTGHcp_.js";
import { a as collectChannelDoctorStaleConfigMutations } from "./channel-doctor-B1TeEP4r.js";
import { a as resolveMarketplaceInstallShortcut, n as updateNpmInstalledPlugins, r as installPluginFromMarketplace } from "./update-BSvq-Ux8.js";
import { r as resolvePluginInstallRequestContext, t as resolvePluginInstallInvalidConfigPolicy } from "./plugin-install-config-policy-DJJI6Gv0.js";
import { n as readInstalledPackageVersion, t as expectedIntegrityForUpdate } from "./package-update-utils-z35QWZTk.js";
...
async function loadHookInstallRuntime() {
	hookInstallRuntimePromise ??= import("./install.runtime-BbIwb5fW.js");
...
async function ensureEnclawedHooks(manifest) {
	const hooks = manifest[MANIFEST_KEY]?.hooks;
	if (!Array.isArray(hooks)) throw new Error("package.json missing enclawed.hooks");
	const list = hooks.map((e) => typeof e === "string" ? e.trim() : "").filter(Boolean);
	if (list.length === 0) throw new Error("package.json enclawed.hooks is empty");
	return list;
}
...
async function loadConfigFromSnapshotForInstall(request) {
	if (resolvePluginInstallInvalidConfigPolicy(request) !== "allow-bundled-recovery") throw buildInvalidPluginInstallConfigError("Config invalid; run `enclawed doctor --fix` before installing plugins.");
	const snapshot = await readConfigFileSnapshot();
	const parsed = snapshot.parsed ?? {};
	if (!snapshot.exists || Object.keys(parsed).length === 0) throw buildInvalidPluginInstallConfigError("Config file could not be parsed; run `enclawed doctor` to repair it.");
	if (snapshot.legacyIssues.length > 0 || snapshot.issues.length === 0 || snapshot.issues.some((issue) => !isAllowedBundledRecoveryIssue(issue, request))) throw buildInvalidPluginInstallConfigError(`Config invalid outside the bundled recovery path for ${request.bundledPluginId ?? "the requested plugin"}; run \`enclawed doctor --fix\` before reinstalling it.`);
