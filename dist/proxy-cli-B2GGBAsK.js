...
async function loadProxyCliRuntime() {
	proxyCliRuntimePromise ??= import("./proxy-cli.runtime-L8D4N_57.js");
...
function registerProxyCli(program) {
	const proxy = program.command("proxy").description("Run the Enclawed debug proxy and inspect captured traffic");
	...
	proxy.command("run").description("Run a child command with Enclawed debug proxy capture enabled").allowUnknownOption(true).allowExcessArguments(true).option("--host <host>", "Bind host", "127.0.0.1").option("--port <port>", "Bind port", parseOptionalNumber).argument("[cmd...]", "Command to run after --").action(async (cmd, opts) => {
