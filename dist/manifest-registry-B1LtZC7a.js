...
import { i as loadPluginManifest } from "./manifest-Cdg6fKQA.js";
import { l as loadBundleManifest, n as isPathInside, r as safeRealpathSync } from "./path-safety-DBwpKwNC.js";
import { n as discoverEnclawedPlugins, r as resolvePluginCacheInputs } from "./discovery-Yk6Slu1A.js";
import { n as defaultSlotIdForKey } from "./slots-BVkLJktp.js";
import { t as checkMinHostVersion } from "./min-host-version-CQGyLjq_.js";
import fs from "node:fs";
import path from "node:path";
//#region src/enclawed/integration/plugin-admit.ts
function admitPluginCandidate(input) {
	const flavor = getFlavor();
	const decisions = getRuntime()?.moduleDecisions ?? null;
	if (!decisions) {
		if (flavor === "enclaved") return Object.freeze({
			admit: false,
			flavor,
			reason: "enclaved flavor: enclawed runtime not bootstrapped, cannot vouch for plugin"
		});
	...
	if (!decision) {
		...
		return Object.freeze({
			admit: true,
			flavor,
			warnings: Object.freeze([`plugin "${input.pluginId}" has no enclawed.module.json (open flavor: warn-only)`])
		});
	}
...
function shouldUseManifestCache(env) {
	if (env.ENCLAWED_DISABLE_PLUGIN_MANIFEST_CACHE?.trim()) return false;
...
function loadPluginManifestRegistry(params = {}) {
	...
	for (const candidate of candidates) {
		...
		for (const w of admission.warnings) diagnostics.push({
			level: "warn",
			pluginId: manifest.id,
			source: manifestRes.manifestPath,
		...
		if (!minHostVersionCheck.ok) {
			...
			diagnostics.push({
				...
				message: minHostVersionCheck.kind === "invalid" ? `plugin manifest invalid | ${minHostVersionCheck.error}` : minHostVersionCheck.kind === "unknown_host_version" ? `plugin requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load` : `plugin requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`
			});
			continue;
		}
