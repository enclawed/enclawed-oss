...
import { g as resolveUserPath, y as shortenHomePath } from "./utils-BcQ7uNrT.js";
import { n as defaultRuntime } from "./runtime-DVd7lkz0.js";
import { t as formatCliCommand } from "./command-format-C19Ii5s1.js";
import { t as CONFIG_PATH, u as resolveGatewayPort } from "./paths-Z7drr_SN.js";
...
import { o as isSecretRef, s as isValidEnvSecretRefId } from "./types.secrets-C7GHgMrh.js";
import { l as resolveDefaultSecretProviderAlias } from "./ref-contract-UQhW34sh.js";
import { a as findTailscaleBinary } from "./tailscale-fQzMWAOJ.js";
import { s as normalizeStringEntries } from "./string-normalization-BWhNVcLY.js";
import { a as loadConfig, l as readConfigFileSnapshot } from "./io-Dzvn-wpi.js";
import { t as isBlockedObjectKey } from "./prototype-keys-DQs5ogOu.js";
import { c as listChatChannels } from "./registry-BRuATCor.js";
...
import { n as isNonFatalSystemdInstallProbeError } from "./systemd-C_DkmzYn.js";
import { r as resolveGatewayService, t as describeGatewayServiceRestart } from "./service-DqJgm0kR.js";
import { r as replaceConfigFile, t as ConfigMutationConflictError } from "./config-DOKACpaG.js";
import { t as resolveControlUiLinks } from "./control-ui-links-CWHYJ4x5.js";
...
import { t as describeCodexNativeWebSearch } from "./codex-native-web-search.shared-CiHH1Xsw.js";
...
import { r as resolveProviderPluginChoice } from "./provider-wizard-BS_xf2yC.js";
import { t as applyPrimaryModel } from "./provider-model-primary-WHRjDruV.js";
import { t as ensureControlUiAssetsBuilt } from "./control-ui-assets-DiGXQLRG.js";
import { a as guardCancel, c as normalizeGatewayTokenInput, f as summarizeExistingConfig, m as waitForGatewayReachable, n as applyWizardMetadata, p as validateGatewayPasswordInput, r as ensureWorkspaceAndSessions, t as DEFAULT_WORKSPACE, u as probeGatewayReachable } from "./onboard-helpers-CHJZcheF.js";
import { t as promptAuthChoiceGrouped } from "./auth-choice-prompt-sqsk8Gzv.js";
import { n as applyAuthChoice } from "./auth-choice-suz089U4.js";
import { t as resolvePreferredProviderForAuthChoice } from "./provider-auth-choice-preference-BbgSttlY.js";
import { i as setupChannels } from "./onboard-channels-Cgd96Eqh.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-u8oair9T.js";
import { t as ensureSystemdUserLingerInteractive } from "./systemd-linger-r_C9aX9C.js";
import { i as promptModelAllowlist, n as applyModelFallbacksFromSelection, r as promptDefaultModel, t as applyModelAllowlist } from "./model-picker-D56oNUJH.js";
import { t as promptCustomApiConfig } from "./onboard-custom-CSOhprE2.js";
import { a as maybeAddTailnetOriginToControlUiAllowedOrigins, i as TAILSCALE_MISSING_BIN_NOTE_LINES, n as TAILSCALE_DOCS_LINES, r as TAILSCALE_EXPOSURE_OPTIONS, t as validateIPv4AddressInput } from "./ipv4-ZmDdhHJH.js";
import { n as formatHealthCheckFailure } from "./health-format-CSVuh8MT.js";
import { n as healthCommand } from "./health-DUkkyEAc.js";
import { t as promptRemoteGatewayConfig } from "./onboard-remote-CoqpVOzb.js";
import { t as setupSkills } from "./onboard-skills-C8JjWFOy.js";
...
async function removeChannelConfigWizard(cfg, runtime) {
	...
	while (true) {
		...
		if (configured.length === 0) {
			note$1(["No channel config found in enclawed.json.", `Tip: \`${formatCliCommand("enclawed channels status")}\` shows what is configured and enabled.`].join("\n"), "Remove channel");
			return next;
		}
	...
	if (authMode === "token") if (guardCancel(await select$1({
		...
		const envVarName = normalizeOptionalString(guardCancel(await text$1({
			...
			validate: (value) => {
				...
				if (!isValidEnvSecretRefId(candidate)) return "Use an env var name like ENCLAWED_GATEWAY_TOKEN.";
		...
		note$1(`Validated ${envVarName}. Enclawed will store a token SecretRef.`, "Gateway token");
	} else {
...
async function runGatewayHealthCheck(params) {
	...
	await waitForGatewayReachable({
		url: wsUrl,
		token: process.env.ENCLAWED_GATEWAY_TOKEN ?? configuredToken,
		password: process.env.ENCLAWED_GATEWAY_PASSWORD ?? configuredPassword,
		deadlineMs: 15e3
	});
	try {
		await healthCommand({
...
async function promptWebToolsConfig(nextConfig, runtime, prompter) {
	...
	const { resolveSearchProviderOptions, setupSearch } = await import("./onboard-search-7moanDAk.js");
	const { isCodexNativeWebSearchRelevant } = await import("./codex-native-web-search-PM9s6vdu.js");
...
async function runConfigureWizard(opts, runtime = defaultRuntime) {
	try {
		intro$1(opts.command === "update" ? "Enclawed update wizard" : "Enclawed configure");
		...
		if (snapshot.exists) {
			...
			if (!snapshot.valid) {
				outro$1(`Config invalid. Run \`${formatCliCommand("enclawed doctor")}\` to repair it, then re-run configure.`);
				runtime.exit(1);
				return;
			}
		}
		...
		if (opts.sections) {
			...
			if (selected.includes("plugins")) {
				const { configurePluginConfig } = await import("./setup.plugin-config-DFIkYLA-.js");
		...
		const newPassword = process.env.ENCLAWED_GATEWAY_PASSWORD ?? await resolveGatewaySecretInputForWizard({
			cfg: nextConfig,
			value: nextConfig.gateway?.auth?.password,
			path: "gateway.auth.password"
		});
		const oldPassword = process.env.ENCLAWED_GATEWAY_PASSWORD ?? await resolveGatewaySecretInputForWizard({
			cfg: baseConfig,
			value: baseConfig.gateway?.auth?.password,
			path: "gateway.auth.password"
		});
		const token = process.env.ENCLAWED_GATEWAY_TOKEN ?? await resolveGatewaySecretInputForWizard({
			cfg: nextConfig,
