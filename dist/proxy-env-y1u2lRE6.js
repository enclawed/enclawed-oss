...
function hasEnvHttpProxyAgentConfigured(env = process.env) {
	return hasEnvHttpProxyConfigured("http", env) || hasEnvHttpProxyConfigured("https", env);
}
/**
...
*/
function shouldUseEnvHttpProxyForUrl(url, env = process.env) {
	let parsed;
	try {
		parsed = new URL(url);
	} catch {
	...
	if (!hasEnvHttpProxyConfigured(parsed.protocol === "http:" ? "http" : "https", env)) return false;
