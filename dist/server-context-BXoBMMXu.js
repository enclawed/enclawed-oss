...
import { d as isPrivateNetworkAllowedByPolicy } from "./ssrf-DQDx1s1G.js";
import "./text-runtime-Co0Dyh7P.js";
import { C as PROFILE_ATTACH_RETRY_TIMEOUT_MS, T as usesFastLoopbackCdpProbeClass, a as fetchOk, b as CHROME_MCP_ATTACH_READY_WINDOW_MS, f as redactCdpUrl, i as fetchJson, j as DEFAULT_BROWSER_LOCAL_CDP_READY_TIMEOUT_MS, l as normalizeCdpHttpBaseForJsonEndpoints, m as withAllowedHostname, n as assertCdpEndpointAllowed, t as appendCdpPath, w as resolveCdpReachabilityTimeouts } from "./cdp.helpers-DZMnXnxy.js";
import { a as BrowserProfileUnavailableError, c as BrowserTabNotFoundError, d as toBrowserErrorResponse, i as BrowserProfileNotFoundError, l as BrowserTargetAmbiguousError, o as BrowserResetUnsupportedError } from "./errors-CIFawFUF.js";
import "./control-auth-CNgUvKUj.js";
import { i as resolveProfile, n as resolveBrowserConfig } from "./config-DyQGXjQ7.js";
import { t as movePathToTrash } from "./trash-BOm2HQqX.js";
import { C as assertBrowserNavigationResultAllowed, T as withBrowserNavigationPolicy, a as resolveEnclawedUserDataDir, b as InvalidBrowserNavigationUrlError, c as formatChromeCdpDiagnostic, d as createTargetViaCdp, i as launchEnclawedChrome, n as isChromeCdpReady, o as stopEnclawedChrome, p as normalizeCdpWsUrl, r as isChromeReachable, s as diagnoseChromeCdp, w as requiresInspectableBrowserNavigationRedirectsForUrl, x as assertBrowserNavigationAllowed, y as resolveBrowserNavigationProxyMode } from "./chrome-Zui3qUOn.js";
import { g as listChromeMcpTabs } from "./chrome-mcp-2AKy0Omh.js";
import { n as getPwAiModule, r as getBrowserProfileCapabilities, t as resolveTargetIdFromTabs } from "./target-id-DCq4gnFS.js";
...
function resolveCdpReachabilityPolicy(profile, ssrfPolicy) {
	if (!getBrowserProfileCapabilities(profile).isRemote && profile.cdpIsLoopback && profile.driver === "enclawed") return;
...
function changedProfileInvariants(current, next) {
	const changed = [];
	const currentUsesLocalManagedLaunch = current.driver === "enclawed" && !current.attachOnly && current.cdpIsLoopback;
	const nextUsesLocalManagedLaunch = next.driver === "enclawed" && !next.attachOnly && next.cdpIsLoopback;
...
async function getChromeMcpModule() {
	return await import("./chrome-mcp-t8CcSwt2.js");
}
//#endregion
...
function formatLocalPortOwnershipHint(profile) {
	const resetHint = `If Enclawed should own this local profile, run action=reset-profile profile=${profile.name} to stop the conflicting process.`;
	if (!profile.cdpIsLoopback) return resetHint;
	return `${resetHint} If this port is an externally managed CDP service such as Browserless, set browser.profiles.${profile.name}.attachOnly=true so Enclawed attaches without trying to manage the local process. For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Enclawed can reach via browser.profiles.<name>.cdpUrl.`;
}
...
function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning }) {
	...
	const formatChromeMcpAttachFailure = (lastError) => {
		...
		if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome")) return `Chrome MCP existing-session attach for profile "${profile.name}" could not connect to Chrome. Enable remote debugging in the browser inspect page, keep the browser open, approve the attach prompt, and retry. If you do not need your signed-in browser session, use the managed "enclawed" profile instead.` + detail;
	...
	const ensureBrowserAvailableOnce = async (options) => {
		...
		if (!profileState.running) {
			const detail = await describeCdpFailure(PROFILE_ATTACH_RETRY_TIMEOUT_MS);
			throw new BrowserProfileUnavailableError(`Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by enclawed. ${formatLocalPortOwnershipHint(profile)} ${detail}`);
		}
...
function createProfileResetOps({ profile, getProfileState, stopRunningBrowser, isHttpReachable, resolveEnclawedUserDataDir }) {
	const capabilities = getBrowserProfileCapabilities(profile);
