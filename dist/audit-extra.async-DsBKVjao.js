...
import { h as resolveOAuthDir } from "./paths-Z7drr_SN.js";
import { i as safeStat, n as formatPermissionRemediation, r as inspectPathPermissions, t as formatPermissionDetail } from "./audit-fs-DWewBKcU.js";
import { n as isPathInside, t as extensionUsesSkippedScannerPath } from "./scan-paths-1r_5YRip.js";
...
import { S as resolveDefaultAgentId } from "./agent-scope-5o1jve-2.js";
import { a as normalizePluginsConfig, i as normalizePluginId } from "./config-state-DbwRF0HU.js";
import "./plugins-BRuVcg7S.js";
import { u as resolveToolProfilePolicy } from "./tool-policy-D994AhXf.js";
import { f as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, n as resolveSandboxConfigForAgent, o as resolveSandboxToolPolicyForAgent } from "./config-CS3yaNEN.js";
import { r as pickSandboxToolPolicy, t as isToolAllowedByPolicies } from "./tool-policy-match-BLA48GX4.js";
import { t as resolveSkillSource } from "./source-vgvxWVnU.js";
import { t as listAgentWorkspaceDirs } from "./workspace-dirs-CzIeCJkJ.js";
import { a as execDockerRaw } from "./docker-CPa7jZlG.js";
import { t as inspectReadOnlyChannelAccount } from "./read-only-account-inspect-BjuFq6j3.js";
import { r as resolveNativeSkillsEnabled } from "./commands-DpeYsnof.js";
import { n as readInstalledPackageVersion } from "./package-update-utils-BcbVZ0Gy.js";
import { t as collectIncludePathsRecursive } from "./includes-scan-MApvR0ky.js";
import { t as scanDirectoryWithSummary } from "./skill-scanner-bbzJT2uf.js";
...
function loadSkillsModule() {
	skillsModulePromise ??= import("./skills-Bqg7YNpZ.js");
	return skillsModulePromise;
}
function loadConfigModule() {
	configModulePromise ??= import("./config-CV5Z1jsi.js");
...
async function listSandboxBrowserContainers(execDockerRawFn) {
	try {
		const result = await execDockerRawFn([
			...
			"-a",
			"--filter",
			"label=enclawed.sandboxBrowser=1",
			"--format",
			"{{.Names}}"
		], { allowFailure: true });
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (staleEpoch.length > 0) findings.push({
		...
		detail: `Containers: ${staleEpoch.join(", ")}. Expected enclawed.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
		remediation: `${formatCliCommand("enclawed sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("enclawed sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	return findings;
}
async function collectPluginsTrustFindings(params) {
	...
	if (pluginDirs.length > 0) {
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = isToolAllowedByPolicies("__enclawed_plugin_probe__", resolveToolPolicies({
					cfg: params.cfg,
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (pluginVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `enclawed plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (hookVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `enclawed hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `enclawed security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		try {
			...
			findings.push({
				...
				remediation: "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Enclawed extensions state directory."
			});
		}
		...
		if (escapedEntries.length > 0) findings.push({
			...
			remediation: "Update the plugin manifest so all enclawed.extensions entries stay inside the plugin directory."
		});
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Enclawed extensions state directory."
			});
...
async function collectInstalledSkillsCodeSafetyFindings(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		for (const entry of entries) {
			if (resolveSkillSource(entry.skill) === "enclawed-bundled") continue;
			...
			const summary = await getCodeSafetySummary({
				...
				findings.push({
					...
					remediation: "Check file permissions and skill layout, then rerun `enclawed security audit --deep`."
				});
				return null;
			});
			if (!summary) continue;
