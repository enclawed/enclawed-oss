...
async function completionCacheExists(shell, binName = "enclawed") {
...
async function isCompletionInstalled(shell, binName = "enclawed") {
...
*/
async function usesSlowDynamicCompletion(shell, binName = "enclawed") {
	const profilePath = getShellProfilePath(shell);
	if (!await pathExists(profilePath)) return false;
...
async function installCompletion(shell, yes, binName = "enclawed") {
	const home = process.env.HOME || os.homedir();
	let profilePath = "";
	let sourceLine = "";
