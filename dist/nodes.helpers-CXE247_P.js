...
import { a as trimToUndefined, n as hasGatewayPasswordEnvCandidate, r as hasGatewayTokenEnvCandidate } from "./credential-planner-BQSKxqU1.js";
...
import { nn as ErrorCodes, rn as errorShape, t as formatValidationErrors } from "./protocol-CAcA9P3Z.js";
import { i as resolveGatewayTokenSecretRefValue, r as resolveGatewayPasswordSecretRefValue, t as hasConfiguredGatewayAuthSecretInput } from "./auth-config-utils-Mib0g5ay.js";
...
function hasGatewayTokenCandidate(params) {
	if (trimToUndefined(params.env.ENCLAWED_GATEWAY_TOKEN)) return true;
...
function assertGatewayAuthNotKnownWeak(auth) {
	if (auth.mode === "token") {
		...
		if (token && KNOWN_WEAK_GATEWAY_TOKENS.has(token)) throw new Error("Invalid config: gateway auth token is set to the example placeholder from .env.example. Generate a real secret (e.g. `openssl rand -hex 32`) and set ENCLAWED_GATEWAY_TOKEN or gateway.auth.token before starting the gateway.");
		return;
	}
	if (auth.mode === "password") {
		const password = auth.password?.trim() ?? "";
		if (password && KNOWN_WEAK_GATEWAY_PASSWORDS.has(password)) throw new Error("Invalid config: gateway auth password is set to the example placeholder from .env.example. Choose a real password and set ENCLAWED_GATEWAY_PASSWORD or gateway.auth.password before starting the gateway.");
	}
}
