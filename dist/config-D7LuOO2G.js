...
import { o as isNotFoundPathError, s as isPathInside } from "./boundary-path-CUdfnLBJ.js";
import { a as normalizeOptionalTrimmedStringList } from "./string-normalization-QLwZvaSN.js";
import { s as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-DbL4T6oL.js";
import "./text-runtime-CAI4yT_W.js";
import "./sdk-security-runtime-DQ2o4Mo0.js";
import { I as DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME, M as DEFAULT_BROWSER_LOCAL_LAUNCH_TIMEOUT_MS, O as DEFAULT_BROWSER_ACTION_TIMEOUT_MS, P as DEFAULT_ENCLAWED_BROWSER_COLOR, d as parseBrowserHttpUrl, j as DEFAULT_BROWSER_LOCAL_CDP_READY_TIMEOUT_MS } from "./cdp.helpers-Dodvjzj3.js";
import { a as deriveDefaultBrowserCdpPortRange, o as deriveDefaultBrowserControlPort, s as parseBooleanValue } from "./control-auth-C-gwKOQn.js";
import "./tmp-enclawed-dir-C24PNM0E.js";
...
const DEFAULT_BROWSER_TMP_DIR = canUseNodeFs() ? resolvePreferredEnclawedTmpDir() : DEFAULT_FALLBACK_BROWSER_TMP_DIR;
...
const ENCLAWED_BROWSER_HEADLESS_ENV = "ENCLAWED_BROWSER_HEADLESS";
function normalizeHexColor(raw) {
	const value = (raw ?? "").trim();
	if (!value) return DEFAULT_ENCLAWED_BROWSER_COLOR;
	const normalized = value.startsWith("#") ? value : `#${value}`;
	if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_ENCLAWED_BROWSER_COLOR;
...
function ensureDefaultProfile(profiles, defaultColor, legacyCdpPort, derivedDefaultCdpPort, legacyCdpUrl) {
	...
	if (!result["enclawed"]) result[DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME] = {
...
function resolveBrowserConfig(cfg, rootConfig) {
	...
	const defaultProfile = defaultProfileFromConfig ?? (profiles["enclawed"] ? "enclawed" : profiles["enclawed"] ? "enclawed" : "user");
...
function getManagedBrowserMissingDisplayError(resolved, profile, params = {}) {
	...
	const sourceHint = mode.source === "request" ? "request override" : mode.source === "env" ? `${ENCLAWED_BROWSER_HEADLESS_ENV}=0` : mode.source === "profile" ? `browser.profiles.${profile.name}.headless=false` : "browser.headless=false";
	return `Headed browser start requested for profile "${profile.name}" via ${sourceHint}, but no Linux display server was detected (\$DISPLAY/\$WAYLAND_DISPLAY unset). Set ${ENCLAWED_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`;
}
//#endregion
