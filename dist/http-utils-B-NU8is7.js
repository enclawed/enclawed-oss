...
import { t as CONFIG_PATH } from "./paths-Z7drr_SN.js";
import { n as authorizeHttpGatewayConnect } from "./auth-BO-EkmdB.js";
import { a as loadConfig, i as getRuntimeConfig } from "./io-Dzvn-wpi.js";
import { o as parseAgentSessionKey } from "./session-key-utils-qXY7QK1H.js";
import { c as normalizeAgentId, r as buildAgentMainSessionKey } from "./session-key-D8SZau-8.js";
import { r as listChannelPlugins } from "./registry-BZZmrGdc.js";
import { S as resolveDefaultAgentId, _ as listAgentIds } from "./agent-scope-5o1jve-2.js";
import "./config-DOKACpaG.js";
import { u as normalizeMessageChannel } from "./message-channel-BZ78qqgM.js";
import { n as normalizeMessageChannel$1 } from "./message-channel-core-DiqXDzpD.js";
import { o as ADMIN_SCOPE, t as CLI_DEFAULT_OPERATOR_SCOPES } from "./method-scopes-BYgvEi9d.js";
import "./plugins-BRuVcg7S.js";
import { f as modelKey, m as parseModelRef } from "./model-selection-cli-DrzfE0gz.js";
import { f as resolveDefaultModelForAgent, t as buildAllowedModelSet } from "./model-selection-3QcDboLc.js";
import { o as resetModelCatalogCacheForTest, r as loadModelCatalog } from "./model-catalog-DQh8b8if.js";
import { c as requestBodyErrorToText, o as readJsonBodyWithLimit } from "./http-body-CrYxTOA6.js";
import { t as resolveAllowedAgentIds } from "./hooks-policy-BYJeVl_B.js";
...
function resolveHookIdempotencyKey(params) {
	return resolveOptionalHookIdempotencyKey(params.headers?.["idempotency-key"]) || resolveOptionalHookIdempotencyKey(params.headers?.["x-enclawed-idempotency-key"]) || resolveOptionalHookIdempotencyKey(params.payload.idempotencyKey);
}
...
const ENCLAWED_MODEL_ID = "enclawed";
const ENCLAWED_DEFAULT_MODEL_ID = "enclawed/default";
...
function resolveAgentIdFromHeader(req) {
	const raw = normalizeOptionalString(getHeader(req, "x-enclawed-agent-id")) || normalizeOptionalString(getHeader(req, "x-enclawed-agent")) || "";
	if (!raw) return;
...
function resolveAgentIdFromModel(model, cfg = loadConfig()) {
	...
	if (lowered === "enclawed" || lowered === "enclawed/default") return resolveDefaultAgentId(cfg);
	const agentId = (raw.match(/^enclawed[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ?? raw.match(/^agent:(?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i))?.groups?.agentId;
	if (!agentId) return;
	return normalizeAgentId(agentId);
}
async function resolveOpenAiCompatModelOverride(params) {
	const requestModel = params.model?.trim();
	if (requestModel && !resolveAgentIdFromModel(requestModel)) return { errorMessage: "Invalid `model`. Use `enclawed` or `enclawed/<agentId>`." };
	...
	if (!parsed) return { errorMessage: "Invalid `x-enclawed-model`." };
