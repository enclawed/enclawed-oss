...
import { o as resolveRequiredHomeDir, s as resolveRequiredOsHomeDir } from "./home-dir-yzjXRaUx.js";
...
import { n as markEnclawedExecEnv } from "./enclawed-exec-env-DnJET5Xz.js";
import { t as formatCliCommand } from "./command-format-C19Ii5s1.js";
import { i as resolvePathViaExistingAncestorSync } from "./boundary-path-DV1omZV7.js";
...
import { t as getBlockedNetworkModeReason } from "./network-mode-BsENi_kX.js";
...
import { r as writeJsonAtomic } from "./json-files-zRqqRJ8X.js";
import { d as SANDBOX_BROWSER_REGISTRY_PATH, l as DEFAULT_SANDBOX_IMAGE, p as SANDBOX_REGISTRY_PATH, u as SANDBOX_AGENT_WORKSPACE_MOUNT } from "./config-CS3yaNEN.js";
...
function formatBindBlockedError(params) {
	...
	if (params.reason.kind === "reserved_target") return /* @__PURE__ */ new Error(`Sandbox security: bind mount "${params.bind}" targets reserved container path "${params.reason.reservedPath}" (resolved target: "${params.reason.targetPath}"). This can shadow Enclawed sandbox mounts. Use a dangerous override only when you fully trust this runtime.`);
...
async function ensureDockerImage(image) {
	...
	if (image === "enclawed-sandbox:bookworm-slim") {
...
function buildSandboxCreateArgs(params) {
	...
	args.push("--label", `enclawed.mountFormatVersion=2`);
	if (params.configHash) args.push("--label", `enclawed.configHash=${params.configHash}`);
	...
	for (const [key, value] of Object.entries(markEnclawedExecEnv(envSanitization.allowed))) args.push("--env", `${key}=${value}`);
...
function formatSandboxRecreateHint(params) {
	if (params.scope === "session") return formatCliCommand(`enclawed sandbox recreate --session ${params.sessionKey}`);
	if (params.scope === "agent") return formatCliCommand(`enclawed sandbox recreate --agent ${resolveSandboxAgentId(params.sessionKey) ?? "main"}`);
