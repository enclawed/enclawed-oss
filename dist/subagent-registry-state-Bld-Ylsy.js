...
function resolveSubagentStateDir(env = process.env) {
	...
	if (env.VITEST || env.NODE_ENV === "test") return path.join(os.tmpdir(), "enclawed-test-state", String(process.pid));
...
function getSubagentRunsSnapshotForRead(inMemoryRuns) {
	...
	if (process.env.ENCLAWED_TEST_READ_SUBAGENT_RUNS_FROM_DISK === "1" || !(process.env.VITEST || false)) try {
