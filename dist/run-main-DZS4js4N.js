...
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-DfzO_W94.js";
import { t as ensureEnclawedCliOnPath } from "./path-env-Dwp9YykY.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-C78jN6sz.js";
import { n as maybeWarnAboutDebugProxyCoverage } from "./coverage-DM6vnVr1.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-CDKDSjkR.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-DUufRc9H.js");
			...
			const { statusCommand } = await import("./status-OARX6FBx.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-qU0XYKnl.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-DDGhGdhN.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-DxHpp3t-.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-Ucywoxjj.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-Ucywoxjj.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-B9tYhkdd.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-B9tYhkdd.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-DIXwjG_-.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-DBzrzVu4.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-BpZZF4i5.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-cxrAtb_Y.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-DGCYZNgw.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BvjhR5Hm.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-DuNlVvDm.js");
