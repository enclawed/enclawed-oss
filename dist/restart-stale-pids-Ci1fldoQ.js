...
import { n as resolveLsofCommandSync } from "./ports-lsof-BMcxzJdf.js";
import { a as splitArgsPreservingQuotes } from "./runtime-parse-B4f0N4H3.js";
...
function isGatewayArgv(args, opts) {
	...
	return exe.endsWith("/enclawed") || exe === "enclawed" || opts?.allowGatewayBinary === true && exe.endsWith("/enclawed-gateway");
}
//#endregion
...
function parsePidsFromLsofOutput(stdout) {
	...
	for (const line of stdout.split(/\r?\n/).filter(Boolean)) if (line.startsWith("p")) {
		if (currentPid != null && currentCmd && normalizeLowercaseStringOrEmpty(currentCmd).includes("enclawed")) pids.push(currentPid);
