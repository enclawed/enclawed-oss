import { i as resolveCodexAppServerRuntimeOptions } from "./config-4fguwYEf.js";
import { n as listCodexAppServerModels, t as listAllCodexAppServerModels } from "./models-9yv4jJnM.js";
import { t as isJsonObject } from "./protocol-BfJSSvII.js";
import { n as describeControlFailure, t as CODEX_CONTROL_METHODS } from "./capabilities-BM6HJeTP.js";
...
async function bindConversation(deps, ctx, pluginConfig, args) {
	if (!ctx.sessionFile) return { text: "Cannot bind Codex because this command did not include an Enclawed session file." };
...
async function resumeThread(deps, ctx, pluginConfig, threadId) {
	...
	if (!ctx.sessionFile) return "Cannot attach a Codex thread because this command did not include an Enclawed session file.";
...
async function stopConversationTurn(deps, ctx, pluginConfig) {
	...
	if (!sessionFile) return "Cannot stop Codex because this command did not include an Enclawed session file.";
...
async function steerConversationTurn(deps, ctx, pluginConfig, message) {
	const sessionFile = await resolveControlSessionFile(ctx);
	if (!sessionFile) return "Cannot steer Codex because this command did not include an Enclawed session file.";
...
async function setConversationModel(deps, ctx, pluginConfig, model) {
	const sessionFile = await resolveControlSessionFile(ctx);
	if (!sessionFile) return "Cannot set Codex model because this command did not include an Enclawed session file.";
...
async function setConversationFastMode(deps, ctx, pluginConfig, value) {
	const sessionFile = await resolveControlSessionFile(ctx);
	if (!sessionFile) return "Cannot set Codex fast mode because this command did not include an Enclawed session file.";
...
async function setConversationPermissions(deps, ctx, pluginConfig, value) {
	const sessionFile = await resolveControlSessionFile(ctx);
	if (!sessionFile) return "Cannot set Codex permissions because this command did not include an Enclawed session file.";
...
async function requestCodexDiagnosticsFeedbackApproval(deps, ctx, pluginConfig, note, commandPrefix) {
	if (!await hasAnyCodexDiagnosticsSessionFile(ctx)) return { text: "Cannot send Codex diagnostics because this command did not include an Enclawed session file." };
	const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
	if (targets.length === 0) return { text: ["No Codex thread is attached to this Enclawed session yet.", "Use /codex threads to find a thread, then /codex resume <thread-id> before sending diagnostics."].join("\n") };
	const now = Date.now();
...
async function previewCodexDiagnosticsFeedbackApproval(deps, ctx, note) {
	if (!await hasAnyCodexDiagnosticsSessionFile(ctx)) return "Cannot send Codex diagnostics because this command did not include an Enclawed session file.";
	const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
	if (targets.length === 0) return ["No Codex thread is attached to this Enclawed session yet.", "Use /codex threads to find a thread, then /codex resume <thread-id> before sending diagnostics."].join("\n");
...
async function confirmCodexDiagnosticsFeedback(deps, ctx, pluginConfig, token) {
	...
	if (!pending.privateRouted && !await hasAnyCodexDiagnosticsSessionFile(ctx)) return "Cannot send Codex diagnostics because this command did not include an Enclawed session file.";
...
function formatCodexDiagnosticsTargetBlock(target, index) {
	...
	if (target.sessionKey) lines.push(`Enclawed session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
	if (target.sessionId) lines.push(`Enclawed session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
...
function formatCodexDiagnosticsTargetLine(target) {
	...
	if (sessionLabel) parts.push(`Enclawed session ${formatCodexValueForDisplay(sessionLabel)}`);
...
async function startThreadAction(deps, ctx, pluginConfig, method, label) {
	const sessionFile = await resolveControlSessionFile(ctx);
	if (!sessionFile) return `Cannot start Codex ${label} because this command did not include an Enclawed session file.`;
	const binding = await deps.readCodexAppServerBinding(sessionFile);
	if (!binding?.threadId) return `No Codex thread is attached to this Enclawed session yet.`;
