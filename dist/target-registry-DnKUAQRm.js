...
import { i as parseDotPath } from "./shared-6rc7ZNul.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-BSUm5sVB.js";
import { n as getPath } from "./path-utils-Dr5vqBxU.js";
...
let compiledCoreEnclawedTargetState = null;
const compiledBundledChannelEnclawedTargets = /* @__PURE__ */ new Map();
...
function getCompiledSecretTargetRegistryState() {
	...
	const enclawedCompiledSecretTargets = compiledSecretTargetRegistry.filter((entry) => entry.configFile === "enclawed.json");
...
function getCompiledCoreEnclawedTargetState() {
	if (compiledCoreEnclawedTargetState) return compiledCoreEnclawedTargetState;
	const enclawedCompiledSecretTargets = getCoreSecretTargetRegistry().filter((entry) => entry.configFile === "enclawed.json").map(compileTargetRegistryEntry);
...
function getCompiledBundledChannelEnclawedTargets(channelId) {
	const normalizedChannelId = channelId.trim();
	if (!normalizedChannelId) return null;
	if (compiledBundledChannelEnclawedTargets.has(normalizedChannelId)) return compiledBundledChannelEnclawedTargets.get(normalizedChannelId) ?? null;
	const compiledEntries = loadBundledChannelSecretContractApi(normalizedChannelId)?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "enclawed.json").map(compileTargetRegistryEntry) ?? null;
...
function resolveConfigSecretTargetByPath(pathSegments) {
	...
	const explicitBundledChannelEntries = explicitBundledChannelId ? getCompiledBundledChannelEnclawedTargets(explicitBundledChannelId) : null;
...
function discoverConfigSecretTargetsByIds(config, targetIds) {
	...
	const registryState = allowedTargetIds !== null && Array.from(allowedTargetIds).every((targetId) => getCompiledCoreEnclawedTargetState().knownTargetIds.has(targetId)) ? getCompiledCoreEnclawedTargetState() : getCompiledSecretTargetRegistryState();
