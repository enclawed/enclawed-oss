...
import { i as openBoundaryFileSync } from "./boundary-file-read-g48ivAAk.js";
import { n as PLUGIN_MANIFEST_FILENAME } from "./manifest-Cdg6fKQA.js";
import { n as discoverEnclawedPlugins } from "./discovery-Yk6Slu1A.js";
import { h as unwrapDefaultModuleExport } from "./bundled-CXs8skTy.js";
import { i as resolveBundledPluginRepoEntryPath, o as normalizeBundledPluginStringList, s as resolveBundledPluginScanDir } from "./bundled-plugin-metadata-CwKLNsbJ.js";
import { g as shouldPreferNativeJiti, l as resolveLoaderPackageRoot, t as buildPluginLoaderAliasMap } from "./sdk-alias-BNzTrpCQ.js";
import { t as getCachedPluginJitiLoader } from "./jiti-loader-cache-C0hYtv34.js";
import { r as resolveManifestContractPluginIds, t as loadPluginManifestRegistry } from "./manifest-registry-B1LtZC7a.js";
import { n as withBundledPluginEnablementCompat, r as withBundledPluginVitestCompat } from "./bundled-compat-DELuo9-N.js";
import { t as createEmptyPluginRegistry } from "./registry-empty-Br8mkWPG.js";
import { n as createCapturedPluginRegistration } from "./captured-registration-BAyezMUN.js";
...
function buildVitestCapabilityShimAliasMap() {
	return Object.fromEntries(CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
		...
		return [[`@enclawed/plugin-sdk/${subpath}`, targetPath], [`@enclawed/plugin-sdk/${subpath}`, targetPath]];
	}));
}
function applyVitestCapabilityAliasOverrides(params) {
	...
	const { ["@enclawed/plugin-sdk"]: _ignoredLegacyRootAlias, ["@enclawed/plugin-sdk"]: _ignoredScopedRootAlias, ...scopedAliasMap } = params.aliasMap;
	return {
		...scopedAliasMap,
...
function readBundledCapabilityManifest(pluginDir) {
	...
	if (normalizeBundledPluginStringList(packageJson?.enclawed && typeof packageJson.enclawed === "object" ? packageJson.enclawed.extensions : void 0).length === 0) return;
...
function loadProviderContractEntriesForPluginId(pluginId) {
	...
	if (process.env.VITEST) {
		const vitestEntries = loadVitestProviderContractRegistry().filter((entry) => entry.pluginId === pluginId);
...
function resolveWebFetchProviderContractEntriesForPluginId(pluginId) {
	if (webFetchProviderContractRegistryCache) return webFetchProviderContractRegistryCache.filter((entry) => entry.pluginId === pluginId);
	const cache = webFetchProviderContractRegistryByPluginIdCache ?? /* @__PURE__ */ new Map();
...
function resolveWebSearchProviderContractEntriesForPluginId(pluginId) {
	if (webSearchProviderContractRegistryCache) return webSearchProviderContractRegistryCache.filter((entry) => entry.pluginId === pluginId);
	const cache = webSearchProviderContractRegistryByPluginIdCache ?? /* @__PURE__ */ new Map();
	webSearchProviderContractRegistryByPluginIdCache = cache;
	const cached = cache.get(pluginId);
	if (cached) return cached;
	if (process.env.VITEST) {
		const vitestEntries = loadVitestWebSearchProviderContractRegistry().filter((entry) => entry.pluginId === pluginId);
		if (vitestEntries.length > 0) {
			cache.set(pluginId, vitestEntries);
			return vitestEntries;
		}
...
//#endregion
export { resolveWebSearchProviderContractEntriesForPluginId as a, resolveWebFetchProviderContractEntriesForPluginId as i, providerContractLoadError as n, resolveProviderContractProvidersForPluginIds as r, pluginRegistrationContractRegistry as t };
