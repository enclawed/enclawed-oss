import { C as debugLog } from "./sender-VizoFyMG.js";
import { c as getQQBotDataDir, d as getQQBotMediaPath, m as isWindows, o as getHomeDir } from "./target-parser-DLnL6VYB.js";
...
function registerBasicBotCommands(registry) {
	...
	registry.register({
		...
		usage: [
			`/bot-version`,
			``,
			`查看当前 QQBot 插件版本和 enclawed 框架版本。`
		].join("\n"),
		handler: async () => {
	...
	registry.register({
		...
		handler: (ctx) => {
			...
			if (!isForce) {
				...
				lines.push(``, `---`, ``, `确认清理后，上述保存在 Enclawed 运行主机磁盘上的文件将永久删除，后续对话过程中 AI 无法再找回相关文件。`, `‼️ 点击指令确认删除`, `<qqbot-cmd-enter text="/bot-clear-storage --force" />`);
				return lines.join("\n");
			}
			const files = scanDirectoryFiles(targetDir);
...
function collectCandidateLogDirs() {
	...
	for (const [key, value] of Object.entries(process.env)) {
		if (!value) continue;
		if (/STATE_DIR$/i.test(key) && /(ENCLAWED|CLAWDBOT|MOLTBOT)/i.test(key)) pushStateDir(value);
	}
	for (const name of [
		".enclawed",
		".clawdbot",
	...
	for (const root of searchRoots) try {
		...
		for (const entry of entries) {
			...
			if (!/(enclawed|clawdbot|moltbot)/i.test(entry.name)) continue;
	...
	for (const dir of logDirs) {
		...
		try {
			...
			for (const entry of entries) {
				...
				if (!/(gateway|enclawed|clawdbot|moltbot)/i.test(entry.name)) continue;
