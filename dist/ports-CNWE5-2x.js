...
import { a as shouldLogVerbose, n as info, s as warn, t as danger } from "./globals-CYDryU7g.js";
import { t as logDebug } from "./logger-CKs2nJWp.js";
import { r as runCommandWithTimeout } from "./exec-_4N1gSjx.js";
import { t as resolveLsofCommand } from "./ports-lsof-BMcxzJdf.js";
import { t as tryListenOnPort } from "./ports-probe-CcewIcUR.js";
...
function classifyPortListener(listener, port) {
	...
	if (raw.includes("enclawed")) return "gateway";
	if (raw.includes("ssh")) {
...
function buildPortHints(listeners, port) {
	...
	const hints = [];
	if (kinds.has("gateway")) hints.push(`Gateway already running locally. Stop it (${formatCliCommand("enclawed gateway stop")}) or use a different port.`);
...
async function handlePortError(err, port, context, runtime = defaultRuntime) {
	if (err instanceof PortInUseError || isErrno(err) && err.code === "EADDRINUSE") {
		...
		if (details) {
			...
			if (/enclawed|src\/index\.ts|dist\/index\.js/.test(details)) runtime.error(warn("It looks like another Enclawed instance is already running. Stop it or pick a different port."));
		}
