...
function buildParseArgv(params) {
	...
	const normalizedArgv = programName && baseArgv[0] === programName ? baseArgv.slice(1) : baseArgv[0]?.endsWith("enclawed") ? baseArgv.slice(1) : baseArgv;
...
function resolveEnvLogLevelOverride() {
	...
	if (loggingState.invalidEnvLogLevelValue !== trimmed) {
		...
		process.stderr.write(`[enclawed] Ignoring invalid ENCLAWED_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`);
	}
}
//#endregion
...
function resolveDefaultLogFile(defaultLogDir) {
	return canUseNodeFs() ? path.join(defaultLogDir, "enclawed.log") : `${POSIX_ENCLAWED_TMP_DIR}/enclawed.log`;
}
...
function canUseSilentVitestFileLogFastPath(envLevel) {
	return process.env.VITEST === "true" && process.env.ENCLAWED_TEST_FILE_LOG !== "1" && !envLevel && !loggingState.overrideSettings;
}
function resolveSettings() {
	...
	const defaultLevel = process.env.VITEST === "true" && process.env.ENCLAWED_TEST_FILE_LOG !== "1" ? "silent" : "info";
		...
		try {
			...
			if (nextBytes > settings.maxFileBytes) {
				if (!warnedAboutSizeCap) {
					...
					process.stderr.write(`[enclawed] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`);
				}
				return;
			}
