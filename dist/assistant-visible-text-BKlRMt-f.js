...
function parseToolCallTagAt(text, start, tagNames = TOOL_CALL_TAG_NAMES) {
	...
	if (!tagNames.has(tagName) || !isToolCallBoundary(text[cursor])) return null;
...
function stripToolCallXmlTags(text, options) {
	...
	const activeToolCallTagNames = options?.stripFunctionCallsXmlPayloads === false ? new Set([...TOOL_CALL_TAG_NAMES].filter((name) => name !== "function_call" && name !== "function_calls")) : TOOL_CALL_TAG_NAMES;
	const codeRegions = findCodeRegions(text);
	let result = "";
	let lastIndex = 0;
