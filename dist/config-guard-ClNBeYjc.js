import { F as shouldMigrateStateFromPath } from "./logger-wuQoU2z2.js";
...
async function ensureConfigReady(params) {
	...
	if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-CAgwhdHE.js")).runDoctorConfigPreflight({
			migrateState: false,
		...
		if (!params.suppressDoctorStdout) preflightSnapshot = (await runDoctorConfigPreflight()).snapshot;
			...
			try {
				...
				if (originalSuppressNotes === void 0) delete process.env.ENCLAWED_SUPPRESS_NOTES;
				else process.env.ENCLAWED_SUPPRESS_NOTES = originalSuppressNotes;
			}
		}
	}
	...
	const { formatConfigIssueLines } = await import("./issue-format-OJ6Yr4Wa.js");
	...
	const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }] = await Promise.all([
		import("./theme-DzcX9uws.js"),
		import("./utils-bUcNW0IF.js"),
		import("./command-format-_PVs6xtl.js")
	]);
	const rich = isRich();
	...
	params.runtime.error(`${muted("Run:")} ${commandText(formatCliCommand("enclawed doctor --fix"))}`);
