...
import { d as pathExists } from "./utils-BcQ7uNrT.js";
...
import { n as inheritOptionFromParent } from "./command-options-DDSrRKei.js";
import "./daemon-cli-BWErmJdL.js";
import { n as resolveCliName, t as replaceCliName } from "./cli-name-B67WgIx7.js";
import { t as formatCliCommand } from "./command-format-C19Ii5s1.js";
import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-Z7drr_SN.js";
import { d as resolveGatewayLaunchAgentLabel, m as resolveGatewayWindowsTaskName, p as resolveGatewaySystemdServiceName } from "./paths-YeCWuIy9.js";
import { o as resolveGatewayInstallEntrypoint } from "./daemon-install-plan.shared-As_fUgdd.js";
import { a as parseSemver, i as nodeVersionSatisfiesEngine } from "./runtime-guard-_x9R7u4P.js";
import { r as runCommandWithTimeout } from "./exec-LGLh3vyF.js";
import { I as asResolvedSourceConfig, L as asRuntimeConfig, l as readConfigFileSnapshot } from "./io-Dzvn-wpi.js";
import { o as quoteCmdScriptArg } from "./restart-stale-pids-BF8tUnEt.js";
import { r as resolveGatewayService } from "./service-DqJgm0kR.js";
import { t as runDaemonInstall } from "./install-CrKkJJQ8.js";
import { r as replaceConfigFile } from "./config-DOKACpaG.js";
import { t as runDaemonRestart } from "./lifecycle-Cyqkiqha.js";
import { n as formatConfigIssueLines } from "./issue-format-DdkTqXUL.js";
import { l as waitForGatewayHealthyRestart, o as renderRestartDiagnostics, s as terminateStaleGatewayPids } from "./restart-health-BYH70A-n.js";
import { o as trimLogTail } from "./restart-sentinel-kYCIe5kw.js";
import { r as formatDurationPrecise } from "./format-duration-BKnM5WuD.js";
import { n as stylePromptMessage } from "./prompt-style-CPWYQlir.js";
import { t as formatHelpExamples } from "./help-format-Cc7aGPm0.js";
import { r as installCompletion } from "./completion-runtime-DUaTEkl_.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-Dg2KD-rV.js";
import { n as updateNpmInstalledPlugins, t as syncPluginsForUpdateChannel } from "./update-DVutEihZ.js";
import { n as doctorCommand, t as selectStyled } from "./prompt-select-styled-oGD5rrYK.js";
import { a as cleanupGlobalRenameDirs, c as detectGlobalInstallManagerByPresence, d as resolveExpectedInstalledVersionFromSpec, f as resolveGlobalInstallSpec, g as readPackageVersion, h as readPackageName, i as canResolveRegistryVersionForPackageTarget, l as detectGlobalInstallManagerForRoot, m as normalizePackageTagInput, o as collectInstalledGlobalPackageErrors, p as resolveGlobalInstallTarget, r as runGatewayUpdate, s as createGlobalInstallEnv, u as globalInstallArgs } from "./lifecycle-startup-DGBTUzBO.js";
import { d as formatUpdateChannelLabel, g as resolveUpdateChannelDisplay, h as resolveEffectiveUpdateChannel, i as fetchNpmTagVersion, m as normalizeUpdateChannel, n as compareSemverStrings, o as resolveNpmChannelTag, r as fetchNpmPackageTargetStatus, t as checkUpdateStatus, u as channelToNpmTag } from "./update-check-BaOFfy0o.js";
import { r as ensureCompletionCacheExists, t as checkShellCompletionStatus } from "./doctor-completion-sE2-RiSC.js";
import { i as resolveUpdateAvailability, n as formatUpdateOneLiner, t as formatUpdateAvailableHint } from "./status.update-iHKJCbwz.js";
...
function resolveDefaultGitDir() {
	...
	if (home.startsWith("/")) return path.posix.join(home, "enclawed");
...
async function resolveUpdateRoot() {
	return await resolveEnclawedPackageRoot({
...
async function ensureGitCheckout(params) {
	...
	if (!await isGitCheckout(params.dir)) {
		if (!await isEmptyDir(params.dir)) throw new Error(`ENCLAWED_GIT_DIR points at a non-git directory: ${params.dir}. Set ENCLAWED_GIT_DIR to an empty folder or an enclawed checkout.`);
	...
	if (!await isCorePackage(params.dir)) throw new Error(`ENCLAWED_GIT_DIR does not look like a core checkout: ${params.dir}.`);
	return null;
}
...
async function tryInstallShellCompletion(opts) {
	...
	if (!status.profileInstalled) {
		...
		if (isCancel(shouldInstall) || !shouldInstall) {
			if (!opts.skipPrompt) defaultRuntime.log(theme.muted(`Skipped. Run \`${replaceCliName(formatCliCommand("enclawed completion --install"), CLI_NAME)}\` later to enable.`));
			return;
		}
...
async function runPackageInstallUpdate(params) {
	...
	const packageName = (pkgRoot ? await readPackageName(pkgRoot) : await readPackageName(params.root)) ?? "enclawed";
...
async function maybeRestartService(params) {
		...
		try {
			...
			if (!params.opts.json && restartInitiated) {
				...
				if (health.healthy) defaultRuntime.log(theme.success("Daemon restart completed."));
					...
					defaultRuntime.log(theme.muted(`Run \`${replaceCliName(formatCliCommand("enclawed gateway status --deep"), CLI_NAME)}\` for details.`));
				}
				defaultRuntime.log("");
			}
		} catch (err) {
			if (!params.opts.json) {
				...
				defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${replaceCliName(formatCliCommand("enclawed gateway restart"), CLI_NAME)}`));
			}
		}
		return;
	}
	if (!params.opts.json) {
		defaultRuntime.log("");
		if (params.result.mode === "npm" || params.result.mode === "pnpm") defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("enclawed doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("enclawed gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
		else defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("enclawed gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
	}
}
...
async function updateCommand(opts) {
	...
	const devTargetRef = channel === "dev" ? process.env.ENCLAWED_UPDATE_DEV_TARGET_REF?.trim() || void 0 : void 0;
	...
	if (result.status === "skipped") {
		if (result.reason === "dirty") {
			...
			defaultRuntime.log(theme.muted("Commit, stash, or discard the local changes, then rerun `enclawed update`."));
		}
		if (result.reason === "not-git-install") {
			defaultRuntime.log(theme.warn(`Skipped: this Enclawed install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("enclawed doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("enclawed gateway restart"), CLI_NAME)}\`.`));
			defaultRuntime.log(theme.muted(`Examples: \`${replaceCliName("npm i -g enclawed@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g enclawed@latest", CLI_NAME)}\``));
		}
		defaultRuntime.exit(0);
		return;
	}
...
async function updateWizardCommand(opts = {}) {
	if (!process.stdin.isTTY) {
		defaultRuntime.error("Update wizard requires a TTY. Use `enclawed update --channel <stable|beta|dev>` instead.");
		defaultRuntime.exit(1);
		return;
	}
	const timeoutMs = parseTimeoutMsOrExit(opts.timeout);
		...
		if (!await isGitCheckout(gitDir)) {
			if (await pathExists(gitDir)) {
				if (!await isEmptyDir(gitDir)) {
					defaultRuntime.error(`ENCLAWED_GIT_DIR points at a non-git directory: ${gitDir}. Set ENCLAWED_GIT_DIR to an empty folder or an enclawed checkout.`);
					defaultRuntime.exit(1);
					return;
				}
			}
			const ok = await confirm({
				message: stylePromptMessage(`Create a git checkout at ${gitDir}? (override via ENCLAWED_GIT_DIR)`),
				initialValue: true
			});
			if (isCancel(ok) || !ok) {
				defaultRuntime.log(theme.muted("Update cancelled."));
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.enclawed.ai/cli/update")}`;
	...
	])}\n\n${theme.heading("Notes:")}\n${theme.muted("- Shows current update channel (stable/beta/dev) and source")}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.enclawed.ai/cli/update")}`).action(async (opts, command) => {
		try {
