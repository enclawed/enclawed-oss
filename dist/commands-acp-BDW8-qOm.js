...
import { a as requireGatewayClientScopeForInternalChannel } from "./command-gates-Bf2p7rM4.js";
import { C as stopWithText, b as resolveAcpAction, d as COMMAND, x as resolveAcpHelpText } from "./shared-BpFRj3wq.js";
...
async function loadAcpActionHandler(action) {
	if (action === "spawn" || action === "cancel" || action === "steer" || action === "close") {
		lifecycleHandlersPromise ??= import("./lifecycle-BYFn0Dr9.js");
	...
	if (action === "status" || action === "set-mode" || action === "set" || action === "cwd" || action === "permissions" || action === "timeout" || action === "model" || action === "reset-options") {
		runtimeOptionHandlersPromise ??= import("./runtime-options-BpCK_PrQ.js");
	...
	diagnosticHandlersPromise ??= import("./diagnostics-DVd5UD1c.js");
