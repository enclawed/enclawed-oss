...
import { c as normalizeAgentId, t as DEFAULT_AGENT_ID } from "./session-key-D8SZau-8.js";
...
import { l as resolveAuthStorePath } from "./source-check-WHC0RF1m.js";
import { n as ensureAuthProfileStore } from "./store-J7Io5AUg.js";
import "./plugins-BRuVcg7S.js";
import "./sessions-Dq3XJ_x5.js";
import { l as resolveSessionTranscriptsDirForAgent } from "./paths-CADYkZ1j.js";
import "./auth-profiles-CWIWzE4_.js";
import { i as listRouteBindings, t as isRouteBinding } from "./bindings-CQlzwnhb.js";
...
import { i as parseIdentityMarkdown, t as identityHasValues } from "./identity-file-DEu9My5k.js";
import { r as ensureWorkspaceAndSessions, s as moveToTrash } from "./onboard-helpers-CHJZcheF.js";
import { i as parseBindingSpecs, n as buildChannelBindings, r as describeBinding$1, t as applyAgentBindings } from "./agents.bindings-BoxiLUgQ.js";
import { n as requireValidConfigSnapshot, t as requireValidConfigFileSnapshot$1 } from "./config-validation-BcdnHxmA.js";
import { i as loadAgentIdentity, n as buildAgentSummaries, o as pruneAgentConfig, r as findAgentEntryIndex, t as applyAgentConfig } from "./agents.config-CTgCEL37.js";
import { t as promptAuthChoiceGrouped } from "./auth-choice-prompt-sqsk8Gzv.js";
import { n as applyAuthChoice, t as warnIfModelConfigLooksOff } from "./auth-choice-suz089U4.js";
...
async function resolveParsedBindingsOrExit(params) {
	...
	const { parseBindingSpecs } = await import("./agents.bindings-DmrMEbbV.js");
...
async function agentsBindCommand(opts, runtime = defaultRuntime) {
	...
	if (!parsed) return;
	const { applyAgentBindings } = await import("./agents.bindings-DmrMEbbV.js");
...
async function agentsUnbindCommand(opts, runtime = defaultRuntime) {
	...
	if (!parsed) return;
	const { removeAgentBindings } = await import("./agents.bindings-DmrMEbbV.js");
...
async function agentsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (nonInteractive) {
		...
		const payload = {
			...
			model,
			bindings: {
				added: bindingResult.added.map(describeBinding$1),
				updated: bindingResult.updated.map(describeBinding$1),
				skipped: bindingResult.skipped.map(describeBinding$1),
				conflicts: bindingResult.conflicts.map((conflict) => `${describeBinding$1(conflict.binding)} (agent=${conflict.existingAgentId})`)
			}
		};
		if (opts.json) writeRuntimeJson(runtime, payload);
			...
			if (bindingResult.conflicts.length > 0) runtime.error(["Skipped bindings already claimed by another agent:", ...bindingResult.conflicts.map((conflict) => `- ${describeBinding$1(conflict.binding)} (agent=${conflict.existingAgentId})`)].join("\n"));
		}
		return;
	}
	...
	try {
		...
		if (selection.length > 0) if (await prompter.confirm({
			...
			nextConfig = result.config;
			if (result.conflicts.length > 0) await prompter.note(["Skipped bindings already claimed by another agent:", ...result.conflicts.map((conflict) => `- ${describeBinding$1(conflict.binding)} (agent=${conflict.existingAgentId})`)].join("\n"), "Routing bindings");
		} else await prompter.note(["Routing unchanged. Add bindings when you're ready.", "Docs: https://docs.enclawed.ai/concepts/multi-agent"].join("\n"), "Routing");
		await replaceConfigFile({
			nextConfig,
			...baseHash !== void 0 ? { baseHash } : {}
		});
...
async function agentsListCommand(opts, runtime = defaultRuntime) {
	...
	if (opts.bindings) for (const summary of summaries) {
		...
		if (bindings.length > 0) summary.bindingDetails = bindings.map((binding) => describeBinding$1(binding));
	}
	...
	lines.push(`Channel status reflects local config/creds. For live health: ${formatCliCommand("enclawed channels status --probe")}.`);
