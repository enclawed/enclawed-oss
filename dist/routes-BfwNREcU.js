...
import { i as normalizeLowercaseStringOrEmpty, s as normalizeOptionalString$4 } from "./string-coerce-BUSzWgUA.js";
...
import { a as getImageMetadata, l as resizeToJpeg, r as buildImageResizeSideGrid, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-DdCDfmES.js";
import { a as ensureMediaDir, l as saveMediaBuffer } from "./store-DrQVGdgl.js";
import "./text-runtime-CAI4yT_W.js";
import { D as DEFAULT_AI_SNAPSHOT_MAX_CHARS, E as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, N as DEFAULT_BROWSER_SCREENSHOT_TIMEOUT_MS, d as parseBrowserHttpUrl, n as assertCdpEndpointAllowed, p as withCdpSocket } from "./cdp.helpers-Dodvjzj3.js";
import { a as BrowserProfileUnavailableError, c as BrowserTabNotFoundError, d as toBrowserErrorResponse, i as BrowserProfileNotFoundError, l as BrowserTargetAmbiguousError, n as BrowserConflictError, r as BrowserError, s as BrowserResourceExhaustedError, u as BrowserValidationError } from "./errors-CIFawFUF.js";
import { a as deriveDefaultBrowserCdpPortRange } from "./control-auth-C-gwKOQn.js";
import { a as DEFAULT_DOWNLOAD_DIR, c as resolveExistingPathsWithinRoot, i as resolveProfile, o as DEFAULT_TRACE_DIR, r as resolveManagedBrowserHeadlessMode, s as DEFAULT_UPLOAD_DIR, u as resolveWritablePathWithinRoot } from "./config-D7LuOO2G.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-B5bjM8O0.js";
import { n as normalizeString } from "./record-shared-C2o3WDz-.js";
import "./sdk-setup-tools-CTPRMojk.js";
import { t as movePathToTrash } from "./trash-B_oynyg8.js";
import { C as assertBrowserNavigationResultAllowed, T as withBrowserNavigationPolicy, _ as INTERACTIVE_ROLES, a as resolveEnclawedUserDataDir, g as CONTENT_ROLES, h as snapshotRoleViaCdp, m as snapshotAria, t as getChromeWebSocketUrl, u as captureScreenshot, v as STRUCTURAL_ROLES, x as assertBrowserNavigationAllowed, y as resolveBrowserNavigationProxyMode } from "./chrome-C2oLKaAJ.js";
import { E as uploadChromeMcpFile, T as takeChromeMcpSnapshot, _ as navigateChromeMcpPage, a as closeChromeMcpTab, c as evaluateChromeMcpScript, f as getChromeMcpPid, l as fillChromeMcpElement, m as hoverChromeMcpElement, n as clickChromeMcpCoords, o as dragChromeMcpElement, r as clickChromeMcpElement, u as fillChromeMcpForm, w as takeChromeMcpScreenshot, x as resizeChromeMcpPage, y as pressChromeMcpKey } from "./chrome-mcp-CSdT2UI6.js";
import { a as shouldUsePlaywrightForAriaSnapshot, i as resolveDefaultSnapshotFormat, n as getPwAiModule$1, o as shouldUsePlaywrightForScreenshot, r as getBrowserProfileCapabilities, t as resolveTargetIdFromTabs } from "./target-id-YIJLvu42.js";
import { c as ACT_MAX_CLICK_DELAY_MS, l as ACT_MAX_WAIT_TIME_MS, o as normalizeBrowserFormField, p as matchBrowserUrlPattern, r as getRoleSnapshotStats, u as normalizeActBoundedNonNegativeMs } from "./pw-role-snapshot-BIFtViiU.js";
...
function browserEvaluateDisabledMessage(action) {
	return [action === "wait" ? "wait --fn is disabled by config (browser.evaluateEnabled=false)." : "act:evaluate is disabled by config (browser.evaluateEnabled=false).", "Docs: /gateway/configuration#browser-enclawed-managed-browser"].join("\n");
}
//#endregion
...
function buildBrowserDoctorReport(params) {
	...
	if (transport === "chrome-mcp") checks.push({
		...
		if (missingDisplay) checks.push({
			...
			fixHint: "Use a desktop session, Xvfb, set ENCLAWED_BROWSER_HEADLESS=1, or remove the headed override."
		});
		...
		checks.push({
			...
			...status.cdpHttp || !status.running ? {} : { fixHint: "Run enclawed browser start or inspect browser.cdpUrl/CDP port reachability." }
		});
		checks.push({
			id: "cdp-websocket",
			label: "CDP WebSocket",
...
async function runBrowserLiveProbe(req, ctx) {
	...
	try {
		...
		if (capabilities.usesChromeMcp) {
			const { takeChromeMcpSnapshot } = await import("./chrome-mcp-pu8TWSla.js");
...
function parseHeadlessStartOverride(params) {
	...
	if (params.profileCtx.profile.driver !== "enclawed" || params.profileCtx.profile.attachOnly || capabilities.isRemote) {
		jsonError(params.res, 400, `Headless start override is only supported for locally launched enclawed profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`);
		return { ok: false };
	}
	return {
		ok: true,
...
function registerBrowserBasicRoutes(app, ctx) {
	...
	app.post("/profiles/create", asyncBrowserRoute(async (req, res) => {
		...
		if (!name) return jsonError(res, 400, "name is required");
		if (driver && driver !== "enclawed" && driver !== "clawd" && driver !== "existing-session") return jsonError(res, 400, `unsupported profile driver "${driver}"; use "enclawed", "clawd", or "existing-session"`);
		await withProfilesServiceMutation({
			...
			run: async (service) => await service.createProfile({
				...
				driver: driver === "existing-session" ? "existing-session" : driver === "enclawed" || driver === "clawd" ? "enclawed" : void 0
			})
		});
	}));
