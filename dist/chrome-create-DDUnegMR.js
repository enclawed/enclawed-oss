...
async function listGoogleMeetNodes(runtime, params) {
	...
	if (!nodesRuntime) throw new Error("Google Meet node inventory unavailable: runtime node access is not enabled");
	try {
		return { nodes: (await nodesRuntime.list(params?.connected === true ? { includeOffline: false } : { includeOffline: true })).nodes.map((node) => ({
			nodeId: node.nodeId,
			displayName: node.displayName,
			caps: node.capabilities ? [...node.capabilities] : void 0,
			commands: node.commands ? [...node.commands] : void 0,
...
async function resolveChromeNodeInfo(params) {
	...
	if (requested) {
		...
		if (matches.length === 1) {
			...
			throw new Error(`Configured Google Meet node ${requested} is not usable (${formatNodeLabel(node)}): ${describeNodeUsabilityIssues(node).join("; ")}. Start or reinstall \`enclawed node run\` on that Chrome host, approve pairing, and allow googlemeet.chrome plus browser.proxy.`);
		}
		...
		throw new Error(`Configured Google Meet node ${requested} was not found. Run \`enclawed nodes status\` and start or approve the Chrome node.`);
	}
	const nodes = (await listGoogleMeetNodes(params.runtime, { connected: true })).nodes.filter(isGoogleMeetNode);
	if (nodes.length === 0) throw new Error("No connected Google Meet-capable node with browser proxy. Run `enclawed node run` on the Chrome host with browser proxy enabled, approve pairing, and allow googlemeet.chrome plus browser.proxy.");
...
async function callBrowserProxyOnNode(params) {
	...
	if (!nodesRuntime) throw new Error("Google Meet browser proxy unavailable: runtime node access is not enabled");
	return parseBrowserProxyResult(await nodesRuntime.invoke({
