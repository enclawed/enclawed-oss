...
function isCurrentEnclawedClient(clientId) {
	return (normalizeOptionalLowercaseString(clientId) ?? "").startsWith("enclawed-");
}
...
function scoreNodeCandidate(node, matchScore) {
	...
	if (isCurrentEnclawedClient(node.clientId)) score += 10;
