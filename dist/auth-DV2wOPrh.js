...
import { i as resolveGatewayCredentialsFromValues } from "./credentials-Dap1S7F2.js";
import { s as readTailscaleWhoisIdentity } from "./tailscale-D0rviK4C.js";
import { t as safeEqualSecret } from "./secret-equal-YvM7lU0c.js";
import { _ as resolveRequestClientIp, a as isLoopbackHost, d as normalizeHostHeader, i as isLoopbackAddress, l as isTrustedProxyAddress, p as resolveClientIp } from "./net-SK24IRt9.js";
import { o as normalizeRateLimitClientIp } from "./auth-rate-limit-BBOwpYDJ.js";
...
function assertGatewayAuthConfigured(auth, rawAuthConfig) {
	if (auth.mode === "token" && !auth.token) {
		...
		throw new Error("gateway auth mode is token, but no token was configured (set gateway.auth.token or ENCLAWED_GATEWAY_TOKEN)");
	}
	if (auth.mode === "password" && !auth.password) {
		if (rawAuthConfig?.password != null && typeof rawAuthConfig.password !== "string") throw new Error("gateway auth mode is password, but gateway.auth.password contains a provider reference object instead of a resolved string — bootstrap secrets (gateway.auth.password) must be plaintext strings or set via the ENCLAWED_GATEWAY_PASSWORD environment variable because the secrets provider system has not initialised yet at gateway startup");
	...
	if (auth.mode === "trusted-proxy") {
		...
		if (auth.token) throw new Error("gateway auth mode is trusted-proxy, but a shared token is also configured; remove gateway.auth.token / ENCLAWED_GATEWAY_TOKEN because trusted-proxy and token auth are mutually exclusive");
	}
}
/**
