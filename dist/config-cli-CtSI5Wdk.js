...
import { d as resolveSecretInputRef, i as coerceSecretRef, s as isValidEnvSecretRefId } from "./types.secrets-C7GHgMrh.js";
import { a as formatExecSecretRefIdValidationMessage, c as isValidSecretProviderAlias, d as validateExecSecretRefId, o as isValidExecSecretRefId, s as isValidFileSecretRefId, u as secretRefKey } from "./ref-contract-UQhW34sh.js";
import { n as info, o as success, t as danger } from "./globals-D8R_YQwt.js";
import { a as resolveSecretRefValue } from "./resolve-DVl7buUu.js";
import { w as SecretProviderSchema } from "./zod-schema.core-VW7GXHL-.js";
import { b as validateConfigObjectRaw, l as readConfigFileSnapshot, v as collectUnsupportedSecretRefPolicyIssues } from "./io-Dzvn-wpi.js";
import { t as isBlockedObjectKey } from "./prototype-keys-DQs5ogOu.js";
import { r as replaceConfigFile } from "./config-DOKACpaG.js";
import { a as normalizeConfigIssues, n as formatConfigIssueLines } from "./issue-format-DdkTqXUL.js";
import { n as discoverConfigSecretTargets, s as resolveConfigSecretTargetByPath } from "./target-registry-se_Ryd0R.js";
import { i as redactConfigObject, n as readBestEffortRuntimeConfigSchema } from "./runtime-schema-D03KS_XC.js";
import { n as setCommandJsonMode } from "./json-mode-TlMMWSNz.js";
...
const CONFIG_SET_DESCRIPTION = [
	...
	"Examples:",
	formatCliCommand("enclawed config set gateway.port 19001 --strict-json"),
	formatCliCommand("enclawed config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN"),
	formatCliCommand("enclawed config set secrets.providers.vault --provider-source file --provider-path /etc/enclawed/secrets.json --provider-mode json"),
	formatCliCommand("enclawed config set --batch-file ./config-set.batch.json --dry-run")
].join("\n");
...
function registerConfigCli(program) {
	const cmd = program.command("config").description("Non-interactive config helpers (get/set/unset/file/schema/validate). Run without subcommand for guided setup.").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.enclawed.ai/cli/config")}\n`).option("--section <section>", "Configuration sections for guided setup (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
		const { configureCommandFromSectionsArg } = await import("./configure-CJv7e5Jf.js");
	...
	cmd.command("schema").description("Print the JSON schema for enclawed.json").action(async () => {
