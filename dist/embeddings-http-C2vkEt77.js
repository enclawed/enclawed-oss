...
import { n as listMemoryEmbeddingProviders, t as getMemoryEmbeddingProvider } from "./memory-embedding-provider-runtime-ChRMtCIK.js";
import { a as getHeader, d as resolveOpenAiCompatibleHttpOperatorScopes, o as resolveAgentIdForRequest, s as resolveAgentIdFromModel, y as sendJson } from "./http-utils-BSyHU5k1.js";
...
async function handleOpenAiEmbeddingsHttpRequest(req, res, opts) {
	...
	if (requestModel !== "enclawed" && !resolveAgentIdFromModel(requestModel, cfg)) {
		sendJson(res, 400, { error: {
	...
	const target = resolveEmbeddingsTarget({
		requestModel: normalizeOptionalString(getHeader(req, "x-enclawed-model")) || normalizeOptionalString(memorySearch?.model) || "",
		configuredProvider,
		cfg
	});
