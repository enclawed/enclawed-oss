...
import { a as enableConsoleCapture } from "./subsystem-DMryAe7X.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-DkSj_7zL.js";
...
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-BnBSHMQy.js";
import { t as ensureEnclawedCliOnPath } from "./path-env-B-wYipvE.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-BuqP--jR.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-Dnqg95m1.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-B8TIRVH_.js");
			...
			const { statusCommand } = await import("./status-DSkTiohA.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-DeMbEkzI.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-DC1yLffz.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-DFFNSOph.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-DbmSa9Tr.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-DbmSa9Tr.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-DtkZnWAn.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-DtkZnWAn.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-B1HRBScH.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-CcF9kKrI.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-DRC9iyw4.js");
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			...
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-B1jt_EI1.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-Dl1afcxo.js");
