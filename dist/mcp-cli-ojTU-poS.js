...
import { g as GATEWAY_CLIENT_NAMES, h as GATEWAY_CLIENT_MODES, u as normalizeMessageChannel } from "./message-channel-CP7lSRvA.js";
import { d as WRITE_SCOPE, l as READ_SCOPE, s as APPROVALS_SCOPE } from "./method-scopes-H5HCSsJN.js";
import { i as extractFirstTextBlock } from "./chat-message-content-C5zS33r4.js";
...
import { t as resolveGatewayClientBootstrap } from "./client-bootstrap-BNwrmYnE.js";
import { t as parseConfigValue } from "./config-value-JkezLiFz.js";
import { t as resolveGatewayAuthOptions } from "./gateway-secret-options-ChTsT76V.js";
...
function resolveMessageId(entry) {
	return normalizeOptionalString(entry.id) ?? (entry.__enclawed && typeof entry.__enclawed === "object" ? normalizeOptionalString(entry.__enclawed.id) : void 0);
}
...
var EnclawedChannelBridge = class {
	...
	async handleClaudePermissionRequest(params) {
		...
		if (this.verbose) process.stderr.write(`enclawed mcp: pending Claude permission ${params.requestId}\n`);
	}
	...
	async sendNotification(notification) {
		...
		try {
			...
			if (this.verbose && !this.closed) process.stderr.write(`enclawed mcp: notification ${notification.method} failed: ${String(error)}\n`);
		}
	}
	async handleHelloOk() {
		try {
...
function registerChannelMcpTools(server, bridge) {
	server.tool("conversations_list", "List Enclawed channel-backed conversations available through session routes.", {
	...
	server.tool("conversation_get", "Get one Enclawed conversation by session key.", { session_key: z.string().min(1) }, async ({ session_key }) => {
	...
	server.tool("messages_read", "Read recent messages for one Enclawed conversation.", {
	...
	server.tool("attachments_fetch", "List non-text attachments for a message in one Enclawed conversation.", {
		session_key: z.string().min(1),
	...
	server.tool("events_poll", "Poll queued Enclawed conversation events since a cursor.", {
	...
	server.tool("events_wait", "Wait for the next queued Enclawed conversation event.", {
		after_cursor: z.number().int().min(0).optional(),
		session_key: z.string().optional(),
	...
	server.tool("messages_send", "Send a message back through the same Enclawed conversation route.", {
		session_key: z.string().min(1),
		text: z.string().min(1)
	...
	server.tool("permissions_list_open", "List open Enclawed exec or plugin approval requests visible through the Gateway.", {}, async () => {
	...
	server.tool("permissions_respond", "Allow or deny one pending Enclawed exec or plugin approval request.", {
...
async function createEnclawedChannelMcpServer(opts = {}) {
...
async function serveEnclawedChannelMcp(opts = {}) {
...
function registerMcpCli(program) {
	...
	mcp.command("serve").description("Expose Enclawed channels over MCP stdio").option("--url <url>", "Gateway WebSocket URL (defaults to gateway.remote.url when configured)").option("--token <token>", "Gateway token (if required)").option("--token-file <path>", "Read gateway token from file").option("--password <password>", "Gateway password (if required)").option("--password-file <path>", "Read gateway password from file").option("--claude-channel-mode <mode>", "Claude channel notification mode: auto, on, or off", "auto").option("-v, --verbose", "Verbose logging to stderr", false).action(async (opts) => {
		try {
