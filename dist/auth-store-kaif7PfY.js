...
import { n as info, o as success } from "./globals-D8R_YQwt.js";
...
import "./text-runtime-D3dEeWWF.js";
import { c as resolveComparableIdentity } from "./identity-CfbBxRHF.js";
...
async function stringifyCreds(creds) {
	const { BufferJSON } = await import("./session.runtime-BvXobI9H.js");
...
async function pickWebChannel(pref, authDir = resolveDefaultWebAuthDir()) {
	...
	if (!auth.exists) throw new Error(`No WhatsApp Web session found. Run \`${formatCliCommand("enclawed channels login --channel whatsapp --verbose")}\` to link.`);
	return choice;
}
//#endregion
