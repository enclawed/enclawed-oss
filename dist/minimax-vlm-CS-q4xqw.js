...
import { d as resolveConfiguredModelRef } from "./model-selection-3QcDboLc.js";
...
function hasImageReasoningOnlyResponse(message) {
	if (extractAssistantText(message).trim().length > 0) return false;
	const content = message.content;
	if (!Array.isArray(content)) return false;
	return content.some((block) => block && typeof block === "object" && block.type === "thinking");
}
...
async function minimaxUnderstandImage(params) {
	...
	const res = await fetch(url, {
		...
		signal: AbortSignal.timeout(typeof params.timeoutMs === "number" && Number.isFinite(params.timeoutMs) && params.timeoutMs > 0 ? params.timeoutMs : 6e4),
		body: JSON.stringify({
			prompt,
