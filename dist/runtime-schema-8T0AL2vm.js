...
import { t as CHANNEL_IDS } from "./ids-BArxFn_a.js";
import { t as createSubsystemLogger } from "./subsystem-WGqvYWrS.js";
import "./zod-schema.sensitive-MFr0aGHM.js";
import { C as collectChannelSchemaMetadata, T as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA, a as loadConfig, l as readConfigFileSnapshot, w as collectPluginSchemaMetadata } from "./io-BPigC1a6.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-BSUm5sVB.js";
import { S as resolveDefaultAgentId, x as resolveAgentWorkspaceDir } from "./agent-scope-Bv7XD6OT.js";
import "./config-BDd-c7HD.js";
import { a as redactSensitiveUrlLikeString, n as hasSensitiveUrlHintTag, r as isSensitiveUrlConfigPath, t as SENSITIVE_URL_HINT_TAG } from "./redact-sensitive-url-DmZf2stv.js";
...
*/
const REDACTED_SENTINEL = "__ENCLAWED_REDACTED__";
...
var RedactionError = class extends Error {
	constructor(key, humanReadableMessage) {
		...
		this.key = key;
		this.humanReadableMessage = humanReadableMessage ?? `Sentinel value "__ENCLAWED_REDACTED__" in key ${key} is not valid as real data`;
...
function assertNoRedactedSentinel(value, path) {
	if (typeof value === "string" && value === "__ENCLAWED_REDACTED__") {
...
function maybeRestoreSecretRefId(params) {
	...
	if (!isSecretRefShape(incomingObj) || incomingObj.id !== "__ENCLAWED_REDACTED__") return { handled: false };
...
function restoreArrayItemWithLookup(params) {
	if (params.item === "__ENCLAWED_REDACTED__") return params.originalArray[params.index];
...
function restoreArrayItemWithGuessing(params) {
	if (!isExplicitlyNonSensitivePath(params.hints, [params.path]) && isSensitivePath(params.path) && params.item === "__ENCLAWED_REDACTED__") return params.originalArray[params.index];
...
function restoreRedactedValuesWithLookup(incoming, original, lookup, prefix, hints) {
	...
	for (const [key, value] of Object.entries(toObjectRecord(incoming))) {
		...
		for (const candidate of [path, wildcardPath]) if (lookup.has(candidate)) {
			matched = true;
			if (value === "__ENCLAWED_REDACTED__" && (hints[candidate]?.sensitive === true || hasSensitiveUrlHintPath(hints, [
				candidate,
				path,
				wildcardPath
		...
		if (!matched) {
			const markedNonSensitive = isExplicitlyNonSensitivePath(hints, [path, wildcardPath]);
			if (!markedNonSensitive && value === "__ENCLAWED_REDACTED__" && (isSensitivePath(path) || hasSensitiveUrlHintPath(hints, [path, wildcardPath]) || isSensitiveUrlPath(path))) result[key] = restoreOriginalValueOrThrow({
				key,
				path,
				original: orig
			});
...
function restoreRedactedValuesGuessing(incoming, original, prefix, hints) {
	...
	for (const [key, value] of Object.entries(toObjectRecord(incoming))) {
		const path = prefix ? `${prefix}.${key}` : key;
		const wildcardPath = prefix ? `${prefix}.*` : "*";
		if (!isExplicitlyNonSensitivePath(hints, [path, wildcardPath]) && value === "__ENCLAWED_REDACTED__" && (isSensitivePath(path) || hasSensitiveUrlHintPath(hints, [path, wildcardPath]) || isSensitiveUrlPath(path))) result[key] = restoreOriginalValueOrThrow({
			key,
			path,
			original: orig
		});
...
const GENERATED_BASE_CONFIG_SCHEMA = {
	schema: {
		...
		properties: {
			meta: {
				...
				properties: {
					lastTouchedVersion: {
						...
						description: "Auto-set when Enclawed writes the config."
					},
					lastTouchedAt: {
						anyOf: [{ type: "string" }, {}],
				...
				title: "Metadata",
				description: "Metadata fields automatically maintained by Enclawed to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history."
			},
			env: {
				type: "object",
				properties: {
					...
					vars: {
						...
						description: "Explicit key/value environment variable overrides merged into runtime process environment for Enclawed. Use this for deterministic env configuration instead of relying only on shell profile side effects."
					}
				},
				additionalProperties: { type: "string" },
				title: "Environment",
			...
			wizard: {
				...
				properties: {
					...
					lastRunVersion: {
						...
						description: "Enclawed version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes."
					},
					lastRunCommit: {
						type: "string",
					...
					cacheTrace: {
						...
						properties: {
							...
							filePath: {
								...
								description: "JSONL output path for cache trace logs (default: $ENCLAWED_STATE_DIR/logs/cache-trace.jsonl)."
							},
							includeMessages: {
								type: "boolean",
			...
			update: {
				...
				additionalProperties: false,
				title: "Updates",
				description: "Update-channel and startup-check behavior for keeping Enclawed runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments."
			},
			browser: {
				type: "object",
				properties: {
						...
						additionalProperties: {
							...
							properties: {
								...
								driver: {
									...
									description: "Per-profile browser driver mode. Use \"enclawed\" (or legacy \"clawd\") for CDP-based profiles, or use \"existing-session\" for host-local Chrome DevTools MCP attachment."
								},
								attachOnly: {
									type: "boolean",
						...
						additionalProperties: {
							...
							properties: {
								...
								injectNumCtxForOpenAICompat: {
									...
									description: "Controls whether Enclawed injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields."
								},
								headers: {
									type: "object",
									propertyNames: { type: "string" },
				...
				properties: { browserProxy: {
					...
					properties: {
						...
						allowProfiles: {
							...
							description: "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Enclawed enforces least-privilege profile access and blocks persistent profile create/delete through the proxy."
						}
					},
					additionalProperties: false,
						...
						properties: {
							...
							experimental: {
								...
								properties: { localModelLean: {
									...
									description: "Experimental local-model prompt trim. When enabled, Enclawed drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends."
								} },
								additionalProperties: false,
									...
									store: {
										...
										properties: {
											...
											path: {
												...
												description: "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.enclawed/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment."
											},
											fts: {
												type: "object",
							...
							embeddedPi: {
								...
								additionalProperties: false,
								title: "Embedded Pi",
								description: "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Enclawed sessions."
							},
									...
									browser: {
										...
										properties: {
											...
											network: {
												...
												description: "Docker network for sandbox browser containers (default: enclawed-sandbox-browser). Avoid bridge if you need stricter isolation."
											},
											cdpPort: {
												type: "integer",
												exclusiveMinimum: 0,
								...
								runtime: {
									anyOf: [{
										...
										properties: { type: {
											...
											description: "Runtime type for this agent: \"embedded\" (default Enclawed runtime) or \"acp\" (ACP harness defaults)."
										} },
										required: ["type"],
										additionalProperties: false
									}, {
											...
											acp: {
												...
												properties: {
													agent: {
														...
														description: "Optional ACP harness agent id to use for this Enclawed agent (for example codex, claude, cursor, gemini, enclawed)."
													},
													backend: {
														type: "string",
									...
									title: "Agent Runtime",
									description: "Optional runtime descriptor for this agent. Use embedded for default Enclawed execution or acp for external ACP harness defaults."
								}
							},
							required: ["id"],
							additionalProperties: false
			...
			commands: {
				...
				properties: {
					...
					mcp: {
						type: "boolean",
						title: "Allow /mcp",
						description: "Allow /mcp chat command to manage Enclawed MCP server config under mcp.servers (default: false)."
					},
					plugins: {
						type: "boolean",
						title: "Allow /plugins",
			...
			session: {
				...
				properties: {
					...
					parentForkMaxTokens: {
						...
						description: "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, Enclawed starts a fresh thread session instead of forking; set 0 to disable this protection."
					},
					mainKey: {
						type: "string",
			...
			hooks: {
				...
				additionalProperties: false,
				title: "Hooks",
				description: "Inbound webhook automation surface for mapping external events into wake or agent actions in Enclawed. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks."
			},
			web: {
				type: "object",
				properties: {
					...
					wideArea: {
						...
						properties: {
							...
							domain: {
								...
								description: "Optional unicast DNS-SD domain for wide-area discovery, such as enclawed.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes."
							}
						},
						additionalProperties: false,
					...
					controlUi: {
						...
						properties: {
							...
							basePath: {
								...
								description: "Optional URL prefix where the Control UI is served (e.g. /enclawed)."
							},
							root: {
								type: "string",
							...
							allowExternalEmbedUrls: {
								...
								description: "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__enclawed__/canvas and /__enclawed__/a2ui documents do not need it."
							},
							allowedOrigins: {
								type: "array",
								items: { type: "string" },
					...
					nodes: {
						...
						properties: {
							...
							allowCommands: {
								...
								description: "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `enclawed security audit`."
							},
							denyCommands: {
								type: "array",
								items: { type: "string" },
			...
			memory: {
				...
				properties: {
					backend: {
						...
						description: "Selects the global memory engine: \"builtin\" uses Enclawed memory internals, while \"qmd\" uses the QMD sidecar pipeline. Keep \"builtin\" unless you intentionally operate QMD."
					},
					citations: {
						anyOf: [
							{
			...
			mcp: {
				...
				properties: { servers: {
					...
					title: "MCP Servers",
					description: "Named MCP server definitions. Enclawed stores them in its own config and runtime adapters decide which transports are supported at execution time."
				} },
				additionalProperties: false,
				title: "MCP",
				description: "Global MCP server definitions managed by Enclawed. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings."
			},
			skills: {
				type: "object",
				properties: {
			...
			plugins: {
				...
				properties: {
					...
					installs: {
						...
						description: "CLI-managed install metadata (used by `enclawed plugins update` to locate install sources)."
					}
				},
				additionalProperties: false,
				title: "Plugins",
	...
	uiHints: {
		...
		update: {
			...
			group: "Update",
			order: 25,
			help: "Update-channel and startup-check behavior for keeping Enclawed runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
			tags: ["advanced"]
		},
		cli: {
			label: "CLI",
		...
		hooks: {
			...
			group: "Hooks",
			order: 110,
			help: "Inbound webhook automation surface for mapping external events into wake or agent actions in Enclawed. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
			tags: ["advanced"]
		},
		ui: {
			label: "UI",
		...
		meta: {
			label: "Metadata",
			help: "Metadata fields automatically maintained by Enclawed to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
			tags: ["advanced"]
		},
		"meta.lastTouchedVersion": {
			label: "Config Last Touched Version",
			help: "Auto-set when Enclawed writes the config.",
			tags: ["media"]
		},
		"meta.lastTouchedAt": {
		...
		"env.vars": {
			...
			help: "Explicit key/value environment variable overrides merged into runtime process environment for Enclawed. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
			tags: ["advanced"]
		},
		"wizard.lastRunAt": {
		...
		"wizard.lastRunVersion": {
			...
			help: "Enclawed version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
			tags: ["advanced"]
		},
		...
		"diagnostics.cacheTrace.filePath": {
			...
			help: "JSONL output path for cache trace logs (default: $ENCLAWED_STATE_DIR/logs/cache-trace.jsonl).",
			tags: ["observability", "storage"]
		},
		...
		"agents.list[].runtime": {
			label: "Agent Runtime",
			help: "Optional runtime descriptor for this agent. Use embedded for default Enclawed execution or acp for external ACP harness defaults.",
			tags: ["advanced"]
		},
		"agents.list[].runtime.type": {
			label: "Agent Runtime Type",
			help: "Runtime type for this agent: \"embedded\" (default Enclawed runtime) or \"acp\" (ACP harness defaults).",
			tags: ["advanced"]
		},
		...
		"agents.list[].runtime.acp.agent": {
			...
			help: "Optional ACP harness agent id to use for this Enclawed agent (for example codex, claude, cursor, gemini, enclawed).",
			tags: ["advanced"]
		},
		...
		"browser.profiles.*.driver": {
			...
			help: "Per-profile browser driver mode. Use \"enclawed\" (or legacy \"clawd\") for CDP-based profiles, or use \"existing-session\" for host-local Chrome DevTools MCP attachment.",
			tags: ["storage"]
		},
		...
		"gateway.controlUi.basePath": {
			...
			help: "Optional URL prefix where the Control UI is served (e.g. /enclawed).",
			placeholder: "/enclawed",
			tags: ["network", "storage"]
		},
		...
		"gateway.controlUi.allowExternalEmbedUrls": {
			...
			help: "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__enclawed__/canvas and /__enclawed__/a2ui documents do not need it.",
			tags: [
				"security",
				"access",
				"network",
		...
		"gateway.nodes.allowCommands": {
			...
			help: "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `enclawed security audit`.",
			tags: ["access", "network"]
		},
		...
		"nodeHost.browserProxy.allowProfiles": {
			...
			help: "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Enclawed enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
			tags: [
				"access",
				"network",
				"storage"
		...
		"agents.defaults.experimental.localModelLean": {
			...
			help: "Experimental local-model prompt trim. When enabled, Enclawed drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
			tags: ["security", "advanced"]
		},
		...
		"agents.defaults.memorySearch.store.path": {
			...
			help: "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.enclawed/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
			tags: ["storage"]
		},
		...
		"memory.backend": {
			...
			help: "Selects the global memory engine: \"builtin\" uses Enclawed memory internals, while \"qmd\" uses the QMD sidecar pipeline. Keep \"builtin\" unless you intentionally operate QMD.",
			tags: ["storage"]
		},
		"memory.citations": {
		...
		"models.providers.*.injectNumCtxForOpenAICompat": {
			...
			help: "Controls whether Enclawed injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
			tags: ["models"]
		},
		...
		"agents.defaults.embeddedPi": {
			label: "Embedded Pi",
			help: "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Enclawed sessions.",
			tags: ["advanced"]
		},
		...
		"agents.defaults.sandbox.browser.network": {
			...
			help: "Docker network for sandbox browser containers (default: enclawed-sandbox-browser). Avoid bridge if you need stricter isolation.",
			tags: ["storage"]
		},
		...
		"commands.mcp": {
			label: "Allow /mcp",
			help: "Allow /mcp chat command to manage Enclawed MCP server config under mcp.servers (default: false).",
			tags: ["advanced"]
		},
		"commands.plugins": {
			label: "Allow /plugins",
		...
		mcp: {
			label: "MCP",
			help: "Global MCP server definitions managed by Enclawed. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
			tags: ["advanced"]
		},
		"mcp.servers": {
			label: "MCP Servers",
			help: "Named MCP server definitions. Enclawed stores them in its own config and runtime adapters decide which transports are supported at execution time.",
			tags: ["advanced"]
		},
		"ui.seamColor": {
			label: "Accent Color",
		...
		"session.parentForkMaxTokens": {
			...
			help: "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, Enclawed starts a fresh thread session instead of forking; set 0 to disable this protection.",
			tags: [
				"security",
				"auth",
				"performance",
		...
		"discovery.wideArea.domain": {
			...
			help: "Optional unicast DNS-SD domain for wide-area discovery, such as enclawed.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
			tags: ["network"]
		},
		"discovery.mdns": {
		...
		"agents.list[].identity.avatar": {
			...
			placeholder: "avatars/enclawed.png",
			tags: ["advanced"]
		},
		"agents.list[].heartbeat.suppressToolErrorWarnings": {
		...
		"plugins.installs": {
			...
			help: "CLI-managed install metadata (used by `enclawed plugins update` to locate install sources).",
			tags: ["advanced"]
		},
