...
import { n as resolveAgentsDirFromSessionStorePath, u as resolveStorePath } from "./paths-CADYkZ1j.js";
...
function resolveSessionStoreTargets(cfg, opts, params = {}) {
	...
	if (hasAgent) {
		...
		if (!knownAgents.includes(requested)) throw new Error(`Unknown agent id "${opts.agent}". Use "enclawed agents list" to see configured agents.`);
		return [{
			agentId: requested,
			storePath: resolveStorePath(cfg.session?.store, {
				agentId: requested,
