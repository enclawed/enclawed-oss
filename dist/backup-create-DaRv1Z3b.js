import { d as pathExists, g as resolveUserPath, h as resolveHomeDir, y as shortenHomePath } from "./utils-CrVQlOZJ.js";
import { _ as resolveStateDir, h as resolveOAuthDir, o as resolveConfigPath } from "./paths-CDjhyzOH.js";
...
import { t as formatSessionArchiveTimestamp } from "./artifacts-CF4LrKnT.js";
import { n as isPathWithin, t as buildCleanupPlan } from "./cleanup-utils-XNfYsaDh.js";
...
async function resolveBackupPlanFromDisk(params = {}) {
	...
	if (includeWorkspace && configSnapshot.exists && !configSnapshot.valid) throw new Error(`Config invalid at ${shortenHomePath(configSnapshot.path)}. Enclawed cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`);
...
async function createBackupArchive(opts = {}) {
	...
	if (plan.included.length === 0) throw new Error(onlyConfig ? "No Enclawed config file was found to back up." : "No local Enclawed state was found to back up.");
	...
	const tempDir = await fs$1.mkdtemp(path.join(os.tmpdir(), "enclawed-backup-"));
