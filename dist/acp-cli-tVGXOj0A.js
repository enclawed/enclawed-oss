...
import { i as asOptionalRecord } from "./record-coerce-CMKPGWB_.js";
import { t as sanitizeTerminalText } from "./safe-text-CNdk9l9T.js";
import { i as omitEnvKeysCaseInsensitive, n as listKnownProviderAuthEnvVarNames } from "./provider-env-vars-reCe_d7o.js";
...
import { f as isKnownCoreToolId, s as resolveOwnerOnlyToolApprovalClass } from "./tool-policy-D994AhXf.js";
import { r as listThinkingLevels } from "./thinking-DWHN3uH-.js";
import { r as isMutatingToolCall } from "./tool-mutation-zn7FdQP3.js";
import { i as resolveWindowsSpawnProgram, n as materializeWindowsSpawnProgram } from "./windows-spawn-0eneg5VK.js";
import { t as getChatCommands } from "./commands-registry.data-XbBN7K15.js";
...
async function createAcpClient(opts = {}) {
	...
	const { getActiveSkillEnvKeys } = await import("./env-overrides.runtime-Cv73KxkV.js");
	...
	return {
		configOptions: [
			buildSelectConfigOption({
				...
				description: "Controls how much deliberate reasoning Enclawed requests from the Gateway model.",
			...
			buildSelectConfigOption({
				...
				name: "Tool verbosity",
				description: "Controls how much tool progress and output detail Enclawed keeps enabled for the session.",
			...
			buildSelectConfigOption({
				...
				name: "Usage detail",
				description: "Controls how much usage information Enclawed attaches to responses for the session.",
...
var AcpGatewayAgent = class {
	...
	assertSupportedSessionSetup(mcpServers) {
		...
		throw new Error("ACP bridge mode does not support per-session MCP servers. Configure MCP on the Enclawed gateway or agent instead.");
	}
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
	...
	if (argv.includes("--token") || argv.includes("--gateway-token")) console.error("Warning: --token can be exposed via process listings. Prefer --token-file or ENCLAWED_GATEWAY_TOKEN.");
	if (argv.includes("--password") || argv.includes("--gateway-password")) console.error("Warning: --password can be exposed via process listings. Prefer --password-file or ENCLAWED_GATEWAY_PASSWORD.");
...
function registerAcpCli(program) {
	...
	acp.command("client").description("Run an interactive ACP client against the local ACP bridge").option("--cwd <dir>", "Working directory for the ACP session").option("--server <command>", "ACP server command (default: enclawed)").option("--server-args <args...>", "Extra arguments for the ACP server").option("--server-verbose", "Enable verbose logging on the ACP server", false).option("-v, --verbose", "Verbose client logging", false).action(async (opts, command) => {
