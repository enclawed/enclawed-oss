...
import { u as listBundledChannelSetupPlugins } from "./bundled-U6EdlhZh.js";
import { n as normalizeAccountId, t as DEFAULT_ACCOUNT_ID } from "./account-id-BV5xNTUp.js";
import { c as listChatChannels, n as formatChannelSelectionLine, t as formatChannelPrimerLine } from "./registry-DWJO7iQk.js";
...
import { t as isChannelConfigured } from "./channel-configured-uhgzt_3t.js";
import { a as getActivePluginRegistryVersion, g as requireActivePluginRegistry, t as getActivePluginChannelRegistry } from "./runtime-v-gfCtZv.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-icsK1N4v.js";
import { t as enablePluginInConfig } from "./enable-ChB25UnK.js";
import { t as buildChannelSetupWizardAdapterFromSetupWizard } from "./setup-wizard-C-fp-mQK.js";
import { n as loadChannelSetupPluginRegistrySnapshotForChannel, t as ensureChannelSetupPluginInstalled } from "./plugin-install-C2sJ168p.js";
import { r as listTrustedChannelPluginCatalogEntries } from "./trusted-catalog-CNWlwVgp.js";
import { i as shouldShowChannelInSetup, r as resolveChannelSetupEntries } from "./discovery-Cx1Iclxz.js";
...
async function maybeConfigureDmPolicies(params) {
	...
	for (const policy of dmPolicies) {
		...
		await prompter.note([
			...
			"Multi-user DMs: run: " + formatCliCommand("enclawed config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.",
...
async function setupChannels(cfg, runtime, prompter, options) {
	...
	const enableBundledPluginForSetup = async (channel) => {
		...
		if (!plugin) {
			if (adapter) {
				await prompter.note(`${channel} plugin not available (continuing with setup). If the channel still doesn't work after setup, run \`${formatCliCommand("enclawed plugins list")}\` and \`${formatCliCommand("enclawed plugins enable " + channel)}\`, then restart the gateway.`, "Channel setup");
				await refreshStatus(channel);
				return true;
			}
