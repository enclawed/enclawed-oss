...
import "./format-pHnEk_L3.js";
import { l as buildStatusOverviewRowsFromSurface } from "./status-runtime-shared-QUAyS2Fs.js";
...
function buildStatusSecurityAuditLines(params) {
	...
	lines.push(params.theme.muted(`Full report: ${params.formatCliCommand("enclawed security audit")}`));
	lines.push(params.theme.muted(`Deep probe: ${params.formatCliCommand("enclawed security audit --deep")}`));
	return lines;
}
...
function buildStatusFooterLines(params) {
	return [
		...
		params.nodeOnlyGateway ? `  Need node service?  ${params.formatCliCommand("enclawed node status")}` : params.gatewayReachable ? `  Need to test channels? ${params.formatCliCommand("enclawed status --deep")}` : `  Fix reachability first: ${params.formatCliCommand("enclawed gateway probe")}`
	];
}
...
function buildStatusPairingRecoveryLines(params) {
	...
	return [
		...
		...params.pairingRecovery.requestId ? [params.muted(`Recovery: ${params.formatCliCommand(`enclawed devices approve ${params.pairingRecovery.requestId}`)}`)] : [],
...
function buildStatusAllOverviewRows(params) {
	return buildStatusOverviewRowsFromSurface({
		...
		middleRows: [{
			Item: "Security",
			Value: `Run: ${formatCliCommand("enclawed security audit --deep")}`
		}],
