...
import { t as buildPluginConfigSchema } from "./config-schema-C7ukmjeN.js";
import "./text-runtime-BSsrP5ac.js";
import { c as unregisterAcpRuntimeBackend, o as registerAcpRuntimeBackend } from "./manager-D_RkNMs4.js";
import "./core-DBv8LeUb.js";
import { t as zod_exports } from "./zod-BjTCjsWs.js";
import "./runtime-api-BbO-gCls.js";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "enclawed-plugin-tools";
function isAcpxPluginRoot(dir) {
	return fs.existsSync(path.join(dir, "enclawed.plugin.json")) && fs.existsSync(path.join(dir, "package.json"));
}
...
function resolveAcpxPluginRootFromEnclawedLayout(moduleUrl) {
	let cursor = path.dirname(fileURLToPath(moduleUrl));
	for (let i = 0; i < 5; i += 1) {
		const candidates = [
...
function resolveAcpxPluginRoot(moduleUrl = import.meta.url) {
	...
	return resolveWorkspaceAcpxPluginRoot(resolvedRoot) ?? resolveRepoAcpxPluginRoot(resolvedRoot) ?? resolveAcpxPluginRootFromEnclawedLayout(moduleUrl) ?? resolvedRoot;
}
...
function resolveEnclawedRoot(currentRoot) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl = import.meta.url) {
	const enclawedRoot = resolveEnclawedRoot(resolveAcpxPluginRoot(moduleUrl));
...
function resolveConfiguredMcpServers(params) {
	...
	if (resolved["enclawed-plugin-tools"]) throw new Error(`mcpServers.${ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME} is reserved when pluginToolsMcpBridge=true`);
...
function createAcpxRuntimeService(params = {}) {
	...
	return {
		...
		async start(ctx) {
			...
			if (process.env.ENCLAWED_SKIP_ACPX_RUNTIME_PROBE === "1") return;
			lifecycleRevision += 1;
