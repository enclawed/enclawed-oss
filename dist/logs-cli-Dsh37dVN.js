...
import { m as isValidTimeZone, p as formatTimestamp } from "./logger-wuQoU2z2.js";
...
import { n as buildGatewayConnectionDetails } from "./call-DtxbZweG.js";
...
async function loadLogsCliRuntime() {
	logsCliRuntimePromise ??= import("./logs-cli.runtime-CN1gbVDi.js");
...
function createLogWriters() {
	const writer = createSafeStreamWriter({
		...
		onBrokenPipe: (err, stream) => {
			...
			const message = `enclawed logs: output ${stream === process.stdout ? "stdout" : "stderr"} closed (${code}). Stopping tail.`;
			try {
