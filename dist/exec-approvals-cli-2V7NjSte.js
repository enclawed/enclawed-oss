...
import { j as saveExecApprovals, y as readExecApprovalsSnapshot } from "./exec-approvals-CSCQWe00.js";
import { n as formatTimeAgo } from "./format-relative-CPp3DocO.js";
import { n as callGatewayFromCli } from "./gateway-rpc-CiMkSrD5.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-Dg2KD-rV.js";
import { t as collectExecPolicyScopeSnapshots } from "./exec-approvals-effective-kB8r0OfL.js";
import { a as resolveNodeId, r as nodesCallOpts } from "./rpc-Cqqfq2WL.js";
...
function registerExecApprovalsCli(program) {
	...
	const allowlist = approvals.command("allowlist").description("Edit the per-agent allowlist").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatExample("enclawed approvals allowlist add \"~/Projects/**/bin/rg\"", "Allowlist a local binary pattern for the main agent.")}\n${formatExample("enclawed approvals allowlist add --agent main --node <id|name|ip> \"/usr/bin/uptime\"", "Allowlist on a specific node/agent.")}\n${formatExample("enclawed approvals allowlist add --agent \"*\" \"/usr/bin/uname\"", "Allowlist for all agents (wildcard).")}\n${formatExample("enclawed approvals allowlist remove \"~/Projects/**/bin/rg\"", "Remove an allowlist pattern.")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.enclawed.ai/cli/approvals")}\n`);
