...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, T as getCommandPositionalsWithRootOptions, k as getVerboseFlag } from "./logger-wuQoU2z2.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-L1bu69lF.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-Bs3b9ucU.js";
import { n as defaultRuntime } from "./runtime-DVd7lkz0.js";
import { a as enableConsoleCapture } from "./subsystem-WGqvYWrS.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-DMXNDA06.js";
import { _ as resolveStateDir } from "./paths-Z7drr_SN.js";
import { t as assertSupportedRuntime } from "./runtime-guard-_x9R7u4P.js";
import { t as resolveManifestCommandAliasOwner } from "./manifest-command-aliases.runtime-CLmHJO3U.js";
import { s as hasMemoryRuntime } from "./memory-state-BV-DKPhJ.js";
import "./logging-DotYzX7j.js";
import { i as initializeDebugProxyCapture, r as finalizeDebugProxyCapture } from "./runtime-B0vRcVVh.js";
import { n as ensureGlobalUndiciEnvProxyDispatcher } from "./undici-global-dispatcher-CEjuidQ3.js";
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-BsIUk0l1.js";
import { t as ensureEnclawedCliOnPath } from "./path-env-eo6X4A_D.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-Dvyf7tUC.js";
import { n as maybeWarnAboutDebugProxyCoverage } from "./coverage-Bq6V39PW.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-CqUB3WIZ.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-BfQ6D9zq.js");
			...
			const { statusCommand } = await import("./status-BKgL3yHD.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-C7tTPZE3.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-BEQ2NUm-.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-tre_nY5m.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-CtSI5Wdk.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-CtSI5Wdk.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-CYlb-nwU.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-CYlb-nwU.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-vVis1eEz.js");
...
async function tryRouteCli(argv) {
	if (isTruthyEnvValue(process.env.ENCLAWED_DISABLE_ROUTE_FIRST)) return false;
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-C-lcS-ph.js");
...
function resolveMissingPluginCommandMessage(pluginId, config, options) {
	...
	if (parentPluginId) {
		...
		if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) return `The \`enclawed ${normalizedPluginId}\` command is unavailable because \`plugins.entries.${parentPluginId}.enabled=false\`. Re-enable that entry if you want the bundled plugin command surface.`;
		if (commandAlias.kind === "runtime-slash") return `"${normalizedPluginId}" is a runtime slash command (/${normalizedPluginId}), not a CLI command. It is provided by the "${parentPluginId}" plugin. ${commandAlias.cliCommand ? `Use \`enclawed ${commandAlias.cliCommand}\` for related CLI operations, or ` : "Use "}\`/${normalizedPluginId}\` in a chat session.`;
	}
	if (allow.length > 0 && !allow.includes(normalizedPluginId)) {
		if (parentPluginId && allow.includes(parentPluginId)) return null;
		return `The \`enclawed ${normalizedPluginId}\` command is unavailable because \`plugins.allow\` excludes "${normalizedPluginId}". Add "${normalizedPluginId}" to \`plugins.allow\` if you want that bundled plugin CLI surface.`;
	}
	if (config?.plugins?.entries?.[normalizedPluginId]?.enabled === false) return `The \`enclawed ${normalizedPluginId}\` command is unavailable because \`plugins.entries.${normalizedPluginId}.enabled=false\`. Re-enable that entry if you want the bundled plugin CLI surface.`;
	return null;
}
...
async function runCli(argv = process$1.argv) {
	...
	if ((parsedContainer.container ?? normalizeOptionalString(process$1.env.ENCLAWED_CONTAINER) ?? null) && parsedProfile.profile) throw new Error("--container cannot be combined with --profile/--dev");
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-CxUOVAL-.js");
	...
	if (shouldEnsureCliPath(normalizedArgv)) ensureEnclawedCliOnPath();
	assertSupportedRuntime();
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-BS1GeSoo.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-nlNVfBBp.js");
		...
		const [{ buildProgram }, { installUnhandledRejectionHandler }, { restoreTerminalState }] = await Promise.all([
			import("./program-DDugyYPA.js"),
			import("./unhandled-rejections-BrEKYAZt.js"),
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-5S_jg4cp.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-DUVRBuUu.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-Dg0TUyyo.js");
