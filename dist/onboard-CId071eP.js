...
import { u as resolveGatewayPort } from "./paths-CDjhyzOH.js";
import { t as assertSupportedRuntime } from "./runtime-guard-DVKiBAxK.js";
import "./daemon-runtime-3pRKLH22.js";
import { d as resolveSecretInputRef, s as isValidEnvSecretRefId } from "./types.secrets-B-g1z55T.js";
...
import { l as readConfigFileSnapshot } from "./io-BPigC1a6.js";
...
import { c as normalizeGatewayTokenInput, m as waitForGatewayReachable, n as applyWizardMetadata, o as handleReset, r as ensureWorkspaceAndSessions, t as DEFAULT_WORKSPACE } from "./onboard-helpers-DnqWC8bO.js";
import { i as resolveDeprecatedAuthChoiceReplacement, n as isDeprecatedAuthChoice, r as normalizeLegacyOnboardAuthChoice, t as formatDeprecatedNonInteractiveAuthChoiceError } from "./auth-choice-legacy-Bp9C6f0M.js";
...
function applyNonInteractiveGatewayConfig(params) {
	...
	if (authMode === "token") if (gatewayTokenRefEnv) {
		if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
			runtime.error("Invalid --gateway-token-ref-env (use env var name like ENCLAWED_GATEWAY_TOKEN).");
			runtime.exit(1);
			return null;
		}
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-ojzYPwE3.js");
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BBbJ7M3l.js");
...
async function runNonInteractiveLocalSetup(params) {
	...
	const inferredAuthChoice = opts.authChoice ? void 0 : (await import("./auth-choice-inference-DUJA6IOw.js")).inferAuthChoiceFromFlags(opts, {
		config: nextConfig,
		workspaceDir,
		env: process.env
	});
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-CqHMEOEk.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-DJJaz-rs.js");
		...
		if (!daemonInstall.installed && !opts.skipHealth) {
			logNonInteractiveOnboardingFailure({
				...
				hints: daemonInstall.skippedReason === "systemd-user-unavailable" ? ["Fix: rerun without `--install-daemon` for one-shot setup, or enable a working user-systemd session and retry.", "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `enclawed gateway run` or `enclawed agent --local`."] : [`Run \`${formatCliCommand("enclawed gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
	...
	if (!opts.skipHealth) {
		...
		if (!probe.ok) {
			...
			logNonInteractiveOnboardingFailure({
				...
				].filter((value) => Boolean(value)) : [`Run \`${formatCliCommand("enclawed gateway status --deep")}\` for more detail.`]
			});
			runtime.exit(1);
			return;
		}
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("enclawed configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.enclawed.ai/tools/web`);
}
//#endregion
...
async function runNonInteractiveRemoteSetup(params) {
	...
	if (opts.json) writeRuntimeJson(runtime, payload);
		...
		runtime.log(`Tip: run \`${formatCliCommand("enclawed configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.enclawed.ai/tools/web`);
	}
}
//#endregion
...
async function runNonInteractiveSetup(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("enclawed doctor")}\` to repair it, then re-run setup.`);
		runtime.exit(1);
		return;
	}
