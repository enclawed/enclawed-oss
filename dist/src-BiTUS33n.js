...
function parseManifest$1(raw) {
	...
	const id = scalarField(raw.id, "manifest.id");
	if (!id) throw new Error("manifest.id is required");
	const publisher = scalarField(raw.publisher, "manifest.publisher");
	if (!publisher) throw new Error("manifest.publisher is required");
	const version = scalarField(raw.version, "manifest.version");
	if (!version) throw new Error("manifest.version is required");
	const clearance = scalarField(raw.clearance, "manifest.clearance");
...
function canonicalize(value) {
	...
	return "{" + Object.keys(value).toSorted().map((k) => JSON.stringify(k) + ":" + canonicalize(value[k])).join(",") + "}";
}
function canonicalManifestBytes$1(manifest) {
	const body = {
		...
		capabilities: [...manifest.capabilities].toSorted((a, b) => a < b ? -1 : a > b ? 1 : 0),
		signerKeyId: manifest.signerKeyId ?? null,
		verification: manifest.verification,
		netAllowedHosts: [...manifest.netAllowedHosts ?? []].toSorted((a, b) => a < b ? -1 : a > b ? 1 : 0)
	};
...
//#endregion
export { registerServer as i, HttpJsonRpcTransport as n, QClearedMcpClient as r, GoogleOAuthProvider as t };
