...
import { n as isDangerousNetworkMode, r as normalizeNetworkMode } from "./network-mode-BsENi_kX.js";
import { u as resolveToolProfilePolicy } from "./tool-policy-D994AhXf.js";
import { n as resolveSandboxConfigForAgent, o as resolveSandboxToolPolicyForAgent } from "./config-CS3yaNEN.js";
import { r as pickSandboxToolPolicy, t as isToolAllowedByPolicies } from "./tool-policy-match-BLA48GX4.js";
import { t as resolvePluginWebSearchProviders } from "./web-search-providers.runtime-DscBV_55.js";
import { f as getBlockedBindReason } from "./docker-CPa7jZlG.js";
import { i as resolveNodeCommandAllowlist, t as DEFAULT_DANGEROUS_NODE_COMMANDS } from "./node-command-policy-cRTZg7hq.js";
...
import { a as collectSandboxBrowserHashLabelFindings, c as readConfigSnapshotForAudit, i as collectPluginsTrustFindings, o as collectStateDeepFilesystemFindings, s as collectWorkspaceSkillSymlinkEscapeFindings, t as collectIncludeFilePermFindings } from "./audit-extra.async-DsBKVjao.js";
...
function collectSecretsInConfigFindings(cfg) {
	...
	if (password && !looksLikeEnvRef(password)) findings.push({
		...
		remediation: "Prefer ENCLAWED_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk."
	});
...
function collectHooksHardeningFindings(cfg, env = process.env) {
	...
	const enclawedGatewayToken = typeof env.ENCLAWED_GATEWAY_TOKEN === "string" && env.ENCLAWED_GATEWAY_TOKEN.trim() ? env.ENCLAWED_GATEWAY_TOKEN.trim() : null;
	const gatewayToken = gatewayAuth.mode === "token" && typeof gatewayAuth.token === "string" && gatewayAuth.token.trim() ? gatewayAuth.token.trim() : enclawedGatewayToken ? enclawedGatewayToken : null;
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-enclawed-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
