...
import { a as resolveHookEntries } from "./config-iLzV83ml.js";
import { i as buildPluginDiagnosticsReport } from "./status-DcaEkiOH.js";
...
import { t as buildWorkspaceHookStatus } from "./hooks-status-BOEHTWek.js";
import { n as runPluginInstallCommand, t as runPluginUpdateCommand } from "./plugins-update-command-BhgOCFR1.js";
...
function formatHooksList(report, opts) {
	...
	if (hooks.length === 0) return opts.eligible ? `No eligible hooks found. Run \`${formatCliCommand("enclawed hooks list")}\` to see all hooks.` : "No hooks found.";
...
function registerHooksCli(program) {
	...
	hooks.command("install").description("Deprecated: install a hook pack via `enclawed plugins install`").argument("<path-or-spec>", "Path to a hook pack or npm package spec").option("-l, --link", "Link a local path instead of copying", false).option("--pin", "Record npm installs as exact resolved <name>@<version>", false).action(async (raw, opts) => {
		defaultRuntime.log(theme.warn("`enclawed hooks install` is deprecated; use `enclawed plugins install`."));
		await runPluginInstallCommand({
			raw,
			opts
		});
	});
	hooks.command("update").description("Deprecated: update hook packs via `enclawed plugins update`").argument("[id]", "Hook pack id (omit with --all)").option("--all", "Update all tracked hooks", false).option("--dry-run", "Show what would change without writing", false).action(async (id, opts) => {
		defaultRuntime.log(theme.warn("`enclawed hooks update` is deprecated; use `enclawed plugins update`."));
