...
import { n as compareSemverStrings, t as checkUpdateStatus } from "./update-check-CkD2wWDp.js";
//#region src/commands/status.update.ts
async function getUpdateCheckResult(params) {
	return await checkUpdateStatus({
		root: await resolveEnclawedPackageRoot({
...
function formatUpdateAvailableHint(update) {
	...
	return `Update available${details.length > 0 ? ` (${details.join(" · ")})` : ""}. Run: ${formatCliCommand("enclawed update")}`;
}
