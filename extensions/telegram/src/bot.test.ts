...
describe("createTelegramBot", () => {
  ...
  it("edits commands list for pagination callbacks", async () => {
    ...
    // The page count is derived from the command registry, so pinning it as a
    // literal made this fail whenever a command was added or removed. What must
    // hold is that the keyboard counter agrees with the header above it.
    const headerPageCount = String(text).match(/Commands \(2\/(\d+)\)/)?.[1];
    expect(headerPageCount, "commands header should carry a page count").toBeTruthy();
    expect(params).toEqual({
      reply_markup: {
        inline_keyboard: [
          [
            { text: "◀ Prev", callback_data: "commands_page_1:main" },
            { text: `2/${headerPageCount}`, callback_data: "commands_page_noop:main" },
