...
type SlackTestState = {
  ...
  reactAddMock: Mock<(...args: unknown[]) => unknown>;
  reactRemoveMock: Mock<(...args: unknown[]) => unknown>;
...
function ensureSlackTestRuntime(): {
  ...
  if (!globalState.__slackClient) {
    globalState.__slackClient = {
      ...
      reactions: {
        // Both feed reactMock so existing call-count assertions keep working,
        // and each also feeds a dedicated mock: a combined list cannot say
        // whether "scream" means the error was set or taken away.
        add: (...args: unknown[]) => {
          slackTestState.reactAddMock(...args);
          return slackTestState.reactMock(...args);
        },
        remove: (...args: unknown[]) => {
          slackTestState.reactRemoveMock(...args);
          return slackTestState.reactMock(...args);
        },
      },
    };
...
export function resetSlackTestState(config: Record<string, unknown> = defaultSlackTestConfig()) {
  ...
  slackTestState.reactAddMock.mockReset();
  slackTestState.reactRemoveMock.mockReset();
