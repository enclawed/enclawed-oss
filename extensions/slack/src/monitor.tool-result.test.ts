...
describe("monitorSlackProvider tool results", () => {
  ...
  it("keeps the error reaction when dispatch fails before any reply is delivered", async () => {
    ...
    expect(sendMock).not.toHaveBeenCalled();
    // Stated as adds and removes rather than one merged list, which could not
    // distinguish "the error was set" from "the error was taken away".
    const added = slackTestState.reactAddMock.mock.calls.map(
      ([args]) => (args as { name: string }).name,
    );
    const removed = slackTestState.reactRemoveMock.mock.calls.map(
      ([args]) => (args as { name: string }).name,
    );
    expect(added).toEqual(["eyes", "scream"]);
    expect(removed).toEqual(["eyes"]);
    expect(removed).not.toContain("scream");
  });
