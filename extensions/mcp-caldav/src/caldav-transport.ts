...
export type CalDavTransportOptions = Readonly<{
   ...
   * Most users want their primary calendar; the executive assistant's
...
export class CalDavTransport implements McpTransport {
  ...
  private async getEvent(args: Record<string, unknown>): Promise<EventDetail> {
    ...
    // API), walk the calendar. Bounded scan: the executive assistant's lookups
...
export function normalizeRrule(raw: unknown): string | undefined {
  ...
  for (const p of parts) {
    ...
    if (key === "FREQ") {
      hasFreq = true;
      if (!FREQ_VOCAB.has(val.toUpperCase())) {
        return undefined;
      }
    }
  }
...
function defaultEnd(start: Date): Date {
  // Default time window: 14 days forward. The executive assistant's
