...
describe("memory cli", () => {
  ...
  it("prints conceptual promotion signals", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      // Recorded relative to now. These were fixed calendar dates, and promote
      // drops candidates older than its maxAgeDays threshold -- so the case
      // worked when written and silently became "No short-term recall
      // candidates." once real time moved past the window.
      const dayMs = 24 * 60 * 60 * 1000;
      const firstRecallMs = Date.now() - 3 * dayMs;
      const secondRecallMs = Date.now() - 1 * dayMs;
      await recordShortTermRecalls({
        workspaceDir,
        query: "router vlan",
        nowMs: firstRecallMs,
