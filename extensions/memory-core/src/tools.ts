  ...
  return createMemoryTool({
    ...
    execute:
      ...
      async (_toolCallId, rawParams) => {
        // AnyAgentTool's `execute` types params as the schema's Static. Through
        // the `any`-parameterised AnyAgentTool alias this widens to unknown,
        // which the readXxxParam helpers (typed against Record<string, unknown>)
        // reject. The schema guarantees it is a record at runtime.
...
export function createMemoryGetTool(options: {
  ...
  return createMemoryTool({
    ...
    execute:
      ({ cfg, agentId }) =>
      async (_toolCallId, rawParams) => {
        // See createMemorySearchTool above for why this cast is required.
        const params = rawParams as Record<string, unknown>;
