...
describe("short-term promotion", () => {
  ...
  it("audits and repairs invalid store metadata plus stale locks", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      // A hardcoded owner pid makes this load-dependent: pid_max is 4194304 on
      // Linux, so 999999 is an allocatable pid, and when the suite runs wide
      // enough for a live process to hold it the audit correctly refuses to call
      // the lock stealable and this case fails. Spawn a child and wait for it to
      // exit -- the kernel allocates pids upward, so the pid it just released is
      // reliably dead, which is the state the case is actually about.
      ...
      await fs.writeFile(lockPath, `${deadPid}:0\n`, "utf-8");
