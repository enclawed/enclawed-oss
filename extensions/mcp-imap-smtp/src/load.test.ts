...
describe("mcp-imap-smtp / loadImapSmtpBridge", () => {
  ...
  it("admits the full canonical tool surface (5 tools) by default", () => {
    ...
    // The canonical list has grown since this was written; pin it to the list
    // itself rather than a number that goes stale silently.
    expect(registered.allowedTools.length).toBe(IMAP_SMTP_TOOLS.length);
  });
