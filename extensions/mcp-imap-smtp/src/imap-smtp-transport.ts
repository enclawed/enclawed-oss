...
import type { JsonRpcResult } from "@enclawed/mcp-attested";
import type { McpTransport } from "@enclawed/mcp-attested";

/**
 * SMTP auth is either a password or a bearer access token.
 *
 * nodemailer distinguishes them by the `type` discriminator; imapflow by
 * whether `accessToken` is present. Both are expressed here so the transport
...
/**
 * How the mailbox is authenticated.
 *
 * `oauth` supplies a function rather than a token because access tokens live
 * about an hour and this transport holds an IMAP connection open for days.
 * The token is therefore fetched at CONNECT time, never at construction, and
 * the supplier owns refresh. Handing in a string would work until the first
 * reconnect after expiry and then fail as an authentication error, which
 * reads exactly like a wrong password.
 */
export type MailAuth =
  | Readonly<{ kind: "password"; password: string }>
  | Readonly<{ kind: "oauth"; getAccessToken: () => Promise<string> }>;
...
export type ImapSmtpTransportOptions = Readonly<{
  ...
  username: string;
  /**
   * Password auth. Retained for existing callers and for hosts with no OAuth;
   * exactly one of `password` or `auth` must be supplied.
   */
  password?: string;
  /** OAuth (or explicit password) auth. Preferred over `password`. */
  auth?: MailAuth;
  /**
...
export type ThreadMessage = Readonly<{
  ...
  subject: string;
  bodyText: string;
  /** Verbatim RFC822 source, base64. Audit-only; never enters LLM context. */
...
export type SendDraftOutput = Readonly<{
  ...
  // recipient the executive assistant intended to send to.
  recipient: string;
}>;
...
export class ImapSmtpTransport implements McpTransport {
  ...
  private readonly auth: MailAuth;
  /** Token the cached SMTP transporter was built with, to detect staleness. */
  private smtpToken: string | null = null;
  ...
  constructor(opts: ImapSmtpTransportOptions) {
    ...
    // A password is required unless OAuth is supplying the credential. The
    // wording stays as it was: without `auth`, a password is exactly what is
    // missing, and existing callers assert on this message.
    ...
    this.fromHeader =
      dispRaw.length > 0
        ...
        : opts.username;
    // `auth` wins when both are given; the validation above guarantees one of
    // them exists.
    this.auth = opts.auth ?? { kind: "password", password: opts.password ?? "" };
    this.imapCfg = {
      host: opts.imap.host,
      port: opts.imap.port,
      secure: opts.imap.secure,
      // Placeholder only. Every connection re-resolves credentials through
      // imapConfig() so an OAuth token is fresh at connect time.
      auth: { user: opts.username, pass: "" },
      // imapflow's default logger pretty-prints to stderr. The
      // executive assistant runs as a background service and the audit log is
      ...
      // "executive assistant went silent after a while". 4 min is well under
  ...
  private async getImap(): Promise<ImapFlow> {
    if (this.imap?.usable) {
      ...
      // with stale/empty results, and the operator sees "executive assistant
    ...
    this.connectingPromise = (async () => {
      const client = this.imapFactory(await this.imapConfig());
      // ImapFlow is an EventEmitter, and a connection-level failure
      // (socket timeout, peer reset) arrives as an 'error' EVENT rather
      // than as a rejection of the command currently being awaited. An
      // 'error' event with no listener is fatal in Node: it rethrows on
      // the event loop and terminates the process. That is how an IMAP
      // server refusing our credentials turned into a restart loop —
      // each poll logged AUTHENTICATIONFAILED and carried on, then the
      // idle socket timed out and killed the service outright.
      //
      // Absorb it and mark the connection unusable so the next
      // getImap() rebuilds. Commands still surface their own errors
      // through their own rejections; this listener exists to keep a
      // dead socket from taking the process down with it.
      client.on("error", () => {
        if (this.imap === client) {
          this.imap = null;
        }
    ...
    try {
      ...
      for await (const msg of imap.fetch(
        ...
        messages.push({
          ...
          subject: env.subject ?? "(no subject)",
          bodyText,
          // Verbatim RFC822 -- every header and the untruncated body --
          // base64'd for the audit trail. Base64 because the audit
          // logger replaces every control character with U+FFFD before
          // hashing, which would shred a raw message; encoded, the
          // recorded bytes are exactly what arrived on the wire and the
          // hash chain covers them. The agent never sees this field: it
          // is carried past the LLM context to the auditor, so raising
          // the audited fidelity does not widen the injection surface.
          rawSourceBase64: msg.source ? Buffer.from(msg.source).toString("base64") : "",
          labels: Object.freeze([...labelSet]),
  ...
  private async sendDraft(args: Record<string, unknown>): Promise<SendDraftOutput> {
    ...
    try {
      ...
      // name ("Alfredo's Executive Assistant <user@gmail.com>") — and Gmail
      ...
      // Mirror to Sent AFTER the draft is gone. Gmail's folders are
      // labels over one message store, deduplicated by Message-ID, and
      // this copy is byte-identical to the draft that was just sent --
      // same Message-ID. Trashing is not an ordinary label removal in
      // Gmail: a trashed message leaves every other view, Sent
      // included. Appending first therefore created the Sent copy and
      // the draft cleanup immediately took it away again, which is
      // exactly what "the Sent folder is empty" looked like from the
      // outside: append succeeded, no error, recipient got the mail,
      // nothing in Sent.
      //
  ...
  // Lets the executive assistant "consume" a thread after handling it so the
  ...
  private async getAttachment(args: Record<string, unknown>): Promise<{
    ...
    if (messageIdx < 0) {
      throw new Error("messageIdx must be a non-negative integer");
    }
    if (attachmentIdx < 0) {
      throw new Error("attachmentIdx must be a non-negative integer");
    }
    ...
    try {
      ...
      const uids: number[] = searchRes === false ? [] : searchRes;
      if (uids.length === 0) {
        throw new Error(`no messages found for threadId=${threadId}`);
      }
      const sorted = [...uids].toSorted((a, b) => a - b);
  ...
  private async getSmtp(): Promise<Transporter> {
    if (this.auth.kind === "password") {
      if (!this.smtp) {
        this.smtp = this.smtpFactory({
          ...this.smtpCfg,
          auth: { user: this.username, pass: this.auth.password },
        });
      }
      return this.smtp;
    }
    // A nodemailer transporter holds whatever token it was created with; it
    // does not refresh. Rebuild whenever the supplier hands back a different
    // one, which is what happens after the hourly refresh. Comparing tokens
    // rather than timestamps means this stays correct however the supplier
    // decides when to refresh.
    const accessToken = await this.auth.getAccessToken();
    if (!this.smtp || this.smtpToken !== accessToken) {
      this.smtp = this.smtpFactory({
        ...this.smtpCfg,
        auth: { type: "OAuth2", user: this.username, accessToken },
      });
      this.smtpToken = accessToken;
    }
    return this.smtp;
  }

  /**
   * IMAP settings with credentials resolved now, not at construction.
   *
   * imapflow authenticates once per connection, so a token fetched here is
   * valid for the life of that connection; the next reconnect gets a fresh
   * one. That is the whole reason `getAccessToken` is a function.
   */
  private async imapConfig(): Promise<ConstructorParameters<typeof ImapFlow>[0]> {
    if (this.auth.kind === "password") {
      return { ...this.imapCfg, auth: { user: this.username, pass: this.auth.password } };
    }
    const accessToken = await this.auth.getAccessToken();
    return { ...this.imapCfg, auth: { user: this.username, accessToken } };
  }
