...
import { canonicalManifestBytes, parseManifest } from "../../../enclawed/src/module-manifest.mjs";
import { generateEd25519KeyPair, signManifest } from "../../../enclawed/src/module-signing.mjs";
...
test("q-cleared MCP server: a Secret-only server is rejected", () => {
  ...
  try {
    const assertion = buildSignedAssertion(privateKey, "lab-secret-signer", "secret", [
      "mcp-server",
    ]);
    const d = checkModule(assertion, { flavor: "enclaved", requiredClearance: "q-cleared" });
...
test("q-cleared MCP server: assertion with wrong signer key is rejected", () => {
  ...
  try {
    ...
    const assertion = buildSignedAssertion(b.privateKey, "lab-q-signer", "q-cleared", [
      "mcp-server",
    ]);
    const d = checkModule(assertion, { flavor: "enclaved", requiredClearance: "q-cleared" });
    assert.equal(d.allowed, false);
