...
export class GoogleOAuthProvider {
  ...
  async getToken(): Promise<string> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Google OAuth refresh failed: HTTP ${res.status}${snippet ? `: ${snippet}` : ""}`,
      );
    }
    ...
    const expiresIn =
      typeof payload.expires_in === "number" && payload.expires_in > 0 ? payload.expires_in : 3300; // safe default ~55min
  ...
  isConfigured(): boolean {
    if (read(this.env, "GOOGLE_OAUTH_TOKEN")) {
      return true;
    }
    return Boolean(
