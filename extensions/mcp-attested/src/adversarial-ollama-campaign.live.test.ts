...
function classify(name: string): string {
  // oxlint-disable-next-line typescript/no-misused-spread -- splitting into code points is exactly what this classifier inspects.
  const codes = [...name].map((c) => c.charCodeAt(0));
  if (codes.some((c) => c > 0x7f)) {
    return "non-ascii (homoglyph/zw/rtl)";
  }
  if (codes.some((c) => c < 0x20) || /\s/.test(name)) {
    return "whitespace/control";
  }
  if (/[;&|]/.test(name)) {
    return "separator/chaining";
  }
  if (name.includes("/") || name.includes("..")) {
    return "path-traversal";
  }
  const lc = name.toLowerCase();
  if (ALLOWED.some((a) => a.toLowerCase() === lc)) {
    return "case-variant";
  }
  if (name.length > 0 && ALLOWED.some((a) => lev(a, name) <= 2)) {
    return "near-miss";
  }
  return "other";
}
...
describe.skipIf(!RUN)("Ollama adversarial CAMPAIGN (coverage)", () => {
  beforeAll(async () => {
    ...
    if (!r || !r.ok) {
      throw new Error(`OLLAMA_LIVE=1 but Ollama not reachable at ${OLLAMA_HOST}`);
    }
  });
  afterAll(() => {
  ...
  it(`runs ${ROUNDS} rounds, dedupes, classifies, and asserts full coverage`, async () => {
    ...
    await pool(CONC, ROUNDS, async (r) => {
      try {
        ...
        for (const s of extractStrings(raw)) {
          toolGenerated++;
          if (!ALLOWED.includes(s as (typeof ALLOWED)[number])) {
            names.add(s);
          }
        }
      } catch {
    ...
    try {
      ...
      // ---- Report ----
      const cats = [...byCat.entries()].toSorted((a, b) => b[1].unique - a[1].unique);
