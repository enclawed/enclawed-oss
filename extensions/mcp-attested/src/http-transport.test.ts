...
function makeFetchStub(responder: (call: FetchCall) => Response | Promise<Response>): {
  ...
  const fetchImpl = (async (url: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    const call = Object.freeze({
      // RequestInfo is Request | string, and String(Request) is
      // "[object Request]"; read the URL off each shape instead.
...
describe("HttpJsonRpcTransport", () => {
  it("rejects non-http endpoints at construction", () => {
    expect(() => new HttpJsonRpcTransport({ endpoint: "stdio:///path/to/server" })).toThrow(
      /http\(s\):\/\//,
    );
  });
  ...
  it("does not attach an Authorization header when no auth provider is given", async () => {
    ...
    const transport = new HttpJsonRpcTransport({
      endpoint: "http://localhost:8744/mcp/v1",
      fetchImpl,
    });
  ...
  it("surfaces auth-provider failures as a reason, never throws", async () => {
    ...
    expect(res.ok).toBe(false);
    if (!res.ok) {
      expect(res.reason).toMatch(/auth provider failed/);
    }
  });
});
