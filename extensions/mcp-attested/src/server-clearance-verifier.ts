...
import {
  ...
  type ModuleManifest,
  parseManifest,
} from "@enclawed/module-trust/module-manifest.js";
import { verifyManifestSignature } from "@enclawed/module-trust/module-signing.js";
import { findSigner } from "@enclawed/module-trust/trust-root.js";
...
export async function verifyServerClearance(
  ...
  try {
    ...
    if (!r.ok) {
      return { ok: false, reason: `clearance fetch HTTP ${r.status}` };
    }
    raw = await r.json();
  } catch (e) {
