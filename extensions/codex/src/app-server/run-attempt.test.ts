...
describe("runCodexAppServerAttempt", () => {
  ...
  it("builds deterministic opaque Codex native hook relay ids", () => {
    ...
    // The digest changed when the hash domain was renamed openclaw: -> enclawed:
    // (verified: the previous expectation is exactly the openclaw: digest for
    // these inputs). The id is opaque -- what matters is that it is stable and
    // leaks neither agent nor session, both asserted below.
    expect(relayId).toBe("codex-e684aec4b3d18e06757e0c2c402d876613c726ec");
