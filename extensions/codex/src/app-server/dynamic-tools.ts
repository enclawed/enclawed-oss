  ...
  return {
    ...
    handleToolCall: async (call, options) => {
      ...
      try {
        ...
        // A refusal is reported to after_tool_call as a RESULT describing the
        // block, not as an error string, so an audit can tell "the policy said
        // no, and which policy" from "the tool threw".
        void runAgentHarnessAfterToolCallHook({
          toolName: tool.name,
          toolCallId: call.callId,
          runId: params.hookContext?.runId,
        ...
        return {
          ...
          // A policy refusal is a completed call whose answer is "no", not a
          // failure: reporting it as a failure invites a retry that only
          // re-runs the gate.
          success: blocked,
        };
      }
