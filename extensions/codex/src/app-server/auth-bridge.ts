...
async function resolveCodexAppServerAuthProfileLoginParamsInternal(params: {
  ...
  // Read-through, not ensure. ensureAuthProfileStore materialises the main
  // agent's secret-bearing profiles into the child agent's directory when the
  // child has no store of its own -- so merely LOOKING UP a credential for a
  // subagent wrote a second copy of the owner's OAuth tokens to disk, with its
  // own lifetime and outside the owner's rotation. The secrets-runtime loader
  // merges the main store for reading without cloning it, which is all this
  // needs; refresh below already resolves and writes through to the owner.
