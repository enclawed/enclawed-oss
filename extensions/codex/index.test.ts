...
describe("codex plugin", () => {
  it("is opt-in by default", () => {
    ...
    // The property is "not on unless asked for", which an absent key and an
    // explicit `false` both express. This used to require the key be absent,
    // so writing the safer thing down explicitly broke the test while changing
    // nothing about the behaviour. Assert the posture, not its spelling.
    expect(manifest.enabledByDefault ?? false).toBe(false);
  });
