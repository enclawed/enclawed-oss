...
describe("parseDocumentExtractConfig", () => {
  ...
  it("rejects unknown keys", () => {
    expect(() => parseDocumentExtractConfig({ allowedSources: ["x"], bogus: 1 })).toThrow(
      DocumentExtractConfigError,
    );
  });

  it("rejects non-array allowedSources", () => {
    expect(() => parseDocumentExtractConfig({ allowedSources: "x" })).toThrow(/allowedSources/);
  });

  it("rejects empty strings inside allowedSources", () => {
    expect(() => parseDocumentExtractConfig({ allowedSources: ["", "ok"] })).toThrow(
      /allowedSources/,
    );
  });

  it("rejects negative maxFileSizeMB", () => {
    expect(() => parseDocumentExtractConfig({ maxFileSizeMB: -1 })).toThrow(/maxFileSizeMB/);
  });
});
