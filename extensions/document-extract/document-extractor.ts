...
const VALID_DOCUMENT_EXTRACT_KEYS = new Set(["allowedSources", "maxFileSizeMB"]);
...
export function parseDocumentExtractConfig(raw: unknown): DocumentExtractConfig {
  ...
  if (typeof raw !== "object" || Array.isArray(raw)) {
    throw new DocumentExtractConfigError("extensions.document-extract config must be an object");
  }
  const cfg = raw as Record<string, unknown>;
  for (const key of Object.keys(cfg)) {
    if (!VALID_DOCUMENT_EXTRACT_KEYS.has(key)) {
      throw new DocumentExtractConfigError(`extensions.document-extract: unknown key "${key}"`);
    }
  }
