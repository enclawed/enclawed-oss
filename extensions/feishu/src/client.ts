...
function createTimeoutHttpInstance(defaultTimeoutMs: number): Lark.HttpInstance {
  ...
  // The SDK holds two incompatible views of this one object. It exports
  // `defaultHttpInstance` as an AxiosInstance, whose methods resolve to the
  // axios response envelope, while `HttpInstance` -- the interface the client
  // is configured with -- declares them as returning the caller's own R.
  // Delegating therefore cannot typecheck on its own: the wrapper promises
  // whatever R the caller picked and can only forward what the base returns.
  // The assertion sits here, at that one boundary, rather than being spread
  // over the call sites or hidden by typing the whole object as unknown.
  return {
    request: <R = unknown, D = unknown>(opts: Lark.HttpRequestOptions<D>) =>
      base.request(injectTimeout(opts)) as Promise<R>,
    get: <R = unknown, D = unknown>(url: string, opts?: Lark.HttpRequestOptions<D>) =>
      base.get(url, injectTimeout(opts)) as Promise<R>,
    post: <R = unknown, D = unknown>(url: string, data?: D, opts?: Lark.HttpRequestOptions<D>) =>
      base.post(url, data, injectTimeout(opts)) as Promise<R>,
    put: <R = unknown, D = unknown>(url: string, data?: D, opts?: Lark.HttpRequestOptions<D>) =>
      base.put(url, data, injectTimeout(opts)) as Promise<R>,
    patch: <R = unknown, D = unknown>(url: string, data?: D, opts?: Lark.HttpRequestOptions<D>) =>
      base.patch(url, data, injectTimeout(opts)) as Promise<R>,
    delete: <R = unknown, D = unknown>(url: string, opts?: Lark.HttpRequestOptions<D>) =>
      base.delete(url, injectTimeout(opts)) as Promise<R>,
    head: <R = unknown, D = unknown>(url: string, opts?: Lark.HttpRequestOptions<D>) =>
      base.head(url, injectTimeout(opts)) as Promise<R>,
    options: <R = unknown, D = unknown>(url: string, opts?: Lark.HttpRequestOptions<D>) =>
      base.options(url, injectTimeout(opts)) as Promise<R>,
  };
}
