// Minimal vCard parser sufficient for the executive assistant's contact-gating
...
// scope for the executive assistant's contact gate. The parser drops what it
...
function parseLine(line: string): ContentLine | null {
  ...
  // parameter values can be DQUOTE-wrapped, but the executive assistant's
  ...
  const namePart = semi < 0 ? head : head.slice(0, semi);
  // Strip Apple/Google "item-group" prefix used to attach an
  // X-ABLabel to a property. Google Contacts CardDAV export uses
  // this form for every email/phone:
  //   item1.EMAIL;TYPE=INTERNET:alice@example.com
  //   item1.X-ABLabel:Personal
  // Without stripping, the literal field name "ITEM1.EMAIL" does
  // not match our `case "EMAIL"` dispatch and every email is
  // dropped — which is exactly the symptom that surfaced in the
  // probe ("3 contact(s)" with "no email" for each).
  const name = namePart.replace(/^item\d+\./i, "").toUpperCase();
