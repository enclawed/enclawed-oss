...
describe("google-meet plugin", () => {
  ...
  it("joins Chrome on a paired node without local Chrome or BlackHole", async () => {
    ...
    expect(respond.mock.calls[0]?.[0]).toBe(true);
    // The bridge asks for the full inventory including offline nodes, so it can
    // say "your configured node is offline" instead of "not found".
    expect(nodesList.mock.calls[0]).toEqual([{ includeOffline: true }]);
