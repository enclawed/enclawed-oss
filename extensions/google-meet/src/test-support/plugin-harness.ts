...
/**
 * What PluginRuntime.nodes.list actually returns.
 *
 * This used to be declared with the google-meet module's OWN field names
 * (`connected`, `caps`). The runtime speaks `online` and `capabilities`, and the
 * bridge translates between them -- so a stub shaped like the module's internal
 * type produced `connected: undefined` after translation, every node read as
 * offline and capability-less, and sixteen tests across two files asserted
 * against a node inventory that could never be healthy. Mock the contract being
 * mocked, not the shape on the far side of the translation.
 */
