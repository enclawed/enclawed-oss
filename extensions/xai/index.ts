import type { EnclawedPluginToolFactory } from "@enclawed/plugin-sdk/core";
...
export default defineSingleProviderPluginEntry({
  ...
  register(api) {
    ...
    // The factory return shape is structurally an AnyAgentTool but TS does
    // not bridge our typebox TObject parameter type to AgentTool's TSchema
    // base across the registerTool generic seam. Cast at the boundary; the
    // runtime contract is unchanged.
    api.registerTool(((ctx) => createLazyCodeExecutionTool(ctx)) as EnclawedPluginToolFactory, {
      name: "code_execution",
    });
    api.registerTool(((ctx) => createLazyXSearchTool(ctx)) as EnclawedPluginToolFactory, {
      name: "x_search",
    });
  },
});
