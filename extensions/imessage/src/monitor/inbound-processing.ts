...
export function resolveIMessageInboundDecision(params: {
  ...
  // Applies to DMs as well as groups: an open dmPolicy grants conversation,
  // not command execution, so letting it through on the strength of not being
  // a group dispatched the command anyway.
  if (shouldBlock) {
    const blockReason = isGroup ? "control command (unauthorized)" : "dmPolicy blocked";
    ...
    return { kind: "drop", reason: blockReason };
  }
