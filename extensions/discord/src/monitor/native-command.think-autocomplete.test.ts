...
describe("discord native /think autocomplete", () => {
  beforeAll(async () => {
    ...
    // Warm the bundled Discord channel plugin here rather than inside a test.
    // Its first lookup loads the plugin's whole source graph through jiti --
    // about a minute in source mode (installed apps load it natively from
    // dist/ instead) -- and the first test to touch it used to be charged the
    // lot against its own 120s budget: fine alone, a timeout under full-suite
    // load. Paying it once, under an explicit fixture budget, leaves each test
    // timing only its own logic.
    findCommandByNativeName("think", "discord");
  }, 300_000);
