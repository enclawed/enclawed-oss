...
vi.mock("./runtime.js", () => ({
  ...
  // The CLI reads config through this helper, and the mock did not provide it,
  // so every command under test threw on "No export is defined" before it ran.
  // The real one delegates to runtime.config.current, so this does too rather
  // than inventing a second source of truth for the same config.
  readMatrixRuntimeConfig: (...args: unknown[]) => matrixRuntimeLoadConfigMock(...args),
  // Same for the write side: the CLI calls this helper, whose real
  // implementation forwards to runtime.config.replaceConfigFile.
  matrixReplaceConfigFile: async (...args: unknown[]) =>
    await matrixRuntimeReplaceConfigFileMock(...args),
}));
