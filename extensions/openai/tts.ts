...
export async function openaiTTS(params: {
  ...
  const requestHeaders = resolveProviderRequestHeaders({
    provider: "openai",
    // Named, like the transcription path, so attribution can tell a native
    // /audio/speech call apart from an OpenAI-compatible endpoint. Without an
    // api the request matched no attribution rule and went out unattributed.
