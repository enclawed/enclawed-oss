...
describe("openai image generation provider", () => {
  ...
  beforeEach(() => {
    // Pin the QA escape hatch off before every case. unstubAllEnvs in the
    // afterEach only removes stubs this file made; it cannot clear the variable
    // when it arrives already set from the surrounding environment, and several
    // assertions below turn on allowPrivateNetwork being false for a loopback
    // base URL. Leaving that to ambient state made those cases pass or fail
    // depending on what else had run in the process.
    // AZURE_OPENAI_API_VERSION is pinned for the same reason: the Azure cases
    // assert the built-in default api-version, so an inherited value silently
    // rewrote the URL they were checking.
    vi.stubEnv("ENCLAWED_QA_ALLOW_LOCAL_IMAGE_PROVIDER", "");
