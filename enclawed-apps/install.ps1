...
#   Install-EnclawedApp executive-assistant
#
# Or from a clone:
#   .\enclawed-apps\install.ps1 executive-assistant
#
...
# Force UTF-8 on the console for both input and output. Without this,
# Windows PowerShell 5.x defaults to the legacy system codepage (Windows-
# 1252 on US installs), and any UTF-8 byte sequence written by the Node
# runtime — em-dashes, smart quotes, accented Latin chars in service.log
# / audit.jsonl — gets decoded as mojibake (E2 80 94 reads as "âEUR""
# instead of "--"). Setting this on every dot-source means every verb
# below (Get-EnclawedAppStatus / Start- / Stop- / Update-) inherits the
# right encoding. No-op on pwsh 7+ which already defaults to UTF-8.
try {
  [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
  [Console]::InputEncoding = [System.Text.Encoding]::UTF8
  $OutputEncoding = [System.Text.Encoding]::UTF8
} catch {
  # If the console can't be reconfigured (e.g. redirected output,
  # restricted host), keep going. The log file on disk is still UTF-8;
  # the only consequence is mojibake on the operator's screen.
}

# Default Get-Content / Out-File / Set-Content to UTF-8 for this
# session. Without this, Windows PowerShell 5.x's Get-Content on
# our UTF-8 service.log / audit.jsonl returns mojibake even when the
# console encoding is UTF-8 — the cmdlet's own file-read decoder
# uses the system codepage independent of Console.OutputEncoding.
# Same effect as passing -Encoding UTF8 to every call manually.
...
# MAINTENANCE: any edit to this file -- not just these constants --
# invalidates the signature under static/build/ and aborts every install
# on this platform. Re-sign before publishing:
#   pnpm sign:install-bootstraps --key <path>
#   pnpm check:install-bootstrap-signatures
# See docs/reference/install-bootstrap-signing.md.
...
$bldPub = "db6RgWCvz88kRfsCiNeTI8U0TJsDMA0tOGMxbkjJwIk="
...
<#
.SYNOPSIS
  Clear a DIRTY hardware-root latch after the enclaweder is reconnected.
.DESCRIPTION
  The guard halts accreditation and latches DIRTY when the bound enclaweder
  disconnects or fails a liveness check, and it refuses to boot afterwards --
  so this cannot be done from inside the running app. It prompts for the bound
  device's serial and requires an interactive console; the device itself still
  has to be present and pass a fresh challenge before anything is cleared.
.EXAMPLE
  Repair-EnclawedHardwareRoot executive-assistant
#>
function Repair-EnclawedHardwareRoot {
  [CmdletBinding()]
  param([Parameter(Position = 0, Mandatory = $true)] [string]$App)
  Invoke-EnclawedAppAction -App $App -Action "--recover-root"
}

<#
.SYNOPSIS
  List every enclawed command, with what it does.
.DESCRIPTION
  The verbs are dot-sourced into the session, so tab-completion only
  helps once you know the prefix and Get-Command needs a wildcard. This
  prints them all, grouped, with the settings commands alongside the
  install ones rather than buried.
.EXAMPLE
  Get-EnclawedHelp
#>
function Get-EnclawedHelp {
  [CmdletBinding()]
  param()
  $app = "executive-assistant"
  Write-Host ""
  Write-Host "enclawed commands" -ForegroundColor Cyan
  Write-Host ""
  Write-Host "  Install / update" -ForegroundColor DarkGray
  Write-Host "    Install-EnclawedApp $app          install, or repair an install"
  Write-Host "    Update-EnclawedApp $app           pull latest code, rebuild, restart"
  Write-Host "    Uninstall-EnclawedApp $app        remove service, env, audit log, keyring"
  Write-Host ""
  Write-Host "  Settings" -ForegroundColor DarkGray
  Write-Host "    Set-EnclawedAppSettings $app              walk every setting, explained"
  Write-Host "    Set-EnclawedAppSettings $app -List        show current values"
  Write-Host "    Set-EnclawedAppSettings $app -Set K=V     change one, non-interactive"
  Write-Host ""
  Write-Host "  Run / inspect" -ForegroundColor DarkGray
  Write-Host "    Get-EnclawedAppStatus $app        is it running, and is it working"
  Write-Host "    Start-EnclawedApp $app            start the service"
  Write-Host "    Stop-EnclawedApp $app             stop it (keeps credentials + audit log)"
  Write-Host "    Test-EnclawedApp $app             probe connectivity and config"
  Write-Host "    Repair-EnclawedHardwareRoot $app  clear a DIRTY hardware-root latch"
  Write-Host ""
  Write-Host "  Any command: Get-Help <name> -Full   e.g. Get-Help Set-EnclawedAppSettings -Full"
  Write-Host "  Credentials are re-provisioned by Install-EnclawedApp; they are not settings."
  Write-Host ""
}

<#
.SYNOPSIS
  Review or change an installed app's settings without reinstalling.
.DESCRIPTION
  Walks every setting, explaining what each one does, and writes only
  what you change. Reinstalling to edit one value re-runs credential
  acquisition, the workspace build and the adversarial gate, and risks
  losing a working app password at the "reuse credentials?" prompt.
  Secrets are not handled here: they live in the OS keyring.
.EXAMPLE
  Set-EnclawedAppSettings executive-assistant
.EXAMPLE
  Set-EnclawedAppSettings executive-assistant -List
.EXAMPLE
  Set-EnclawedAppSettings executive-assistant -Set ENCLAWEDER_ROOT=required
#>
function Set-EnclawedAppSettings {
  [CmdletBinding()]
  param(
    [Parameter(Position = 0, Mandatory = $true)] [string]$App,
    [switch]$List,
  ...
  # Not $args: that is an automatic variable in PowerShell and writing
  # to it inside an advanced function is asking for trouble.
  $cliArgs = @()
  if ($List) { $cliArgs += "--list" }
  if ($Set) { $cliArgs += "--set=$Set" }
  Invoke-EnclawedAppAction -App $App -Action $cliArgs -Script "settings.mjs"
}
...
# Idempotently install a loader line into the current user's PowerShell
# profile so Start-/Stop-/Get-EnclawedAppStatus / Update-EnclawedApp are
# defined automatically in every new PowerShell session.
#
# Without this, the verb functions vanish on every new shell and the
# operator has to dot-source install.ps1 again. The loader line is
# guarded by Test-Path so an uninstall (removed clone) leaves a graceful
# no-op rather than a startup error.
function Register-EnclawedCommandsInProfile {
  $loaderPath = Join-Path $env:USERPROFILE ".enclawed\enclawed-oss\enclawed-apps\install.ps1"
  $marker = "# Enclawed: auto-load Start-/Stop-/Get-EnclawedAppStatus / Update-EnclawedApp"
  $loaderLine = "if (Test-Path '$loaderPath') { . '$loaderPath' }"

  # Use CurrentUserCurrentHost — the profile that PowerShell auto-loads
  # for this user's invocations of this host (powershell.exe vs pwsh).
  ...
  if (Test-Path $profilePath) {
    ...
    if ($existing -match [regex]::Escape($marker)) {
      Write-Host "  Enclawed commands already registered in $profilePath; skipping."
      return
    }
  }
  Add-Content -Path $profilePath -Value "`n$marker`n$loaderLine`n"
  Write-Host "  Registered Enclawed commands in $profilePath."
  Write-Host "  New PowerShell sessions will load Update-EnclawedApp / Get-EnclawedAppStatus / etc. automatically."
}

function Invoke-EnclawedAppAction {
  # $Script lets a verb target a sibling entry point (settings.mjs)
  # instead of install.mjs, and $Action takes an array so a verb can
  # pass more than one flag. Both default to the previous behaviour.
  param([string]$App, [string[]]$Action, [string]$Script = "install.mjs")
  # Force UTF-8 on every verb call. The script-top setting only fires
  # at dot-source time, so an operator whose session was already loaded
  # before the fix landed would still see mojibake. Setting it here too
  # means the FIRST Get-EnclawedAppStatus / Update-EnclawedApp after
  # an Update-EnclawedApp pulls the fix in immediately.
  try {
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
    [Console]::InputEncoding = [System.Text.Encoding]::UTF8
  } catch {
    # see install.ps1 top-of-file note; non-fatal
  }
  $repoDir = Join-Path $env:USERPROFILE ".enclawed\enclawed-oss"
  $installScript = Join-Path $repoDir (Join-Path "enclawed-apps" $Script)
...
function Invoke-EnclawedBootstrap {
  ...
  if (-not $needsClone) {
    ...
    # executive assistant's launcher holds open file handles in $repoDir, so
    ...
    if (-not (Test-Path (Join-Path $repoDir ".git"))) {
      if (Test-Path $repoDir) {
        ...
        # lock on enclawed-apps/executive-assistant/, and aborted after .git
  ...
  # After a successful install, drop the loader line into the user's
  # PowerShell profile so the verb functions are defined automatically
  # in every new shell. Idempotent — re-running Install-EnclawedApp does
  # not duplicate the line. Failures here are non-fatal: the install
  # already succeeded; the operator can always re-source install.ps1
  # by hand if profile-write was blocked.
  if ($LASTEXITCODE -eq 0) {
    try {
      Register-EnclawedCommandsInProfile
    } catch {
      Write-Warning "Could not register Enclawed commands in your PowerShell profile: $_"
      Write-Warning "Workaround: in any new PowerShell session, run"
      Write-Warning "  . `"$env:USERPROFILE\.enclawed\enclawed-oss\enclawed-apps\install.ps1`""
    }
  }
}
