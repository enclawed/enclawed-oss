...
export async function acquireCredentials({ config, principal, ask, ok, warn, fail }) {
  ...
  console.log("");
  console.log("To grant the executive assistant access, generate an app-specific password:");
  console.log("");
  ...
  console.log('  3. In "App name", type "Enclawed Executive Assistant" and click Create.');
...
// the SMTP/IMAP login is ENCLAWED_EXECUTIVE_ASSISTANT_MAILBOX_EMAIL. The legacy
// ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_EMAIL (which, pre-rename, held the
...
export function reuseCredentials(existingEnv, opts = {}) {
  const mailbox =
    existingEnv?.ENCLAWED_EXECUTIVE_ASSISTANT_MAILBOX_EMAIL ??
    existingEnv?.ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_EMAIL;
  if (!mailbox) {
    return null;
  }
