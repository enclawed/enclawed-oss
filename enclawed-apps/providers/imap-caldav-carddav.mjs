...
// principal email comes back via the env file (principal is not
// secret — it's the keyring lookup key, not the secret itself).
// Returns null if either piece is missing.
export function reuseCredentials(existingEnv, opts = {}) {
  const principal = existingEnv?.ENCLAWED_SECRETARY_PRINCIPAL_EMAIL;
  if (!principal) {
    return null;
  }
  ...
  const appPassword = lookup({ service: opts.service, account: principal });
