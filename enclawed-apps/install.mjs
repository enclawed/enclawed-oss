...
async function runAdversarialGate({ exitOnFailure = true } = {}) {
  ...
  const classifierModel =
    process.env.ENCLAWED_SECRETARY_CLASSIFIER_MODEL ?? config.llm?.classifierModel ?? "llama3.1:8b";
  ...
  if (failures.length > 0) {
    ...
    console.error("");
    console.error("  One or more components of the secretary's prompt-injection");
    console.error("  shield did NOT pass its self-test. The secretary will not be");
    console.error("  started in this state — a broken shield is strictly worse");
    console.error("  than no secretary at all, because inbound mail can hijack");
