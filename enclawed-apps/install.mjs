...
if (
  ...
  console.error(
    "usage: install.mjs <app-id> [--start | --stop | --status | --probe | --update | --adversarial-gate | --uninstall]",
  );
  process.exit(64);
}
...
const TWENTY_TECHNIQUES_COVERAGE = [
  ...
  ["#11", "Image / Steganography (surface trigger)", "regex"],
...
// for the secretary as a JSON blob: {mailbox, principal, aliases[]}.
...
const IDENTITIES_KEYRING_ACCOUNT = "secretary-identities";
...
function scrubIdentitiesFromEnvFile(p) {
  ...
  const drop = new Set([
    ...
    "ENCLAWED_SECRETARY_PRINCIPAL_ALIASES",
  ]);
...
if (existingIdentitiesBlob?.principal) {
  ...
  existingPrincipalFromKeyring = directKeyringGet(
    appDir,
    config.id,
...
//     registered principal, but they sometimes email the secretary
...
if (!principalAliases) {
  ...
  console.log(`address you also email the secretary from). Comma-separated. Leave blank to skip;`);
...
// unified `secretary-identities` JSON blob below carries every
...
if (!braveSearchKey) {
  console.log("Enclawed can call Brave Search from the secretary's tool loop to look up");
...
if (keyring && braveSearchKey.length > 0) {
  ...
  ok("Skipping web search — secretary will run without it.");
}

// All operator-identity addresses (mailbox, principal, aliases) go
// into ONE keyring blob — `secretary-identities` — as JSON. Replaces
...
if (keyring) {
  try {
    ...
    ok(`Operator identities stored in OS keyring (encrypted JSON: secretary-identities).`);
  } catch (err) {
    fail(
...
const envEntries = {
  ...
  // in the OS keyring under the `secretary-identities` JSON blob and
  ...
  ...(config.llm?.type === "ollama" && config.llm.classifierModel
    ? { ENCLAWED_SECRETARY_CLASSIFIER_MODEL: config.llm.classifierModel }
    : {}),
};
const envLines = [
...
writeFileSync(
  ...
  buildLauncher({
    ...
    aliasesEnvVar: keyring ? "ENCLAWED_SECRETARY_PRINCIPAL_ALIASES" : "",
...
// Adversarial gate. The secretary will not be started — on install or
...
async function runAdversarialGate({ exitOnFailure = true } = {}) {
  ...
  const classifierModel =
    process.env.ENCLAWED_SECRETARY_CLASSIFIER_MODEL ??
  ...
  const runHeader = [
    ...
    ...TWENTY_TECHNIQUES_COVERAGE.map(
      ([n, name, suites]) => `  ${n.padEnd(4)} ${String(name).padEnd(42)} [${suites}]`,
    ),
    "",
  ].join("\n");
  ...
  const SUITES = [
    ...
    },
    {
      ...
      env: {
        ...
        ENCLAWED_SECRETARY_CLASSIFIER_MODEL: classifierModel,
      },
    },
  ];
  ...
  if (failures.length > 0) {
    ...
    console.error("");
    console.error(
      "  One or more components of the secretary's prompt-injection",
    );
    console.error(
      "  shield did NOT pass its self-test. The secretary will not be",
    );
    console.error(
      "  started in this state — a broken shield is strictly worse",
    );
    console.error(
      "  than no secretary at all, because inbound mail can hijack",
    );
    console.error(
      "  your mailbox, calendar, and contacts.",
    );
    console.error("");
    ...
    console.error(
      "  Attach that file to your message. After Alfredo confirms a",
    );
    console.error(
      "  fix, re-run the gate standalone with:",
    );
    console.error(
      `    node enclawed-apps/install.mjs ${config.id} --adversarial-gate`,
    );
    console.error("");
    if (exitOnFailure) process.exit(2);
    return false;
...
async function runUpdate() {
  ...
  // so the secretary imports the COMPILED output, not src/. Without
  ...
  // to do with the secretary. ciArtifacts still runs tsdown
  ...
  ok("Workspace rebuilt — dist/ is in sync with src/.");
...
async function regenerateLauncher() {
  ...
  const principalVarLocal = config.principal?.envVar ?? "ENCLAWED_SECRETARY_PRINCIPAL_EMAIL";
  ...
  // ─── Migration: identities live in the unified secretary-identities
    ...
    if (!existingBlob) {
      ...
      if (mailboxFromEnv) {
        ...
        const legacyAliasesRaw =
          existingEnv["ENCLAWED_SECRETARY_PRINCIPAL_ALIASES"] ||
          directKeyringGet(appDir, config.id, `principal-aliases:${mailboxFromEnv}`);
        const aliases = legacyAliasesRaw
          ? legacyAliasesRaw.split(",").map((s) => s.trim()).filter((s) => s.length > 0)
          : [];
  ...
  writeFileSync(
    ...
    buildLauncher({
      ...
      aliasesEnvVar: usesKeyringLocal ? "ENCLAWED_SECRETARY_PRINCIPAL_ALIASES" : "",
...
// LLM schema. Non-65 failure logged but not fatal — the secretary can
