...
const recoverRootOnly = argv.includes("--recover-root");
if (
  !appId ||
  [
    uninstall,
...
function printUsage() {
  ...
  console.log("    --recover-root      clear a DIRTY hardware-root latch (device must be present)");
...
async function runRecoverRoot() {
  // The guard latches DIRTY when the enclaweder disconnects or fails a liveness
  // check, and refuses to boot afterwards. Recovery deliberately cannot happen
  // inside the agent -- boot() throws while DIRTY -- so it lives here.
  const home = homedir();
  const candidates = [];
  try {
    for (const f of readdirSync(home)) {
      ...
      const full = join(home, f);
      try {
        candidates.push({ path: full, state: JSON.parse(readFileSync(full, "utf8")) });
      } catch {
        /* unreadable or malformed -- nothing to recover from */
      }
    }
  } catch {
    /* no home listing */
  ...
  const dirty = candidates.filter((c) => c.state?.status === "DIRTY");
  if (dirty.length === 0) {
    const statuses = candidates.map((c) => `${c.path}: ${c.state?.status ?? "unknown"}`);
    console.log("");
    console.log("  No DIRTY hardware-root latch to clear.");
  ...
  if (dirty.length > 1) {
    console.log("");
    console.log("  More than one DIRTY latch found; recover them one at a time:");
  ...
  const boundSerial = target.state?.bound?.serial ?? "";
  console.log("");
  console.log("  Hardware root is DIRTY — accreditation is halted.");
  console.log(`    state file:   ${target.path}`);
  console.log(`    bound device: ${boundSerial || "(unknown)"}`);
  console.log(`    reason:       ${target.state?.dirty?.reason ?? "(unrecorded)"}`);
  if (target.state?.dirty?.at) {
    console.log(`    latched at:   ${new Date(target.state.dirty.at).toLocaleString()}`);
  }
  console.log("");

  // A recovery that a script can perform is not a recovery -- the latch exists
  // for the case where the host is doing things its owner did not ask for. Only
  // proceed from a real console, and make the operator read the serial off the
  // device they are holding rather than press a key.
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.log("  Refusing: recovery requires an interactive terminal.");
    console.log("  Run this command yourself at a console, with the device plugged in.");
    console.log("");
    process.exitCode = 1;
    return;
  }
  ...
  let typed;
  try {
    console.log("  Confirm you are physically holding the bound enclaweder.");
    typed = (
      await rl.question(`  Type its serial to proceed (or anything else to abort): `)
    ).trim();
  } finally {
    rl.close();
  }
  if (!boundSerial || typed.toUpperCase() !== boundSerial.toUpperCase()) {
    console.log("");
    console.log("  Aborted — serial did not match. Nothing was changed.");
    console.log("");
    process.exitCode = 1;
    return;
  }

  const frameworkPath = join(repoRoot, "dist", "framework.js");
  if (!existsSync(frameworkPath)) {
    console.log("");
    console.log(`  Cannot recover: ${frameworkPath} is missing. Build first (--update).`);
    console.log("");
    process.exitCode = 1;
    return;
  }
  const fw = await import(pathToFileURL(frameworkPath).href);
  try {
    const res = await fw.recoverHardwareRootStandalone({
      humanConfirmed: true,
      statePath: target.path,
    });
    console.log("");
    if (res?.recovered) {
      // The guard re-verified the device identity and a fresh challenge before
      // clearing; reaching here means those passed, not merely that we asked.
      console.log(`  Recovered — hardware root is CLEAN again (${res.serial ?? boundSerial}).`);
      console.log("  The clearing is recorded in the guard's recovery history.");
      console.log("  Start the app when ready.");
    } else {
      console.log("  Recovery did not complete; the latch is unchanged.");
      process.exitCode = 1;
    }
    console.log("");
  } catch (err) {
    console.log("");
    console.log(`  Recovery REFUSED: ${(err && err.message) || err}`);
    console.log("  The latch is unchanged. A refusal here means the device present is");
    console.log("  not the bound one, or it failed the challenge — investigate before retrying.");
    console.log("");
    process.exitCode = 1;
  }
}
...
// 2.4) Formal verification — prove the app's code stays inside its declared
//      capabilities, and write the evidence the app re-checks at every boot.
//      Like the adversarial gate below, a failure stops here, before the
//      operator is asked for any credential.
await runFormalVerificationSetup({ exitOnFailure: true });
...
async function runStatus() {
  ...
  try {
    ...
    if (providerModule.usesKeyring) {
      ...
      if (account && existsSync(helper)) {
        ...
        // Two unrelated causes used to print the same line, which left the
        // reader unable to tell a missing helper from an env file that simply
        // carries no mailbox -- and the env file is reported as present on the
        // line above either way. Name the one that actually applies.
        const missing = [];
        if (!account) {
          missing.push("no mailbox in env");
        }
        if (!existsSync(helper)) {
          missing.push(`helper missing at ${helper}`);
        }
        console.log(`  Keyring entry:  cannot check (${missing.join("; ")})`);
      }
    }
  } catch (err) {
  ...
  // Formal verification: what setup proved. The authoritative verdict is the
  // one the app re-derives at every boot (audit type skill.formal-verification);
  // this reports the evidence it re-derives it from.
  if (existsSync(join(appDir, "skill.manifest.json"))) {
    const attestPath = join(envDir, "formal", "manifest.attest.json");
    if (!existsSync(attestPath)) {
      console.log(`  Formal verif.:  NO EVIDENCE — run Update-EnclawedApp to produce it`);
    } else {
      try {
        const att = JSON.parse(readFileSync(attestPath, "utf8"));
        console.log(
          `  Formal verif.:  ${att.contained ? "contained in manifest" : "NOT CONTAINED"}` +
            ` (proved ${new Date(att.producedAt).toLocaleString()}; re-checked at every boot)`,
        );
      } catch (err) {
        console.log(`  Formal verif.:  evidence unreadable (${(err && err.message) || err})`);
      }
    }
  }
...
async function runFormalVerificationSetup({ exitOnFailure = true } = {}) {
  // Prove the app's code stays inside the capabilities its manifest declares,
  // and leave the evidence where the app re-checks it at every boot. An app
  // without a skill.manifest.json has nothing to prove and is left alone.
  //
  // It runs on install AND on every update because the proof is about the
  // code: an update that lets the app reach a capability it does not declare
  // must not start, for the same reason a broken prompt-injection shield must
  // not -- the audit log would no longer describe what the app does.
  const manifestPath = join(appDir, "skill.manifest.json");
  if (!existsSync(manifestPath)) {
    return true;
  }
  header("Formal verification: app code against its declared capabilities");
  const formal = await import(
    pathToFileURL(join(repoRoot, "enclawed", "src", "skill-formal-bundle.mjs")).href
  );
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  const started = Date.now();
  const bundle = formal.produceFormalBundle({
    skillDir: join(appDir, "src"),
    manifest,
    bound: 8,
  });
  const bundleDir = join(envDir, "formal");
  mkdirSync(envDir, { recursive: true });
  formal.writeFormalBundle(bundle, bundleDir);
  const elapsed = Date.now() - started;
  if (bundle.attestation.contained) {
    ok(
      `Contained in its manifest (${manifest.caps.join(", ")}), ${elapsed} ms. ` +
        `Re-checked at every boot; evidence in ${bundleDir}.`,
    );
    return true;
  }
  ...
  const top = Object.entries(a.perScript ?? {})
    .filter(([, r]) => r.top)
    .map(([f, r]) => `${f} (${r.reason ?? "unanalysable"})`);
  fail(
    "The app's code is NOT contained in its declared capabilities.\n" +
      (a.escapingCapabilities?.length
        ? `  Reaches but does not declare: ${a.escapingCapabilities.join(", ")}\n`
        : "") +
      (top.length ? `  Cannot be analysed: ${top.join(", ")}\n` : "") +
      (!bundle.evidence.types.contained ? "  Method B (refined dispatch) failed.\n" : "") +
      (!bundle.evidence.smt_unsat.contained ? "  Method C (bounded model check) failed.\n" : "") +
      `  The service will not start on code whose audit log would misdescribe it.\n` +
      `  Report this to alfredo.metere@enclawed.com with the evidence in ${bundleDir}.`,
  );
  if (exitOnFailure) {
    process.exit(2);
  }
...
async function runUpdate() {
  ...
  // Re-prove the updated code against its manifest before it may start. The
  // boot re-check would catch the drift too, but only by quietly admitting the
  // app at a lower level; an update that breaks containment should stop here,
  // visibly, the way a broken shield does just below.
  await runFormalVerificationSetup({ exitOnFailure: true });
