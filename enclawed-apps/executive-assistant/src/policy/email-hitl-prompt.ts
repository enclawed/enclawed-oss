...
import type { AuditLogger, BrokerDecision, BrokerRequest } from "enclawed/framework";
...
export type EmailHitlPromptOptions = Readonly<{
   ...
   * executive assistant authenticates as (matches the keyring account) and the
   ...
   * mailbox the executive assistant is signed in to). Override when the operator
   * reads / replies from a different identity than the mailbox the
   * executive assistant is sending from — typical configurations:
   *
   *   - principal mailbox is a dedicated service account
   *     (`executive assistant@…`), operator replies from a personal address
   ...
   * principal mailbox's INBOX (because that's where the executive assistant
  ...
  /**
   * Additional addresses to accept as the principal's reply From:
   * header — the same set the broker carve-out treats as principal-
   * equivalent (registered principal + ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_ALIASES).
   * Without these, an operator who registers `principal@corp.com` but
   * replies from `principal@personal.com` has every HITL reply silently
   * rejected, never labeled, and the inbox accumulates "Re: Enclawed
   * HITL approval needed" mail that the daily loop keeps re-fetching.
   */
  acceptedReplyFromAddresses?: ReadonlySet<string>;
  ...
  hitlLabel?: string;
  /**
   * Hash-chained audit logger. The HITL email channel is intentionally NOT
   * gated (routing the broker's own question through the gate would
   * recurse), but its lifecycle still belongs in the audit trail: it sends
   * mail and mutates the mailbox (labels/append) over raw IMAP/SMTP. When
   * supplied, each request-sent / reply-received / timeout / send-error
   * event is appended so the broker-internal channel is auditable even
   * though it can't be broker-mediated. Best-effort: append failures never
   * block or alter the HITL decision.
   */
  audit?: AuditLogger;
...
// Subject prefix the executive assistant stamps on every HITL email. The principal
...
export function buildEmailHitlPrompt(
  ...
  // Best-effort audit append. The HITL channel must never fail or stall a
  // decision because the audit log is momentarily unwritable, so every
  // append is fire-and-forget with a swallowed rejection.
  const recordHitl = (type: string, payload: Record<string, unknown>): void => {
    audit?.append({ type, actor: "email-hitl", level: null, payload }).catch(() => {});
  };
  const replyFromAddress = (opts.replyFromAddress ?? opts.principalEmail).toLowerCase().trim();
  const acceptedReplyFromSet = new Set(
    [
      replyFromAddress,
      ...(opts.acceptedReplyFromAddresses
        ? [...opts.acceptedReplyFromAddresses].map((s) => s.toLowerCase().trim())
        : []),
    ].filter((s) => s.length > 0),
  );
  ...
  return async (req) => {
    ...
    try {
      ...
      sentAt = new Date();
      recordHitl("hitl.email.request-sent", {
        requestId: req.requestId,
        correlation,
        to: maskAddress(replyFromAddress),
        cap: req.call.cap,
        target: truncate(req.call.target, 120),
      });
    } catch (err) {
      const reason = `email-hitl: SMTP send failed: ${(err as Error).message}`;
      log("warn", `${reason} — defaulting to DENY`);
      recordHitl("hitl.email.send-error", {
        requestId: req.requestId,
        correlation,
        reason,
        decision: "deny",
    ...
    // Operator asked for an explicit reminder + a timeout confirmation.
    // The reminder fires once, at the 5-minute mark, so a principal who
    // missed the original mail gets a second nudge with time still on
    // the clock. The timeout confirmation always fires when the deadline
    // passes (alongside the deny return) so the principal knows the
    // request died rather than silently dropping. Both messages thread
    // against the original (same subject line) so the principal's
    // mailbox shows the full lifecycle in one thread.
    let reminderSent = false;
    const reminderAt = sentAt.getTime() + 5 * 60_000;
    while (Date.now() < deadline) {
      ...
      // 5-minute reminder: fires once, only if we're past reminderAt
      // and still before the deadline. Failure to send the reminder is
      // logged but doesn't collapse the prompt — the original request
      // is still in-flight.
      if (!reminderSent && Date.now() >= reminderAt) {
        reminderSent = true;
        const minutesLeft = Math.max(1, Math.floor((deadline - Date.now()) / 60_000));
        try {
          await smtp.sendMail({
            from: fromHeader,
            to: replyFromAddress,
            subject: `Re: ${subject}`,
            text:
              `Reminder: an enclawed-executive-assistant HITL approval is still waiting on your reply.\n\n` +
              `Original request:\n\n${body}\n\n` +
              `Reply YES or NO on the first line within ~${minutesLeft} more minutes ` +
              `or the request will time out and be denied.`,
            headers: {
              "X-Enclawed-HITL-Correlation": correlation,
              "X-Enclawed-HITL-RequestId": req.requestId,
              "X-Enclawed-HITL-Kind": "reminder",
            },
          });
          log("info", `email-hitl: 5min reminder sent for ${correlation}`);
        } catch (err) {
          log(
            "warn",
            `email-hitl: reminder send failed for ${correlation}: ${(err as Error).message}`,
          );
        }
      }
      try {
    ...
    recordHitl("hitl.email.timeout", {
      requestId: req.requestId,
      correlation,
      timeoutMs,
      decision: "deny",
    });
    // Timeout confirmation: tell the principal the request died, so a
    // late YES/NO reply doesn't leave them assuming it took effect. Same
    // best-effort failure handling as the reminder.
    try {
      await smtp.sendMail({
        from: fromHeader,
        to: replyFromAddress,
        subject: `Re: ${subject} — TIMED OUT`,
        text:
          `Your approval did not arrive within ${Math.floor(timeoutMs / 60_000)} minutes; the ` +
          `request has been DENIED and will NOT execute.\n\n` +
          `Original request:\n\n${body}\n\n` +
          `If you still want the action, re-send the original instruction to the ` +
          `executive assistant and a fresh HITL request will be issued.`,
        headers: {
          "X-Enclawed-HITL-Correlation": correlation,
          "X-Enclawed-HITL-RequestId": req.requestId,
          "X-Enclawed-HITL-Kind": "timeout-confirmation",
        },
      });
      log("info", `email-hitl: timeout confirmation sent for ${correlation}`);
    } catch (err) {
      log(
        "warn",
        `email-hitl: timeout-confirmation send failed for ${correlation}: ${(err as Error).message}`,
      );
    }
    return { decision: "deny", reason };
  };
...
function renderRequestBody(req: BrokerRequest, correlation: string, timeoutMs: number): string {
  ...
  return [
    `The executive assistant needs your approval:`,
    ``,
    summary,
    ``,
...
function renderHumanSummary(req: BrokerRequest): string {
  ...
  // mail:draft-review/<threadId> — the draft-for-review surface.
  // Renders the entire body so the principal can decide YES/NO on
  // what's about to be sent without leaving the mail client.
  if (target.startsWith("mail:draft-review/")) {
    const recipient = typeof args["recipient"] === "string" ? args["recipient"] : "(unknown)";
    const subject = typeof args["subject"] === "string" ? args["subject"] : "(no subject)";
    const body = typeof args["body"] === "string" ? args["body"] : "";
    return [
      `Action:    REVIEW reply draft to ${recipient}`,
      `Subject:   ${subject}`,
      "",
      "Body:",
      body
...
async function fetchReply(args: {
  ...
  /** IMAP login identity — the executive assistant's mailbox credentials. */
  mailboxLogin: string;
  /**
   ...
   * differs from the mailbox the executive assistant signs in to.
   */
  expectedReplyFrom: string;
  /**
   * Full lowercase set of From: addresses to accept. Includes
   * expectedReplyFrom plus every alias the operator registered.
   */
  acceptedReplyFromSet: ReadonlySet<string>;
  password: string;
  correlation: string;
  sinceMs: number;
  log: (level: "info" | "warn" | "error", msg: string) => void;
    ...
    try {
      ...
      for (const uid of sorted) {
        ...
        const seenFrom = fromAddrs
          ...
          .filter((s) => s.length > 0);
        const fromMatches = seenFrom.some((f) => args.acceptedReplyFromSet.has(f));
        ...
        if (subjMatches && !fromMatches) {
          ...
          // from a different account / alias / send-as identity than any
          // address in the accepted set (principal + aliases captured at
          // install). Addresses are masked so the log file (which sits
          // at rest unencrypted under ~/.enclawed) does not leak the
          // operator's personal identity to anyone with read access.
          const acceptedMasked = [...args.acceptedReplyFromSet].map(maskAddress).join(",");
          args.log(
            "warn",
            `email-hitl: ignoring Re:-prefixed reply uid=${uid} ` +
              `from=[${seenFrom.map(maskAddress).join(",") || "(none)"}] — ` +
              `reply must come from one of [${acceptedMasked}] (case-insensitive)`,
          );
          continue;
        }
        if (!fromMatches) {
        ...
        args.log(
          "info",
          `email-hitl: received reply for ${args.correlation} uid=${uid} ` +
            `from=${seenFrom.map(maskAddress).join(",")} subject="${truncate(subj, 80)}" ` +
            `bodyLen=${text.length}`,
        );
...
async function openImap(
  ...
  // Absorb connection-level 'error' events. ImapFlow delivers a socket
  // timeout or peer reset as an EventEmitter 'error', and an 'error'
  // with no listener terminates the Node process rather than rejecting
  // the awaited command. This client polls for the operator's approval
  // reply and can sit idle for a long time, so it is exactly the one
  // most likely to have its socket time out. Callers still see real
  // failures through the rejections of the commands they await.
  client.on("error", () => {});
