// Bicriterion-calibrated HITL broker for the executive assistant demo.
//
...
import type { SecretarySeverity } from "./dlp-executive-assistant.js";
...
// Target-string shapes the executive assistant tool wrappers produce. The broker
...
export function buildSecretaryBroker(opts: SecretaryBrokerOptions): Broker {
  ...
  return {
    id: `executive assistant-bicriterion(${opts.hitl})`,
...
function classify(
  ...
  // write by emailing the executive assistant about it. Auto-approve every
  ...
  if (cap === CAPABILITY.PUBLISH) {
    ...
    // The carve-out did NOT fire. Surface why so a follow-up status
    // paste pinpoints the cause: either origin was null (the dispatch
    // didn't pass originSenderEmail through), or the principal-self
    // set is missing the email the operator actually used. Without
    // this line a missed carve-out is invisible until the HITL email
    // times out — minutes later, and looks like a different bug class.
    warn(
      `bicriterion: principal-authored carve-out skipped — origin=${origin ?? "(null)"} ` +
        `principalSelf=[${[...principalSelf].join(",")}]`,
    );
  }
  ...
  switch (cap) {
    ...
    case CAPABILITY.SPAWN_PROC: {
      // The executive assistant never dispatches the first three. spawn.proc
      // it does use -- the self-update check's git plumbing -- but it is
      // declared in skill.manifest.json, so the gate runs it directly and it
      // never reaches this broker. Arriving here therefore means a call the
      // manifest does not cover: a programming bug, denied rather than put to
      // a human who should not have to adjudicate it.
      return {
        kind: "deny",
        reason: `bicriterion: capability ${cap} is not in the executive assistant contract`,
      };
    }
...
/**
 * Build a stdin-based prompt suitable for `enclawed demo run executive assistant`.
...
export function buildStdinKeypressPrompt(input: {
  ...
  return async (req) => {
    stdout.write(
      [
        "",
        "── executive assistant HITL keypress ─────────────────────────────────",
 ...
 * executive assistant runs as a detached service (Task Scheduler / launchd /
 ...
 * can re-run the executive assistant in a real terminal or install a dialog
...
export function buildDialogPrompt(): (req: BrokerRequest) => Promise<BrokerDecision> {
  return async (req) => {
    ...
    const body =
      `The executive assistant is asking for your approval before this action.\n\n` +
      `cap:    ${req.call.cap}\n` +
      `target: ${truncate(req.call.target, 200)}\n\n` +
      `args:\n${argsLine}\n\n` +
      `Approve?`;
    const title = `Enclawed Executive Assistant — approval required (req ${req.requestId.slice(0, 8)})`;
    try {
      ...
      process.stderr.write(`[executive-assistant warn] ${reason}; defaulting to DENY\n`);
