...
// the principal when an update is available. The executive assistant never
// applies the update itself — that is `Update-EnclawedApp executive-assistant`,
...
import { gitUpdateCheckAllowed, updateRefusalReason } from "../deployment.js";
...
export async function checkForUpdates(opts: {
  ...
  // Refuse before anything is spawned. A packaged build has no repo and no
  // git, but that is not the reason: an application store owns updates for
  // the software it distributes, and a store build that probes a remote for
  // a newer version of itself is doing the one thing it must not, whatever
  // it does with the answer.
  if (!gitUpdateCheckAllowed()) {
    return { kind: "unavailable", reason: updateRefusalReason() };
  }
...
type GitResult = Readonly<{ ok: true; stdout: string }> | Readonly<{ ok: false; reason: string }>;
  ...
  return gatedSpawn(
    ...
    () =>
      new Promise<GitResult>((resolve) => {
        ...
        const timer = setTimeout(() => settleErr(`timeout after ${timeoutMs}ms`), timeoutMs);
...
async function resolveGit(gatedSpawn: GatedSpawn = PASSTHROUGH_SPAWN): Promise<string | null> {
  ...
  const fromPath = await gatedSpawn(
    `${finder} git`,
    () =>
 ...
 *      exec, so this is correct when the executive assistant is started via the
