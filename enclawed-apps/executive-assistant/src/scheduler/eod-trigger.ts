...
async function fireSummary(opts: EodTriggerOptions): Promise<void> {
  ...
  // Project only records produced during THIS executive assistant's runtime
  // window. The audit log is append-only across restarts; without the
  // sinceMs filter the projection sums every publish since the file
  // was first created, the in-memory counts cover only the current
  // process lifetime, and the RT-11 check triggers a false alarm on
  // every restart. Aligning the window — startedAt of the runtime
  // state ≤ ts — restores apples-to-apples comparison.
  const projection = await projectAuditCounts(opts.auditPath, opts.state.startedAt()).catch(
    () => null,
  );
  ...
  const subject = `[Executive Assistant] Daily Summary ${nowIso.slice(0, 10)}`;
...
function renderSummaryBody(input: {
  ...
  return [
    `Daily summary — executive assistant run started ${input.startedAtIso}`,
    ...
    "",
    "— Executive Assistant (enclawed demo)",
  ].join("\n");
}
 ...
 */
export async function projectAuditCounts(path: string, sinceMs?: number): Promise<AuditProjection> {
  const raw = await readFile(path, "utf8");
  const lines = raw.split("\n").filter((l) => l.length > 0);
  let publishExecuted = 0;
  const h = createHash("sha256");
  for (const ln of lines) {
    // Hash always sees the full line stream — the chain-hash check is
    // file-wide tamper detection, independent of the count window.
    h.update(ln, "utf8");
    try {
      const r = JSON.parse(ln) as {
        ts?: number;
        type?: string;
        payload?: { call?: { cap?: string } };
      };
      if (typeof sinceMs === "number" && typeof r.ts === "number" && r.ts < sinceMs) {
        continue;
      }
...
function defaultLog(level: "info" | "warn" | "error", msg: string): void {
  ...
  stream.write(`${logTimestamp()} [executive assistant-eod ${level}] ${msg}\n`);
}
