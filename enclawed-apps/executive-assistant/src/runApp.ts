...
import { readAppManifest, verifyAppAtBoot } from "./policy/formal-verification.js";
...
export async function runApp(opts: AppRunOptions = {}): Promise<number> {
  ...
  // The manifest comes from skill.manifest.json, the same file setup proved
  // the code against. caps=[fs.read, fs.write.rev, net.egress, spawn.proc]
  // are the declared set; publish is intentionally OUTSIDE it so every
  // send_draft walks the broker (and the bicriterion routing fires precisely
  // on the F5-risky calls). net.egress covers the web tools, which are
  // network reads and are logged as such. spawn.proc covers the periodic
  // self-update check's read-only git plumbing (fetch / rev-parse /
  // rev-list): declared, it stays AUDITED without an HITL keypress on every
  // 12h tick, while a non-declared spawn would still walk the broker.
  //
  // The verification level is re-derived here, at every boot, by re-running
  // the formal proofs against the bundle setup produced. It used to be
  // hand-asserted as TESTED, a level nothing had checked.
  const formal = verifyAppAtBoot(dirname(auditPath));
  const manifest = buildSecretarySkillManifest(formal.level);
  gate.loadSkill(manifest);
  process.stderr.write(
    `${logTimestamp()} executive-assistant: formal verification: admitted at ` +
      formal.level.toUpperCase() +
      (formal.reasons.length > 0 ? ` (${formal.reasons.join("; ")})` : "") +
      "\n",
  );
  await auditLogger.append({
    type: "skill.formal-verification",
    actor: EXECUTIVE_ASSISTANT_SKILL_ID,
    level: null,
    payload: {
      verification: formal.level,
      reasons: formal.reasons,
      bundleDir: formal.bundleDir,
    },
  });
...
function buildSecretarySkillManifest(level: "formal" | "declared"): SkillManifest {
  // Read, not hand-built: the installer produces the formal-verification
  // bundle from this same file, so the manifest the gate enforces and the one
  // the proofs were computed against cannot diverge.
  const m = readAppManifest();
  if (m.id !== EXECUTIVE_ASSISTANT_SKILL_ID) {
    throw new Error(
      `executive-assistant: skill.manifest.json id "${m.id}" does not match ` +
        `the id the tools dispatch under ("${EXECUTIVE_ASSISTANT_SKILL_ID}")`,
    );
  }
  const known = new Set<string>(Object.values(CAPABILITY));
  const unknown = m.caps.filter((c) => !known.has(c));
  if (unknown.length > 0) {
    throw new Error(`executive-assistant: unknown capability in manifest: ${unknown.join(", ")}`);
  }
  return Object.freeze({
    v: 1 as const,
    id: m.id,
    label: makeLabel({ level: m.label.level }),
    caps: Object.freeze([...m.caps] as CapabilityToken[]),
    signer: m.signer,
    version: m.version,
    verification: level === "formal" ? VERIFICATION.FORMAL : VERIFICATION.DECLARED,
  });
}
