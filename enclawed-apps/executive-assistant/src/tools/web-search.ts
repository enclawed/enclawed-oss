...
//
// Both backends go through the SkillGate dispatch as cap=NET_EGRESS, which
// is what they do. This used to be declared FS_READ so the reversible path
// would skip the broker keypress -- but that recorded outbound network traffic
// in the audit log as a local file read, and formal verification (Method A)
// rightly refused the app for it. net.egress is now in skill.manifest.json,
// so these calls still run without a keypress, via the declared path, and are
// logged as what they are. Egress hosts for each backend are added to the
// executive assistant's allowlist at startup.
//
