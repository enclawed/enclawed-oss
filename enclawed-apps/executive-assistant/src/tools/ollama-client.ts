// Minimal Ollama client for the executive assistant's draft-composition step.
//
...
// pure function for projection purposes — the executive assistant's tool wrappers
...
export type OllamaChatMessage = Readonly<{
  ...
  content: string;
  /**
   * Base64 images for a multimodal model, per Ollama's /api/chat
   * contract. Only ever set for the vision pass, and only with bytes
   * that have already been through image sanitisation -- the raw
   * attachment is never handed to a model.
   */
  images?: ReadonlyArray<string>;
}>;

// Tool-use types. Mirrors Ollama's /api/chat tools contract; the
// executive assistant's daily loop hands the model a list of these and parses
...
export type OllamaChatOptions = Readonly<{
  ...
  /** Sampling temperature. Default 0.2 (the executive assistant wants reproducible drafts). */
  temperature?: number;
  /** Hard cap on response tokens. Default 512 — drafts are short. */
  numPredict?: number;
  /**
   * Context window size in tokens (Ollama `num_ctx`). Default 32768.
   * Ollama's built-in default is 2048, which is far smaller than what
   * modern instruct models can actually handle (qwen2.5:32b supports
   * 128k). Without setting this explicitly, long email threads +
   * tool-call iterations get silently truncated and the LLM produces
   * empty or hallucinated output. The default of 32768 covers every
   * realistic executive assistant workload while leaving room for VRAM headroom
   * on smaller cards.
   */
  numCtx?: number;
...
export type OllamaChatWithToolsOptions = Readonly<{
  ...
  temperature?: number;
  numPredict?: number;
  /** Context window size in tokens. See OllamaChatOptions.numCtx. */
  numCtx?: number;
  timeoutMs?: number;
}>;
