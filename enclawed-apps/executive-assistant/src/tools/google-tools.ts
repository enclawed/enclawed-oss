...
import type { SecretarySeverity } from "../policy/dlp-executive-assistant.js";
...
export type ThreadAttachmentMeta = Readonly<{
  filename: string;
  contentType: string;
  size: number;
}>;
...
export type ThreadDetail = Readonly<{
  ...
  bodyExcerpt: string;
  /**
   * Verbatim RFC822 source of every message in the thread, base64, in
   * receive order. Audit-only: never rendered into LLM context.
   */
  rawMessagesBase64?: ReadonlyArray<string>;
  /**
   * Per-message attachment metadata for the thread, in message order
   * (oldest first — same order as the IMAP UID set behind get_thread).
   * The LLM sees this listing in the user prompt and can call
   * read_attachment(messageIdx, attachmentIdx) to fetch + extract
   * text from a specific entry.
   */
  attachmentsByMessage: ReadonlyArray<ReadonlyArray<ThreadAttachmentMeta>>;
}>;
...
export type CreateEventInput = Readonly<{
  ...
  /**
   * RFC 5545 RRULE value for a recurring event (the part after
   * "RRULE:" — e.g. "FREQ=WEEKLY;BYDAY=TU" or
   * "FREQ=DAILY;COUNT=10"). `startsAtIso` / `endsAtIso` describe the
   * first occurrence; the CalDAV server expands the recurrence on
   * the read side. Omit for one-off events. The mcp-caldav bridge
   * validates and canonicalizes the value before emitting it.
   */
  recurrence?: string;
  /**
   * IANA timezone identifier (e.g. "America/Los_Angeles") for the
   * event's local time. Required for recurring events with BYDAY /
   * BYMONTHDAY rules so the CalDAV server iterates weekday-aware
   * recurrence in the user's timezone instead of UTC. The dispatcher
   * fills this from the executive assistant's configured TZ; the LLM does not
   * supply it.
   */
  timezone?: string;
  /**
...
export type AddContactInput = Readonly<{
  name: string;
  email: string;
  relationship?: string;
  dlpSummary: DlpSummaryArg | null;
...
export type AddedContact = Readonly<{
  uid: string;
  href: string;
}>;
...
// recurrence). Same contract as canonicalDraftSha256 — keys are sorted,
// attendees lowercased and deduped so reordering inside the model doesn't
// shift the hash. Lives in its own module so unit tests can pin the
// contract without pulling the framework graph in. The local import is
// needed because `export { ... } from "..."` re-exports the symbol to
// downstream consumers but does NOT make it available inside THIS
// module's value scope — calls to canonicalEventSha256(...) below in
// createEvent / updateEvent would otherwise be unresolved at runtime.
import { canonicalEventSha256 } from "./canonical-event-sha.js";
export { canonicalEventSha256 } from "./canonical-event-sha.js";
...
export class GoogleTools {
  ...
  async getAttachment(input: {
  ...
  }): Promise<{ filename: string; contentType: string; size: number; bytes: Buffer }> {
    let captured: {
      filename: string;
      contentType: string;
      size: number;
    ...
    const call = makeCall({
      cap: CAPABILITY.FS_READ,
      target: `mail:attachment/${encodeURIComponent(input.threadId)}#m=${input.messageIdx};a=${input.attachmentIdx}`,
      args: {
        threadId: input.threadId,
        messageIdx: input.messageIdx,
        attachmentIdx: input.attachmentIdx,
      },
    });
    const outcome = await this.dispatchRead(call, async () => {
      const r = await this.client.invoke({
      ...
      const base64 = typeof out?.base64 === "string" ? out.base64 : "";
      if (!base64) {
        return {
          ok: false as const,
          reason: "F3: get_attachment returned ok but no base64 payload",
        };
      }
      captured = {
        filename: typeof out?.filename === "string" ? out.filename : "(unnamed)",
        contentType:
          typeof out?.contentType === "string" ? out.contentType : "application/octet-stream",
        size: typeof out?.size === "number" ? out.size : 0,
  ...
  async addContact(input: AddContactInput): Promise<AddedContact> {
    const emailNorm = input.email.toLowerCase().trim();
    const target = `contacts:vcard/${encodeURIComponent(emailNorm)}#add`;
    let captured: AddedContact | null = null;
    const call = makeCall({
    ...
    const outcome = await this.gate.dispatch({
      ...
      execute: async () => {
        ...
        const out = r.output as { uid?: unknown; href?: unknown } | null;
        const uid = typeof out?.uid === "string" ? out.uid : "";
        const href = typeof out?.href === "string" ? out.href : "";
        if (!uid) {
          return {
            ok: false as const,
            reason: "F3: add_contact returned ok but no uid — silent host failure",
          };
        }
        captured = { uid, href };
        return { ok: true as const };
  ...
  // after the executive assistant has fully handled a thread so the next
  ...
  // strict sense, but it's an inbox-cleanup step the executive assistant
  ...
  async markThreadSeen(threadId: string): Promise<void> {
    ...
    // The first version of this method awaited the dispatch but never
    // checked the outcome — so on transient IMAP failures ("Connection
    // not available") the gate logged the failure, this function
    // returned void as if it succeeded, the daily-loop never saw a
    // warn line, the IMAP \Seen flag was never set, and the same
    // threads came back through is:unread at the next restart.
    // Throw on non-"executed" so the daily-loop's catch surfaces it
    // in service.log and the in-memory dedup is the only guard for
    // that thread until the next successful mark — which at least
    // bounds the regression to "won't re-handle within this process
    // lifetime" instead of "will re-handle on next restart."
    if (outcome.kind !== "executed") {
      throw new GoogleToolsError(outcome);
    }
  }
...
function parseThread(raw: unknown): ThreadDetail | null {
  ...
  const bodyText =
    typeof latest.bodyText === "string"
      ...
      : typeof latest.body === "string"
        ? latest.body
        : "";
  // Collect per-message attachment metadata in receive order. The
  // bridge already filters out inline-only MIME parts; we just pass
  // through whatever it surfaced.
  const attachmentsByMessage: ReadonlyArray<ThreadAttachmentMeta>[] = [];
  for (const m of messages) {
    const mr = m as { attachments?: unknown };
    if (!Array.isArray(mr.attachments)) {
      attachmentsByMessage.push(Object.freeze([]));
      continue;
    }
    const list: ThreadAttachmentMeta[] = [];
    for (const att of mr.attachments) {
      if (!att || typeof att !== "object") {
        continue;
      }
      const ar = att as Record<string, unknown>;
      list.push({
        filename: typeof ar.filename === "string" ? ar.filename : "(unnamed)",
        contentType:
          typeof ar.contentType === "string" ? ar.contentType : "application/octet-stream",
        size: typeof ar.size === "number" ? ar.size : 0,
      });
    }
    attachmentsByMessage.push(Object.freeze(list));
  }
  // Verbatim RFC822 for every message in the thread, in receive order.
  // bodyExcerpt stays capped at 4000 chars because that is what the LLM
  // is shown; this is the auditor's copy and is deliberately kept off
  // the model's context so full fidelity in the record does not mean a
  // larger injection surface.
  const rawMessages: string[] = [];
  for (const m of messages) {
    const mr = m as { rawSourceBase64?: unknown };
    if (typeof mr.rawSourceBase64 === "string" && mr.rawSourceBase64.length > 0) {
      rawMessages.push(mr.rawSourceBase64);
    }
  }
  return Object.freeze({
    ...
    rawMessagesBase64: Object.freeze(rawMessages),
    attachmentsByMessage: Object.freeze(attachmentsByMessage),
  });
}
