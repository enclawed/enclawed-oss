#!/usr/bin/env node
// Live probe of the three protocol bridges that the executive assistant uses.
...
//   ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_EMAIL    The IMAP/CalDAV/CardDAV login
//   ENCLAWED_IMAP_APP_PASSWORD            The 16-letter app password
//   ENCLAWED_PROBE_CONFIG                 Path to the app.config.json

import { readFileSync } from "node:fs";

const principal = process.env.ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_EMAIL;
...
if (!principal || !password || !configPath) {
  process.stderr.write(
    "probe: missing env (need ENCLAWED_EXECUTIVE_ASSISTANT_PRINCIPAL_EMAIL, ENCLAWED_IMAP_APP_PASSWORD, ENCLAWED_PROBE_CONFIG)\n",
  );
  process.exit(64);
}
