...
An AI executive assistant that reads inbound Gmail, replies to senders in the
...
**https://www.enclawed.com/enclawed-apps/executive-assistant**

The short version, once you have a Google OAuth Client ID and secret:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/enclawed/enclawed-oss/main/enclawed-apps/install.sh) executive assistant
```

The installer detects your OS, installs Node 22 and Ollama if missing,
runs the Google OAuth flow in your browser, writes
`~/.enclawed/enclawed-apps/executive-assistant/.env`, and registers the agent as a
...
bypass the audit. Production closure: run the executive assistant inside a network
namespace pinned to Google hosts (see `security/nft.rules.example`).

## Files

```
enclawed-apps/executive-assistant/
...
├── bin/install.sh          backward-compat shim → enclawed-apps/install.sh executive-assistant
├── security/
│   └── nft.rules.example   process-level egress filter
└── src/
    ├── main.ts             CLI entrypoint (argv → runApp)
    ├── runApp.ts           orchestrator (synthesizes the SkillManifest in-process)
...
- **Raw socket egress** — the user space egress guard wraps `fetch`, not
  `node:net`. Closure for production: run the app inside the network
...
- **OAuth token storage** — refresh tokens live in
  `~/.enclawed/enclawed-apps/executive-assistant/.env` at chmod 600. For a real deployment,
