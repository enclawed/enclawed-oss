// Web search for the secretary. Two backends, picked automatically:
//
...
//      operator who's installing the secretary as an end-user product
...
// are added to the secretary's allowlist at startup.
//
...
function parseDuckDuckGoHtml(html: string, count: number): ReadonlyArray<WebSearchResult> {
  ...
  const blockRe =
    /<div[^>]*class="[^"]*\bresult\b[^"]*results_links\b[^"]*"[\s\S]*?<\/div>/gi;
  const titleRe =
