...
export type EmailHitlPromptOptions = Readonly<{
   ...
   * equivalent (registered principal + ENCLAWED_SECRETARY_PRINCIPAL_ALIASES).
      ...
      if (!reminderSent && Date.now() >= reminderAt) {
        ...
        try {
          await smtp.sendMail({
            ...
            subject: `Re: ${subject}`,
            text:
              `Reminder: an enclawed-secretary HITL approval is still waiting on your reply.\n\n` +
              `Original request:\n\n${body}\n\n` +
    ...
    try {
      await smtp.sendMail({
        ...
        text:
          ...
          `secretary and a fresh HITL request will be issued.`,
        headers: {
          "X-Enclawed-HITL-Correlation": correlation,
          "X-Enclawed-HITL-RequestId": req.requestId,
