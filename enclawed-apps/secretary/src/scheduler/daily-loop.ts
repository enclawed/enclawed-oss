...
// Skill id the gate matches against the loaded secretary manifest. Must
...
export async function runDailyLoop(opts: DailyLoopOptions): Promise<void> {
  ...
  while (!opts.signal.aborted) {
    ...
    if (opts.updateCheckIntervalMs > 0) {
    ...
    if (pollsSinceHeartbeat >= heartbeatEveryPolls) {
      const lastSearchTag =
        lastSearchHitCount === null ? "n/a" : String(lastSearchHitCount);
      log(
        "info",
    ...
    try {
      // Adaptive sleep: when the loop is fully idle (no in-flight
      // threads AND no fresh threads were just dispatched), poll fast
      // so a newly arrived email gets picked up in ~½ s instead of
      // waiting up to a full pollMs. When there's work in flight,
      // keep the configured pollMs — the concurrency cap is already
      // backpressuring; faster polling just spins.
      const idleSleepMs = 500;
      const sleepMs =
        !activity && opts.state.inFlightCount() === 0 ? idleSleepMs : opts.pollMs;
...
async function processOnePoll(
  ...
  for (const t of threads) {
    ...
    if (seenUid !== undefined && t.lastUid <= seenUid) {
      droppedReasons.push(
        `${t.threadId} (lastUid=${t.lastUid} <= seen=${seenUid})`,
      );
      continue;
    }
  ...
  if (fresh.length === 0) {
    if (droppedReasons.length > 0) {
      ...
      // secretary did nothing" — surface why each was dropped so
...
// for the rationale. Override via ENCLAWED_SECRETARY_MAX_CONCURRENT.
const MAX_CONCURRENT_THREADS = Math.max(
  1,
  Number.parseInt(process.env.ENCLAWED_SECRETARY_MAX_CONCURRENT ?? "4", 10) || 4,
);
...
async function handleContact(
  ...
  const activeTools = CALENDAR_TOOLS.filter((t) => {
    if (t.function.name === "web_search" && !opts.webSearch) {return false;}
    if (t.function.name === "read_url" && !opts.readUrl) {return false;}
    if (t.function.name === "get_weather" && !opts.weather) {return false;}
    if (t.function.name === "schedule_followup" && !opts.followups) {return false;}
    if (t.function.name === "read_attachment" && !hasAttachments) {return false;}
    return true;
  });
  const activeToolNames = new Set(activeTools.map((t) => t.function.name));
  log(
    "info",
    `thread ${thread.threadId} active tools: [${[...activeToolNames].join(",")}]`,
  );
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    if (resp.toolCalls.length === 0) {
      ...
      // proof to the principal that the secretary refused to do
      ...
      if (
        ...
        messages.push({
          ...
          content:
            ...
            "secretary refusing to do the work. Call read_url on the most " +
  ...
  try {
    ...
    if (placeholderHit || deferralHit || scopeHit) {
      const issueKind = placeholderHit
        ? "placeholder"
        : deferralHit
          ? "deferral"
          : "out-of-scope";
          ...
          messages: [
            {
              ...
              content:
                ...
                "secretary's scope of email-triage / calendar / contacts / brief " +
      ...
      if (
        ...
        log(
          "warn",
          "stage-2 quality retry still contains the issue; sending original",
        );
      }
    }
  } catch (err) {
...
async function runDraftReview(args: {
  ...
  try {
    ...
    args.log(
      "warn",
      `draft-review prompt threw (${(err as Error).message}); treating as deny`,
    );
...
function renderFollowupBody(f: Followup): string {
  const lines = [
    `This is the secretary's scheduled follow-up reminder.`,
    ``,
    `When: ${f.triggerAtIso}`,
    `Why:  ${f.summary}`,
  ];
...
function renderSplitBody(bodyExcerpt: string): string[] {
  const split = extractNewMessageContent(bodyExcerpt);
  ...
  if (quotedContext.length > 0) {
    ...
    const quoted = quotedContext.length > 2000
      ? quotedContext.slice(0, 2000) + "\n…[truncated]"
      : quotedContext;
    out.push("");
...
// helper is just the secretary-side bookkeeping + notification body.
//
...
async function runUpdateCheckTick(opts: DailyLoopOptions): Promise<void> {
  ...
  // SPAWN_PROC. SPAWN_PROC is declared in the secretary manifest at
  ...
  const subject = "[Secretary] Update available";
...
function buildUpdateNotificationBody(opts: {
  ...
  return [
    ...
    "",
    "To apply, run on the host this secretary is installed on:",
    "",
    "  Update-EnclawedApp secretary    (Windows / PowerShell)",
    "  node ~/.enclawed/enclawed-oss/enclawed-apps/install.mjs secretary --update    (macOS / Linux / WSL)",
    "",
    "The update re-runs the prompt-injection shield self-test before",
    "restarting the secretary. If any test fails, the update aborts",
    "and the running secretary continues on the current version.",
    "",
...
function looksLikeEnglish(s: string): boolean {
  ...
  const englishHits = (t.match(/\b(the|and|of|to|in|is|for|on|with|that|this|you|your|have|are|was|were|will|been|but|not|from|i'?ll|i'?m|i'?ve)\b/g) ?? []).length;
...
async function translateToEnglishIfNeeded(args: {
  ...
  if (!trimmed || looksLikeEnglish(trimmed)) {return trimmed;}
  try {
...
/** Detect content that's outside the secretary's scope of email-
...
function containsOutOfScopeContent(s: string): boolean {
  ...
  if (/\b(?:import|from)\s+[a-z_.][a-z0-9_.]*\s+(?:import|;)/i.test(s)) {return true;}
  ...
  if (/\b[A-Z][a-z]+\s+(?:knew|felt|thought|wondered|realized|noticed|remembered|decided|hoped|wished|feared|imagined|dreamed)\s+(?:that|how|what|why|the|a|an|of)\b/.test(s)) {
    return true;
  }
  ...
  if (numberedSteps >= 4) {return true;}
  // 5. Word-count cap. Secretary replies are short (the prompt caps
  ...
  if (/\b(?:in\s+conclusion|in\s+summary|to\s+sum\s+up|overall|moreover|furthermore|additionally|on\s+the\s+other\s+hand|in\s+this\s+essay|in\s+this\s+story)\b/i.test(s)) {
    return true;
  }
  ...
  if (
    /\bencrypt\w*\b[\s\S]{0,300}\b(?:delete|remove|wipe|overwrit|shred|unlink)\w*\b/i.test(
      s,
    ) ||
 ...
 *  because the secretary's reply body precedes the quoted block, and
...
function estimateMessagesChars(messages: ReadonlyArray<OllamaMessage>): number {
  ...
  for (const m of messages) {
    ...
    if ("name" in m && typeof m.name === "string") {total += m.name.length;}
  }
  return total;
}
...
function compressEarlierToolUseMessages(
  ...
  if (lastAssistantIdx < 4) {return [...messages];}
...
function detectConflicts(
  ...
  for (const e of events) {
    if (excludeId && e.eventId === excludeId) {continue;}
...
export function parseQuietHours(raw: string | undefined | null): {
  ...
  if (!m) {return null;}
  const [, start, end] = m;
  if (!start || !end || !QUIET_HHMM_RE.test(start) || !QUIET_HHMM_RE.test(end)) {return null;}
  ...
  Object.freeze({
    ...
    function: Object.freeze({
      ...
      description:
        ...
        "URLs, and snippets. This is the secretary's window onto live " +
        ...
        "the principal explicitly asks the secretary to look something " +
      ...
      parameters: {
        ...
        properties: {
          ...
          attachmentIdx: {
            type: "integer",
            description:
              "0-based index into that message's attachments list.",
          },
        },
...
async function dispatchCalendarToolCall(args: {
  ...
  try {
    if (name === "web_search") {
      ...
      return maybeCache(Object.freeze({
        kind: "executed",
    ...
    if (name === "schedule_followup") {
      ...
      const followupOutcome = await opts.gate.dispatch({
        ...
        rollback: async () => {
          if (persisted) {await store.markFired(persisted.id);}
        },
      });
    ...
    if (name === "read_attachment") {
      ...
      const attachmentIdx =
        typeof a.attachmentIdx === "number" ? Math.floor(a.attachmentIdx) : -1;
...
function buildToolUseSystemPrompt(
  ...
  if (!has("web_search")) {unavailable.push("web_search (no BRAVE_SEARCH_API_KEY configured)");}
  if (!has("add_contact")) {unavailable.push("add_contact");}
  if (!has("schedule_followup")) {unavailable.push("schedule_followup");}
  if (!has("read_attachment")) {unavailable.push("read_attachment");}
  ...
  return [
    ...
    "  You are a secretary, not a general-purpose AI assistant. You",
    ...
    "repetition as the secretary being broken.",
    "",
...
function buildReplySystemPrompt(persona: { displayName: string; personaPrompt: string }): string {
  ...
  return [
    ...
    "    output text are the secretary's single worst failure mode and",
