...
const SECRETARY_CONTACTS_TOOLS = Object.freeze([
  "search_contacts",
  "list_contacts",
  "add_contact",
]);
...
export async function runApp(opts: AppRunOptions = {}): Promise<number> {
  ...
  const updateCheckIntervalMs = (() => {
    const raw = env.ENCLAWED_SECRETARY_UPDATE_CHECK_INTERVAL_MS;
    if (raw === undefined) {return 12 * 60 * 60 * 1000;}
  ...
  // this window so the secretary doesn't reply to inbound mail at
  // 3 AM. Format: "HH:MM-HH:MM" in the configured local TZ. Wraps
  // midnight when end < start.
  const quietHours = parseQuietHours(env.ENCLAWED_SECRETARY_QUIET_HOURS);
  ...
  // back to auto.
  const draftModeRaw = (env.ENCLAWED_SECRETARY_DRAFT_MODE ?? "auto").toLowerCase().trim();
  ...
  const classifierModel =
    env.ENCLAWED_SECRETARY_CLASSIFIER_MODEL ?? "llama3.1:8b";
  ...
  const rawDisplayName = env.ENCLAWED_SECRETARY_DISPLAY_NAME ?? "Secretary";
  const rawSystemPrompt = env.ENCLAWED_SECRETARY_SYSTEM_PROMPT ?? "";
  ...
  // tool itself is always wired so the secretary can answer "look this
  ...
  // principal-equivalent (e.g. they send the secretary instructions
  ...
  const principalAliases = (env.ENCLAWED_SECRETARY_PRINCIPAL_ALIASES ?? "")
    .split(",")
  ...
  const broker = buildSecretaryBroker({
    hitl,
    prompt,
    // principalSelfAddresses distinguishes the identities the secretary
    ...
    //     captured.
    //   - principalAliases = ENCLAWED_SECRETARY_PRINCIPAL_ALIASES,
    ...
    // loop's 30 min deadline, and the secretary denies the action
