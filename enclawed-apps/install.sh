...
#   bash <(curl -fsSL https://raw.githubusercontent.com/enclawed/enclawed-oss/main/enclawed-apps/install.sh) <app-name>
#
...
_BLD_REF="https://raw.githubusercontent.com/enclawed/enclawed-oss/main/enclawed-apps/install.sh"
...
if [ -z "$HERE" ] || [ ! -d "$HERE/$APP_NAME" ]; then
  ...
  if [ ! -d "$REPO_DIR/.git" ]; then
    ...
    (cd "$REPO_DIR" && git fetch --depth=1 origin main && git reset --hard origin/main >/dev/null)
  fi
