...
export const sharedVitestConfig = {
  root: repoRoot,
  // Vite deprecated `envFile: false` in favour of `envDir: false`; both mean
  // "do not load .env files", but the old key warns once per project config,
  // and this object is spread into every scoped project.
  // `as const` keeps this literal `false` rather than widening to
  // `boolean`, which Vite's UserConfig (string | false | undefined) rejects.
  ...
  resolve: {
    alias: [
      ...
      // The bundled MCP bridges are imported by package name so that src/ and
      // the sibling bridges do not reach into each other's files -- see the
      // extension import-boundary checks. tsgo and tsdown resolve those through
      // tsconfig paths, but Vitest resolves only what is listed here, so they
      // need entries or every load test that touches them fails with
      // "Cannot find package".
      {
        find: "@enclawed/module-trust/",
        replacement: `${path.join(repoRoot, "enclawed", "ts")}/`,
      },
      ...["mcp-attested", "mcp-google-workspace", "mcp-imap-smtp", "mcp-caldav", "mcp-carddav"].map(
        (name) => ({
          find: `@enclawed/${name}`,
          replacement: path.join(repoRoot, "extensions", name, "src", "index.ts"),
        }),
      ),
    ],
  },
