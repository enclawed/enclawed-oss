...
describe("projects vitest config", () => {
  ...
  it("disables vite env-file loading for vitest lanes", () => {
    // The property under test is that .env files are NOT loaded into test
    // lanes -- a test run must never pick up real credentials from the
    // developer's environment files. Vite deprecated `envFile: false` in
    // favour of `envDir: false`, and the configs migrated; this assertion did
    // not, so it was checking a key nothing reads any more -- failing whether
    // or not the real protection was in place.
    expect(baseConfig.envDir).toBe(false);
    expect(sharedVitestConfig.envDir).toBe(false);
    // Guard the migration itself: if `envFile` ever comes back it must not be
    // set to something that re-enables loading.
    expect(baseConfig.envFile ?? false).toBe(false);
    expect(sharedVitestConfig.envFile ?? false).toBe(false);
  });
