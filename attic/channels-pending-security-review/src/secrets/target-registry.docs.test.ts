...
describe("secret target registry docs", () => {
  ...
  it("stays in sync with docs/reference/secretref-credential-surface.md", () => {
    ...
    expect([...supportedFromDocs].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0))).toEqual(
      [...supportedFromMatrix].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
    );
    expect([...unsupportedFromDocs].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0))).toEqual(
      [...unsupportedFromMatrix].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
    );
  });
});
