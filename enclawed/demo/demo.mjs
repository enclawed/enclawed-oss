...
import { TIER, makeLabel, dominates, canRead, canWrite, format } from "../src/classification.mjs";
...
const COLOR = !args.has("--no-color") && process.stdout.isTTY;
...
const c = COLOR
  ...
  : Object.fromEntries(
      ["reset", "dim", "bold", "red", "green", "yellow", "blue", "magenta", "cyan"].map((k) => [
        k,
        "",
      ]),
    );
...
function header(n, total, title) {
  ...
  console.log(
    `${c.cyan}│${c.reset}${c.bold}${line}${c.reset}${" ".repeat(width - line.length)}${c.cyan}│${c.reset}`,
  );
  console.log(`${c.cyan}└${bar}┘${c.reset}`);
}
function pass(msg) {
...
process.on("exit", () => {
  /* sync best-effort handled by `cleanup` below */
});

// ── banner ──────────────────────────────────────────────────────────────
console.log();
console.log(
  `${c.bold}${c.magenta}enclawed${c.reset}${c.dim} — guided tour of the OSS hardening framework${c.reset}`,
);
...
note(
  "active scheme: US_GOVERNMENT (UNCLASSIFIED < CUI < CONFIDENTIAL < SECRET < TOP_SECRET < TOP_SECRET//SCI)",
);
...
if (canRead(SECRET_CLEARANCE, SECRET_DATA)) {
  pass("SECRET-cleared analyst MAY read SECRET report (no read-up)");
} else {
  fail("expected to allow read");
}

if (!canRead(SECRET_CLEARANCE, TOP_SECRET_DATA)) {
  pass("SECRET-cleared analyst MAY NOT read TOP_SECRET report (read-up blocked)");
} else {
  fail("expected to deny read-up");
}
...
if (!canWrite(SECRET_CLEARANCE, makeLabel({ level: TIER.PUBLIC }))) {
  ...
  fail("expected to deny write-down");
}
...
note(
  `scan() returned ${findings.length} findings (highest severity: ${highestSeverity(findings)})`,
);
for (const f of findings.slice(0, 5)) {
  note(`  • ${f.id} (${f.severity})`);
}
...
try {
  ...
  if (e instanceof EgressDeniedError) {
    pass(`fetch https://api.openai.com/... → blocked (${e.reason})`);
  } else {
...
const baseManifest = {
  v: 1,
  id: "demo-image-classifier",
  publisher: "metere-consulting",
  version: "1.0.0",
  clearance: "restricted",
  capabilities: ["inference.image"],
  signerKeyId: "demo-lab-2026",
};
...
if (enclaved.allowed) {
  pass(
    `enclaved flavor: signed module loaded (clearance=${enclaved.clearance}, signer=${enclaved.signerKeyId})`,
  );
} else {
  fail(`expected load to succeed: ${enclaved.reason}`);
}
...
if (!tamperResult.allowed) {
  pass(`enclaved flavor: tampered manifest BLOCKED (${tamperResult.reason})`);
} else {
  fail("expected tamper to be caught");
}
...
if (!unsignedEnclaved.allowed) {
  pass(`enclaved flavor: unsigned module BLOCKED (${unsignedEnclaved.reason})`);
}
...
for (const evt of [
  ...
  },
  {
    ...
    actor: "analyst-7",
    level: "restricted",
    payload: { tool: "image-classifier" },
  },
  {
    type: "tool.result",
    actor: "image-classifier",
    level: "restricted",
    payload: { resultBytes: 4096 },
  },
...
if (initial.ok) {
  pass(`chain verified: ${initial.count} records, no tampering`);
} else {
  fail(`chain broke: ${initial.reason}`);
}
...
if (!after.ok) {
  pass(`chain re-verified: TAMPER DETECTED at record #${after.brokenAt} (${after.reason})`);
} else {
  fail("expected tamper to break chain");
}
...
for (const d of detections.slice(0, 4)) {
  note(`  • ${d}`);
}
...
const proposal = session.proposeAction("network.outbound", {
  url: "https://leak-site.example/upload",
});
...
try {
  ...
  if (e instanceof ApprovalDeniedError) {
    pass(`operator DENIED action — agent received ApprovalDeniedError("${e.actionType}")`);
  } else {
    fail(`unexpected error: ${e.message}`);
  }
}
...
tx.record({
  ...
  inverse: () => {
    sideEffects.files.delete("draft.md");
  },
});
...
sideEffects.dbRows += 3;
note(
  `side-effects after 2 actions: files={${[...sideEffects.files].join(", ")}}, dbRows=${sideEffects.dbRows}`,
);
...
note(
  `side-effects after rollback: files={${[...sideEffects.files].join(", ")}}, dbRows=${sideEffects.dbRows}`,
);
...
note(
  `envelope: algo=${env.algo}, kdf=${env.kdf}, salt=${env.salt.slice(0, 16)}…, iv=${env.iv}, ct=${env.ct.slice(0, 24)}…`,
);
const round = decryptAtRest(env, "demo-passphrase").toString("utf8");
if (round === plaintext) {
  pass('round-trip OK (AES-256-GCM, scrypt KDF, AAD-bound to "analyst-7")');
} else {
  fail(`round-trip mismatch: got "${round}"`);
}
try {
  decryptAtRest(env, "wrong-passphrase");
  fail("expected GCM auth failure");
} catch {
  pass("wrong passphrase → GCM authentication failed (rejected)");
}
...
console.log(
  `${c.dim}207 unit + adversarial pen-tests cover these primitives — see \`npm test\`.${c.reset}`,
);
console.log(
  `${c.dim}framework reference: enclawed/src/   ·   paper: enclawed/paper/enclawed.tex${c.reset}`,
);
console.log();
