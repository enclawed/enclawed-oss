...
function freezeScheme(s) {
  return Object.freeze({
    ...
    levels: Object.freeze(s.levels.slice().toSorted((a, b) => a.rank - b.rank)),
    validCompartments: s.validCompartments ? Object.freeze(s.validCompartments.slice()) : undefined,
    validReleasability: s.validReleasability
      ? Object.freeze(s.validReleasability.slice())
      : undefined,
  });
}
 ...
 *
 * `String(value ?? "")` turns a supplied object into the literal
 * "[object Object]" -- a non-empty string that then passes every "is it
 * present" check below it, so a manifest claiming `{"id": {}}` was accepted
 * with that as its id. Numbers stay acceptable (an unquoted version is an
 * ordinary mistake, not an attack); objects, arrays and booleans are
 * malformed input and say so.
 */
...
export function parseClassificationScheme(raw) {
  ...
  const id = scalarField(raw.id, "scheme.id");
  if (!id) {
    throw new Error("scheme.id is required");
  }
  const description = scalarField(raw.description, "scheme.description");
  ...
  for (const lv of raw.levels) {
    ...
    if (!Number.isInteger(rank) || rank < 0) {
      throw new Error(`level.rank must be a non-negative integer, got ${lv.rank}`);
    }
    ...
    if (!canonicalName) {
      throw new Error(`level rank=${rank} missing canonicalName`);
    }
  ...
  const sorted = [...seenRanks].toSorted((a, b) => a - b);
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i] !== i) {
      throw new Error(
        `scheme ranks must be contiguous 0..${sorted.length - 1}, got ${JSON.stringify(sorted)}`,
      );
    }
  }
  const validCompartments =
    Array.isArray(raw.validCompartments) && raw.validCompartments.length > 0
      ? raw.validCompartments.map(String)
      : undefined;
  const validReleasability =
    Array.isArray(raw.validReleasability) && raw.validReleasability.length > 0
      ? raw.validReleasability.map(String)
      : undefined;
...
export function setActiveScheme(scheme) {
  activeScheme = scheme;
}
export function resetActiveScheme() {
...
export function clearanceNameToRank(name, scheme = activeScheme) {
  ...
  for (const lv of scheme.levels) {
    if (normalizeName(lv.canonicalName) === norm) {
      return lv.rank;
    }
...
export async function loadSchemeByName(name, opts = {}) {
  ...
  try {
    ...
    throw new Error(`scheme JSON parse failed at ${name}: ${e.message}`, { cause: e });
  }
