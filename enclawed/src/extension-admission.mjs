...
export function admitExtension({ manifest, flavor }) {
  ...
  if (f === "enclaved") {
    ...
    if (
      !verifyManifestSignature(
        canonicalExtensionManifestBytes(manifest),
        manifest.signature,
        signer.publicKeyPem,
      )
    ) {
  ...
  if (declaresNet && rank(manifest.verification) < rank(VERIFICATION.TESTED)) {
    warnings.push(
      `net.egress declared at verification "${manifest.verification}" — operator should promote to ≥ tested`,
    );
  }
  return Object.freeze({
    allowed: true,
...
export function installPerExtensionEgressGuard({ manifest, audit, extraAllowedHosts = [] }) {
  if (!manifest) {
    throw new TypeError("installPerExtensionEgressGuard: manifest required");
  }
