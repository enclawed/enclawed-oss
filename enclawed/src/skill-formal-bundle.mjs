 ...
 */
export function produceFormalBundle({ skillDir, manifest, signerKeyId, privateKeyPem, bound = 8 }) {
  if (!skillDir) {
    throw new TypeError("produceFormalBundle: skillDir required");
  }
  if (!manifest) {
    throw new TypeError("produceFormalBundle: manifest required");
  }
  ...
  const declared = [...new Set(manifest.caps)].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0));
  ...
  if (signerKeyId && privateKeyPem) {
    const sig = signManifest(Buffer.from(canonicalize(attestation), "utf8"), privateKeyPem);
...
export function readFormalBundle(bundleDir) {
  ...
  const attestation = JSON.parse(
    readFileSync(path.join(bundleDir, "manifest.attest.json"), "utf8"),
  );
...
export function verifyFormalBundle({
  ...
  if (authorisedFormalSigners) {
    const authorised =
      authorisedFormalSigners instanceof Set
        ? authorisedFormalSigners
  ...
  // -- 2. re-run Method A --------------------------------------------
  const declared = [...new Set(manifest.caps ?? [])].toSorted((a, b) =>
    a < b ? -1 : a > b ? 1 : 0,
  );
  ...
  if (bundle.attestation?.evidenceHashes) {
    ...
    if (eh.static !== aHashCached) {
      admit = false;
      reasons.push("attestation-static-hash-mismatch");
    }
    if (eh.types !== bHashCached) {
      admit = false;
      reasons.push("attestation-types-hash-mismatch");
    }
    if (eh.smt_unsat !== sha256OfCanonical(cachedC)) {
      admit = false;
      reasons.push("attestation-smt-hash-mismatch");
    }
  }
