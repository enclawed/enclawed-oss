...
function normalizeFrozenList(arr) {
  ...
  const dedup = [...new Set(src.map(String))].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0));
...
export function format(label, opts = {}) {
  ...
  const parts = [head];
  if (listSize(label.compartments) > 0) {
    parts.push(label.compartments.join("/"));
  }
  if (listSize(label.releasability) > 0) {
