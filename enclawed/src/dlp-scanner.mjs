...
const PATTERNS = [
  ...
  { id: "us-sci-codeword", severity: "high", re: /\b(NOFORN|ORCON|PROPIN|HCS|TK|SI|G|HUMINT)\b/g },
  {
  ...
  { id: "phone-e164", severity: "medium", re: /(?<![\w-])\+[1-9]\d{6,14}(?!\w)/g },
  { id: "credit-card-pan", severity: "high", re: /\b(?:\d[ -]?){13,19}\b/g },
  { id: "iban", severity: "high", re: /\b[A-Z]{2}\d{2}[A-Z0-9]{10,30}\b/g },
  {
    id: "us-ssn",
    severity: "high",
...
export function highestSeverity(findings) {
  ...
  for (const f of findings) {
    if (max === null || order[f.severity] > order[max]) {
      max = f.severity;
    }
  }
  return max;
...
export function redact(text, { placeholder = "[REDACTED]", minSeverity = "medium" } = {}) {
  ...
  const sorted = [...PATTERNS].toSorted((a, b) => order[b.severity] - order[a.severity]);
  for (const p of sorted) {
    if ((order[p.severity] ?? 0) < threshold) {
      continue;
    }
