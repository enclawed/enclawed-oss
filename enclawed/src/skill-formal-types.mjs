...
export function normaliseDeclaredCaps(caps) {
  ...
  return Object.freeze([...out].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)));
}
...
export function buildRefinedDispatch(manifest) {
  ...
  return Object.freeze(function refinedDispatch(envelope) {
    if (envelope === null || typeof envelope !== "object") {
      throw new RefinementError(`dispatch_M(${skillId}): envelope must be an object`);
    }
    const cap = envelope.capability;
    if (typeof cap !== "string") {
      throw new RefinementError(`dispatch_M(${skillId}): envelope.capability must be a string`);
    }
    if (!ALL_CAPS.has(cap)) {
    ...
    if (!declared.has(cap)) {
      throw new RefinementError(
        `dispatch_M(${skillId}): out-of-manifest envelope rejected at refinement boundary; ` +
          `capability "${cap}" not in declared caps [${[...declared].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)).join(",")}]`,
      );
    }
    return envelope;
  });
...
export function methodB({ manifest }) {
  if (!manifest) {
    throw new RefinementError("methodB: manifest required");
  }
  ...
  for (const c of [...ALL_CAPS].toSorted()) {
  ...
  return {
    ...
    schemaVocabulary: [...ALL_CAPS].toSorted(),
    probe,
    reason: allMatch
