...
// oxlint-disable-next-line no-control-regex -- naming control characters is the point: they are what gets stripped.
...
const ROLE_BOUNDARY_TOKEN =
  /(<\|(?:im_start|im_end|endoftext|system|user|assistant|tool|function)\|>|<\/(?:system|user|assistant|tool|function)>|\[\/?INST\]|<\|reserved_special_token_\d+\|>)/gi;
...
export function detectInjection(text) {
  ...
  if (CONTROL_CHARS.test(text)) {
    findings.push("control-chars");
  }
  CONTROL_CHARS.lastIndex = 0;
  if (BIDI_OVERRIDES.test(text)) {
    findings.push("bidi-overrides");
  }
  BIDI_OVERRIDES.lastIndex = 0;
  if (ZERO_WIDTH.test(text)) {
    findings.push("zero-width");
  }
  ZERO_WIDTH.lastIndex = 0;
  if (ROLE_BOUNDARY.test(text)) {
    findings.push("role-boundary");
  }
  ROLE_BOUNDARY.lastIndex = 0;
  if (ROLE_BOUNDARY_TOKEN.test(text)) {
    findings.push("role-boundary-token");
  }
  ...
  if (
    /\b(IGNORE|DISREGARD|OVERRIDE|BYPASS|FORGET|PURGE)\s+(?:(?:THE|ALL|ANY|YOUR|OF)\s+){0,3}(PREVIOUS|PRIOR|ABOVE|EARLIER|FOREGOING)\s+(INSTRUCTIONS?|RULES?|MESSAGES?|PROMPTS?|CONTEXT|SAFEGUARDS?|SAFETY|GUIDELINES?|POLICIES|POLICY|CONSTRAINTS?)/i.test(
      text,
    )
  ) {
  ...
  if (
    /\b(IGNORE|DISREGARD|OVERRIDE|BYPASS|FORGET|PURGE)\s+(?:(?:THE|ALL|ANY|YOUR|OF)\s+){0,3}(SAFEGUARDS?|SAFETY|GUIDELINES?|POLICIES|POLICY|CONSTRAINTS?|RULES?)\b/i.test(
      text,
    )
  ) {
    findings.push("imperative-override");
