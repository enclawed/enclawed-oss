...
export function getTrustRoot() {
  return runtimeTrustRoot;
}

export function setTrustRoot(signers) {
  if (trustRootLocked) {
    throw new TrustRootLockedError();
  }
  if (!Array.isArray(signers)) {
    throw new TypeError("setTrustRoot: signers must be array");
  }
  for (const s of signers) {
    if (!s || typeof s.keyId !== "string" || !s.publicKeyPem || !s.approvedClearance) {
      throw new TypeError(
        "setTrustRoot: each signer must define keyId, publicKeyPem, approvedClearance",
      );
    }
  }
...
export function lockTrustRoot() {
  trustRootLocked = true;
}

export function isTrustRootLocked() {
...
export function _unsafeUnlockTrustRootForTest() {
  trustRootLocked = false;
}
