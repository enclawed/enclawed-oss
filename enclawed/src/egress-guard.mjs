...
export function createEgressGuard({ allowedHosts, fetchImpl = globalThis.fetch, onDeny }) {
...
export function installEgressGuard(opts = {}) {
  ...
  if (opts.freeze) {
    ...
    const alreadyFrozenGuard =
      ...
      desc.value.__enclawedGuard === true;
...
export function installRawSocketGuard(opts = {}) {
  ...
  // The policy is host- and CIDR-based, so the port is not consulted; the
  // parameter stays to match the call at the connect hook below.
  ...
  // oxlint-disable-next-line typescript/unbound-method -- re-invoked with .apply(this, ...) by the patch below; binding it would defeat the interception.
  ...
  if (opts.freeze) {
    ...
    const alreadyFrozenGuard =
      ...
      desc.writable === false &&
      typeof desc.value === "function" &&
      desc.value.name === "enclawedRawSocketGuardConnect";
    if (!alreadyFrozenGuard) {
