...
test('Python: open(path,"w") → fs.write.rev; bare open() → fs.read', () => {
  const r = analyzeSource("open('/tmp/out', 'w')\nopen('/etc/passwd')\n", ".py");
  const caps = new Set(
    r.effects.map((e) => e.capability).toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
  );
  assert.ok(caps.has(CAPABILITY.FS_WRITE_REV));
  assert.ok(caps.has(CAPABILITY.FS_READ));
});
...
test("Python: exec()/eval() taints program to TOP", () => {
  const r = analyzeSource("exec(some_string)\n", ".py");
...
test("shell: curl → net.egress; > redirect → fs.write.rev; rm → fs.write.irrev", () => {
  const r = analyzeSource("curl https://example.com/foo > /tmp/out\nrm -f /tmp/old\n", ".sh");
  const caps = new Set(r.effects.map((e) => e.capability));
...
test("Python comments do not contribute spurious matches", () => {
  const r = analyzeSource("# requests.get('https://no.real.call/')\nx = 1\n", ".py");
...
test("analyzeSkillDirectory walks scripts and skips SKILL.md / docs / configs", () => {
  ...
  assert.deepEqual(
    [...out.unionEffects].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
    [CAPABILITY.FS_WRITE_REV, CAPABILITY.NET_EGRESS],
  );
...
test("checkContainment: contained when union ⊆ M.caps", () => {
  ...
  const r = checkContainment(analysis, [
