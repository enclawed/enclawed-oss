...
const STATE_DIR =
  process.env.ENCLAWED_INVIVO_ENCLAWED_STATE_DIR ??
...
function checkBootPrerequisites() {
  ...
  if (!existsSync(STATE_DIR)) {
    try {
      ...
      _bootError = `cannot create state dir ${STATE_DIR}: ${e.message}`;
      return _bootError;
    }
  }
  return null;
...
function targetForChannel(channelId, target) {
  ...
  if (channelId === "discord" && /^\d+$/.test(String(target))) {
...
export async function mediateEnclawedSample({ channelId, content, target }) {
  ...
  const args = [
    ...
    channelId,
    "--target",
    targetForChannel(channelId, target),
    "--message",
    content,
    "--dry-run",
    "--json",
  ...
  // rejection.
  const accepted =
    parsed.action === "send" && parsed.payload != null && !parsed.error && r.status === 0;
  return {
    delivered: accepted,
    reason: accepted
