...
if (!FORCE_DISABLE_ACCREDITOR) {
  try {
    ...
    const providerPrivPem = providerKp.privateKey
...
function splitKey(key) {
  try {
    ...
    if (Array.isArray(a) && a.length === 2) return { op: a[0], target: a[1] };
  } catch {}
...
function checkBiconditional({ delta, audit: records }) {
  ...
  for (const [k, dC] of D) {
    const sC = S.get(k) ?? 0;
    if (dC > sC) f1.push({ ...splitKey(k), count: dC - sC });
  }
  for (const [k, sC] of S) {
    const dC = D.get(k) ?? 0;
    if (sC > dC) f2.push({ ...splitKey(k), count: sC - dC });
  }
...
function* walk(root) {
  ...
  for (const e of entries) {
    if (e.isDirectory()) {
      if (!SKIP_DIRS.has(e.name)) yield* walk(path.join(root, e.name));
...
function randAlnum(n, charset = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789") {
  let s = "";
  for (let i = 0; i < n; i++) s += charset[Math.floor(randUnit() * charset.length)];
  return s;
}
  ...
  if (!STATS_ONLY) {
    ...
    for (const [subj, m] of [
      ...
      try {
        ...
        console.error(`matrix ${subj}: ${e.message}`);
      }
      await new Promise((r) => setTimeout(r, 600));
    }
    try {
...
for (const r of channelResults) {
  ...
  for (const [k, v] of [...r.blockReasonHist.oss.entries()].sort((a, b) => b[1] - a[1]))
    md += `- \`${k}\`: ${v}\n`;
  md += `\n### enclawed-enclaved top block reasons\n\n`;
  for (const [k, v] of [...r.blockReasonHist.enclaved.entries()].sort((a, b) => b[1] - a[1]))
    md += `- \`${k}\`: ${v}\n`;
  md += "\n";
}
