...
function* walk(root) {
  ...
  for (const e of entries) {
    if (e.isDirectory()) {
      if (!SKIP_DIR_NAMES.has(e.name)) yield* walk(path.join(root, e.name));
...
function deriveCapTarget(extDir, name) {
  ...
  if (!role && enclawedManifest) {
    ...
    if (Array.isArray(enclawedManifest.channels) && enclawedManifest.channels.length > 0) {
      role = "channel";
      cap = "publish";
      target = `channel://${enclawedManifest.channels[0]}/message`;
    } else if (Array.isArray(enclawedManifest.providers) && enclawedManifest.providers.length > 0) {
      role = "provider";
      cap = "tool.invoke";
      target = `provider://${enclawedManifest.providers[0]}/inference`;
    } else if (
      Array.isArray(enclawedManifest.commandAliases) &&
      enclawedManifest.commandAliases.length > 0
    ) {
      role = "tool";
      cap = "tool.invoke";
      target = `tool://${name}/${enclawedManifest.commandAliases[0].name}`;
    } else if (typeof enclawedManifest.id === "string") {
      role = "generic";
      cap = "tool.invoke";
      target = `tool://${enclawedManifest.id}/op`;
    }
  }
  if (!role && pkg) {
