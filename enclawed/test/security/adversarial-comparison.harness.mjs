...
function multisetFromAudit(records) {
  ...
  for (const r of records) {
    if (r.type !== "irreversible.executed") {
      continue;
    }
...
function checkBiconditional({ delta, audit }) {
  const D = multisetFromDelta(delta),
    S = multisetFromAudit(audit);
  const f1 = [],
    f2 = [];
  for (const [k, dC] of D) {
    const sC = S.get(k) ?? 0;
...
const PROBE_PATTERNS = [
  ...
  {
    id: "auditChain",
    primitive: "hash-chained AuditLogger",
    regex: /class\s+AuditLogger\b|recordHash[\s\S]{0,200}prevHash|verifyChain[\s\S]{0,40}audit/,
  },
  {
    id: "admission",
    primitive: "extension admission gate",
    regex: /\badmitExtension\b|\bparseExtensionManifest\b/,
  },
  {
    id: "egressGuard",
    primitive: "two-layer egress guard",
    regex: /installEgressGuard|installRawSocketGuard/,
  },
  {
    id: "classification",
    primitive: "Bell-LaPadula classification",
    regex: /defaultEnclavedPolicy|defaultClassifiedPolicy|maxOutputClearance/,
  },
  {
    id: "trustRoot",
    primitive: "module-signing + trust root",
    regex: /lockTrustRoot|verifyManifestSignature/,
  },
  {
    id: "bootstrapSeal",
    primitive: "bootstrap seal",
    regex: /sealBootstrap|BootstrapAlreadySealedError/,
  },
];
...
function* walk(root) {
  ...
  for (const e of entries) {
    ...
    } else if (e.isFile() && SCAN_EXTENSIONS.has(path.extname(e.name))) {
...
function probeTree(root) {
  ...
  if (!existsSync(root) || !statSync(root).isDirectory()) {
    return { available: false, root, found, files: 0, ms: 0 };
  }
  let files = 0;
  for (const file of walk(root)) {
    ...
    for (const p of PROBE_PATTERNS) {
      if (!found[p.id] && p.regex.test(c)) {
        found[p.id] = true;
      }
    }
    if (PROBE_PATTERNS.every((p) => found[p.id])) {
      break;
    }
  }
...
function deriveCapTarget(extDir, name) {
  ...
  // enclawed.plugin.json; fall back to a generic role
  // inferred from the extension's name.
  const moduleManifest = readJSON(path.join(extDir, "enclawed.module.json"));
  const pluginManifest = readJSON(path.join(extDir, "enclawed.plugin.json"));
  ...
  if (moduleManifest && Array.isArray(moduleManifest.capabilities)) {
    const caps = moduleManifest.capabilities;
    ...
    if (caps.includes("channel") || caps.includes("publish")) {
      ...
      role = "provider";
      cap = "tool.invoke";
      target = `provider://${name}/inference`;
      ...
      role = "net";
      cap = "net.egress";
      target = `host://${name}.local/op`;
    } else if (caps.includes("fs.write.irrev")) {
      role = "fs-irrev";
      cap = "fs.write.irrev";
      target = `path://${name}/data`;
  ...
  if (!role && pluginManifest) {
    manifestSource = manifestSource || "enclawed.plugin.json";
    if (Array.isArray(pluginManifest.channels) && pluginManifest.channels.length > 0) {
      role = "channel";
      cap = "publish";
      target = `channel://${pluginManifest.channels[0]}/message`;
    } else if (Array.isArray(pluginManifest.providers) && pluginManifest.providers.length > 0) {
      role = "provider";
      cap = "tool.invoke";
      target = `provider://${pluginManifest.providers[0]}/inference`;
    } else if (
      Array.isArray(pluginManifest.commandAliases) &&
      pluginManifest.commandAliases.length > 0
    ) {
      role = "tool";
      cap = "tool.invoke";
      target = `tool://${name}/${pluginManifest.commandAliases[0].name}`;
    } else if (typeof pluginManifest.id === "string") {
      role = "generic";
      cap = "tool.invoke";
      target = `tool://${pluginManifest.id}/op`;
    }
  }
  if (!role && pkg) {
      ...
      audit: [
        {
          ...
          payload: { ok: true, call: { cap: extInfo.cap, target: A } },
        },
      ],
    },
    F3: {
      delta: [{ op: extInfo.cap, target: A, count: 1 }],
      audit: [
        {
          type: "irreversible.executed",
          payload: { ok: false, call: { cap: extInfo.cap, target: A } },
        },
      ],
    },
    F4: {
      delta: [{ op: extInfo.cap, target: B, count: 1 }],
      audit: [
        {
          type: "irreversible.executed",
          payload: { ok: true, call: { cap: extInfo.cap, target: A } },
...
function detect(probe, scenario, key) {
  ...
  if (missing.length > 0) {
    return { detected: false, reason: `missing: ${missing.join(",")}` };
  }
  const r = checkBiconditional(scenario);
  if (r.ok) {
    return { detected: false, reason: "check ok (unexpected)" };
  }
...
const allNames = [
  ...new Set([...extLists.enclawed, ...extLists.oss, ...extLists.enclaved]),
...
const allRolesUsed = [...new Set(rows.map((r) => r.role).filter((x) => x !== "-"))].toSorted(
  (a, b) => (a < b ? -1 : a > b ? 1 : 0),
);
...
for (const r of rows) {
  ...
  const target = (r.target ?? "-").replace(/\\/g, "\\\\").replace(/\|/g, "\\|");
