...
// Not every tree carries the macOS app. Where apps/macos is absent
// there is nothing to keep in sync, and the old behaviour was to fail
// --check with "Out of date" and crash --write with ENOENT on a
// directory that was never supposed to exist there. Since `pnpm check`
// runs this on every commit, that made a tree without apps/ unable to
// commit any non-docs change at all. Absent target, nothing to do.
if (!fs.existsSync(path.dirname(outputPath))) {
  console.log(`Skipping ${path.relative(repoRoot, outputPath)} — no macOS app in this tree.`);
  process.exit(0);
}
