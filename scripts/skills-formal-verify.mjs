...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    const need = () => {
      i += 1;
      if (i >= argv.length) {
        die(`${a} expects a value`);
      }
      return argv[i];
    };
    switch (a) {
      case "--manifest":
        out.manifestPath = need();
        break;
      case "--out":
        out.outDir = need();
        break;
      case "--bound":
        out.bound = parseInt(need(), 10);
        break;
      case "--gen-key":
        out.genKey = true;
        break;
  ...
  if (positional.length !== 1) {
    die("exactly one positional <skill-dir> argument required");
  }
...
function resolveSigner(args) {
  if (args.keyPath && args.genKey) {
    die("--key and --gen-key are mutually exclusive");
  }
  if (args.keyPath) {
    return {
