...
function emitRow(row) {
  ...
  return [chan, fcat, label, oc, oss, enc, csvEsc(ossBR), csvEsc(encBR), csvEsc(content)].join(",");
}
...
function decompressGz(p) {
  ...
  if (r.status !== 0) {
    process.stderr.write(
      `  ${path.basename(p)}: gunzip recovered ${r.stdout.length} bytes ` +
...
function scrubFile(p, { checkOnly = false } = {}) {
  ...
  if (checkOnly) {
    // Report what WOULD be redacted; leave the file alone. This is the mode
    // the publish gate runs in, so a dirty artifact stops the publish
    // instead of being quietly rewritten under the operator.
    return result;
  }
  // Write back.
  let outBuf;
...
function expandDefaults() {
  ...
  if (existsSync(xllmDir) && statSync(xllmDir).isDirectory()) {
    for (const f of readdirSync(xllmDir).toSorted()) {
...
const resolved = targets.length > 0 ? targets : expandDefaults();
...
for (const p of resolved) {
  process.stderr.write(
    `${checkOnly ? "checking" : "scrubbing"} ${path.relative(REPO_ROOT, p)} ...\n`,
  );
  const r = scrubFile(p, { checkOnly });
  if (!r) {
    anyError = true;
    continue;
  }
...
if (checkOnly) {
  process.stderr.write(
    `\nChecked ${totalRows} rows across ${resolved.length} file(s): ` +
      `${totalChanged} content cell(s) still carry secret-shaped text.\n`,
  );
  if (totalChanged > 0) {
    process.stderr.write(
      "\nThese artifacts are PUBLISHED. Run without --check to scrub them:\n" +
        "  node scripts/scrub-invivo-csv.mjs\n",
    );
  }
  process.exit(anyError || totalChanged > 0 ? 1 : 0);
}
process.stderr.write(
  `\nDone. ${totalRows} rows scanned, ${totalChanged} content cells rewritten across ${resolved.length} files.\n`,
);
