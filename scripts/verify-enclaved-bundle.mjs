...
async function checkExtensions(trustKeys) {
  ...
  for (const ent of entries) {
    ...
    const body = {
      ...
      capabilities: [...m.capabilities].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
...
async function main() {
  ...
  console.log(
    `enclaved-bundle verifier: ${ext.signed} extensions signed, ${trustKeys.size} signers in trust root`,
  );
  ...
  if ((ext.gatedUnsigned ?? []).length > 0) {
    console.log(
      `  gated-unsigned (rejected by enclaved admission gate): ${ext.gatedUnsigned.length} extensions`,
    );
    for (const id of ext.gatedUnsigned.slice(0, 10)) {
      console.log(`    - ${id}`);
    }
    if (ext.gatedUnsigned.length > 10) {
      console.log(`    ... and ${ext.gatedUnsigned.length - 10} more`);
    }
  }
  if (all.length > 0) {
  ...
  console.log(
    "PASS: every signed manifest verifies; unsigned plugins are gated by the host admission layer; attic/ unreachable from tsconfig include.",
  );
}
