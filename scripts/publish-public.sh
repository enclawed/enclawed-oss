#!/usr/bin/env bash
# Snapshot the current working tree to every public mirror as a single orphan
# commit on `main`, after wiping every branch and tag on each mirror. The SAME
# snapshot commit is pushed to all mirrors, so they end up byte-identical (same
# commit SHA, same flattened single-commit history). The private remote
# (`origin`) and the local working tree / HEAD are not touched.
#
# Public mirrors = every remote whose name matches `public-*` (NOT the bare
# `public` remote — that name is reserved for the deprecated
# metereconsulting/enclawed repository, which now hosts only a redirect README
# and is no longer kept in sync with the workspace). Add another with:
#   git remote add public-<name> <url>
#
# Usage: scripts/publish-public.sh [--yes]
#   --yes  skip the interactive confirmation
#
# Requires:
#   - at least one remote named `public` or `public-*`
#   - push access to each
#
# Notes:
#   - Snapshot includes uncommitted changes and new files (anything `git add -A`
#     would stage). It respects .gitignore.
#   - GitHub-managed `refs/pull/*` refs cannot be deleted by push; left as-is.

set -euo pipefail

# Resolve this script's own directory BEFORE changing directory. Everything
# below runs from the repo root, so a "$(dirname "$0")" taken afterwards means
# the root, not scripts/, whenever the script was invoked as ./publish-public.sh
# from inside scripts/ -- which is how the scrubber below went missing.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

cd "$(git rev-parse --show-toplevel)"

mapfile -t PUBLIC_REMOTES < <(git remote | grep -E '^public-.+$' | sort)
if [ "${#PUBLIC_REMOTES[@]}" -eq 0 ]; then
  echo "error: no public mirror configured (need a remote named 'public-*')" >&2
  echo "  add one with: git remote add public-<name> <url>" >&2
  exit 1
fi

# Cheap filename sanity check — flag, don't block.
SUSPECT=$(
  { git ls-files; git ls-files --others --exclude-standard; } \
    | grep -iE '(^|/)\.env($|\.[^e]|_)|\.pem$|\.key$|id_rsa|id_ed25519|\.netrc|\.p12$|\.pfx$' \
    || true
)
if [ -n "$SUSPECT" ]; then
  echo "warning: filenames that look sensitive will be included in the snapshot:" >&2
  echo "$SUSPECT" >&2
  echo >&2
fi

# HARD GATE: the published in-vivo corpora must carry no secret-shaped text.
#
# The content column of those CSVs is LLM output, and the adversarial suite
# asks the model to fabricate keys ("an OpenAI key starting sk- with at least
# 20 chars") -- so a fresh harness run reliably produces strings that GitHub's
# secret scanner flags the moment they are published. That already happened
# once: the 2026-07-10 snapshot shipped 49 of them to the public mirror, and
# they had to be dismissed one by one. The scrubber existed then too; nothing
# made publishing depend on it. Now it does.
echo "checking published artifacts for secret-shaped text ..."
SCRUBBER="$SCRIPT_DIR/scrub-invivo-csv.mjs"
# Fail closed, but say which failure it is. A missing scrubber used to exit
# non-zero like any other error and was reported as "secret-shaped text": a
# false alarm about the one thing this check exists to be exact about, and one
# that invites the operator to work around the check rather than trust it.
if [ ! -f "$SCRUBBER" ]; then
  echo >&2
  echo "REFUSING TO PUBLISH: the secret scrubber is missing at $SCRUBBER," >&2
  echo "so the published corpora cannot be checked. This is not a finding" >&2
  echo "about the CSVs; restore the scrubber and run again." >&2
  exit 1
fi
if ! node "$SCRUBBER" --check >/dev/null 2>&1; then
  echo >&2
  echo "REFUSING TO PUBLISH: an in-vivo CSV still carries secret-shaped text." >&2
  echo >&2
  node "$SCRUBBER" --check >&2 || true
  exit 1
fi
echo "  ok — no secret-shaped text in the published corpora"
echo
echo "checking the dependency tree for known vulnerabilities ..."
# Zero, every time. A public snapshot is the tree people install from, so an
# audit that cannot run is a refusal too -- publishing unverified is not a
# fallback. See scripts/check-audit-zero.mjs.
if ! node "$SCRIPT_DIR/check-audit-zero.mjs"; then
  echo >&2
  echo "REFUSING TO PUBLISH: the dependency audit is not at zero, or could not run." >&2
  exit 1
fi
echo

DIRTY=$(git status --short | wc -l | tr -d ' ')
HEAD_SHA=$(git rev-parse --short HEAD)
BRANCH=$(git symbolic-ref --short HEAD 2>/dev/null || echo "(detached)")

echo "About to publish one identical snapshot to ${#PUBLIC_REMOTES[@]} public mirror(s):"
for r in "${PUBLIC_REMOTES[@]}"; do
  echo "  - $r  ($(git remote get-url "$r"))"
done
echo "  source branch : $BRANCH @ $HEAD_SHA"
echo "  dirty files   : $DIRTY (will be included in snapshot)"
echo "  destination   : <remote>/main (force-replace, single orphan commit)"
echo

if [ "${1:-}" != "--yes" ]; then
  read -r -p "Proceed? [y/N] " ans
  case "$ans" in
    y|Y|yes|YES) ;;
    *) echo "aborted"; exit 1 ;;
  esac
fi

# Build the orphan snapshot ONCE via plumbing — no branch switch, no working-tree
# touch, no real-index touch. The same commit object is pushed to every mirror,
# so all mirrors are byte-identical (same SHA).
SNAP_INDEX=$(mktemp -u)
trap 'rm -f "$SNAP_INDEX"' EXIT
GIT_INDEX_FILE="$SNAP_INDEX" git add -A
# Public mirrors carry no CI/automation: drop GitHub Actions workflows and the
# Dependabot config from the snapshot, so each publish does NOT re-arm Actions or
# Dependabot on the mirror. (Removed from the snapshot index only; the working
# tree and origin are untouched.)
GIT_INDEX_FILE="$SNAP_INDEX" git rm -r --cached --quiet --ignore-unmatch \
  .github/workflows .github/dependabot.yml >/dev/null 2>&1 || true

# The platform clients are PRODUCT, not open source: they are sold as
# subscriptions. They are excluded from every public snapshot. (Removed from
# the snapshot index only; the working tree and the private origin keep them.)
GIT_INDEX_FILE="$SNAP_INDEX" git rm -r --cached --quiet --ignore-unmatch \
  clients >/dev/null 2>&1 || true

# Fail loudly rather than ship one by accident: if anything closed-source is
# still in the snapshot tree, stop. A silent exclusion that stops matching
# after a directory rename is exactly how this leaks.
CLOSED_LEFT=$(GIT_INDEX_FILE="$SNAP_INDEX" git ls-files --cached \
  -- clients .github/workflows .github/dependabot.yml 2>/dev/null | head -5)
if [ -n "$CLOSED_LEFT" ]; then
  echo >&2
  echo "REFUSING TO PUBLISH: closed-source paths are still in the snapshot:" >&2
  echo "$CLOSED_LEFT" >&2
  exit 1
fi

TREE=$(GIT_INDEX_FILE="$SNAP_INDEX" git write-tree)
COMMIT=$(git commit-tree "$TREE" -m "Snapshot $(date -u +%Y-%m-%d)")
echo "built snapshot commit $COMMIT (tree $TREE)"
echo

for r in "${PUBLIC_REMOTES[@]}"; do
  # Per-remote SSH key override, if configured (e.g. an org mirror with a
  # dedicated deploy key):  git config remote.<name>.identityFile <path>
  KEY=$(git config "remote.$r.identityFile" 2>/dev/null || true)
  if [ -n "$KEY" ]; then
    export GIT_SSH_COMMAND="ssh -i $KEY -o IdentitiesOnly=yes"
    echo "publishing to $r (key: $KEY) ..."
  else
    unset GIT_SSH_COMMAND
    echo "publishing to $r ..."
  fi
  # Wipe every branch except main (overwritten next).
  git ls-remote --heads "$r" \
    | awk '$2 != "refs/heads/main" {print $2}' \
    | sed 's|^|:|' \
    | xargs -r -n1 git push "$r"
  # Wipe every tag.
  git ls-remote --tags "$r" \
    | awk '{print $2}' \
    | sed 's|^|:|' \
    | xargs -r -n1 git push "$r"
  # Force-push the orphan commit as main.
  git push "$r" "$COMMIT:refs/heads/main" --force
  echo "  $r/main → $COMMIT"
done

echo
echo "done. all public mirrors → $COMMIT"
