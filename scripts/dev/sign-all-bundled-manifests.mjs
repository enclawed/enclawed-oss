...
function deriveCapabilitiesFromPlugin(pkg, plugin) {
  ...
  if (Array.isArray(plugin?.providers) && plugin.providers.length > 0) {
    caps.add(ENCLAWED_CAPS.TOOL_INVOKE);
  }
  if (Array.isArray(plugin?.channels) && plugin.channels.length > 0) {
    caps.add(ENCLAWED_CAPS.PUBLISH);
  }
  ...
  if (typeof plugin?.runtime === "object") {
    caps.add(ENCLAWED_CAPS.FS_READ);
  }
  if (caps.size === 0) {
    caps.add(ENCLAWED_CAPS.FS_READ);
  }
  return [...caps].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0));
}
...
async function main() {
  ...
  for (const ent of entries) {
    ...
    const manifestBody = {
      ...
      version,
      clearance,
      capabilities: [...capabilities].toSorted((a, b) => (a < b ? -1 : a > b ? 1 : 0)),
      signerKeyId: SIGNER_KEY_ID,
      verification: "tested", // bundled extensions ship at verification=tested
      netAllowedHosts: [...netAllowedHosts].toSorted(),
    };
    ...
    await writeFile(out, JSON.stringify({ ...manifestBody, signature }, null, 2) + "\n");
    written.push({
      id,
    ...
    JSON.stringify(
      {
        ...
        description:
          "Bundled dev signer used for the shipped extension set. Replace with an HSM-anchored signer for any non-dev deployment.",
  ...
  for (const target of [TRUST_ROOT_TS, TRUST_ROOT_MJS]) {
    ...
    if (patched === src) {
      console.warn(
        `WARN: BUNDLED_DEV_PUBKEY not found in ${path.relative(REPO_ROOT, target)}; skipping`,
      );
    } else {
  ...
  if (skipped.length > 0) {
    ...
    for (const id of skipped.toSorted()) {
      console.log(`  ${id}`);
    }
  }
  console.log(`\nbundled signer pubkey written to ${path.relative(REPO_ROOT, SIGNER_OUT)}`);
  console.log(
    "Bundled signer private key was generated in-memory and discarded; re-run to regenerate.",
  );
}
