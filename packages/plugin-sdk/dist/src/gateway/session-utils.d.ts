...
export { archiveFileOnDisk, archiveSessionTranscripts, attachEnclawedTranscriptMeta, capArrayByJsonBytes, readFirstUserMessageFromTranscript, readLastMessagePreviewFromTranscript, readLatestSessionUsageFromTranscript, readSessionTitleFieldsFromTranscript, readSessionPreviewItemsFromTranscript, readSessionMessages, resolveSessionTranscriptCandidates, } from "./session-utils.fs.js";
...
export declare function listAgentsForGateway(cfg: EnclawedConfig): {
    defaultId: string;
    mainKey: string;
    scope: SessionScope;
...
export declare function loadCombinedSessionStoreForGateway(cfg: EnclawedConfig): {
    storePath: string;
    store: Record<string, SessionEntry>;
};
export declare function getSessionDefaults(cfg: EnclawedConfig): GatewaySessionsDefaults;
export declare function resolveSessionModelRef(cfg: EnclawedConfig, entry?: SessionEntry | Pick<SessionEntry, "model" | "modelProvider" | "modelOverride" | "providerOverride">, agentId?: string): {
    provider: string;
    model: string;
};
...
export declare function resolveSessionModelIdentityRef(cfg: EnclawedConfig, entry?: SessionEntry | Pick<SessionEntry, "model" | "modelProvider" | "modelOverride" | "providerOverride">, agentId?: string, fallbackModelRef?: string): {
    provider?: string;
    model: string;
};
