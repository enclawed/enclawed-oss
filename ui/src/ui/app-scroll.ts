...
export function scheduleChatScroll(host: ScrollHost, force = false, smooth = false) {
  ...
  const pickScrollTarget = () => {
    // The retry below fires on a 120-150ms timer, so this can run after the
    // host is gone -- a torn-down test environment, or a page being unloaded.
    // There is nothing to scroll in that state, and callers already handle a
    // null target; reaching for `document` instead threw an uncaught
    // ReferenceError out of the timer. Guarded the same way `window` is below.
