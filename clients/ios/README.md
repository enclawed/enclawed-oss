...
4. If signing fails on a personal team:
   - Use unique local bundle IDs via `clients/ios/LocalSigning.xcconfig`.
   - Start from `clients/ios/LocalSigning.xcconfig.example`.
...
- Beta release uses canonical `ai.enclawed.client*` bundle IDs through a temporary generated xcconfig in `clients/ios/build/BetaRelease.xcconfig`.
...
- The beta flow does not modify `clients/ios/.local-signing.xcconfig` or `clients/ios/LocalSigning.xcconfig`.
- `clients/ios/version.json` is the pinned iOS release version source.
- `clients/ios/CHANGELOG.md` is the iOS-only changelog and release-note source.
...
- See `clients/ios/VERSIONING.md` for the full workflow.
...
This should create `clients/ios/fastlane/.env` with the non-secret ASC variables while the private key stays in Keychain.
...
6. Expected behavior:
   - Fastlane reads `clients/ios/version.json`
   - verifies synced iOS versioning artifacts
   - resolves the next TestFlight build number for that short version
   - generates `clients/ios/build/BetaRelease.xcconfig`
   - archives `Enclawed`
   - uploads the IPA to TestFlight

7. Expected outputs after a successful run:
   - `clients/ios/build/beta/Enclawed-<version>.ipa`
   - `clients/ios/build/beta/Enclawed-<version>.app.dSYM.zip`
   - Fastlane log line like `Uploaded iOS beta: version=<version> short=<short> build=<build>`

8. If this is a fresh clone on a maintainer machine that already works elsewhere, it is OK to copy the non-secret `clients/ios/fastlane/.env` from another trusted local clone on the same Mac. The Keychain-backed private key remains machine-local and is not stored in the repo.

## iOS Versioning Workflow

- Pinned iOS release version: `clients/ios/version.json`
- iOS-only changelog: `clients/ios/CHANGELOG.md`
- Generated checked-in artifacts:
  - `clients/ios/Config/Version.xcconfig`
  - `clients/ios/fastlane/metadata/en-US/release_notes.txt`
- Useful commands:
...
1. Keep `clients/ios/version.json` pinned to the current train version.
2. Update `clients/ios/CHANGELOG.md`, usually under `## Unreleased` while iterating.
...
2. Update `clients/ios/CHANGELOG.md` for the new release as needed.
...
See `clients/ios/VERSIONING.md` for the detailed spec.

## APNs Expectations For Local/Manual Builds

- The app calls `registerForRemoteNotifications()` at launch.
- `clients/ios/Sources/Enclawed.entitlements` sets `aps-environment` to `development`.
...
- `clients/ios/fastlane/.env` only covers App Store Connect / Fastlane auth; it does not provide gateway APNs credentials for local direct-push testing.
