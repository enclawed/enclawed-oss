...
The pinned iOS release version lives in `clients/ios/version.json`.
...
- `clients/ios/version.json`
  - pinned iOS release version
- `clients/ios/CHANGELOG.md`
  - iOS-only changelog and release-note source
- `clients/ios/VERSIONING.md`
  - workflow and constraints

### Generated or derived files

- `clients/ios/Config/Version.xcconfig`
  - checked-in defaults derived from `clients/ios/version.json`
- `clients/ios/fastlane/metadata/en-US/release_notes.txt`
  - generated from `clients/ios/CHANGELOG.md`
- `clients/ios/build/Version.xcconfig`
...
- `scripts/ios-write-version-xcconfig.sh`
  ...
  - writes the local numeric build override file in `clients/ios/build/Version.xcconfig`
- `scripts/ios-beta-prepare.sh`
  - prepares beta signing and bundle settings against the pinned iOS version
- `clients/ios/fastlane/Fastfile`
...
When generating `clients/ios/fastlane/metadata/en-US/release_notes.txt`, the tooling reads the first available changelog section in this order:
...
1. keep `clients/ios/version.json` pinned to the current TestFlight train version
2. update `clients/ios/CHANGELOG.md` under `## Unreleased` while iterating
...
2. review the generated changes in:
   ...
   - `clients/ios/Config/Version.xcconfig`
   - `clients/ios/fastlane/metadata/en-US/release_notes.txt`
3. update `clients/ios/CHANGELOG.md` for the new release if needed
...
Fastlane and Xcode should consume only the pinned iOS version from `clients/ios/version.json`.
