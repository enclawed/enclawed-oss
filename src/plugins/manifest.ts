...
export type PackageManifest = {
  ...
  Partial<Record<LegacyManifestKey, EnclawedPackageManifest>>;
...
export function getPackageManifestMetadata(
  ...
  // Backward-compatible read for legacy package.json keys (e.g. "openclaw":
  // {...}). legacy-names.ts states the contract outright -- new code writes
  // only the current MANIFEST_KEY, the legacy keys stay readable -- and without
  // this an existing third-party plugin published under the old key declares no
  // extensions at all and fails to install.
  for (const legacyKey of LEGACY_MANIFEST_KEYS) {
    const legacy = manifest[legacyKey];
    if (legacy) {
      return legacy;
    }
  }
