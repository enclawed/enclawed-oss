...
/**
 * The override pnpm actually applies for a package, if the repo declares one.
 *
 * Pins live in pnpm-workspace.yaml: pnpm stopped reading the package.json
 * `pnpm` field, so the repo migrated them out (see the note above `overrides:`
 * in that file). Anything still declared under `pnpm.overrides` is inert, and
 * a top-level `overrides` entry is shadowed whenever the workspace file names
 * the same package.
 */
function readWorkspaceOverride(name: string): string | undefined {
  const raw = fs.readFileSync(path.resolve(process.cwd(), "pnpm-workspace.yaml"), "utf8");
  const line = raw.split("\n").find((entry) => new RegExp(`^\\s+"?${name}"?:`).test(entry));
  return line
...
describe("dependency denylist guardrails", () => {
  ...
  it("pins the axios override to an exact version", () => {
    ...
    // Checked where the pin actually takes effect. This used to also assert a
    // pin under package.json `pnpm.overrides`, a field pnpm no longer reads --
    // so it demanded a declaration with no bearing on resolution and failed
    // permanently once the pins moved to pnpm-workspace.yaml.
    const effective = readWorkspaceOverride("axios") ?? manifest.overrides?.axios;
    expect(effective).toMatch(/^\d+\.\d+\.\d+$/);
    // The npm-style block is shadowed by the workspace entry, so a stale value
    // there reads as protection while having none. Keep them in step.
    expect(manifest.overrides?.axios).toBe(effective);
  });
