...
/**
 * The version an alias actually pins.
 *
 * A dependency written as `npm:@scope/name@version` carries the package name
 * inside the spec string. Comparing whole specs therefore compares names, which
 * never agree across four differently-named packages, and the range check never
 * saw a leading `^` because the string begins with "npm:". Both checks below
 * were reading the alias envelope instead of the version inside it.
 */
function readSpecVersion(spec: string): string {
  const at = spec.lastIndexOf("@");
  return at > 0 ? spec.slice(at + 1) : spec;
}

function isExactPinnedVersion(spec: string): boolean {
  const version = readSpecVersion(spec);
...
describe("pi package graph guardrails", () => {
  it("keeps root Pi packages aligned to the same exact version", () => {
    ...
    const presentSpecs = specs.map((entry) => readSpecVersion(entry.spec));
