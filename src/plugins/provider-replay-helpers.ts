...
export function buildOpenAICompatibleReplayPolicy(
  ...
  // An explicit opt-out means the transport carries provider-native tool-call
  // ids that have to survive replay untouched. Emitting `sanitizeToolCallIds:
  // false` next to `toolCallIdMode: "strict"` still handed the replayer an id
  // policy to enforce, so the ids were validated and rewritten anyway.
  const toolCallIdPolicy =
    options.sanitizeToolCallIds === false
      ? {}
      : ({
