...
type ThinkingProviderPlugin = {
  ...
  /**
   * The exact set of levels this model offers, when the provider knows it.
   * Overrides the generic ladder: a model advertising {off, max} has no
   * "medium" to offer, and the ladder has no "max" to give it.
   */
  resolveThinkingProfile?: (ctx: ProviderThinkingPolicyContext) =>
    | {
