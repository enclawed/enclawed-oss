...
export async function dispatchReplyFromConfig(
  ...
  // Also honour the caller's delivery mode. `message_tool_only` is the privacy
  // default for group and channel surfaces: the agent still processes the
  // message, but an unaddressed reply must not be posted for the whole room.
  // Channels resolve this and pass it in replyOptions, and nothing here ever
  // read it -- so the reply was delivered anyway and the default did nothing.
  const suppressDelivery =
    sendPolicy === "deny" || params.replyOptions?.sourceReplyDeliveryMode === "message_tool_only";
