...
describe("mention helpers", () => {
  ...
  it("normalizes zero-width characters", () => {
    // The zero-width space sits inside the name: stripping it is what stops a
    // mention from being hidden from the matcher. The input carried the old
    // brand while the expectation had been renamed to the new one, so it
    // asserted a mapping this function does not and should not perform.
    expect(normalizeMentionText("encl\u200bawed")).toBe("enclawed");
  });
