...
function normalizeFrozenList(arr?: Iterable<string>): ReadonlyArray<string> {
  ...
  const dedup = [...new Set([...arr].map(String))].toSorted();
  return Object.freeze(dedup);
}

export function makeLabel(input: {
  // Any numeric rank: Level names the canonical set, but a scheme may carry
  // ranks outside it, so the wider type is the real contract.
  level: number;
...
export function format(label: Label, opts?: { nameStyle?: NameStyle }): string {
  ...
  if (label.compartments.length > 0) {
    parts.push(label.compartments.join("/"));
  }
  if (label.releasability.length > 0) {
    parts.push(label.releasability.join("/"));
  }
  return parts.join("//");
}
...
export function parse(input: string): Label {
  ...
  if (segments.length === 0) {
    throw new Error("empty classification string");
  }
