...
function makeLevel(rank: number, canonicalName: string, aliases: string[] = []): SchemeLevel {
 ...
 */
function scalarField(value: unknown, field: string): string {
...
export function parseClassificationScheme(raw: unknown): ClassificationScheme {
  ...
  const id = scalarField(o.id, "scheme.id");
  if (!id) {
    throw new Error("scheme.id is required");
  }
  const description = scalarField(o.description, "scheme.description");
...
export async function loadSchemeByName(
  ...
  try {
    ...
    throw new Error(`scheme JSON parse failed at ${name}: ${(e as Error).message}`, { cause: e });
  }
