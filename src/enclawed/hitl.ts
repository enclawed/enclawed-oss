...
export class AgentSession {
  ...
  pause(): void {
    if (this.state !== SESSION_STATE.RUNNING) {
      return;
    }
    this.state = SESSION_STATE.PAUSED;
    this._pauseGate = new Promise<void>((res) => {
      this._pauseResolve = res;
    });
  ...
  async checkpoint(): Promise<void> {
    if (this.isStopped()) {
      throw new AgentStoppedError(this.stopReason);
    }
  ...
  async proposeAction(actionType: string, payload: unknown): Promise<ApprovalDecision> {
    ...
    if (decision === APPROVAL.DENY) {
      throw new ApprovalDeniedError(actionType);
    }
    return decision;
  }
}
...
export class HitlController extends EventEmitter {
  ...
  createSession(opts: { agentId: string; requireApprovalFor?: Iterable<string> }): AgentSession {
    if (!opts.agentId) {
      throw new Error("createSession: agentId required");
    }
  ...
  liveSessions(): AgentSession[] {
    return this.listSessions().filter((s) => s.isLive());
  }
  ...
  resolveApproval(approvalId: string, decision: ApprovalDecision): void {
    ...
    if (decision !== APPROVAL.ALLOW && decision !== APPROVAL.DENY) {
      throw new Error(`decision must be allow|deny, got ${String(decision)}`);
    }
