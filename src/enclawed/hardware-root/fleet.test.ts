...
import { HaltError, type AttestingDevice } from "./guard.js";
...
function mockDevice(o: MockOpts) {
  ...
  // The mock DOES enrol and attest, so say so: typing it as a bare
  // GuardDevice erased the two methods every job in this file calls.
  return dev as unknown as AttestingDevice & { state: typeof state };
}
...
describe("hardware-root fleet", () => {
  ...
  it("runs work in parallel ACROSS devices but serialised PER device", async () => {
    ...
    await Promise.all(
      Array.from({ length: 9 }, () =>
        // The mock ignores both, but attest() is a two-argument call.
        fleet.run((d) =>
          (d as never as (typeof mocks)[0]).attest(Buffer.alloc(32), Buffer.alloc(32)),
        ),
      ),
    );
