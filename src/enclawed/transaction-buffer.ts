...
function txCanonicalBody(
  tx: Pick<Transaction, "id" | "ts" | "agentId" | "description" | "payload" | "byteSize">,
): string {
...
export class TransactionBuffer {
  ...
  record(input: {
    ...
    while (this._bytesUsed + byteSize > this._maxBytes && this._txs.length > 0) {
      ...
      this._totalEvicted++;
      this._auditAppend("transaction.evicted", {
        id: evicted.id,
    ...
    this._lastHash = hash;
    this._auditAppend("transaction.recorded", {
      id,
      description: input.description,
      byteSize,
  ...
  async rollback(n: number = 1): Promise<RollbackResult> {
    ...
    this._lastHash = this._txs.length > 0 ? this._txs[this._txs.length - 1].hash : GENESIS_HASH;
  ...
  private _auditAppend(type: string, payload: unknown): void {
    if (this._audit) {
      this._audit
        .append({ type, actor: "transaction-buffer", level: null, payload })
        .catch(() => {});
    }
  }
}
