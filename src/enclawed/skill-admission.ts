...
import { ALL_CAPABILITIES, type CapabilityToken } from "./skill-capabilities.js";
import { SkillGate } from "./skill-gate.js";
import { type SkillManifest, VERIFICATION, parseSkillManifest } from "./skill-manifest.js";
import { SkillMutationGuard, type LoadedSkillRecord } from "./skill-mutation-guard.js";
...
export async function admitUpstreamSkill(
  ...
  const declaredCaps = (s.declaredCaps ?? []).filter((c): c is CapabilityToken =>
    (ALL_CAPABILITIES as ReadonlyArray<string>).includes(c),
  );
