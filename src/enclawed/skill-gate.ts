...
import { assertHardwareClean, hardwareRootInfo } from "./hardware-root/index.js";
...
import { type SkillManifest, VERIFICATION } from "./skill-manifest.js";
...
export class SkillGate {
  ...
  async dispatch(input: {
    ...
    // Fail-secure hardware root: if a bound enclaweder disconnected or zeroized, HALT every call
    // (reversible or not) until a human recovers. No-op in software-only deployments.
    assertHardwareClean();
    ...
    if (
      (v === VERIFICATION.DECLARED || v === VERIFICATION.TESTED || v === VERIFICATION.FORMAL) &&
      inDeclared
    ) {
      ...
      if (res.ok) {
        ...
        await this.audit.append({
          ...
          payload: {
            ...
            ok: true,
            hardwareRoot: hardwareRootInfo(), // was this action accredited via the enclaweder hardware?
          },
        });
