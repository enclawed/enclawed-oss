...
import { ALL_CAPABILITIES, type CapabilityToken, isCapabilityToken } from "./skill-capabilities.js";
...
export function parseSkillManifest(raw: unknown): SkillManifest {
  ...
  if (!id) {
    throw new Error("skill manifest: id is required");
  }
  ...
  if (!signer) {
    throw new Error("skill manifest: signer is required");
  }
...
export function canonicalSkillBytes(manifest: SkillManifest, contentSha256: string): Buffer {
  const body = {
    ...
    label: {
      ...
      compartments: [...manifest.label.compartments].toSorted(),
      releasability: [...manifest.label.releasability].toSorted(),
    },
    caps: [...manifest.caps].toSorted(),
...
export function canonicalSkillHash(manifest: SkillManifest, contentHashHex: string): string {
  return createHash("sha256").update(canonicalSkillBytes(manifest, contentHashHex)).digest("hex");
}
