...
// Zod 4 made ZodTypeAny (= ZodType<unknown, unknown, $ZodTypeInternals<...>>)
// invariant in its third (Internals) generic, so a concrete ZodObject /
// ZodPipe / ZodDiscriminatedUnion is no longer assignable. The only methods
// we touch here are `.safeParse` and `.toJSONSchema`; use a version-agnostic
// structural alias so callers can pass any concrete zod schema class.
type AnyZodSchema = {
...
type ZodSchemaWithToJsonSchema = AnyZodSchema & {
