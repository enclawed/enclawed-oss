...
export const WhatsAppConfigSchema = WhatsAppSharedSchema.extend({
  ...
  // Defaults belong to the top level only. The shared shape is reused for
  // `accounts.*`, so defaulting there stamped an explicit value onto every
  // account -- which then shadowed the top-level setting it was supposed to
  // inherit.
  ...
  .superRefine((value, ctx) => {
    ...
    // An account without its own allowFrom inherits it, in order, from the
    // account acting as the default and then from the channel-level list. The
    // default account is where a multi-account setup keeps the shared
    // allowlist. Matched case-insensitively because `accounts.Default` reaches
    // the runtime the same way `accounts.default` does.
    const defaultAccountKey =
      value.defaultAccount ??
      Object.keys(value.accounts).find((key) => key.toLowerCase() === "default");
    const defaultAccountAllowFrom = defaultAccountKey
      ? value.accounts[defaultAccountKey]?.allowFrom
      : undefined;
    for (const [accountId, account] of Object.entries(value.accounts)) {
      ...
      const effectiveAllowFrom = account.allowFrom ?? defaultAccountAllowFrom ?? value.allowFrom;
      enforceOpenDmPolicyAllowFromStar({
