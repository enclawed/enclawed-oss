...
export const TelegramAccountSchemaBase = z
  .object({
    ...
    network: z
      ...
      .strict()
      .optional(),
    // Bounded to the range the watchdog can actually act on: below 30s it trips
    // on ordinary long-poll gaps and restarts a healthy loop, above 10 minutes
    // a genuinely stalled poller sits undetected for longer than the stall it
    // exists to catch.
...
const DiscordVoiceSchema = z
  .object({
    ...
    // Capped at two minutes. Without an upper bound a typo like 120000000 was
    // accepted and turned a failed voice connect into a twenty-minute hang
...
export const DiscordAccountSchema = z
  .object({
    ...
    // Snowflake, so a value written unquoted above 2^53 has already lost
    // precision before it reaches us. DiscordIdSchema normalises a safe number
    // to its string form and reports an unsafe one instead of silently using a
    // rounded value as somebody else's application ID.
...
export const SlackAccountSchema = z
  .object({
    ...
    socketMode: z
      .object({
        // Timeouts, so zero or negative is never a configuration -- it is a
        // ping that has already expired, which disconnects Socket Mode on the
        // first heartbeat instead of tuning it.
        clientPingTimeout: z.number().positive().optional(),
        serverPingTimeout: z.number().positive().optional(),
