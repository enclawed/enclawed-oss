...
export function resolveChannelGroupRequireMention(params: {
  ...
  // Only for a group that actually has its own config entry -- which is what
  // the flag name says. It was applied to every group instead, so a group that
  // was merely present in an allowlist lost mention gating too.
  return !(params.configuredGroupDefaultsToNoMention === true && Boolean(groupConfig));
}
