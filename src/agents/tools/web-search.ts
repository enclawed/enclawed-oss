...
export function createWebSearchTool(options?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      // AgentTool.execute's `params` widens to `unknown` after the schema
      // validates them; runWebSearch wants Record<string, unknown>. The
      // schema guarantees it is an object, but TS does not propagate that.
