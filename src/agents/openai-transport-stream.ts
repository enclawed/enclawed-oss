...
import { stripNullHeaderValues } from "../plugin-sdk/provider-stream-shared.js";
...
function convertResponsesTools(
  ...
  return tools.map((tool) => ({
    ...
    strict,
    // FunctionTool's `parameters` field is structurally Record<string, unknown>;
    // our typebox-derived TSchema is structurally compatible but TS will not
    // confirm it across the parameterised generics. Same shape, same cast as
    // the strict-undefined branch above.
  })) as unknown as FunctionTool[];
}
