...
function resolveLiveXaiModel() {
  // Upstream pi-ai dropped "grok-4-1-fast-reasoning" and "grok-4" from the
  // xai catalog; both are intentional fallback IDs that may exist server-side
  // for live xAI tenants even when not in the local model registry. Cast
  // through `never` so the lookup still compiles — getModel returns
  // `undefined` for unknown IDs at runtime, which is what the chain consumes.
  return getModel("xai", "grok-4-1-fast-reasoning" as never) ?? getModel("xai", "grok-4" as never);
}
