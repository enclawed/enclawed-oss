...
type OpenAIResponsesPayloadModel = {
  ...
  // Model<Api>.compat now widens to a discriminated union per upstream pi-ai
  // (Anthropic, OpenAI Completions, OpenAI Responses). Take it as unknown and
  // narrow at the read site — the only field consumed here is supportsStore.
  compat?: unknown;
};
...
function readSupportsStoreFlag(compat: unknown): boolean | undefined {
  if (compat && typeof compat === "object" && "supportsStore" in compat) {
    const value = (compat as { supportsStore?: unknown }).supportsStore;
...
function narrowSupportsStoreCompat(
  compat: unknown,
): { supportsStore?: boolean; supportsPromptCacheKey?: boolean } | undefined {
  if (!compat || typeof compat !== "object") {
    return undefined;
  }
  const record = compat as Record<string, unknown>;
  const out: { supportsStore?: boolean; supportsPromptCacheKey?: boolean } = {};
  if (typeof record.supportsStore === "boolean") {
    out.supportsStore = record.supportsStore;
  }
  if (typeof record.supportsPromptCacheKey === "boolean") {
    out.supportsPromptCacheKey = record.supportsPromptCacheKey;
  }
  return out;
}
...
export function resolveOpenAIResponsesPayloadPolicy(
  ...
  const capabilities = resolveProviderRequestPolicyConfig({
    ...
    compat: narrowSupportsStoreCompat(model.compat),
    capability: "llm",
    transport: "stream",
  }).capabilities;
  ...
  return {
    ...
    shouldStripStore:
      explicitStore !== true && readSupportsStoreFlag(model.compat) === false && isResponsesApi,
    useServerCompaction:
