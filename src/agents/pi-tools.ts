// pi-coding-agent no longer exports a pre-instantiated `codingTools` array or a
// `readTool` value; both are now factories parameterised by `cwd`. Instantiate
// against the resolved workspace root at call time below.
...
// Runtime callers pass either the local config-side `ModelCompatConfig` or the
// upstream pi-ai discriminated union (`AnthropicMessagesCompat |
// OpenAICompletionsCompat | OpenAIResponsesCompat`). We never narrow per call
// site — the downstream helpers (resolveUnsupportedToolSchemaKeywords,
// resolveToolCallArgumentsEncoding) already type-check on
// `{ compat?: unknown } | ModelCompatConfig | undefined`. Accept any object
// shape to bridge the two surfaces without forcing every caller to assert.
type RuntimeModelCompatConfig = object;
...
function applyModelProviderToolPolicy(
  ...
  params?: {
    ...
    modelId?: string;
    agentDir?: string;
    modelCompat?: RuntimeModelCompatConfig;
  },
): AnyAgentTool[] {
...
export function createEnclawedCodingTools(options?: {
  ...
  // pi-coding-agent moved from a pre-instantiated `codingTools` value export
  // to a `createCodingTools(cwd)` factory. The tool array shape is unchanged;
  // the workspace root is what feeds into the bash/read/write/edit defaults.
  const base = (createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === "read") {
      ...
      // createReadTool returns AgentTool<TObject<...>, any>; createEnclawedReadTool
      // wants AnyAgentTool (= AgentTool<any, unknown>). `any` and `unknown` are not
      // assignable in either direction at the generic position, but the result is a
      // valid concrete AgentTool — coerce through unknown to bridge the variance.
      const freshReadTool = createReadTool(workspaceRoot) as unknown as AnyAgentTool;
