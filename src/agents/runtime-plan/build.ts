...
export function buildAgentRuntimeDeliveryPlan(
  // Nothing in the plan reads the params yet: followup routing is not wired
  // through provider-runtime (see resolveFollowupRoute below).
  _params: BuildAgentRuntimeDeliveryPlanParams,
): AgentRuntimeDeliveryPlan {
  return {
  ...
  return {
    ...
    tools: {
      ...
      logDiagnostics(
        ...
        logProviderToolSchemaDiagnostics({
          ...resolveToolContext(overrides),
          // AgentTool defaults its TResult to `any`; the diagnostics helper
          // defaults TResult to `unknown`. The two are not assignable in
          // either direction at a generic position. Coerce through unknown.
          tools: tools as unknown as AgentTool<TSchema, unknown>[],
        });
      },
    },
    transcript: {
