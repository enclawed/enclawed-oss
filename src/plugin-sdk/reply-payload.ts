...
>(params: {
  ...
  chunker?:
    | ((
        text: string,
        limit: number,
        ctx?: { formatting?: { maxLinesPerMessage?: number } },
      ) => string[])
    | null;
  ...
  // The formatting context matters: a chunker that splits on line count sees
  // no limit without it and returns the text whole.
  const chunks =
    limit && params.chunker
      ? params.chunker(text, limit, {
          formatting: (params.ctx as { formatting?: { maxLinesPerMessage?: number } }).formatting,
        })
      : [text];
...
export async function sendTextMediaPayload(params: {
    ...
    const lastResult = await sendPayloadMediaSequence({
      ...
      send: async ({ text, mediaUrl }) =>
        await params.adapter.sendMedia!({
          ...
          text,
          mediaUrl,
          // Lifted out of the payload: adapters take this as a send option, not
          // as a payload field, so leaving it nested sent every voice note as a
          // plain audio attachment.
          ...(params.ctx.payload.audioAsVoice === undefined
            ? {}
            : { audioAsVoice: params.ctx.payload.audioAsVoice }),
        }),
    });
  ...
  const chunks =
    limit && params.adapter.chunker
      ...
      : [text];
  // An implicit reply belongs on the first chunk only. Repeating it made every
  // chunk of a split message a reply to the same source message, which is what
  // `replyToMode: "first"` exists to prevent.
  const replyToCtx = params.ctx as {
    replyToId?: string | null;
    replyToMode?: Parameters<typeof createReplyToFanout>[0]["replyToMode"];
    replyToIdSource?: Parameters<typeof createReplyToFanout>[0]["replyToIdSource"];
  };
  const nextReplyToId = createReplyToFanout({
    replyToId: replyToCtx.replyToId,
    replyToMode: replyToCtx.replyToMode,
    replyToIdSource: replyToCtx.replyToIdSource,
  });
