...
export function isChannelExecApprovalClientEnabledFromConfig(params: {
  ...
  // Explicit opt-in. Configuring approvers says WHO may approve; it does not
  // say the channel may run an approval client at all. Treating an unset flag
  // as "on" turned contact configuration into an enablement decision, and this
  // gate governs command execution -- the wrong direction to fail. `auto` is
  // still an explicit opt-in spelling, so only an absent flag is a refusal.
