...
import { verifyHistory } from "../../enclawed/audit-log.js";
...
export function registerAuditCommand(program: Command) {
  ...
  audit
    .command("verify [path]")
    .description("Verify a hash-chained audit log end-to-end, across all rotated segments")
    ...
    .action(async (pathArg: string | undefined, opts: { json?: boolean }) => {
      ...
      await runCommandWithRuntime(defaultRuntime, async () => {
        let result;
        try {
          // verifyHistory, not verifyChain: a rotated log lives in several
          // segments, and checking only the newest would report a clean chain
          // for a history whose earlier segments had been deleted.
        ...
        if (result.ok) {
          ...
          if (result.segments.length > 1) {
            defaultRuntime.log(
              `segments: ${result.segments.length} (one chain across all of them)`,
            );
          }
          defaultRuntime.log(`path:  ${path}`);
          // Say how the head was attested. "matched-offline" is a weaker claim
          // than "matched" and must not be reported as if it were the same:
          // without the device's counter, a restored older anchor cannot be
          // ruled out.
          if (result.anchor === "matched") {
            defaultRuntime.log(`chain ok (head attested by ${result.anchorId})`);
          } else if (result.anchor === "matched-offline") {
            defaultRuntime.log(
              `chain ok (head matches a ${result.anchorId}-signed record, but the device was not ` +
                `present: completeness is not proven)`,
            );
          } else {
            defaultRuntime.log("chain ok (no head anchor: completeness is not proven)");
          }
          return;
        }
        defaultRuntime.error(
          result.brokenAt === null
            ? `chain broken between segments (${result.reason}); ${result.count} entries verified before failure`
            : `chain broken at entry ${result.brokenAt} of ${result.file} (${result.reason}); ` +
                `${result.count} entries verified before failure`,
        );
