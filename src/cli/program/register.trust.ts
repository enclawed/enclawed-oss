...
import { getTrustRoot, isTrustRootLocked } from "../../enclawed/trust-root.js";
    ...
    .action((opts: { json?: boolean }) => {
      void runCommandWithRuntime(defaultRuntime, async () => {
        ...
        for (const s of signers) {
          ...
          if (s.notAfter) {
            defaultRuntime.log(`  notAfter:    ${s.notAfter}`);
          }
        ...
        await runCommandWithRuntime(defaultRuntime, async () => {
          ...
          try {
            const result = await addPersistedSigner({
              ...
              description:
                opts.description ?? `Signer added via 'enclawed trust add' (${opts.signer}).`,
