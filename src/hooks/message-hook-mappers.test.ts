...
describe("message hook mappers", () => {
  ...
  it("maps canonical inbound context to plugin/internal received payloads", () => {
    ...
    // messageId and senderId are part of the context now: a hook that receives a
    // message has to be able to name the turn, not just the conversation.
  ...
  it("maps sent context consistently for plugin/internal hooks", () => {
    ...
    // The sent context carries the message id for the same reason the inbound
    // one does: a hook must be able to correlate the two halves of a turn.
    expect(toPluginMessageContext(canonical)).toEqual({
      channelId: "demo-chat",
      accountId: "acc-1",
      conversationId: "demo-chat:chat:456",
