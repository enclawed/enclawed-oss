...
async function ensureEnclawedHooks(manifest: HookPackageManifest) {
  // Read-only fallback to the legacy manifest key, matching how every other
  // package manifest is read here (see getPackageManifestMetadata). Without it
  // a hook pack published under "openclaw" -- which legacy-names.js explicitly
  // promises to keep reading -- failed to install with "missing enclawed.hooks"
  // even though its hooks were declared.
  const metadata =
    manifest[MANIFEST_KEY] ?? LEGACY_MANIFEST_KEYS.map((key) => manifest[key]).find(Boolean);
