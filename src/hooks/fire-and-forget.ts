...
export function formatHookErrorForLog(err: unknown): string {
  // `String(err)` for a real Error, so the constructor name survives: this line
  // is the only record a fire-and-forget hook leaves behind, and "boom" alone
  // does not say whether it was a TypeError, a timeout, or a rejected string.
  const described = err instanceof Error ? String(err) : formatErrorMessage(err);
  const formatted = replaceLogControlCharacters(described).replace(/\s+/g, " ").trim();
