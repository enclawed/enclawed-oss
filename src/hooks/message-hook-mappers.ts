...
export type CanonicalInboundMessageHookContext = {
  /** The session this message belongs to, when the caller knows it. */
  sessionKey?: string;
  from: string;
  to?: string;
  content: string;
...
export function toPluginMessageContext(
  ...
  return {
    ...
    // Guarded because this mapper serves both the inbound and the sent context,
    // and only the inbound one carries a sender.
    ...("sessionKey" in canonical && canonical.sessionKey
      ? { sessionKey: canonical.sessionKey }
      : {}),
    ...("messageId" in canonical && canonical.messageId ? { messageId: canonical.messageId } : {}),
    ...("senderId" in canonical && canonical.senderId ? { senderId: canonical.senderId } : {}),
  };
}
...
export function toPluginMessageReceivedEvent(
  ...
  return {
    ...
    content: canonical.content,
    timestamp: canonical.timestamp,
    // Promoted out of metadata: a hook must be able to correlate the event to a
    // session and a turn without digging through a bag of optional strings.
    // Omitted rather than set to undefined so the payload keeps a stable shape.
