...
/**
 * Whether a tag opens its own line (only whitespace before it).
 *
 * A real emitted tool-call block starts a line; an example quoted inside a
 * sentence does not. That is the whole difference between markup the user was
 * never meant to see and prose deliberately talking about the markup, so the
 * XML-payload strip below is limited to the standalone form.
 */
function isAtLineStart(text: string, tagStart: number): boolean {
  let idx = tagStart - 1;
...
export function stripToolCallXmlTags(
  ...
  for (let idx = 0; idx < text.length; idx += 1) {
    ...
    if (!inToolCallBlock) {
      ...
      // `function_call` / `function_calls` carry an <invoke>/<parameter> XML
      // body in the Anthropic tool-call format, not just a JSON one. Detecting
      // only JSON for those tags left that entire shape unstripped, so a model
      // that emitted the XML form leaked the whole block into user-visible
      // text -- which is precisely what `stripFunctionCallsXmlPayloads` is
      // named for and documented to prevent.
      const allowsXmlPayload =
        tag.tagName === "tool_call" ||
        tag.tagName === "function" ||
        ((tag.tagName === "function_call" || tag.tagName === "function_calls") &&
          options?.stripFunctionCallsXmlPayloads !== false &&
          // A truncated `<function_calls` opening stays visible: unlike
          // `tool_call`, the contract preserves those rather than swallowing
          // the rest of the message on a tag that never closed.
          !tag.isTruncated &&
          isAtLineStart(text, idx));
      const payloadKind = allowsXmlPayload
