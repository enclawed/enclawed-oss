...
export function resolveDmGroupAccessWithCommandGate(
  ...
  return {
    ...
    // Applies to DMs too. `shouldBlock` already means "a control command
    // arrived from a sender who is not authorised for commands"; discarding it
    // for direct messages let exactly that command through to dispatch on the
    // strength of it not being a group.
    shouldBlockControlCommand: commandGate.shouldBlock,
  };
}
