...
// The macOS client is not checked out in every tree -- the OSS repo has no
// apps/ directory -- and a parity test has nothing to compare when the other
// side is absent. Skipped explicitly rather than failing on a missing file.
const macosClientPresent = fs.existsSync(path.join(process.cwd(), "apps", "macos", "Sources"));

describe("host env security policy parity", () => {
  it.skipIf(!macosClientPresent)(
