 ...
 */
export function resolveDefaultConfigPath(options: ResolveWorkspaceDirOptions = {}): {
  path: string;
...
export function formatRuleReason(rule: WorkspaceDirResolution["rule"]): string {
  switch (rule) {
    ...
    default: {
      // Exhaustive above: this keeps the compile-time check while giving the
      // function a return on every path.
