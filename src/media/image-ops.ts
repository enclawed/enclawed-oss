...
// The CALLABLE, not the module. `typeof import("sharp")` is the namespace
// object -- it carries the default export, it is not itself the factory --
// so ReturnType<> of it has nothing to return and calling it is not callable.
// Sharp is loaded dynamically here (it is optional, and unavailable under
// some runtimes), which is exactly where that distinction gets lost.
...
async function loadSharp(): Promise<(buffer: Buffer) => ReturnType<SharpFactory>> {
  ...
  return (buffer) =>
    sharp(buffer, {
      // Sharp renamed this: `failOnError: false` became `failOn: "none"`,
      // and the old key is simply not read any more. It has been inert
      // here, which left decoding at the default "warning" -- stricter
      // than intended, so a merely warty image failed instead of being
