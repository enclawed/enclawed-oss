...
const { fetchWithSsrFGuardMock, hasEnvHttpProxyConfiguredMock, matchesNoProxyMock } = vi.hoisted(
  () => ({
    ...
    matchesNoProxyMock: vi.fn((_url?: string) => false),
  }),
);
...
vi.mock("../infra/net/proxy-env.js", async () => {
  ...
  return {
    ...
    // The auto-upgrade gate calls `shouldUseEnvHttpProxyForUrl`, and THAT
    // function calls the two above from inside this same module -- direct
    // calls that `vi.mock` cannot intercept. Mocking only those two therefore
    // leaves the real implementation reading `process.env`, where no proxy is
    // set, so the auto-upgrade never fires and every assertion that it did
    // silently tests nothing. Mock the seam the code actually imports, derived
    // from the same two mocks so each test's setup still means what it reads
    // as (including the NO_PROXY bypass case).
    shouldUseEnvHttpProxyForUrl: (url: string) =>
      hasEnvHttpProxyConfiguredMock() && !matchesNoProxyMock(url),
  };
});
